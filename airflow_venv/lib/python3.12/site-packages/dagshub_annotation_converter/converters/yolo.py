    ...
    for dirpath, subdirs, files in os.walk(data_dir_path):
        ...
        for filename in files:
            ...
            annotation = replace_folder(
                ...
                context.label_extension,
            )
            if annotation is None:
                logger.warning(
                    f"Couldn't generate annotation file path for image file [{img}]"
                )
                continue
            if not annotation.exists():
                ...
                continue
            annotations[str(relpath)] = parse_annotation(
                context, data_dir_path, img, annotation
            )
...
def parse_annotation(
    ...
    for ann in annotation_strings:
        res.append(
            convert_func(ann, context, img_width, img_height, img).with_filename(
                rel_path
            )
        )
...
def load_yolo_from_fs(
    ...
    return (
        load_yolo_from_fs_with_context(context, import_dir=meta_file_path.parent),
        context,
    )
...
def annotations_to_string(
    annotations: Sequence[IRImageAnnotationBase], context: YoloContext
) -> Optional[str]:
    """
    ...
    filtered_annotations = [
        ...
        if isinstance(ann, allowed_annotation_types[context.annotation_type])
    ]
...
def _guess_train_val_test_split(
...
def export_to_fs(
    ...
    if context.path is None:
        print(
            f"`YoloContext.path` was not set. Exporting to {os.path.join(os.getcwd(), 'data')}"
        )
    ...
    for filename, anns in grouped_annotations.items():
        ...
        if annotation_filepath is None:
            logger.warning(
                f"Couldn't generate annotation file path for image file [{filename}]"
            )
            continue
    ...
    (
        ...
        guessed_val_path,
        guessed_test_path,
    ) = _guess_train_val_test_split(list(grouped_annotations.keys()))
    ...
    logger.warning(
        f"Saved annotations to {context.path}\nand .YAML file at {yaml_file_path}"
    )
