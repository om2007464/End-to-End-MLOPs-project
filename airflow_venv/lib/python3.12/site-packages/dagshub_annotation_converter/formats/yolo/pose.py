...
from dagshub_annotation_converter.ir.image.annotations.pose import (
...
def import_pose_from_string(
    ...
    if context.keypoint_dim == 2:
        points = [
            (float(parts[i]), float(parts[i + 1]), None)
            for i in range(5, len(parts), 2)
        ]
    elif context.keypoint_dim == 3:
        points = [
            (float(parts[i]), float(parts[i + 1]), parts[i + 2] == "1")
...
def export_pose(annotation: IRPoseImageAnnotation, context: YoloContext) -> str:
    if context.keypoint_dim == 2:
        point_list = [
            ...
            if point.visible is not False
        ]
    else:
        point_list = [
            f"{point.x} {point.y} {0 if point.visible is False else 1}"
            for point in annotation.points
        ]
