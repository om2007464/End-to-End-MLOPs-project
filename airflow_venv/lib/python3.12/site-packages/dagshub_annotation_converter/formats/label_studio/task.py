...
from dagshub_annotation_converter.formats.label_studio.base import (
    AnnotationResultABC,
    ImageAnnotationResultABC,
)
from dagshub_annotation_converter.formats.label_studio.ellipselabels import (
    EllipseLabelsAnnotation,
)
from dagshub_annotation_converter.formats.label_studio.keypointlabels import (
    KeyPointLabelsAnnotation,
)
from dagshub_annotation_converter.formats.label_studio.polygonlabels import (
...
ir_annotation_lookup: Dict[
    Type[IRImageAnnotationBase], Type[ImageAnnotationResultABC]
] = {
...
AnnotationsList = Annotated[
    List[SerializeAsAny[AnnotationResultABC]], BeforeValidator(ls_annotation_validator)
]
...
class LabelStudioTask(ParentModel):
    ...
    def log_pose_metadata(
        self, bbox: RectangleLabelsAnnotation, keypoints: List[KeyPointLabelsAnnotation]
    ):
        """
    ...
    def _reimport_poses(
        ...
        annotation_lookup = {
            ann.imported_id: ann for ann in annotations if ann.imported_id is not None
        }
        ...
        for bbox_id, point_ids in zip(pose_bboxes, pose_points):
            ...
            if maybe_bbox is None:
                ...
                logger.warning(
                    f"Bounding box of pose with annotation ID {bbox_id} is not a bounding box annotation"
                )
            else:
                bbox = maybe_bbox
            ...
            for point_id in point_ids:
                ...
                if maybe_point is None:
                    ...
                    logger.warning(
                        f"Point of pose with annotation ID {point_id} is not a point annotation"
                    )
                    continue
                else:
                    if category is None:
        ...
        logger.debug(
            f"Consolidated {len(poses)} pose annotations for LS Task {self.id}"
        )
        ...
        annotations = list(
            filter(
                lambda ann: ann.imported_id not in annotations_to_remove, annotations
            )
        )
