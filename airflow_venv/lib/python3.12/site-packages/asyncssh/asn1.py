...
class DERTag:
    ...
    def __init__(
        self, tag: int, types: Sequence[object] = (), constructed: bool = False
    ):
        self._tag = tag
        self._types = types
...
class RawDERObject:
    ...
    def __repr__(self) -> str:
        return (
            f"RawDERObject({_asn1_class[self.asn1_class]}, "
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.asn1_class == other.asn1_class
...
class TaggedDERObject:
    ...
    def __init__(self, tag: int, value: object, asn1_class: int = CONTEXT_SPECIFIC):
        self.asn1_class = asn1_class
        self.tag = tag
        self.value = value
    ...
    def __repr__(self) -> str:
        if self.asn1_class == CONTEXT_SPECIFIC:
            ...
            return (
                f"TaggedDERObject({_asn1_class[self.asn1_class]}, "
...
class BitString(DERType):
    ...
    def __init__(self, value: object, unused: int = 0, named: bool = False):
        ...
        if isinstance(value, bytes):
            if unused:
                if not value:
                    raise ASN1EncodeError("Can't have unused bits with empty " "value")
                elif value[-1] & ((1 << unused) - 1):
                    raise ASN1EncodeError("Unused bits in value should be " "zero")
        elif isinstance(value, str):
            if unused:
                raise ASN1EncodeError(
                    "Unused bit count should not be set " "when providing a string"
                )
...
class ObjectIdentifier(DERType):
    ...
    def encode(value: object) -> bytes:
        ...
        def _bytes(component: int) -> bytes:
            ...
            if component < 0:
                raise ASN1EncodeError(
                    "Components of object identifier must "
        ...
        try:
            ...
            raise ASN1EncodeError("Component values must be " "integers") from None

        if len(components) < 2:
            raise ASN1EncodeError(
                "Object identifiers must have at least two " "components"
            )
        elif components[0] < 0 or components[0] > 2:
            raise ASN1EncodeError(
                "First component of object identifier must " "be between 0 and 2"
            )
        elif components[0] < 2 and (components[1] < 0 or components[1] > 39):
            raise ASN1EncodeError(
                "Second component of object identifier must " "be between 0 and 39"
            )
    ...
    def decode(cls, constructed: bool, content: bytes) -> "ObjectIdentifier":
        ...
        if constructed:
            raise ASN1DecodeError("OBJECT IDENTIFIER should not be " "constructed")
