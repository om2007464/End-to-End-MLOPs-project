...
class SSHKeySignKeyPair(SSHKeyPair):
    ...
    def __init__(self, keysign_path: str, sock_fd: int, key_or_cert: _KeySignKey):
        ...
        super().__init__(
            algorithm, algorithm, sig_algorithms, sig_algorithms, public_data, comment
        )
    ...
    async def sign_async(self, data: bytes) -> bytes:
        ...
        proc = await asyncio.create_subprocess_exec(
            ...
            pass_fds=[self._sock_fd],
        )

        request = String(Byte(KEYSIGN_VERSION) + UInt32(self._sock_fd) + String(data))
...
def get_keysign_keys(
    keysign_path: str, sock_fd: int, keys: Iterable[_KeySignKey]
