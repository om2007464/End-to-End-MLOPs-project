...
async def _lookup_host(
    loop: asyncio.AbstractEventLoop, host: str, family: int
) -> Sequence[str]:
    """Look up IPv4 or IPv6 addresses of a host name"""

    try:
        addrinfo = await loop.getaddrinfo(
            host, 0, family=family, type=socket.SOCK_STREAM
        )
...
class SSHX11ClientListener:
    ...
    def attach(
        self, display: str, chan: "SSHChannel", single_connection: bool
    ...
    async def forward_connection(self) -> SSHX11ClientForwarder:
        ...
        try:
            _, peer = await self._connect_coro(SSHForwarder, *self._connect_args)
...
async def lookup_xauth(
    loop: asyncio.AbstractEventLoop, auth_path: Optional[str], host: str, dpynum: str
) -> Tuple[bytes, bytes]:
...
async def update_xauth(
    ...
    new_entry = SSHXAuthorityEntry(
        XAUTH_FAMILY_HOSTNAME, host, dpynum, auth_proto, auth_data
    )
    ...
    for entry in walk_xauth(auth_path):
        if (
            entry.family != new_entry.family
            or entry.addr != new_entry.addr
            or entry.dpynum != new_entry.dpynum
        ):
...
async def create_x11_client_listener(
    loop: asyncio.AbstractEventLoop, display: str, auth_path: Optional[str]
...
async def create_x11_server_listener(
    ...
    for dpynum in range(X11_DISPLAY_START, X11_MAX_DISPLAYS):
        ...
        try:
            await update_xauth(
                auth_path, X11_LISTEN_HOST, str(dpynum), auth_proto, auth_data
            )
        except ValueError:
            tcp_listener.close()
            break
