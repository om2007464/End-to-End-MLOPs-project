...
async def _open_tunnel(
    tunnels: object, passphrase: Optional[BytesOrStr], config: DefTuple[ConfigPaths]
...
class SSHConnection(SSHPacketHandler, asyncio.Protocol):
    ...
    def send_packet(
        ...
        if (
            (
                (
                    pkttype in {MSG_SERVICE_REQUEST, MSG_SERVICE_ACCEPT}
...
class SSHClientConnectionOptions(SSHConnectionOptions):
    ...
    def prepare(
        ...
        config = SSHClientConfig.load(
            last_config, config, reload, local_username, username, host, port
        )
