...
class _KexRSA(Kex):
    ...
    async def start(self) -> None:
        ...
        if self._conn.is_server():
            ...
            self._trans_key = generate_private_key("ssh-rsa", key_size=self._key_size)
    ...
    def _process_pubkey(self, _pkttype: int, _pktid: int, packet: SSHPacket) -> None:
        ...
        self._encrypted_k = cast(
            bytes, trans_key.encrypt(MPInt(self._k), self.algorithm)
        )

        self.send_packet(MSG_KEXRSA_SECRET, String(self._encrypted_k))

    def _process_secret(self, _pkttype: int, _pktid: int, packet: SSHPacket) -> None:
    ...
    def _process_done(self, _pkttype: int, _pktid: int, packet: SSHPacket) -> None:
