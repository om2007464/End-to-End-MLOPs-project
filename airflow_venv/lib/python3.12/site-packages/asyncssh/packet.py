...
class SSHPacketLogger:
    ...
    def _log_packet(
        ...
        self.logger.packet(pktid, packet, "%s %s, %s%s", msg, name, count, note)

    def log_sent_packet(
        self, pkttype: int, pktid: Optional[int], packet: _LoggedPacket, note: str = ""
    ) -> None:
        """Log a sent packet"""

        self._log_packet("Sent", pkttype, pktid, packet, note)

    def log_received_packet(
        self, pkttype: int, pktid: Optional[int], packet: _LoggedPacket, note: str = ""
    ) -> None:
...
class SSHPacketHandler(SSHPacketLogger):
    ...
    ) -> Union[bool, Awaitable[None]]:
        """Log and process a received packet"""

        if pkttype in self._packet_handlers:
            return self._packet_handlers[pkttype](self, pkttype, pktid, packet) or True
        else:
            return False
