...
class _PageantTransport:
    ...
    def __init__(self) -> None:
        ...
        try:
            self._mapfile = mmapfile.mmapfile(
                "", self._mapname, _AGENT_MAX_MSGLEN, 0, 0
            )
        except mmapfile.error as exc:
            raise OSError(errno.EIO, str(exc)) from None

        self._cds = _CopyDataStruct(
            _AGENT_COPYDATA_ID, len(self._mapname) + 1, self._mapname.encode()
        )
    ...
    async def readexactly(self, n: int) -> bytes:
        ...
        if self._writing:
            ...
            if not cwnd.SendMessage(win32con.WM_COPYDATA, 0, cast(int, self._cds)):
