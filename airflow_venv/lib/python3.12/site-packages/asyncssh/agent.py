...
class SSHAgentClient:
    ...
    async def connect(self) -> None:
        ...
        if isinstance(self._agent_path, str):
            ...
            self._reader, self._writer = await self._agent_path.open_agent_connection()

    async def _make_request(self, msgtype: int, *args: bytes) -> Tuple[int, SSHPacket]:
    ...
    async def get_keys(
        self, identities: Optional[Sequence[bytes]] = None
    ) -> Sequence[SSHKeyPair]:
        ...
        resptype, resp = await self._make_request(SSH_AGENTC_REQUEST_IDENTITIES)
        ...
        if resptype == SSH_AGENT_IDENTITIES_ANSWER:
            ...
            for _ in range(num_keys):
                ...
                result.append(SSHAgentKeyPair(self, algorithm, key_blob, comment))
    ...
    async def sign(self, key_blob: bytes, data: bytes, flags: int = 0) -> bytes:
        """Sign a block of data with the requested key"""

        resptype, resp = await self._make_request(
    ...
    async def add_smartcard_keys(
        ...
        msgtype = (
            ...
            if constraints
            else SSH_AGENTC_ADD_SMARTCARD_KEY
        )

        resptype, resp = await self._make_request(
            msgtype, String(provider), String(pin or ""), constraints
        )
    ...
    async def remove_keys(self, keylist: Sequence[SSHKeyPair]) -> None:
        ...
        for keypair in keylist:
            resptype, resp = await self._make_request(
                SSH_AGENTC_REMOVE_IDENTITY, String(keypair.public_data)
            )
    ...
    async def remove_smartcard_keys(
        self, provider: str, pin: Optional[str] = None
    ) -> None:
        ...
        resptype, resp = await self._make_request(
            SSH_AGENTC_REMOVE_SMARTCARD_KEY, String(provider), String(pin or "")
        )
    ...
    async def remove_all(self) -> None:
        ...
        resptype, resp = await self._make_request(SSH_AGENTC_REMOVE_ALL_IDENTITIES)
    ...
    async def lock(self, passphrase: str) -> None:
        ...
        resptype, resp = await self._make_request(SSH_AGENTC_LOCK, String(passphrase))
    ...
    async def unlock(self, passphrase: str) -> None:
        ...
        resptype, resp = await self._make_request(SSH_AGENTC_UNLOCK, String(passphrase))
    ...
    async def query_extensions(self) -> Sequence[str]:
        ...
        resptype, resp = await self._make_request(SSH_AGENTC_EXTENSION, String("query"))
