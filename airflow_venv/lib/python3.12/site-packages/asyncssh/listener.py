...
class SSHTCPClientListener(SSHClientListener, Generic[AnyStr]):
    ...
    async def _close(self) -> None:
        ...
        if self._conn:  # pragma: no branch
            await self._conn.close_client_tcp_listener(
    ...
    def process_connection(
        self, orig_host: str, orig_port: int
    ) -> Tuple["SSHTCPChannel[AnyStr]", MaybeAwait[SSHTCPSession[AnyStr]]]:
        ...
        chan.set_inbound_peer_names(
            self._listen_host, self._listen_port, orig_host, orig_port
        )
...
class SSHUNIXClientListener(SSHClientListener, Generic[AnyStr]):
    ...
    def process_connection(
        self,
    ) -> Tuple["SSHUNIXChannel[AnyStr]", MaybeAwait[SSHUNIXSession[AnyStr]]]:
...
async def create_tcp_local_listener(
    ...
    protocol_factory: _LocalListenerFactory,
    listen_host: str,
    listen_port: int,
) -> "SSHForwardListener":
    ...
    for addrinfo_entry in addrinfo:
        ...
        try:
            ...
            raise OSError(
                exc.errno,
                f"error while attempting "  # type: ignore
...
async def create_tcp_forward_listener(
    conn: "SSHConnection",
    loop: asyncio.AbstractEventLoop,
    ...
    return await create_tcp_local_listener(
        conn, loop, protocol_factory, listen_host, listen_port
    )


async def create_unix_forward_listener(
    conn: "SSHConnection",
    loop: asyncio.AbstractEventLoop,
    coro: SSHForwarderCoro,
    listen_path: str,
...
async def create_socks_listener(
    conn: "SSHConnection",
    loop: asyncio.AbstractEventLoop,
    coro: SSHForwarderCoro,
    listen_host: str,
