...
class _ECKey(SSHKey):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self._key.x == other._key.x
            and self._key.y == other._key.y
            and self._key.d == other._key.d
        )

    def __hash__(self) -> int:
        return hash((self._key.curve_id, self._key.x, self._key.y, self._key.d))
        ...
        if isinstance(alg_params, ObjectIdentifier):
            try:
                ...
                raise KeyImportError(
                    "Unknown elliptic curve OID " f"{alg_params}"
                ) from None
        elif (
            isinstance(alg_params, tuple)
            and len(alg_params) >= 5
            and alg_params[0] == 1
            and isinstance(alg_params[1], tuple)
            and len(alg_params[1]) == 2
            and alg_params[1][0] == PRIME_FIELD
            and isinstance(alg_params[2], tuple)
            and len(alg_params[2]) >= 2
            and isinstance(alg_params[3], bytes)
            and isinstance(alg_params[2][0], bytes)
            and isinstance(alg_params[2][1], bytes)
            and isinstance(alg_params[4], int)
        ):
            p = alg_params[1][1]
    ...
    def make_private(cls, key_params: object) -> SSHKey:
        ...
        curve_id, private_value, public_value = cast(_PrivateKeyArgs, key_params)

        if isinstance(private_value, bytes):
            private_value = int.from_bytes(private_value, "big")

        return cls(ECDSAPrivateKey.construct(curve_id, public_value, private_value))
    ...
    @classmethod
    def decode_pkcs1_private(cls, key_data: object) -> Optional[_PrivateKeyArgs]:
        ...
        if (
            ...
            and len(key_data) > 2
            and key_data[0] == 1
            and isinstance(key_data[1], bytes)
            and isinstance(key_data[2], TaggedDERObject)
            and key_data[2].tag == 0
        ):
            ...
            if (
                len(key_data) > 3
                and isinstance(key_data[3], TaggedDERObject)
                and key_data[3].tag == 1
                and isinstance(key_data[3].value, BitString)
                and key_data[3].value.unused == 0
            ):
    ...
    @classmethod
    def decode_pkcs1_public(cls, key_data: object) -> Optional[_PublicKeyArgs]:
    ...
    @classmethod
    def decode_pkcs8_private(
        cls, alg_params: object, data: bytes
        ...
        if (
            ...
            if (
                len(key_data) > 2
                and isinstance(key_data[2], TaggedDERObject)
                and key_data[2].tag == 1
                and isinstance(key_data[2].value, BitString)
    ...
    def encode_pkcs8_private(self) -> Tuple[object, object]:
        ...
        return self._alg_oid, der_encode(
            (1, self._key.private_value, self.encode_public_tagged())
        )
    ...
    def encode_ssh_public(self) -> bytes:
        ...
        return b"".join((String(self._key.curve_id), String(self._key.public_value)))
...
for _curve_id, _oid_str in (
    (b"nistp521", "1.3.132.0.35"),
    ...
    register_certificate_alg(
        1, _algorithm, _cert_algorithm, _ECKey, SSHOpenSSHCertificateV01, True
    )
