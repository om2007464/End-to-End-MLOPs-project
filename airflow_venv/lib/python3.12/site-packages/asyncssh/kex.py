...
class Kex(SSHPacketHandler):
    ...
    def compute_key(
        self, k: bytes, h: bytes, x: bytes, session_id: bytes, keylen: int
    ) -> bytes:
...
def expand_kex_algs(
    kex_algs: Sequence[bytes], mechs: Sequence[bytes], host_key_available: bool
) -> List[bytes]:
