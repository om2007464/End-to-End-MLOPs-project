...
class RSAKey(SSHKey):
    ...
    sig_algorithms = (
        b"rsa-sha2-256",
        b"rsa-sha2-512",
        b"ssh-rsa-sha224@ssh.com",
        b"ssh-rsa-sha256@ssh.com",
        b"ssh-rsa-sha384@ssh.com",
        b"ssh-rsa-sha512@ssh.com",
        b"ssh-rsa",
    )
    cert_sig_algorithms = (b"rsa-sha2-256", b"rsa-sha2-512", b"ssh-rsa")
    cert_algorithms = tuple(
        alg + b"-cert-v01@openssh.com" for alg in cert_sig_algorithms
    )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self._key.n == other._key.n
            and self._key.e == other._key.e
            and self._key.d == other._key.d
        )

    def __hash__(self) -> int:
        return hash((self._key.n, self._key.e, self._key.d, self._key.p, self._key.q))
    ...
    def make_private(cls, key_params: object) -> SSHKey:
        ...
        n, e, d, p, q, dmp1, dmq1, iqmp, unsafe_skip_rsa_key_validation = cast(
            _PrivateKeyConstructArgs, key_params
        )
    ...
    def decode_pkcs1_private(cls, key_data: object) -> Optional[_PrivateKeyArgs]:
        ...
        if isinstance(key_data, tuple) and all_ints(key_data) and len(key_data) >= 9:
    ...
    def decode_pkcs1_public(cls, key_data: object) -> Optional[_PublicKeyArgs]:
        ...
        if isinstance(key_data, tuple) and all_ints(key_data) and len(key_data) == 2:
...
for _alg in RSAKey.cert_sig_algorithms:
    register_certificate_alg(
        1, _alg, _alg + b"-cert-v01@openssh.com", RSAKey, SSHOpenSSHCertificateV01, True
    )
