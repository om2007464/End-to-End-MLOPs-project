...
class _RFC1423Pad:
    ...
    def __init__(self, cipher_name: str, block_size: int, key: bytes, iv: bytes):
    ...
    def decrypt(self, data: bytes) -> bytes:
        ...
        if data:
            pad = data[-1]
            if 1 <= pad <= self._block_size and data[-pad:] == pad * bytes((pad,)):
                return data[:-pad]
...
def _pbkdf1(
    hash_alg: HashType, passphrase: BytesOrStr, salt: bytes, count: int, key_size: int
) -> bytes:
    ...
    if len(key) <= key_size:
        return key + _pbkdf1(
            hash_alg, key + passphrase, salt, count, key_size - len(key)
        )
    else:
        return key[:key_size]
...
def _pbes1(
    params: object, passphrase: BytesOrStr, hash_alg: HashType, cipher_name: str
) -> _Cipher:
    ...
    if (
        ...
        or len(params) != 2
        or not isinstance(params[0], bytes)
...
def _pbes2_pbkdf2(
    kdf_params: Sequence, passphrase: BytesOrStr, default_key_size: int
) -> bytes:
    ...
    if (
        len(kdf_params) != 1
        or not isinstance(kdf_params[0], tuple)
        or len(kdf_params[0]) < 2
    ):
        raise KeyEncryptionError("Invalid PBES2 key derivation parameters")

    kdf_params = list(kdf_params[0])

    if not isinstance(kdf_params[0], bytes) or not isinstance(kdf_params[1], int):
        raise KeyEncryptionError("Invalid PBES2 key derivation parameters")
    ...
    if kdf_params:
        if (
            isinstance(kdf_params[0], tuple)
            and len(kdf_params[0]) == 2
            and isinstance(kdf_params[0][0], ObjectIdentifier)
        ):
            ...
            raise KeyEncryptionError(
                "Invalid PBES2 pseudo-random function " "parameters"
            )
    else:
        hash_name = "sha1"
...
def _pbes2(params: object, passphrase: BytesOrStr) -> _Cipher:
    ...
    if (
        not isinstance(params, tuple)
        or len(params) != 2
        or not isinstance(params[0], tuple)
        or len(params[0]) < 1
        or not isinstance(params[1], tuple)
        or len(params[1]) < 1
    ):
        raise KeyEncryptionError("Invalid PBES2 encryption parameters")
...
def register_pkcs1_cipher(
    pkcs1_cipher_name: str, pkcs1_dek_name: bytes, cipher_name: str
) -> None:
...
def register_pkcs8_cipher(
    pkcs8_cipher_name: str,
    hash_name: str,
    pkcs8_cipher_oid: ObjectIdentifier,
    handler: _PKCS8CipherHandler,
    hash_alg: HashType,
    cipher_name: str,
) -> None:
...
def register_pbes2_cipher(
    pbes2_cipher_name: str,
...
def register_pbes2_kdf(
    kdf_name: str, kdf_oid: ObjectIdentifier, handler: _PBES2KDFHandler, *args: object
) -> None:
...
def pkcs1_encrypt(
    data: bytes, pkcs1_cipher_name: str, passphrase: BytesOrStr
...
def pkcs1_decrypt(
    data: bytes, pkcs1_dek_name: bytes, iv: bytes, passphrase: BytesOrStr
) -> bytes:
...
def pkcs8_decrypt(key_data: object, passphrase: BytesOrStr) -> object:
    ...
    if (
        not isinstance(alg_params, tuple)
        or len(alg_params) != 2
...
_pbes2_kdf_list = (("pbkdf2", _ES2_PBKDF2, _pbes2_pbkdf2),)
