...
if pkcs11_available:
    encoders = {KeyType.RSA: encode_rsa_public_key, KeyType.EC: encode_ec_public_key}

    mechanisms = {
        b"ssh-rsa": Mechanism.SHA1_RSA_PKCS,
    ...
    class SSHPKCS11Session:
        ...
        def __init__(self, token_id: _TokenID, token: Token, pin: Optional[str]):
        ...
        def get_keys(
            ...
            if load_certs:
                certs = [
                    import_certificate_chain(cast(bytes, cert[Attribute.VALUE]))
                    for cert in self._session.get_objects(cert_attrs)
                ]

                certdict = {
                    cert.key.public_data: cert
                    for cert in certs
            ...
            for key in self._session.get_objects(key_attrs):
                ...
                if encoder:
                    ...
                    if cert:
                        keys.append(SSHPKCS11KeyPair(self, privkey, pubkey, cert))
    ...
    def load_pkcs11_keys(
        ...
        *,
        ...
        for token in lib.get_tokens(token_label=token_label, token_serial=token_serial):
