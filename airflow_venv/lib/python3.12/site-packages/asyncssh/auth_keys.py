...
class _SSHAuthorizedKeyEntry:
    ...
    def _import_key_or_cert(self, line: str) -> None:
        ...
        try:
            ...
            if (
                "cert-authority" in self.options
                and self.cert.subject != self.cert.issuer
            ):
                raise ValueError(
                    "X.509 cert-authority entries must " "contain a root CA certificate"
                )
    ...
    def _add_from(self, option: str, value: str) -> None:
        ...
        from_patterns = cast(List[HostPatternList], self.options.setdefault(option, []))
    ...
    def _add_permitopen(self, option: str, value: str) -> None:
        ...
        permitted_opens = cast(
            Set[Tuple[str, Optional[int]]], self.options.setdefault(option, set())
        )
    ...
    def _add_principals(self, option: str, value: str) -> None:
        ...
        principal_patterns = cast(
            List[WildcardPatternList], self.options.setdefault(option, [])
        )
    ...
    def _add_subject(self, option: str, value: str) -> None:
        ...
        if _x509_available:  # pragma: no branch
            subject_patterns = cast(
                List[X509NamePattern], self.options.setdefault(option, [])
            )
    ...
    def match_options(
        ...
        cert_subject: Optional["X509Name"] = None,
    ) -> bool:
        ...
        principal_patterns = cast(
            List[WildcardPatternList], self.options.get("principals")
        )

        if cert_principals is not None and principal_patterns is not None:
            if not all(
                any(pattern.matches(principal) for principal in cert_principals)
                for pattern in principal_patterns
            ):
                return False

        subject_patterns = cast(List["X509NamePattern"], self.options.get("subject"))

        if cert_subject is not None and subject_patterns is not None:
            if not all(pattern.matches(cert_subject) for pattern in subject_patterns):
                return False
...
class SSHAuthorizedKeys:
    ...
    def load(self, authorized_keys: str) -> None:
        ...
        if not self._user_entries and not self._ca_entries and not self._x509_entries:
    ...
    def validate(
        ...
        for entry in self._ca_entries if ca else self._user_entries:
            if entry.key == key and entry.match_options(
                client_host, client_addr, cert_principals
            ):
    ...
    def validate_x509(
        self, cert: SSHX509CertificateChain, client_host: str, client_addr: str
    ) -> Tuple[Optional[_EntryOptions], Optional[SSHX509Certificate]]:
        """Return whether an X.509 certificate is valid for authentication"""

        for entry in self._x509_entries:
            if (
                entry.cert
                and "cert-authority" not in entry.options
                and (cert.key != entry.cert.key or cert.subject != entry.cert.subject)
            ):
                continue  # pragma: no cover (work around bug in coverage tool)

            if entry.match_options(
                client_host, client_addr, cert.user_principals, cert.subject
            ):
...
def read_authorized_keys(filelist: Union[str, Sequence[str]]) -> SSHAuthorizedKeys:
