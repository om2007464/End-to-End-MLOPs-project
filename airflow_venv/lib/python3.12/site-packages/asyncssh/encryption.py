...
class Encryption:
    ...
    def encrypt_packet(
        self, seq: int, header: bytes, packet: bytes
    ) -> Tuple[bytes, bytes]:
    ...
    def decrypt_header(
        self, seq: int, first_block: bytes, header_len: int
    ) -> Tuple[bytes, bytes]:
    ...
    def decrypt_packet(
        self, seq: int, first: bytes, rest: bytes, header_len: int, mac: bytes
    ) -> Optional[bytes]:
...
class GCMEncryption(Encryption):
    ...
    def decrypt_packet(
        ...
        return self._cipher.verify_and_decrypt(
            first[:header_len], first[header_len:] + rest, mac
        )
...
class ChachaEncryption(Encryption):
    ...
    def decrypt_header(
        ...
        return (
            first_block,
            self._cipher.decrypt_header(first_block[:header_len], UInt64(seq)),
        )
    ...
    def decrypt_packet(
        ...
        return self._cipher.verify_and_decrypt(
            first[:header_len], first[header_len:] + rest, UInt64(seq), mac
        )


def register_encryption_alg(
    enc_alg: bytes, encryption: Type[Encryption], cipher_name: str, default: bool
) -> None:
...
def get_encryption_params(enc_alg: bytes, mac_alg: bytes = b"") -> _EncParams:
    ...
    return (enc_keysize, enc_ivsize, enc_blocksize, mac_keysize, mac_hashsize, etm)
...
_enc_alg_list = (
    (b"chacha20-poly1305@openssh.com", ChachaEncryption, "chacha20-poly1305", True),
    (b"aes256-gcm@openssh.com", GCMEncryption, "aes256-gcm", True),
    (b"aes128-gcm@openssh.com", GCMEncryption, "aes128-gcm", True),
    (b"aes256-ctr", BasicEncryption, "aes256-ctr", True),
    (b"aes192-ctr", BasicEncryption, "aes192-ctr", True),
    (b"aes128-ctr", BasicEncryption, "aes128-ctr", True),
    (b"aes256-cbc", BasicEncryption, "aes256-cbc", False),
    (b"aes192-cbc", BasicEncryption, "aes192-cbc", False),
    (b"aes128-cbc", BasicEncryption, "aes128-cbc", False),
    (b"3des-cbc", BasicEncryption, "des3-cbc", False),
    (b"blowfish-cbc", BasicEncryption, "blowfish-cbc", False),
    (b"cast128-cbc", BasicEncryption, "cast128-cbc", False),
    (b"seed-cbc@ssh.com", BasicEncryption, "seed-cbc", False),
    (b"arcfour256", BasicEncryption, "arcfour256", False),
    (b"arcfour128", BasicEncryption, "arcfour128", False),
    (b"arcfour", BasicEncryption, "arcfour", False),
)
