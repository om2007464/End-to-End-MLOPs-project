...
if backend.poly1305_supported():
    ...
    def chacha20(key: bytes, data: bytes, nonce: bytes, ctr: int) -> bytes:
        ...
        return (
            Cipher(ChaCha20(key, (_CTR_1 if ctr else _CTR_0) + nonce), mode=None)
            .encryptor()
            .update(data)
        )
...
class ChachaCipher:
    ...
    def encrypt_and_sign(
        self, header: bytes, data: bytes, nonce: bytes
    ) -> Tuple[bytes, bytes]:
    ...
    def verify_and_decrypt(
        self, header: bytes, data: bytes, nonce: bytes, tag: bytes
    ) -> Optional[bytes]:
