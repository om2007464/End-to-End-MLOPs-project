...
def _to_purpose_oids(purposes: _Purposes) -> _PurposeOIDs:
    ...
    if not purposes or "any" in purposes or _purpose_any in purposes:
        ...
        purpose_oids = {
            _purpose_to_oid.get(p) or x509.ObjectIdentifier(p) for p in purposes
        }
...
class X509Name(x509.Name):
    ...
    def _parse_name(self, name: str) -> Iterable[x509.RelativeDistinguishedName]:
...
class X509NamePattern:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return self._pattern == other._pattern and self._prefix_len == other._prefix_len
...
class X509Certificate:
    ...
    def __init__(self, cert: x509.Certificate, data: bytes):
        ...
        try:
            self.purposes: Optional[Set[bytes]] = set(
        ...
        try:
            ...
            self.comment: Optional[bytes] = cast(
                IA5String, der_decode(comment_der)
            ).value
        except x509.ExtensionNotFound:
            self.comment = None
    ...
    def validate(
        self,
        trust_store: Sequence["X509Certificate"],
        purposes: _Purposes,
        user_principal: str,
        host_principal: str,
    ) -> None:
        ...
        try:
            x509_ctx = crypto.X509StoreContext(x509_store, self.openssl_cert, None)
...
def generate_x509_certificate(
    ...
    if ca:
        basic_constraints = x509.BasicConstraints(ca=True, path_length=ca_path_len)
    ...
    if purpose_oids:
        builder = builder.add_extension(
            x509.ExtendedKeyUsage(purpose_oids), critical=False
        )
    ...
    sans = _encode_user_principals(user_principals) + _encode_host_principals(
        host_principals
    )
    ...
    if comment:
        ...
        builder = builder.add_extension(
            x509.UnrecognizedExtension(_nscomment_oid, comment_bytes), critical=False
        )
    ...
    cert = builder.sign(cast(PyCAPrivateKey, signing_key), hash_alg)  # type: ignore
