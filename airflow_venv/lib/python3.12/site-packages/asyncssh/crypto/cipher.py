...
class GCMCipher:
    ...
    def verify_and_decrypt(
        self, header: bytes, data: bytes, mac: bytes
    ) -> Optional[bytes]:
        """Verify the signature of and decrypt a block of data"""

        try:
            decrypted_data: Optional[bytes] = AESGCM(self._key).decrypt(
                self._iv, data + mac, header
            )
        except InvalidTag:
            decrypted_data = None
...
def register_cipher(
    cipher_name: str, key_size: int, iv_size: int, block_size: int
) -> None:
