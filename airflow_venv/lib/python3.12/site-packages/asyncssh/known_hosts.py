...
class _HashedHost:
    ...
    def __init__(self, pattern: str):
        try:
            ...
            raise ValueError(f"Invalid known hosts hash entry: {pattern}") from None

        if magic != self._HMAC_SHA1_MAGIC:
            # Only support HMAC SHA-1 for now
            raise ValueError(f"Invalid known hosts hash type: {magic}") from None
...
class SSHKnownHosts:
    ...
    def load(self, known_hosts: str) -> None:
        ...
        for line in known_hosts.splitlines():
            ...
            try:
                ...
                raise ValueError(f"Invalid known hosts entry: {line}") from None

            if marker not in (None, "cert-authority", "revoked"):
                raise ValueError(f"Invalid known hosts marker: {marker}") from None
    ...
    def _match(
        self, host: str, addr: str, port: Optional[int] = None
    ) -> _KnownHostsResult:
        ...
        matches += (
            match
            for (entry, match) in self._pattern_entries
    ...
    def match(self, host: str, addr: str, port: Optional[int]) -> _KnownHostsResult:
...
def match_known_hosts(
    known_hosts: KnownHostsArg, host: str, addr: str, port: Optional[int]
) -> _KnownHostsResult:
    ...
    if isinstance(known_hosts, str) or (
        known_hosts
        and isinstance(known_hosts, list)
        and isinstance(known_hosts[0], str)
    ):
    ...
    if isinstance(known_hosts, SSHKnownHosts):
        ...
        result = (
            tuple(map(load_public_keys, result[:3]))
    ...
    for cert in list(known_hosts[3]) + list(known_hosts[4]):
        if not cert.is_x509:
            raise ValueError(
                "OpenSSH certificates not " "allowed in known hosts"
            ) from None
