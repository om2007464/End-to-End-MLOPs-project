...
class SSHTunTapOSXTransport(SSHTunTapTransport):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, mode: int, unit: Optional[int]):
...
class SSHDarwinUTunTransport(SSHTunTapTransport):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, unit: Optional[int]):
        ...
        try:
            arg = struct.pack(DARWIN_CTLIOCGINFO_FMT, 0, b"com.apple.net.utun_control")
...
def create_tuntap(
    protocol_factory: Callable[[], asyncio.BaseProtocol], mode: int, unit: Optional[int]
    ...
    if sys.platform == "darwin":
        ...
        raise OSError(errno.EPROTONOSUPPORT, f"TunTap not supported on {sys.platform}")
