...
def _create_dataset_source_for_artifact_repo(scheme: str, dataset_source_name: str):
    ...
    if scheme in ["", "file"]:
        source_type = "local"
        class_docstring = (
            "Represents the source of a dataset stored on the local filesystem."
        )
    else:
        source_type = scheme
        class_docstring = (
    ...
    class ArtifactRepoSource(FileSystemDatasetSource):
        ...
        def _can_resolve(raw_source: Any):
            ...
            try:
                if is_local_source_type:
                    return is_local_uri(
                        str(raw_source), is_tracking_or_registry_uri=False
                    )
                else:
        ...
        @classmethod
        def from_dict(
            cls, source_dict: Dict[Any, Any]
        ) -> DatasetForArtifactRepoSourceType:
    ...
    ArtifactRepoSource.uri.__doc__ = ArtifactRepoSource.uri.__doc__.format(
        scheme=scheme
    )
