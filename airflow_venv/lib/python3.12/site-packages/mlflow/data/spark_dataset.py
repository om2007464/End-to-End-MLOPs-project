...
class SparkDataset(Dataset, PyFuncConvertibleDatasetMixin):
    ...
    def schema(self) -> Optional[Schema]:
        ...
        try:
            ...
            _logger.warning(
                "Failed to infer schema for Spark dataset. Exception: %s", e
            )
            return None
...
def load_delta(
    ...
    source = DeltaDatasetSource(
        path=path, delta_table_name=table_name, delta_table_version=version
    )
    df = source.load()
...
def from_spark(
    ...
    if sql is not None:
        ...
        if _is_delta_table(table_name):
            version = version or _try_get_delta_table_latest_version_from_table_name(
                table_name
            )
            source = DeltaDatasetSource(
