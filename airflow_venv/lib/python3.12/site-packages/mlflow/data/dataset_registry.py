...
class DatasetRegistry:
    ...
    def _validate_constructor(constructor_fn: callable, constructor_name: str):
        if not constructor_name.startswith("load_") and not constructor_name.startswith(
            "from_"
        ):
        ...
        for expected_kwarg in ["name", "digest"]:
            if expected_kwarg not in parameters or parameters[
                expected_kwarg
            ].kind not in [
...
def register_constructor(
    constructor_fn: callable, constructor_name: Optional[str] = None
) -> str:
    """Registers a dataset constructor.
