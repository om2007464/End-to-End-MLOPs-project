...
def _log_dataset(dataset, source, context, name=None, targets=None):
    ...
    if isinstance(dataset, np.ndarray):
        dataset = from_numpy(
            features=dataset, targets=targets, source=source, name=name
        )
    elif is_tf_tensor:
        ...
        _logger.warning(
            f"Unrecognized dataset type {type(dataset)}. Dataset logging skipped."
        )
        return
...
def _log_keras_model(
    ...
    log_model(
        ...
        registered_model_name=get_autologging_config(
            "keras", "registered_model_name", None
        ),
        ...
        def _infer_batch_size(self, inst, *args, **kwargs):
            ...
            if "batch_size" in kwargs:
                ...
                if _batch_size := getattr(training_data, "batch_size", None):
                    ...
                    batch_size = (
                        _batch_size
                        if isinstance(_batch_size, int)
                        else _batch_size.numpy()
                    )
        ...
        def _patch_implementation(self, original, inst, *args, **kwargs):
            ...
            if log_datasets:
                try:
                    ...
                    _logger.warning(
                        f"Failed to log dataset information to MLflow. Reason: {e}"
                    )
            ...
            if log_models:
                _log_keras_model(
                    inst, save_exported_model, log_model_signatures, save_model_kwargs
                )
            return history

    safe_patch(
        "keras", keras.Model, "fit", FitPatch, manage_run=True, extra_tags=extra_tags
    )
