...
def _get_permission_from_experiment_name() -> Permission:
    ...
    return _get_permission_from_store_or_default(
        lambda: store.get_experiment_permission(
            store_exp.experiment_id, username
        ).permission
    )
...
) -> Callable[[], Union[Authorization, Response]]:
    """
...
def filter_search_experiments(resp: Response):
    ...
    while (
        ...
        refetched = refetched[
            : request_message.max_results - len(response_message.experiments)
        ]
        if len(refetched) == 0:
        ...
        refetched_readable_proto = [
            e.to_proto()
            for e in refetched
            if can_read.get(e.experiment_id, default_can_read)
        ]
