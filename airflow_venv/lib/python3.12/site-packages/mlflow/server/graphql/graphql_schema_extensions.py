...
class MlflowRunExtension(MlflowRun):
    ...
    def resolve_model_versions(self, info):
        ...
        parse_dict(input_dict, request_message)
        return mlflow.server.handlers.search_model_versions_impl(
            request_message
        ).model_versions
...
class Query(QueryType):
    test = graphene.Field(
        Test, input_string=graphene.String(), description="Simple echoing field"
    )
...
schema = graphene.Schema(
    query=Query, mutation=Mutation, directives=[ComponentDirective]
)
