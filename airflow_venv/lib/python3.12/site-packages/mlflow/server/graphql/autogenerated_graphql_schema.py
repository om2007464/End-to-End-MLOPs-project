...
class MlflowSearchRunsResponse(graphene.ObjectType):
    runs = graphene.List(
        graphene.NonNull(
            "mlflow.server.graphql.graphql_schema_extensions.MlflowRunExtension"
        )
    )
    next_page_token = graphene.String()
    apiError = graphene.Field(ApiError)
...
class QueryType(graphene.ObjectType):
    mlflow_get_experiment = graphene.Field(
        MlflowGetExperimentResponse, input=MlflowGetExperimentInput()
    )
    mlflow_get_metric_history_bulk_interval = graphene.Field(
        MlflowGetMetricHistoryBulkIntervalResponse,
        input=MlflowGetMetricHistoryBulkIntervalInput(),
    )
    mlflow_get_run = graphene.Field(MlflowGetRunResponse, input=MlflowGetRunInput())
    mlflow_list_artifacts = graphene.Field(
        MlflowListArtifactsResponse, input=MlflowListArtifactsInput()
    )
    mlflow_search_model_versions = graphene.Field(
        MlflowSearchModelVersionsResponse, input=MlflowSearchModelVersionsInput()
    )
    ...
    def resolve_mlflow_get_metric_history_bulk_interval(self, info, input):
        ...
        parse_dict(input_dict, request_message)
        return mlflow.server.handlers.get_metric_history_bulk_interval_impl(
            request_message
        )
...
class MutationType(graphene.ObjectType):
    mlflow_search_datasets = graphene.Field(
        MlflowSearchDatasetsResponse, input=MlflowSearchDatasetsInput()
    )
    mlflow_search_runs = graphene.Field(
        MlflowSearchRunsResponse, input=MlflowSearchRunsInput()
    )
