...
class GatewayAPI(FastAPI):
    def __init__(
        self, config: GatewayConfig, limiter: Limiter, *args: Any, **kwargs: Any
    ):
...
def create_app_from_config(config: GatewayConfig) -> GatewayAPI:
    ...
    async def list_endpoints(page_token: Optional[str] = None) -> ListEndpointsResponse:
        start_idx = (
            SearchRoutesToken.decode(page_token).index if page_token is not None else 0
        )
        ...
        result = {
            "endpoints": [
                route.to_route().to_endpoint() for route in routes[start_idx:end_idx]
            ]
        }
    ...
    async def get_limits(endpoint: str) -> LimitsConfig:
        raise HTTPException(
            status_code=501, detail="The get_limits API is not available yet."
        )
    ...
    async def set_limits(payload: SetLimitsModel) -> LimitsConfig:
        raise HTTPException(
            status_code=501, detail="The set_limits API is not available yet."
        )
    ...
    async def openai_chat_handler(
        ...
        payload.model = (
            None  # provider rejects a request with model field, must be set to None
        )
        if payload.stream:
            return await make_streaming_response(prov.chat_stream(payload))
        else:
