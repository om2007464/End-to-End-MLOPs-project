...
class TogetherAIAdapter(ProviderAdapter):
        ...
        return completions_schema.StreamResponsePayload(
            ...
            choices=[
                completions_schema.StreamChoice(
                    ...
                    delta=completions_schema.StreamDelta(
                        role=None, content=choice.get("text")
                    ),
                )
    ...
    def completions_to_model(cls, payload, config):
        ...
        openai_top_logprobs_in_payload_condition = (
            "top_logprobs" in payload and not isinstance(payload["top_logprobs"], int)
        )
...
class TogetherAIProvider(BaseProvider):
    ...
    def __init__(self, config: RouteConfig) -> None:
        ...
        if config.model.config is None or not isinstance(
            config.model.config, TogetherAIConfig
        ):
    ...
    async def completions_stream(
        ...
        stream = await self._stream_request(
            ...
            payload={
                "model": self.config.model.name,
                **TogetherAIAdapter.completions_streaming_to_model(
                    payload, self.config
                ),
            },
        )
