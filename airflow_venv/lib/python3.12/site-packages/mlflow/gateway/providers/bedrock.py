...
class AmazonBedrockAnthropicAdapter(AnthropicAdapter):
    ...
    def completions_to_model(cls, payload, config):
        ...
        payload["max_tokens_to_sample"] = min(
            payload.get(
                "max_tokens_to_sample", MLFLOW_AI_GATEWAY_ANTHROPIC_DEFAULT_MAX_TOKENS
            ),
...
class AmazonBedrockProvider(BaseProvider):
    ...
    def __init__(self, config: RouteConfig):
        ...
        if config.model.config is None or not isinstance(
            config.model.config, AmazonBedrockConfig
        ):
    ...
    async def completions(
        ...
        payload = self.underlying_provider_adapter.completions_to_model(
            payload, self.config
        )
        response = self._request(payload)
        return self.underlying_provider_adapter.model_to_completions(
