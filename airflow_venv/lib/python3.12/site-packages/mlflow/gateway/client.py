...
class MlflowGatewayClient:
    ...
    def _is_databricks_host(self) -> bool:
        return (
            self._gateway_uri == "databricks"
            or get_uri_scheme(self._gateway_uri) == "databricks"
        )
    ...
    def get_route(self, name: str):
        ...
        response["route_url"] = resolve_route_url(
            self._gateway_uri, response["route_url"]
        )
    ...
    def query(self, route: str, data: Dict[str, Any]):
        ...
        query_route = assemble_uri_path(
            [MLFLOW_GATEWAY_ROUTE_BASE, route, MLFLOW_QUERY_SUFFIX]
        )
    ...
    def get_limits(self, route: str) -> LimitsConfig:
        ...
        if not route:
            raise MlflowException(
                "A non-empty string is required for the route.", BAD_REQUEST
            )
