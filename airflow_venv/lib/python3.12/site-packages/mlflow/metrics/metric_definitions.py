    ...
    for index, value in data.items():
        if not (
            (
                ...
                and all(isinstance(val, (str, int)) for val in value)
            )
            or (
...
def _flesch_kincaid_eval_fn(predictions, targets=None, metrics=None):
    if not _validate_text_data(
        predictions, "flesch_kincaid", predictions_col_specifier
    ):
        return
...
def _ari_eval_fn(predictions, targets=None, metrics=None):
    ...
    scores = [
        textstat.automated_readability_index(prediction) for prediction in predictions
    ]
    return MetricValue(
        scores=scores,
        aggregate_results=standard_aggregations(scores),
...
def _rouge1_eval_fn(predictions, targets=None, metrics=None):
    if not _validate_text_data(
        targets, "rouge1", targets_col_specifier
    ) or not _validate_text_data(predictions, "rouge1", predictions_col_specifier):
        return
...
def _rouge2_eval_fn(predictions, targets=None, metrics=None):
    if not _validate_text_data(
        targets, "rouge2", targets_col_specifier
    ) or not _validate_text_data(predictions, "rouge2", predictions_col_specifier):
        return
...
def _rougeL_eval_fn(predictions, targets=None, metrics=None):
    if not _validate_text_data(
        targets, "rougeL", targets_col_specifier
    ) or not _validate_text_data(predictions, "rougeL", predictions_col_specifier):
        return
...
def _rmse_eval_fn(predictions, targets=None, metrics=None, sample_weight=None):
    if targets is not None and len(targets) != 0:
        ...
        rmse = mean_squared_error(
            targets, predictions, squared=False, sample_weight=sample_weight
        )
...
def _mape_eval_fn(predictions, targets=None, metrics=None, sample_weight=None):
    if targets is not None and len(targets) != 0:
        ...
        mape = mean_absolute_percentage_error(
            targets, predictions, sample_weight=sample_weight
        )
...
def _precision_at_k_eval_fn(k):
    ...
    def _fn(predictions, targets):
        ...
        ) or not _validate_array_like_id_data(
        ...
        return MetricValue(
            scores=scores, aggregate_results=standard_aggregations(scores)
        )
...
def _expand_duplicate_retrieved_docs(predictions, targets):
    ...
    for doc_id in predictions:
        if doc_id not in counter:
            ...
            counter[doc_id] += 1
            new_doc_id = f"{doc_id}_bc574ae_{counter[doc_id]}"  # adding a random string to avoid collisions
...
def _prepare_row_for_ndcg(predictions, targets):
    ...
    n_labels = max(
        len(doc_id_to_index), 2
    )  # sklearn.metrics.ndcg_score requires at least 2 labels
...
def _ndcg_at_k_eval_fn(k):
    ...
    def _fn(predictions, targets):
        ...
        if not _validate_array_like_id_data(
            ...
            targets, "ndcg_at_k", targets_col_specifier
        ):
            return
...
def _recall_at_k_eval_fn(k):
    ...
    def _fn(predictions, targets):
        if not _validate_array_like_id_data(
            ...
            targets, "recall_at_k", targets_col_specifier
        ):
            return
