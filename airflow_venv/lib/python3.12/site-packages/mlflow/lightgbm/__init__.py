...
def save_model(
    ...
    model_data_subpath = (
        "model.lgb" if isinstance(lgb_model, lgb.Booster) else "model.pkl"
    )
...
def _patch_metric_names(metric_dict):
    ...
    patched_metrics = {
        metric_name.replace("@", "_at_"): value
...
def autolog(
                    ...
                    if valid_names is None:
                        for valid_set in valid_sets:
                            _log_lightgbm_dataset(
                                valid_set, source, "eval", autologging_client
                            )
                    else:
    ...
    def train(_log_models, _log_datasets, original, *args, **kwargs):
        with _SklearnTrainingSession(
            estimator=lightgbm.train, allow_children=False
        ) as t:
            if t.should_log():
