...
<html lang="en">
  ...
  <body>
    ...
    MlflowClient().log_text(
        run_id, estimator_html_string, artifact_file="estimator.html"
    )
...
def _create_child_runs_for_parameter_search(  # noqa: D417
    ...
    if max_tuning_runs is None:
        ...
        cv_results_best_n_df = cv_results_df.nsmallest(
    ...
    for _, result_row in cv_results_best_n_df.iterrows():
        ...
        autologging_client.set_terminated(
            run_id=pending_child_run_id, end_time=child_run_end_time
        )
...
def _backported_all_estimators(type_filter=None):
    ...
    if type_filter is not None:
        ...
        type_filter = (
            list(type_filter) if isinstance(type_filter, list) else [type_filter]
        )
        filtered_estimators = []
        filters = {
