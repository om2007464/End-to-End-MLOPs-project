...
def update_and_persist_config(profile, databricks_config):  # noqa: D417
    ...
    _set_option(
        raw_config, profile, JOBS_API_VERSION, databricks_config.jobs_api_version
    )
...
def set_config_provider(provider):
    ...
    if provider and not isinstance(provider, DatabricksConfigProvider):
        raise Exception(
            f"Must be instance of DatabricksConfigProvider: {_config_provider}"
        )
...
class SparkTaskContextConfigProvider(DatabricksConfigProvider):
    ...
    def set_insecure(x):
        ...
        SparkContext._active_spark_context.setLocalProperty(
            "spark.databricks.ignoreTls", new_val
        )
...
class ProfileConfigProvider(DatabricksConfigProvider):
    ...
    def get_config(self):
        ...
        jobs_api_version = _get_option_if_exists(
            raw_config, self.profile, JOBS_API_VERSION
        )
...
class DatabricksModelServingConfigProvider(DatabricksConfigProvider):
    ...
    def get_config(self):
        from mlflow.utils.databricks_utils import (
            should_fetch_model_serving_environment_oauth,
        )

        try:
            if should_fetch_model_serving_environment_oauth():
                config = (
                    DatabricksModelServingConfigProvider._get_databricks_model_serving_config()
                )
                if config.is_valid:
                    return config
            else:
            ...
            _logger.warning(
                "Unexpected error resolving Databricks Model Serving config: %s", e
            )
...
class DatabricksConfig:
    ...
    @classmethod
    def from_token(
        cls, host, token, refresh_token=None, insecure=None, jobs_api_version=None
    ):
        return DatabricksConfig(
            host=host,
            username=None,
    ...
    @classmethod
    def from_password(
        cls, host, username, password, insecure=None, jobs_api_version=None
    ):
        return DatabricksConfig(
            host=host,
            username=username,
