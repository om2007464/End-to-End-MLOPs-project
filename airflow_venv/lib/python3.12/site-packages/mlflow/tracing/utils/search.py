...
def traces_to_df(traces: List[Trace]) -> "pandas.DataFrame":
    ...
    return pd.DataFrame.from_records(
        data=rows, columns=Trace.pandas_dataframe_columns()
    )
...
def extract_span_inputs_outputs(
    ...
    if isinstance(traces, pd.DataFrame):
        return _extract_from_traces_pandas_df(
            df=traces, col_name=col_name, fields=parsed_fields
        )
...
class _FieldParser:
    ...
    def parse(self) -> _ParsedField:
        ...
        return _ParsedField(
            span_name=span_name, field_type=field_type, field_name=field_name
        )
