...
def set_llama_index_tracer():
    ...
    for handler in dsp.span_handlers:
        if isinstance(handler, MlflowSpanHandler):
            _logger.debug(
                "MlflowSpanHandler is already set to the dispatcher. Skip setting."
            )
    ...
    for handler in dsp.event_handlers:
        if isinstance(handler, MlflowEventHandler):
            _logger.debug(
                "MlflowEventHandler is already set to the dispatcher. Skip setting."
            )
            break
    else:
...
def remove_llama_index_tracer():
    ...
    dsp = get_dispatcher()
    dsp.span_handlers = [
        h for h in dsp.span_handlers if h.class_name() != "MlflowSpanHandler"
    ]
    dsp.event_handlers = [
        h for h in dsp.event_handlers if h.class_name() != "MlflowEventHandler"
    ]
...
def _end_span(span: LiveSpan, status=SpanStatusCode.OK, outputs=None):
    ...
    if isinstance(
        outputs, (StreamingResponse, AsyncStreamingResponse, StreamingAgentChatResponse)
    ):
        _logger.warning(
    ...
    if (
        ...
        try:
            outputs = [
                Document.from_llama_index_node_with_score(node) for node in outputs
            ]
        except Exception as e:
            _logger.debug(
                f"Failed to convert NodeWithScore to Document objects: {e}",
                exc_info=True,
            )
    ...
    if span.parent_id is None:
        ...
        MlflowClient().end_span(
            span.request_id, span.span_id, status=status, outputs=outputs
        )
        ...
        span.set_attributes(
            {
                ...
                "template_arguments": {
                    var: template_args.get(var) for var in template_args
                },
            }
        )
...
class StreamResolver:
    ...
    def is_streaming_result(self, result: Any) -> bool:
        return (
            ...
            or (
                isinstance(result, TaskStepOutput)
    ...
    def resolve(self, span: LiveSpan, event: _StreamEndEvent):
        ...
        if isinstance(event, (LLMChatEndEvent, LLMCompletionEndEvent)):
            ...
            raise ValueError(
                f"Unsupported event type to resolve streaming: {type(event)}"
            )
        ...
        while span.parent_id in self._span_id_to_span_and_gen:
            if span_and_stream := self._span_id_to_span_and_gen.pop(
                span.parent_id, None
            ):
