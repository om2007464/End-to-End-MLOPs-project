...
def save_model(
    ...
    if signature is None and saved_example is not None:
        wrapped_model = create_pyfunc_wrapper(
            llama_index_model, engine_type, model_config
        )
    ...
    if not isinstance(model_code_path, str) and isinstance(
        llama_index_model, BaseIndex
    ):
...
def _load_llama_model(path, flavor_conf):
    ...
    if model_code_path := pyfunc_flavor_conf.get(MODEL_CODE_PATH):
        ...
        model_config = pyfunc_flavor_conf.get(MODEL_CONFIG) or flavor_conf.get(
            MODEL_CONFIG, {}
        )
        if isinstance(model_config, str):
