...
def _format_predict_input_query_engine_and_retriever(data) -> "QueryBundle":
    ...
    if isinstance(data, str):
        ...
        prediction_input = [
            _format_predict_input_query_engine_and_retriever(d) for d in data
        ]
...
class ChatEngineWrapper(_LlamaIndexModelWrapperBase):
    ...
    def _format_predict_input(self, data) -> Union[str, Dict, List]:
        ...
        if isinstance(data, str):
            ...
            return (
                prediction_input if len(prediction_input) > 1 else prediction_input[0]
            )
        else:
            raise ValueError(
                f"Unsupported input type: {type(data)}. It must be one of "
...
class WorkflowWrapper(_LlamaIndexModelWrapperBase):
    ...
    async def _predict_single(self, x: Dict[str, Any]) -> Any:
        if not isinstance(x, dict):
            raise ValueError(
                f"Unsupported input type: {type(x)}. It must be a dictionary."
            )
...
def _create_wrapper_from_engine(
    engine: Any, model_config: Optional[Dict[str, Any]] = None
):
...
def _create_wrapper_from_workflow(
    workflow: Any, model_config: Optional[Dict[str, Any]] = None
):
