        ...
        if config.log_traces and request_id:
            try:
                ...
                mlflow_client.end_trace(
                    request_id=request_id, status=SpanStatusCode.ERROR
                )
            except Exception as inner_e:
                _logger.warning(
                    f"Encountered unexpected error when ending trace: {inner_e}"
                )
        raise e
    ...
    if isinstance(result, Stream):
        ...
        def _stream_output_logging_hook(stream: Iterator) -> Iterator:
            ...
            try:
                ...
                _logger.warning(
                    f"Encountered unexpected error during openai autologging: {e}"
                )
