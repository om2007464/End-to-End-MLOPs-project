...
if len(installed_targets) > 0:
    supported_targets_msg = (
        "Support is currently installed for deployment to: {targets}".format(
...
deployment_name = click.option(
    "--name", "name", required=True, help="Name of the deployment"
)
...
required_endpoint_param = click.option(
    "--endpoint", required=True, help="Name of the endpoint"
)
...
def create_deployment(flavor, model_uri, target, name, config, endpoint):
    ...
    if "endpoint" in sig.parameters:
        ...
        deployment = client.create_deployment(
            name, model_uri, flavor, config=config_dict
        )
    click.echo(
...
def update_deployment(flavor, model_uri, target, name, config, endpoint):
    ...
    if "endpoint" in sig.parameters:
        ...
        ret = client.update_deployment(
            name, model_uri=model_uri, flavor=flavor, config=config_dict
        )
