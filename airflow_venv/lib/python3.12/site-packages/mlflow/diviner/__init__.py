...
def save_model(
    ...
    **kwargs,
    ...
    flavor_conf.update(
        {_MODEL_TYPE_KEY: diviner_model.__class__.__name__}, **model_bin_kwargs
    )
...
def _load_model_fit_in_spark(local_model_path: str, flavor_conf, **kwargs):
    ...
    dfs_temp_directory = generate_tmp_dfs_path(
        kwargs.get("dfs_tmpdir", MLFLOW_DFS_TMP.get())
    )
    dfs_fuse_directory = dbfs_hdfs_uri_to_fuse_path(dfs_temp_directory)
    os.makedirs(dfs_fuse_directory)
    shutil_copytree_without_file_permissions(
        src_dir=local_model_path, dst_dir=dfs_fuse_directory
    )
    ...
    def predict(
        ...
        if isinstance(self.diviner_model, GroupedProphet):
            ...
            if not predict_groups:
                prediction_df = self.diviner_model.forecast(
                    horizon=n_periods, frequency=frequency
                )
            else:
                group_kwargs = {
                    k: v for k, v in conf.items() if k in {"predict_col", "on_error"}
                }
            ...
            if not predict_groups:
                prediction_df = self.diviner_model.predict(
                    n_periods=n_periods, **predict_conf
                )
            else:
                prediction_df = self.diviner_model.predict_groups(
