...
def _load_model_from_config(path, model_config):
    from langchain.chains.loading import (
        type_to_loader_dict as chains_type_to_loader_dict,
    )
    ...
    config_path = os.path.join(
        path, model_config.get(_MODEL_DATA_KEY, _MODEL_DATA_YAML_FILE_NAME)
    )
...
def _load_runnable_with_steps(file_path: Union[Path, str], model_type: str):
    ...
    if model_type == RunnableSequence.__name__:
        steps = [
            value for _, value in sorted(steps.items(), key=lambda item: int(item[0]))
        ]
...
def runnable_sequence_from_steps(steps):
    ...
    if len(steps) < 2:
        raise ValueError(
            f"RunnableSequence must have at least 2 steps, got {len(steps)}."
        )
...
def _save_runnable_with_steps(
    model, file_path: Union[Path, str], loader_fn=None, persist_dir=None
):  # noqa: D417
...
def _save_runnable_binding(model, file_path, loader_fn=None, persist_dir=None):
    ...
    model_config["bound"] = _save_internal_runnables(
        model.bound, save_path, loader_fn, persist_dir
    )
...
def _load_runnables(path, conf):
    ...
    raise MlflowException.invalid_parameter_value(
        get_unsupported_model_message(model_type)
    )
