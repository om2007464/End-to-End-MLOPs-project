...
def _get_embedding_model_endpoint_names(index):
    ...
    for column in embedding_source_columns:
        embedding_model_endpoint_name = column.get(
...
def _extract_databricks_dependencies_from_tools(
    tools,
        ...
        for tool in tools:
            if _isinstance_with_multiple_modules(
                ...
                if hasattr(tool.func, "keywords") and "retriever" in tool.func.keywords:
                    ...
                    if (
                        "self" in nonlocal_vars
                        and _is_langchain_community_uc_function_toolkit(
...
def _extract_databricks_dependencies_from_tool_nodes(
    tool_node,
) -> Generator[Resource, None, None]:
    try:
...
def _extract_dependency_list_from_lc_model(lc_model) -> Generator[Resource, None, None]:
    ...
    for attr_name in _LEGACY_MODEL_ATTR_SET:
        yield from _extract_dependency_list_from_lc_model(
            getattr(lc_model, attr_name, None)
        )
