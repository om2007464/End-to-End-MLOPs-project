    ...
    if config.should_log_optional_artifacts():
        with _setup_autolog_run(config, self) as run_id:
            ...
            _log_optional_artifacts(
                config, run_id, result, self, func_name, *args, **kwargs
            )
    else:
        result = _invoke(self, *args, **kwargs)
    return result
...
def _get_args_with_mlflow_tracer(func_name, args, kwargs):
    ...
    if func_name in ["invoke", "batch", "stream", "ainvoke", "abatch", "astream"]:
        ...
        _logger.warning(
            f"Unsupported function `{func_name}`. Skipping injecting MLflow callbacks."
        )
        return args, kwargs
...
def _get_runnable_config_with_callback(
    ...
    if original_config is None:
        ...
        return [
            _get_runnable_config_with_callback(c, new_callback) for c in original_config
        ]
...
def _inject_callback(
    ...
    if isinstance(original_callbacks, BaseCallbackManager):
        ...
        if not any(
            isinstance(cb, type(new_callback)) for cb in callback_manager_copy.handlers
        ):
...
def _setup_autolog_run(config, model):
    ...
    if propagated_run_id := getattr(model, "run_id", None):
        ...
        mlflow.MlflowClient().log_batch(
            run_id, tags=[RunTag(k, str(v)) for k, v in tags.items()]
        )
        should_terminate_run = False
    else:
...
def _resolve_tags(extra_tags, active_run=None):
    ...
    if active_run:
        ...
        excluded_tags = {
            tag for tag in active_run.data.tags.keys() if tag.startswith("mlflow.")
        }
...
def _get_input_data_from_function(func_name, model, args, kwargs):
    ...
    if param_name := func_param_name_mapping.get(func_name):
        ...
        if (
            next(iter(inspect.signature(inference_func).parameters.keys()))
            != param_name
        ):
...
def _runnable_with_retriever(model):
    ...
    with contextlib.suppress(ImportError):
        ...
        if isinstance(model, RunnableBranch):
            return any(
                _runnable_with_retriever(runnable) for _, runnable in model.branches
            )
        ...
        if isinstance(model, RunnableSequence):
            return any(
                _runnable_with_retriever(runnable)
                for runnable in get_runnable_steps(model)
            )
...
def _log_optional_artifacts(
    autolog_config, run_id, result, self, func_name, *args, **kwargs
):
    input_example = None
