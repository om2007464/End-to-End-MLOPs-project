...
def save_model(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        lc_model_or_path = _validate_and_prepare_lc_model_or_path(
            lc_model, loader_fn, temp_dir
        )
    ...
    if (
        ...
        and mlflow_model.resources is None
    ):
...
class _LangChainModelWrapper:
    ...
    def predict(
        ...
        if (
            ...
            and os.environ.get("MLFLOW_ENABLE_TRACE_IN_SERVING", "false").lower()
            == "true"
            ...
            and (
                tracer := next(
                    (
                        ...
                        if isinstance(c, MlflowLangchainTracer)
                    ),
                    None,
                )
            )
...
def _load_pyfunc(
    path: str, model_config: Optional[Dict[str, Any]] = None
):  # noqa: D417
...
def _load_model_from_local_fs(local_model_path, model_config_overrides=None):
    ...
    if MODEL_CODE_PATH in pyfunc_flavor_conf or MODEL_CODE_PATH in flavor_conf:
        model_config = pyfunc_flavor_conf.get(
        ...
        try:
            model = _load_model_code_path(
                model_code_path,
                {**(model_config or {}), **(model_config_overrides or {})},
            )
        finally:
        ...
        try:
            for _, obj in inspect.getmembers(importlib.import_module(module_name)):
                if inspect.ismodule(obj) and (
                    obj.__name__.startswith("langchain")
                    or obj.__name__.startswith("langgraph")
                ):
                    _inspect_module_and_patch_cls(
                        obj.__name__, inspected_modules, patched_classes
                    )
                elif (
                    inspect.isclass(obj)
    ...
    with contextlib.suppress(ImportError):
        ...
        for pkg in importlib.metadata.distributions():
            if pkg.metadata["Name"].startswith("langchain"):
                ...
                _inspect_module_and_patch_cls(
                    module_name, inspected_modules, patched_classes
                )
            ...
            if pkg.metadata["Name"] == "langgraph":
                ...
                for submodule in langgraph_submodules:
                    _inspect_module_and_patch_cls(
                        submodule, inspected_modules, patched_classes
                    )
