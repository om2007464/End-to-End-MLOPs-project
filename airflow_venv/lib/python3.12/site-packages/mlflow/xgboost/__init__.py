...
def autolog(
    ...
    def train_impl(_log_models, _log_datasets, original, *args, **kwargs):
        ...
        def log_feature_importance_plot(features, importance, importance_type):
            ...
            colors_to_cycle = plt.rcParams["axes.prop_cycle"].by_key()["color"][
                :num_classes
            ]
            ...
            offsets_per_yloc = (
                np.linspace(-0.5, 0.5, num_classes) / 2 if num_classes > 1 else [0]
            )
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                try:
                    filepath = os.path.join(
                        tmpdir, f"feature_importance_{imp_type}.png"
                    )
        ...
        if _log_datasets and dtrain is not None:
            try:
                ...
                if evals is not None:
                    for d, name in evals:
                        _log_xgboost_dataset(
                            d, source, "eval", autologging_client, name
                        )
        ...
        for imp_type in importance_types:
            ...
            if imp is not None:
                with tempfile.TemporaryDirectory() as tmpdir:
                    filepath = os.path.join(
                        tmpdir, f"feature_importance_{imp_type}.json"
                    )
