...
def save_model(
    ...
    if isinstance(model, KerasModel):
        ...
        if custom_objects is not None:
            _save_keras_custom_objects(
                data_path, custom_objects, _CUSTOM_OBJECTS_SAVE_PATH
            )
    ...
    include_cloudpickle = (
        custom_objects is not None or get_global_custom_objects() is not None
    )
...
def _load_keras_model(model_path, keras_module, save_format, **kwargs):
    ...
    if saved_custom_objects := _load_custom_objects(
        model_path, _CUSTOM_OBJECTS_SAVE_PATH
    ):
        saved_custom_objects.update(custom_objects)
        custom_objects = saved_custom_objects

    if global_custom_objects := _load_custom_objects(
        model_path, _GLOBAL_CUSTOM_OBJECTS_SAVE_PATH
    ):
...
def load_model(
    model_uri, dst_path=None, saved_model_kwargs=None, keras_model_kwargs=None
):
    """
    ...
    if model_type == _MODEL_TYPE_KERAS:
        ...
        keras_module = importlib.import_module(
            flavor_conf.get("keras_module", "tensorflow.keras")
        )
    ...
    if model_type == _MODEL_TYPE_TF1_ESTIMATOR:
        tf_saved_model_dir = os.path.join(
            local_model_path, flavor_conf["saved_model_dir"]
        )
...
def _load_tf1_estimator_saved_model(
...
def _load_pyfunc(path):
    ...
    if os.path.isfile(model_meta_path1):
        ...
        raise MlflowException(
            f"Cannot find file {MLMODEL_FILE_NAME} for the logged model."
        )
    ...
    if model_type == _MODEL_TYPE_TF1_ESTIMATOR:
        ...
        return _TF2Wrapper(
            model=loaded_model, infer=loaded_model.signatures[tf_signature_def_key]
        )
    if model_type == _MODEL_TYPE_TF2_MODULE:
        flavor_conf = _get_flavor_configuration(path, FLAVOR_NAME)
        tf_saved_model_dir = os.path.join(path, flavor_conf["saved_model_dir"])
    ...
    def predict(
        ...
        if isinstance(data, pandas.DataFrame):
            ...
            return pandas.DataFrame(
                self.keras_model.predict(data.values), index=data.index
            )
...
def _setup_callbacks(callbacks, log_every_epoch, log_every_n_steps):
    ...
    model_checkpoint = get_autologging_config(
        mlflow.tensorflow.FLAVOR_NAME, "checkpoint", True
    )
    if model_checkpoint:
    ...
    **tf.keras.callbacks.EarlyStopping**
    ...
    def _log_keras_model(history, args):
        ...
        def _get_tf_keras_input_example_slice():
            ...
            keras_input_example_slice = extract_tf_keras_input_example(
                input_training_data
            )
        ...
        def _patch_implementation(self, original, inst, *args, **kwargs):
            ...
            try:
                ...
                if isinstance(training_data, tf.data.Dataset) and hasattr(
                    ...
                    batch_size = (
                        len(peek[0]) if is_single_input_model else len(peek[0][0])
                    )
...
def _log_tensorflow_dataset(
    tensorflow_dataset, source, context, name=None, targets=None
):
    import tensorflow as tf
