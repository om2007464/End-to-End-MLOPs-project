...
class LiveSpan(Span):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Span":
        raise NotImplementedError(
            "The `from_dict` method is not supported for the LiveSpan class."
        )
...
class _SpanAttributesRegistry:
    ...
    def set(self, key: str, value: Any):
        if not isinstance(key, str):
            _logger.warning(
                f"Attribute key must be a string, but got {type(key)}. Skipping."
            )
            return
        ...
        self._span.set_attribute(
            key, json.dumps(value, cls=TraceJSONEncoder, ensure_ascii=False)
        )
...
class _CachedSpanAttributesRegistry(_SpanAttributesRegistry):
    ...
    def set(self, key: str, value: Any):
        raise MlflowException(
            "The attributes of the immutable span must not be updated.",
            INVALID_PARAMETER_VALUE,
        )
