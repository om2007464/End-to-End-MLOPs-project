...
class SparkMLVector(Array):
    ...
    def _merge(self, arr: Array) -> Array:
        if isinstance(arr, SparkMLVector):
            return deepcopy(self)
        raise MlflowException(
            "SparkML vector type can't be merged with another Array type."
        )
...
class ColSpec:
    ...
    def __init__(
        ...
        required: Optional[
            bool
        ] = None,  # TODO: update to required=True after deprecating optional
    ):
        self._name = name
    ...
    def from_json_dict(cls, **kwargs):
        ...
        if kwargs["type"] == ARRAY_TYPE:
            return cls(
                name=name,
                type=Array.from_json_dict(**kwargs),
                optional=optional,
                required=required,
            )
        if kwargs["type"] == OBJECT_TYPE:
        ...
        if kwargs["type"] == MAP_TYPE:
            return cls(
                name=name,
                type=Map.from_json_dict(**kwargs),
                optional=optional,
                required=required,
            )
        if kwargs["type"] == SPARKML_VECTOR_TYPE:
            return cls(
                name=name, type=SparkMLVector(), optional=optional, required=required
            )
