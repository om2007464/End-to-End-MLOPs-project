...
def put_adls_file_creation(sas_url, headers):
    ...
    for name, value in headers.items():
        if _is_valid_adls_put_header(name):
            ...
            _logger.debug(
                "Removed unsupported '%s' header for ADLS Gen2 Put operation", name
            )
...
def patch_adls_file_upload(
    sas_url, local_file, start_byte, size, position, headers, is_single
):
    """
    ...
    for name, value in headers.items():
        if _is_valid_adls_patch_header(name):
            ...
            _logger.debug(
                "Removed unsupported '%s' header for ADLS Gen2 Patch operation", name
            )
...
def patch_adls_flush(sas_url, position, headers):
    ...
    request_url = _append_query_parameters(
        sas_url, {"action": "flush", "position": str(position)}
    )
