...
def _deploy(
    ...
    sage_client = boto3.client(
        "sagemaker", region_name=region_name, **assume_role_credentials
    )

    endpoint_exists = (
        _find_endpoint(endpoint_name=app_name, sage_client=sage_client) is not None
    )
...
def _delete(
    ...
    def cleanup_fn():
        ...
        _logger.info(
            "Deleted associated endpoint configuration with arn: %s", config_arn
        )
    ...
    delete_operation = _SageMakerOperation(
...
def terminate_transform_job(
    ...
    def status_check_fn():
        transform_job_info = _find_transform_job(
            job_name=job_name, sage_client=sage_client
        )
    ...
    stop_operation = _SageMakerOperation(
        status_check_fn=status_check_fn, cleanup_fn=cleanup_fn
    )

    if synchronous:
        _logger.info("Waiting for the termination operation to complete...")
        operation_status = stop_operation.await_completion(
            timeout_seconds=timeout_seconds
        )
        if operation_status.state == _SageMakerOperationStatus.STATE_SUCCEEDED:
            _logger.info(
...
def _get_default_image_url(region_name):
    ...
    repository_conf = ecr_client.describe_repositories(
        repositoryNames=[DEFAULT_IMAGE_NAME]
    )["repositories"][0]
    return (repository_conf["repositoryUri"] + ":{version}").format(
...
def _upload_s3(
    local_model_path, bucket, prefix, region_name, s3_client, **assume_role_credentials
):  # noqa: D417
    """
...
def _prepare_sagemaker_tags(
    ...
    parsed = [
        {"Key": key, "Value": str(value)} for key, value in sagemaker_tags.items()
    ]
...
def _create_sagemaker_transform_job(
    ...
    transform_input = {
        "DataSource": {
            "S3DataSource": {"S3DataType": s3_input_data_type, "S3Uri": s3_input_uri}
        },
    ...
    def status_check_fn():
        transform_job_info = sage_client.describe_transform_job(
            TransformJobName=job_name
        )
...
def _create_sagemaker_endpoint(  # noqa: D417
    ...
    def status_check_fn():
        endpoint_info = _find_endpoint(
            endpoint_name=endpoint_name, sage_client=sage_client
        )
...
def _delete_sagemaker_endpoint_configuration(
    endpoint_config_name, sage_client
):  # noqa: D417
    """
    Args:
        sage_client: A boto3 client for SageMaker.
...
def _find_transform_job(job_name, sage_client):  # noqa: D417
    ...
    transform_jobs_page = sage_client.list_transform_jobs(
        MaxResults=100, NameContains=job_name
    )
...
class _SageMakerOperation:
    def __init__(self, status_check_fn, cleanup_fn):
        ...
        self.status = _SageMakerOperationStatus(
            _SageMakerOperationStatus.STATE_IN_PROGRESS, None
        )
