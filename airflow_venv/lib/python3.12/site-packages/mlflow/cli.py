...
def gc(older_than, backend_store_uri, artifacts_destination, run_ids, experiment_ids):
    ...
    if not skip_experiments:
        if experiment_ids:
            ...
            active_experiment_ids = [
                ...
                if e.lifecycle_stage != LifecycleStage.DELETED
            ]
            ...
            if older_than:
                non_old_experiment_ids = [
                    e.experiment_id
                    for e in experiments
                    if e.last_update_time is None
                    or e.last_update_time >= time_threshold
                ]
            ...
            filter_string = (
                f"last_update_time < {time_threshold}" if older_than else None
            )
    ...
    if not skip_experiments:
        for experiment_id in experiment_ids:
            ...
            click.echo(
                f"Experiment with ID {experiment_id} has been permanently deleted."
            )


@cli.command(
    short_help="Prints out useful information for debugging issues with MLflow."
)
@click.option(
    "--mask-envs",
    is_flag=True,
