...
def _do_pipeline_transform(
    df: DataFrame, transformer: Union[Transformer, PipelineModel]
):  # noqa: D417
    """
...
def _get_struct_type_by_cols(
    input_fields: Set[str], df_schema: t.StructType
) -> t.StructType:
    """
    Args:
    ...
    if len(input_fields) > 0:
        return t.StructType(
            [_field for _field in df_schema.fields if _field.name in input_fields]
        )
    return []
...
def get_feature_cols(
    ...
    for column in df.columns:
        try:
            ...
            if re.search(
                "does not exist|no such struct field", str(iae), re.IGNORECASE
            ):
