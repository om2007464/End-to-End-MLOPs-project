        ...
        with databricks_utils.MlflowCredentialContext(
            ...
            try:
                ...
                raise MlflowException(
                    "failed to save spark model via mlflowdbfs"
                ) from e
...
class _HadoopFileSystem:
    ...
    def _fs(cls):
        if not cls._filesystem:
            cls._filesystem = cls._jvm().org.apache.hadoop.fs.FileSystem.get(
                cls._conf()
            )
    ...
    def copy_to_local_file(cls, src, dst, remove_src):
        cls._fs().copyToLocalFile(
            remove_src, cls._remote_path(src), cls._local_path(dst)
        )

    @classmethod
    def copy_from_local_file(cls, src, dst, remove_src):
        cls._fs().copyFromLocalFile(
            remove_src, cls._local_path(src), cls._remote_path(dst)
        )
    ...
    def maybe_copy_from_uri(cls, src_uri, dst_path, local_model_path=None):
        ...
        _logger.info(
            "File '%s' not found on DFS. Will attempt to upload the file.", src_uri
        )
...
def _should_use_mlflowdbfs(root_uri):
    ...
    try:
        _get_active_spark_session().read.load(
            "mlflowdbfs:///artifact?run_id=foo&path=/bar"
        )
    except Exception as e:
...
def _save_model_metadata(
    ...
    if pip_constraints:
        write_to(
            os.path.join(dst_dir, _CONSTRAINTS_FILE_NAME), "\n".join(pip_constraints)
        )

    # Save `requirements.txt`
    write_to(
        os.path.join(dst_dir, _REQUIREMENTS_FILE_NAME), "\n".join(pip_requirements)
    )
...
def _load_model(model_uri, dfs_tmpdir_base=None, local_model_path=None):
    ...
    model_uri = _HadoopFileSystem.maybe_copy_from_uri(
        model_uri, dfs_tmpdir, local_model_path
    )
...
def load_model(model_uri, dfs_tmpdir=None, dst_path=None):
    ...
    if model_class is not None and model_class.startswith("pyspark.ml.connect."):
        spark_model_local_path = os.path.join(
            local_mlflow_model_path, flavor_conf["model_data"]
        )
    ...
    local_sparkml_model_path = os.path.join(
        local_mlflow_model_path, flavor_conf["model_data"]
    )
    return _load_model(
            ...
            for col_spec in self.signature.outputs.inputs:
                if isinstance(col_spec.type, SparkMLVector):
                    ...
                    prediction_df = prediction_df.withColumn(
                        col_name, vector_to_array(col_name)
                    )
...
def autolog(disable=False, silent=False):
    ...
    safe_patch(
        FLAVOR_NAME, SparkSession, "stop", patched_session_stop, manage_run=False
    )
