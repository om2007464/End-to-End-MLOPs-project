...
MIN_REQ_VERSION = Version(
    _ML_PACKAGE_VERSIONS["pytorch-lightning"]["autologging"]["minimum"]
)
MAX_REQ_VERSION = Version(
    _ML_PACKAGE_VERSIONS["pytorch-lightning"]["autologging"]["maximum"]
)
...
class __MlflowPLCallback(pl.Callback, metaclass=ExceptionSafeAbstractClass):
    ...
    def on_train_batch_end(self, trainer, pl_module, *args):
        ...
        metric_items = [
            ...
            if (name not in self._epoch_metrics)
            and (f"{name}_step" not in metrics.keys())
        ]
        self._step_metrics.update(name for (name, _) in metric_items)
        step = trainer.global_step
        if (
            (step // self._global_steps_per_training_step) + 1
        ) % self.log_every_n_step == 0:
...
def _log_early_stop_metrics(early_stop_callback, client, run_id):
    ...
    metrics = {
        ...
        "restored_epoch": early_stop_callback.stopped_epoch
        - max(1, early_stop_callback.patience),
    }
...
def patched_fit(original, self, *args, **kwargs):
    ...
    with _pytorch_autolog.disable_pytorch_autologging():
        ...
        log_models = get_autologging_config(
            mlflow.pytorch.FLAVOR_NAME, "log_models", True
        )
        ...
        if log_models:
            ...
            if (
                early_stop_callback is not None
