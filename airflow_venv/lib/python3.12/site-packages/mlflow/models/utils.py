...
class _Example:
    ...
    def __init__(self, input_example: ModelInputExample):
        ...
        if isinstance(model_input, dict):
            ...
            if any(isinstance(values, np.ndarray) for values in model_input.values()):
                if not all(
                    isinstance(values, np.ndarray) for values in model_input.values()
                ):
        ...
        if self.serving_input:
            self.json_serving_input = json.dumps(
                self.serving_input, cls=NumpyEncoder, indent=2
            )
    ...
    def save(self, parent_dir_path: str):
        ...
        if self.json_serving_input:
            with open(
                os.path.join(parent_dir_path, self.info[SERVING_INPUT_PATH]), "w"
            ) as f:
    ...
    try:
        ...
        try:
            ...
            response = w.workspace.export(
                path=model_code_path, format=ExportFormat.SOURCE
            )
...
def _load_model_code_path(
    code_path: str, model_config: Optional[Union[str, Dict[str, Any]]]
):
