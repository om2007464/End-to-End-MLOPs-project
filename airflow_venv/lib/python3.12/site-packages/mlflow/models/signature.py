        ...
        for arg in [arg_name, "return"]:
            if hint_str := f.__annotations__.get(arg, None):
                if hint := _infer_hint_from_str(hint_str):
                    ...
                    _logger.info(
                        "Unsupported type hint: %s, skipping schema inference", hint_str
                    )
    except Exception as e:
        _logger.warning(
            "Failed to extract type hints from function %s: %s", f.__name__, repr(e)
        )
        return _TypeHints()
...
def _infer_signature_from_type_hints(func, input_arg_index, input_example=None):
    ...
    input_schema = (
        _infer_schema_from_type_hint(hints.input, input_example)
        if hints.input
        else None
    )
    ...
    if input_example:
        ...
        output_example = None
    output_schema = (
        _infer_schema_from_type_hint(hints.output, output_example)
        if hints.output
        else None
    )
