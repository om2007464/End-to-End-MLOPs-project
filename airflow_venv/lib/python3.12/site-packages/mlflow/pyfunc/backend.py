...
class PyFuncBackend(FlavorBackend):
    ...
    def __init__(  # noqa: D417
        ...
        **kwargs,
        ...
        self._env_id = (
            os.environ.get("MLFLOW_HOME", VERSION) if install_mlflow else None
        )
    ...
    def prepare_env(
        self, model_uri, capture_output=False, pip_requirements_override=None
    ):
        ...
        if self._create_env_root_dir:
            if self._env_root_dir is not None:
                raise Exception(
                    "env_root_dir can not be set when create_env_root_dir=True"
                )
        ...
        if self._env_manager == em.VIRTUALENV:
            ...
            conda_env_path = os.path.join(
                local_path, _extract_conda_env(self._config[ENV])
            )
        ...
        if sys.platform.startswith("linux"):
            ...
            def setup_sigterm_on_parent_death():
                ...
                try:
                    ...
                    #  not support prctl.
                    warnings.warn(
                        f"Setup libc.prctl PR_SET_PDEATHSIG failed, error {e!r}."
                    )
    ...
    def build_image(
        ...
        with TempDir() as tmp:
            ...
            docker_utils.build_image_from_context(
                context_dir=cwd, image_name=image_name
            )
    ...
    def generate_dockerfile(
        ...
        _logger.debug(
            "Created all folders in path", extra={"output_directory": output_dir}
        )
        ...
        if model_uri:
            ...
            if base_image.startswith("python"):
                ...
                if env_manager == em.LOCAL:
                    raise MlflowException.invalid_parameter_value(
                        LOCAL_ENV_MANAGER_ERROR_MESSAGE
                    )
            ...
            entrypoint = (
                f"from mlflow.models import container as C; C._serve('{env_manager}')"
            )
        ...
        # if no model_uri specified, user must use virtualenv or conda env based on ubuntu image
            ...
            entrypoint = (
                self._get_install_pyfunc_deps_cmd(
                    env_manager, install_mlflow, enable_mlserver
                )
        ...
        _logger.debug(
            "generated dockerfile at {output_dir}",
            extra={"dockerfile": dockerfile_text},
        )
    ...
    def _get_base_image(self, model_path: str, install_java: bool) -> str:
        ...
        try:
            env_conf = Model.load(model_path).flavors[pyfunc.FLAVOR_NAME][pyfunc.ENV][
                em.VIRTUALENV
            ]
    ...
    def _model_installation_steps(
        self, model_path, env_manager, install_mlflow, enable_mlserver
    ):
