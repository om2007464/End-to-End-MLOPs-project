...
class __MlflowPaddleCallback(
    paddle.callbacks.Callback, metaclass=ExceptionSafeAbstractClass
):
    ...
    def _log_metrics(self, logs, current_epoch):
        metrics = {
            key: (metric[0] if isinstance(metric, list) else metric)
...
def patched_fit(original, self, *args, **kwargs):
    ...
    log_every_n_epoch = get_autologging_config(
        mlflow.paddle.FLAVOR_NAME, "log_every_n_epoch", 1
    )
