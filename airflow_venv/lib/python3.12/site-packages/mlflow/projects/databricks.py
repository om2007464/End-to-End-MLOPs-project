...
class DatabricksJobRunner:
    ...
    def run_databricks_spark_job(
        ...
        if project_spec.databricks_spark_job_spec.python_file is not None:
            ...
            command = project_spec.get_entry_point(entry_point).compute_command(
                parameters, None
            )
        ...
        ) = _get_project_dir_and_extracting_tar_command(dbfs_fuse_uri)
...
def _get_cluster_mlflow_run_cmd(
    project_dir, run_id, entry_point, parameters, env_manager
):
    cmd = [
        "mlflow",
        "run",
    ...
    if run_id:
        mlflow_run_arr.extend(
            ["-c", json.dumps({MLFLOW_LOCAL_BACKEND_RUN_ID_CONFIG: run_id})]
        )
    if parameters:
...
def _get_project_dir_and_extracting_tar_command(dbfs_fuse_tar_uri):
    ...
    tar_hash = posixpath.splitext(
        posixpath.splitext(posixpath.basename(dbfs_fuse_tar_uri))[0]
    )[0]
...
def _get_databricks_run_cmd(
    dbfs_fuse_tar_uri, run_id, entry_point, parameters, env_manager
):
    """
