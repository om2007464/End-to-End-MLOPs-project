...
def _resolve_experiment_id(experiment_name=None, experiment_id=None):
    ...
    if experiment_name:
        ...
        if exp:
            ...
            _logger.info(
                "'%s' does not exist. Creating a new experiment", experiment_name
            )
...
def _run(
    ...
    if backend_name not in {"databricks", "kubernetes"}:
        ...
        if backend:
            ...
            if run_name is not None:
                tracking.MlflowClient().set_tag(
                    submitted_run.run_id, MLFLOW_RUN_NAME, run_name
                )
    ...
    if run_name is not None:
        tracking.MlflowClient().set_tag(
            active_run.info.run_id, MLFLOW_RUN_NAME, run_name
        )
