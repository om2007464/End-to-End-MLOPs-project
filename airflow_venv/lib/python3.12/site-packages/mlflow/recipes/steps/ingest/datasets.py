...
class _Dataset:
    ...
    @classmethod
    def from_config(
        cls, dataset_config: Dict[str, Any], recipe_root: str
    ) -> "_Dataset":
        """
...
class _LocationBasedDataset(_Dataset):
    ...
    def __init__(
        ...
        self.location = _LocationBasedDataset._sanitize_local_dataset_multiple_locations_if_necessary(
    ...
    def _from_config(
        ...
        return cls(
            location=cls._get_required_config(
                dataset_config=dataset_config, key="location"
            ),
            recipe_root=recipe_root,
            dataset_format=cls._get_required_config(
                dataset_config=dataset_config, key="using"
            ),
        )
    ...
    def _sanitize_local_dataset_multiple_locations_if_necessary(
        ...
        if isinstance(dataset_location, str):
            ...
            raise MlflowException(
                f"Unsupported location type: {type(dataset_location)}"
            )
    ...
    def _sanitize_local_dataset_location_if_necessary(
        ...
        """
        local_dataset_path_or_none = get_local_path_or_none(
            path_or_uri=dataset_location
        )
            ...
            if os.path.isdir(local_dataset_path):
                ...
                if self.dataset_format == "custom":
                    dataset_file_paths = sorted(
                        pathlib.Path(local_dataset_path).glob("*")
                    )
                else:
                    dataset_file_paths = sorted(
                        pathlib.Path(local_dataset_path).glob(
...
class _SparkDatasetMixin:
    ...
    def _convert_spark_df_to_pandas(self, spark_df):
        ...
        pandas_df[datetime_cols] = pandas_df[datetime_cols].apply(
...
class DeltaTableDataset(_SparkDatasetMixin, _LocationBasedDataset):
    ...
    def __init__(
        ...
        """
        super().__init__(
            location=location, dataset_format=dataset_format, recipe_root=recipe_root
        )
