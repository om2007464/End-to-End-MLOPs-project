...
def generate_equal_height_histogram(quantiles, num_buckets: int) -> Histogram:
    ...
    for low_index in range(0, len(quantiles) - step, step):
        ...
        histogram.buckets.append(
            Histogram.Bucket(
                low_value=quantiles[low_index], high_value=quantiles[high_index]
            )
        )
...
def generate_equal_width_histogram(
    quantiles, num_buckets: int, total_freq: float
) -> Histogram:
    """
    ...
    if min_val == max_val:
        ...
        for i in range(num_buckets):
            if i == half_buckets:
                histogram.buckets.append(
                    Histogram.Bucket(
                        low_value=bucket_left,
                        high_value=bucket_left,
                        sample_count=total_freq,
                    )
                )
            else:
                histogram.buckets.append(
...
def _generate_equal_width_histogram_internal(
    ...
    for i in range(len(quantiles) - 1):
        ...
        if quantile_low == quantile_high:
            ...
            quantile_contribution_ratio = overlap_length / (
