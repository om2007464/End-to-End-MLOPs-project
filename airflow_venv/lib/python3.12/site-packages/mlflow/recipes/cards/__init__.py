...
class CardTab:
    ...
    def add_image(
        ...
        if not os.path.exists(image_file_path):
            ...
            _logger.warning(
                f"Unable to locate image file {image_file_path} to render {name}."
            )
            return
...
class BaseCard:
    ...
    def render_table(table, columns=None, hide_index=True):
        ...
        styler = table.set_table_attributes(
        ...
        return (
            f'<div style="max-height: 500px; overflow: scroll;">{rendered_table}</div>'
        )
