...
class RecipeMetric:
    ...
    def __init__(
        self, name: str, greater_is_better: bool, custom_function: Optional[str] = None
    ):
        self.name = name
    ...
    def from_custom_metric_dict(cls, custom_metric_dict):
        ...
        greater_is_better = custom_metric_dict.get(
            RecipeMetric._KEY_METRIC_GREATER_IS_BETTER
        )
...
def _get_error_fn(
    tmpl: str, use_probability: bool = False, positive_class: Optional[str] = None
):  # noqa: D417
    """
    Args:
...
def transform_multiclass_metrics_dict(
    eval_metrics: Dict[str, Any], ext_task
) -> Dict[str, Any]:
    return {
        transform_multiclass_metric(k, ext_task): v for k, v in eval_metrics.items()
    }
...
def _get_custom_metrics(step_config: Dict, ext_task: str) -> List[Dict]:  # noqa: D417
    ...
    custom_metrics = [
        RecipeMetric.from_custom_metric_dict(metric_dict)
        for metric_dict in custom_metric_dicts
    ]
...
def _load_custom_metrics(
    recipe_root: str, metrics: List[RecipeMetric]
) -> List[EvaluationMetric]:
    custom_metrics = [
        metric for metric in metrics if metric.custom_function is not None
    ]
