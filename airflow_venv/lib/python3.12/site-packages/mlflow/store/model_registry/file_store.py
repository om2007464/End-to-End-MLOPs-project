...
def _default_root_dir():
    return MLFLOW_REGISTRY_DIR.get() or os.path.abspath(
        DEFAULT_LOCAL_FILE_AND_ARTIFACT_PATH
    )
...
class FileStore(AbstractStore):
    ...
    def _save_registered_model_as_meta_file(
        self, registered_model, meta_dir=None, overwrite=True
    ):
    ...
    def _get_registered_model_from_path(self, model_path):
        ...
        registered_model.latest_versions = self.get_latest_versions(
    ...
    def get_latest_versions(self, name, stages=None) -> List[ModelVersion]:
        ...
        model_versions = self._list_file_model_versions_under_path(
    ...
    def get_all_registered_model_tags_from_path(self, model_path):
        parent_path, tag_files = self._get_resource_files(
            model_path, FileStore.TAGS_FOLDER_NAME
        )
        tags = []
    ...
    def _get_model_version_tags_from_dir(self, directory) -> List[ModelVersionTag]:
        parent_path, tag_files = self._get_resource_files(
            directory, FileStore.TAGS_FOLDER_NAME
        )
        tags = []
        for tag_file in tag_files:
            tags.append(
    ...
    def _get_model_version_aliases(self, directory):
        aliases = self.get_all_registered_model_aliases_from_path(
    ...
    def update_model_version(self, name, version, description) -> ModelVersion:
        ...
        """
        updated_time = get_current_time_millis()
        model_version = self._fetch_file_model_version_if_exists(
    ...
    def transition_model_version_stage(
        ...
        if archive_existing_versions:
            ...
            for mv in model_versions:
                if mv.version != version and mv.current_stage == get_canonical_stage(
                    stage
                ):
    ...
    def _fetch_file_model_version_if_exists(self, name, version) -> FileModelVersion:
        ...
        model_version = self._get_file_model_version_from_dir(
            registered_model_version_dir
        )
    ...
    def get_model_version(self, name, version) -> ModelVersion:
        ...
        Returns:
            A single :py:class:`mlflow.entities.model_registry.ModelVersion` object.
        """
        return self._fetch_file_model_version_if_exists(
            name, version
        ).to_mlflow_entity()
    ...
    def _get_registered_model_version_tag_path(self, name, version, tag_name):
        ...
        return os.path.join(
            registered_model_version_path, FileStore.TAGS_FOLDER_NAME, tag_name
        )
    ...
    def get_model_version_by_alias(self, name, alias) -> ModelVersion:
        ...
        if exists(alias_path):
            version = read_file(
                os.path.dirname(alias_path), os.path.basename(alias_path)
            )
