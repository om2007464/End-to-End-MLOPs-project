...
def _default_root_dir():
    return MLFLOW_TRACKING_DIR.get() or os.path.abspath(
        DEFAULT_LOCAL_FILE_AND_ARTIFACT_PATH
    )
...
class FileStore(AbstractStore):
    ...
    def __init__(self, root_directory=None, artifact_root_uri=None):
        ...
        self.root_directory = local_file_uri_to_path(
            root_directory or _default_root_dir()
        )
        ...
        self.trash_folder = os.path.join(
            self.root_directory, FileStore.TRASH_FOLDER_NAME
        )
    ...
    def _get_experiment_path(
        self, experiment_id, view_type=ViewType.ALL, assert_exists=False
    ):
        parents = []
    ...
    def _get_run_dir(self, experiment_id, run_uuid):
        ...
        if not self._has_experiment(experiment_id):
            return None
        return os.path.join(
            self._get_experiment_path(experiment_id, assert_exists=True), run_uuid
        )
    ...
    def _create_experiment_with_id(self, name, experiment_id, artifact_uri, tags):
        if not artifact_uri:
            resolved_artifact_uri = append_to_uri_path(
    ...
    def _get_deleted_runs(self, older_than=0):
        ...
        experiment_ids = (
            self._get_active_experiments() + self._get_deleted_experiments()
        )
        ...
        for deleted_run in deleted_runs:
            ...
            if (
                "deleted_time" not in meta
                or current_time - int(meta["deleted_time"]) >= older_than
            ):
    ...
    def update_run_info(self, run_id, run_status, end_time, run_name):
        ...
        new_info = run_info._copy_with_overrides(
            run_status, end_time, run_name=run_name
        )
        if run_name:
    ...
    def _get_experiment_files(self, experiment_id):
        ...
        return self._get_resource_files(
            experiment_dir, FileStore.EXPERIMENT_TAGS_FOLDER_NAME
        )
        ...
        for metric_file in metric_files:
            metrics.append(
                self._get_metric_from_file(
                    parent_path, metric_file, run_info.experiment_id
                )
            )
        return metrics
    ...
    def get_metric_history(self, run_id, metric_key, max_results=None, page_token=None):
        ...
        return PagedList(
            [
                FileStore._get_metric_from_line(
                    metric_key, line, run_info.experiment_id
                )
        ...
        for r_dir in run_dirs:
            try:
                ...
                if run_info.experiment_id != experiment_id:
                    ...
                    continue
                if LifecycleStage.matches_view_type(
                    view_type, run_info.lifecycle_stage
                ):
    ...
    def _log_run_metric(self, run_info, metric):
        metric_path = self._get_metric_path(
            run_info.experiment_id, run_info.run_id, metric.key
        )
    ...
    def _log_run_param(self, run_info, param):
        param_path = self._get_param_path(
            run_info.experiment_id, run_info.run_id, param.key
        )
    ...
    def _overwrite_run_info(self, run_info, deleted_time=None):
        ...
        write_yaml(
            run_dir, FileStore.META_DATA_FILE_NAME, run_info_dict, overwrite=True
        )
            ...
            for tag in tags:
                ...
                if tag.key == MLFLOW_RUN_NAME:
                    ...
                    self.update_run_info(
                        run_id, run_status, run_info.end_time, tag.value
                    )
                self._set_run_tag(run_info, tag)
    ...
    def record_logged_model(self, run_id, mlflow_model):
        ...
        run_info = self._get_run_info(run_id)
        path = self._get_tag_path(
            run_info.experiment_id, run_info.run_id, MLFLOW_LOGGED_MODELS
        )
    ...
    def log_inputs(self, run_id: str, datasets: Optional[List[DatasetInput]] = None):
        ...
        experiment_dir = self._get_experiment_path(
            run_info.experiment_id, assert_exists=True
        )
        ...
        for dataset_input in datasets:
            ...
            dataset_dir = os.path.join(
                experiment_dir, FileStore.DATASETS_FOLDER_NAME, dataset_id
            )
    ...
    class _FileStoreInput(NamedTuple):
        ...
        def from_yaml(cls, root, file_name):
            ...
            return cls(
                ...
                destination_type=InputVertexType.Value(
                    dict_from_yaml["destination_type"]
                ),
    ...
    def _get_all_inputs(self, run_info: RunInfo) -> RunInputs:
        ...
        datasets_parent_path = os.path.join(
            experiment_dir, FileStore.DATASETS_FOLDER_NAME
        )
        if not os.path.exists(inputs_parent_path) or not os.path.exists(
        ...
        for input_dir in os.listdir(inputs_parent_path):
            ...
            dataset_input = DatasetInput(
                dataset=dataset,
                tags=[
                    InputTag(key=key, value=value)
                    for key, value in fs_input.tags.items()
                ],
            )
    ...
    def _search_datasets(self, experiment_ids) -> List[_DatasetSummary]:
        ...
        for experiment_id in experiment_ids:
            experiment_dir = self._get_experiment_path(
                experiment_id, assert_exists=True
            )
            run_dirs = list_all(
                experiment_dir,
                filter_func=lambda x: all(
            ...
            for run_dir in run_dirs:
                ...
                for dataset_input in run_inputs.dataset_inputs:
                    ...
                    summaries.add(
                        _SummaryTuple(
                            experiment_id, dataset.name, dataset.digest, context
                        )
                    )
    ...
    def _get_trace_info_from_dir(self, trace_dir) -> Optional[TraceInfo]:
        if not os.path.exists(os.path.join(trace_dir, FileStore.TRACE_INFO_FILE_NAME)):
            return None
        trace_info_dict = FileStore._read_yaml(
            trace_dir, FileStore.TRACE_INFO_FILE_NAME
        )
    ...
    def _delete_traces(
        ...
        if max_timestamp_millis:
            trace_paths = list_all(
                traces_path, lambda x: os.path.isdir(x), full_path=True
            )
            ...
            deleted_traces = min(
                len(trace_info_and_paths), max_traces or len(trace_info_and_paths)
            )
    ...
    def search_traces(
        ...
        traces, next_page_token = SearchTraceUtils.paginate(
            sorted_traces, page_token, max_results
        )
