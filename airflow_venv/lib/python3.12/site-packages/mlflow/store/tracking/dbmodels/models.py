...
class SqlTraceInfo(Base):
    ...
    __table_args__ = (
        ...
        # in the where clause.
        Index(
            f"index_{__tablename__}_experiment_id_timestamp_ms",
            "experiment_id",
            "timestamp_ms",
        ),
    )
...
class SqlTraceTag(Base):
    ...
    request_id = Column(
        String(50),
        ForeignKey("trace_info.request_id", ondelete="CASCADE"),
        nullable=False,
    )
    """
...
class SqlTraceRequestMetadata(Base):
    ...
    trace_info = relationship(
        "SqlTraceInfo", backref=backref("request_metadata", cascade="all")
    )
    """
    SQLAlchemy relationship (many:one) with
    :py:class:`mlflow.store.dbmodels.models.SqlTraceInfo`.
