...
def log_artifact(local_file, run_id, artifact_path):
    ...
    _logger.info(
        "Logged artifact from local file %s to artifact_path=%s",
        local_file,
        artifact_path,
    )
...
@click.option(
    "--local-dir", "-l", required=True, help="Directory of local artifacts to log"
)
@click.option(
    "--run-id",
    "-r",
...
def log_artifacts(local_dir, run_id, artifact_path):
    ...
    _logger.info(
        "Logged artifact from local dir %s to artifact_path=%s",
        local_dir,
        artifact_path,
    )
