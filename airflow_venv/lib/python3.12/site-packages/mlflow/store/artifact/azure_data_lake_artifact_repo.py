...
class AzureDataLakeArtifactRepository(CloudArtifactRepository):
    ...
    def _parse_credentials(self, credential):
        ...
        (filesystem, account_name, domain_suffix, path) = _parse_abfss_uri(
            self.artifact_uri
        )
    ...
    def list_artifacts(self, path=None):
        ...
        for result in self.fs_client.get_paths(path=directory_to_list, recursive=False):
            ...
            if result.is_directory:
                subdir = posixpath.relpath(
                    path=result.name, start=self.base_data_lake_directory
                )
                if subdir.endswith("/"):
                    subdir = subdir[:-1]
                infos.append(FileInfo(subdir, is_dir=True, file_size=None))
            else:
                file_name = posixpath.relpath(
                    path=result.name, start=self.base_data_lake_directory
                )
                infos.append(
                    FileInfo(file_name, is_dir=False, file_size=result.content_length)
                )
