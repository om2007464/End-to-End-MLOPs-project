...
class UCVolumesArtifactRepository(ArtifactRepository):
    ...
    def _api_request(self, endpoint, method, **kwargs):
        ...
        return http_request(
            host_creds=creds, endpoint=endpoint, method=method, **kwargs
        )
    ...
    def _paginated_list_directory_contents(
        ...
        if next_page_token := response_json.get("next_page_token"):
            next_contents = self._paginated_list_directory_contents(
                directory_path, next_page_token
            )
    ...
    def _download(self, output_path: str, file_path: str):
        ...
        with open(output_path, "wb") as f:
            with self._api_request(
                endpoint=endpoint, method="GET", stream=True
            ) as resp:
    ...
    def log_artifact(self, local_file, artifact_path=None):
        ...
        artifact_path = (
            posixpath.join(artifact_path, basename) if artifact_path else basename
        )
    ...
    def list_artifacts(self, path=None):
        ...
        for content in self._paginated_list_directory_contents(self._get_path(path)):
            ...
            infos.append(
                FileInfo(rel_path, content["is_directory"], content.get("file_size"))
            )
        return sorted(infos, key=lambda f: f.path)

    def _download_file(self, remote_file_path, local_path):
        resp = self._download(
            output_path=local_path, file_path=self._get_path(remote_file_path)
        )
        augmented_raise_for_status(resp)
...
def uc_volume_artifact_repo_factory(artifact_uri):
    ...
    if (
        ...
        uri_without_profile = remove_databricks_profile_info_from_artifact_uri(
            artifact_uri
        )
