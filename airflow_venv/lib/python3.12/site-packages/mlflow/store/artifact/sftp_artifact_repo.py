...
class SFTPArtifactRepository(ArtifactRepository):
    ...
    def __init__(self, artifact_uri):
        ...
        connections = [
            pysftp.Connection(**self.config) for _ in range(self.max_workers)
        ]
    ...
    def log_artifact(self, local_file, artifact_path=None):
        artifact_dir = (
            posixpath.join(self.path, artifact_path) if artifact_path else self.path
        )
        with self.pool.get_sftp_connection() as sftp:
            sftp.makedirs(artifact_dir)
            sftp.put(
                local_file, posixpath.join(artifact_dir, os.path.basename(local_file))
            )
    ...
    def _is_directory(self, artifact_path):
        ...
        path = (
            posixpath.join(artifact_dir, artifact_path)
            if artifact_path
            else artifact_dir
        )
        with self.pool.get_sftp_connection() as sftp:
    ...
    def list_artifacts(self, path=None):
        ...
        with self.pool.get_sftp_connection() as sftp:
            ...
            for file_name in artifact_files:
                file_path = (
                    file_name if path is None else posixpath.join(path, file_name)
                )
                ...
                if sftp.isdir(full_file_path):
                    ...
                    infos.append(
                        FileInfo(file_path, False, sftp.stat(full_file_path).st_size)
                    )
            return infos
