...
_AWS_MIN_CHUNK_SIZE = (
    5 * 1024**2
)  # 5 MB is the minimum chunk size for S3 multipart uploads
_AWS_MAX_CHUNK_SIZE = (
    5 * 1024**3
)  # 5 GB is the maximum chunk size for S3 multipart uploads
...
class CloudArtifactRepository(ArtifactRepository):
    ...
    def log_artifacts(self, local_dir, artifact_path=None):
            ...
            for name in filenames:
                ...
                staged_uploads.append(
                    StagedArtifactUpload(
                        ...
                        artifact_file_path=posixpath.join(
                            artifact_subdir, src_file_name
                        ),
                    )
                )
        ...
        def upload_artifacts_iter():
            for staged_upload_chunk in chunk_list(
                staged_uploads, _ARTIFACT_UPLOAD_BATCH_SIZE
            ):
                write_credential_infos = self._get_write_credential_infos(
                    remote_file_paths=[
                        staged_upload.artifact_file_path
    ...
    def _parallelized_download_from_cloud(
        self, file_size, remote_file_path, local_path
    ):
    ...
    def _download_file(self, remote_file_path, local_path):
        ...
        if (
            ...
            self._parallelized_download_from_cloud(
                file_size, remote_file_path, local_path
            )
