...
class SearchUtils:
    ...
    def _get_identifier(cls, identifier, valid_attributes):
        ...
        if identifier == cls._ATTRIBUTE_IDENTIFIER and key not in valid_attributes:
            ...
            identifier == cls._DATASET_IDENTIFIER and key not in cls.DATASET_ATTRIBUTES
        ):
            raise MlflowException.invalid_parameter_value(
    ...
    def _get_comparison(cls, comparison):
        ...
        comp["value"] = cls._get_value(
            comp.get("type"), comp.get("key"), stripped_comparison[2]
        )
        return comp
    ...
    def is_attribute(cls, key_type, key_name, comparator):
        return cls.is_string_attribute(
            key_type, key_name, comparator
        ) or cls.is_numeric_attribute(key_type, key_name, comparator)

    @classmethod
    def is_string_attribute(cls, key_type, key_name, comparator):
        if (
            key_type == cls._ATTRIBUTE_IDENTIFIER
            and key_name not in cls.NUMERIC_ATTRIBUTES
        ):
            ...
            if key == "context":
                return any(
                    SearchUtils.get_comparison_func(comparator)(
                        tag.value if tag else None, value
                    )
    ...
    def _get_value_for_sort(cls, run, key_type, key, ascending):
        ...
        return (
            (is_none_or_nan, sort_value)
            if ascending
            else (not is_none_or_nan, sort_value)
        )
...
class SearchModelUtils(SearchUtils):
    ...
    def filter(cls, registered_models, filter_string):
        ...
        def registered_model_matches(model):
            return all(
                cls._does_registered_model_match_clauses(model, s) for s in parsed
            )
    ...
    def _get_sort_key(cls, order_by_list):
        order_by = []
        parsed_order_by = map(
            cls.parse_order_by_for_search_registered_models, order_by_list or []
        )
        for type_, key, ascending in parsed_order_by:
            if type_ == "attribute":
                order_by.append((key, ascending))
        ...
        return lambda model: tuple(
            _apply_reversor(model, k, asc) for (k, asc) in order_by
        )
    ...
    def _process_statement(cls, statement):
        tokens = _join_in_comparison_tokens(statement.tokens)
        invalids = list(
            filter(cls._invalid_statement_token_search_model_registry, tokens)
        )
        if len(invalids) > 0:
            invalid_clauses = ", ".join(map(str, invalids))
            raise MlflowException.invalid_parameter_value(
    ...
    def _get_comparison(cls, comparison):
        ...
        cls._validate_comparison(stripped_comparison)
        left, comparator, right = stripped_comparison
        comp = cls._get_model_search_identifier(
            left.value, cls.VALID_SEARCH_ATTRIBUTE_KEYS
        )
...
class SearchModelVersionUtils(SearchUtils):
    ...
    def filter(cls, model_versions, filter_string):
        ...
        model_versions = [
            mv for mv in model_versions if mv.current_stage != STAGE_DELETED_INTERNAL
        ]
        if not filter_string:
    ...
    def _get_sort_key(cls, order_by_list):
        order_by = []
        parsed_order_by = map(
            cls.parse_order_by_for_search_model_versions, order_by_list or []
        )
        for type_, key, ascending in parsed_order_by:
            if type_ == "attribute":
    ...
    def _get_comparison(cls, comparison):
        ...
        cls._validate_comparison(stripped_comparison)
        left, comparator, right = stripped_comparison
        comp = cls._get_model_version_search_identifier(
            left.value, cls.VALID_SEARCH_ATTRIBUTE_KEYS
        )
        comp["comparator"] = comparator.value.upper()
        comp["value"] = cls._get_value(comp.get("type"), comp.get("key"), right)
    ...
    def _process_statement(cls, statement):
        tokens = _join_in_comparison_tokens(statement.tokens)
        invalids = list(
            filter(cls._invalid_statement_token_search_model_version, tokens)
        )
        if len(invalids) > 0:
            invalid_clauses = ", ".join(map(str, invalids))
            raise MlflowException.invalid_parameter_value(
...
class SearchTraceUtils(SearchUtils):
    ...
    def _get_sort_key(cls, order_by_list):
        ...
        return lambda trace: tuple(
            _apply_reversor(trace, k, asc) for (k, asc) in order_by
        )
