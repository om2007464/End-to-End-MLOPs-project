...
def _get_conda_env_name(conda_env_path, env_id=None, env_root_dir=None):
    ...
    if env_root_dir:
        ...
        env_name += "-{}".format(
            insecure_hash.sha1(env_root_dir.encode("utf-8")).hexdigest()
        )
    ...
    for attempt in range(num_attempts):
        try:
            ...
            if (num_attempts - attempt - 1) > 0 and (
                ...
                or "ChunkedEncodingError" in error_str
            ):
                _logger.warning(
                    "Conda env creation failed due to network issue. Retrying..."
                )
                continue
            raise
...
def get_or_create_conda_env(
    ...
    project_env_name = _get_conda_env_name(
        conda_env_path, env_id=env_id, env_root_dir=env_root_dir
    )
    ...
    try:
        ...
        if pip_requirements_override:
            ...
            process._exec_cmd(
                cmd, extra_env=conda_extra_env_vars, capture_output=capture_output
            )
