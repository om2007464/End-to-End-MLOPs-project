...
class _PythonEnv:
    ...
    def __init__(self, python=None, build_dependencies=None, dependencies=None):
        ...
        if dependencies is not None and not isinstance(dependencies, list):
            raise TypeError(
                f"`dependencies` must be a list but got {type(dependencies)}"
            )
...
def infer_pip_requirements(
    model_uri, flavor, fallback=None, timeout=None, extra_env_vars=None
):
...
def _process_pip_requirements(
    ...
    if pip_requirements is not None:
        ...
        extra_pip_requirements, constraints = _parse_pip_requirements(
            extra_pip_requirements
        )
    ...
    conda_env = _mlflow_conda_env(
        additional_pip_deps=sanitized_pip_reqs, install_mlflow=False
    )
        ...
        try:
            ...
            if not existing_req:
                ...
                if (
                    ...
                    parsed_req.extras = sorted(
                        set(existing_req.extras).union(parsed_req.extras)
                    )
...
class Environment:
    ...
    def execute(
        ...
        command = (
            ["bash", "-c", command] if not is_windows() else ["cmd", "/c", command]
        )
