...
def _capture_imported_modules(
    model_uri, flavor, record_full_module=False, extra_env_vars=None
):
...
def _infer_requirements(model_uri, flavor, raise_on_error=False, extra_env_vars=None):
    ...
    modules = _capture_imported_modules(
        model_uri, flavor, extra_env_vars=extra_env_vars
    )
    ...
    unrecognized_packages = (
        packages - _PYPI_PACKAGE_INDEX.package_names - {"mlflow[gateway]"}
    )
...
class _MismatchedPackageInfo(NamedTuple):
    ...
    def __str__(self):
        current_status = (
            self.installed_version if self.installed_version else "uninstalled"
        )
...
def _check_requirement_satisfied(requirement_str):
    ...
    try:
        installed_version = _get_installed_version(
            pkg_name, _PACKAGES_TO_MODULES.get(pkg_name)
        )
    ...
    try:
        ...
        for req in model_requirements:
            ...
            if mismatch_info is not None:
                if (
                    _normalize_package_name(mismatch_info.package_name)
                    in ignore_packages
                ):
                    continue
