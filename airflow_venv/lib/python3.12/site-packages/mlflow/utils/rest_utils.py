...
def http_request(
    ...
    **kwargs,
    ...
    max_retries = (
        MLFLOW_HTTP_REQUEST_MAX_RETRIES.get() if max_retries is None else max_retries
    )
    backoff_factor = (
        MLFLOW_HTTP_REQUEST_BACKOFF_FACTOR.get()
    ...
    backoff_jitter = (
        MLFLOW_HTTP_REQUEST_BACKOFF_JITTER.get()
...
def verify_rest_response(response, endpoint):
    ...
    if response.status_code == 200 and response.text.strip() == _ARMERIA_OK:
        response._content = (
            b"{}"  # Update response content to be an empty JSON dictionary
        )
        return response
...
def call_endpoint(
    host_creds, endpoint, method, json_body, response_proto, extra_headers=None
):
...
def call_endpoints(
    host_creds, endpoints, json_body, response_proto, extra_headers=None
):
