...
class NumpyEncoder(JSONEncoder):
    ...
    def try_convert(self, o):
        ...
        if isinstance(
            o, (pd.Timestamp, datetime.date, datetime.datetime, datetime.time)
        ):
...
def cast_df_types_according_to_schema(pdf, schema):
    ...
    for col_name, col_type_spec in dtype_list:
        ...
        if col_name in actual_cols:
            try:
                if (
                    isinstance(col_type_spec, DataType)
                    and col_type_spec == DataType.binary
                ):
                ...
                elif schema.is_tensor_spec() and isinstance(
                    pdf[col_name].iloc[0], list
                ):
                    ...
                    pdf[col_name] = pdf[col_name].map(
                        lambda x: _enforce_array(x, col_type_spec)
                    )
                elif isinstance(col_type_spec, Object):
                    pdf[col_name] = pdf[col_name].map(
                        lambda x: _enforce_object(x, col_type_spec)
                    )
                elif isinstance(col_type_spec, Map):
                    pdf[col_name] = pdf[col_name].map(
                        lambda x: _enforce_map(x, col_type_spec)
                    )
                else:
...
def parse_instances_data(data, schema=None):
    ...
    if isinstance(data, dict):
        ...
        check_data = {
            k: v for k, v in data.items() if isinstance(v, (list, np.ndarray))
        }
        if schema and schema.has_input_names():
            ...
            check_cols = {
                col
                for col, spec in schema.input_dict().items()
                if not isinstance(spec.type, Array)
            }
...
def parse_tf_serving_input(inp_dict, schema=None):
    ...
    if "signature_name" in inp_dict:
        raise MlflowInvalidInputException(
            '"signature_name" parameter is currently not supported'
        )
