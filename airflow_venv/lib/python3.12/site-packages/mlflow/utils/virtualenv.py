...
def _install_python(version, pyenv_root=None, capture_output=False):
    ...
    if not is_windows():
        if pyenv_root is None:
            pyenv_root = _exec_cmd(
                [pyenv_bin_path, "root"], capture_output=True
            ).stdout.strip()
...
def _create_virtualenv(
    ...
    with remove_on_error(
        ...
        _logger.info(
            "Creating a new environment in %s with %s", env_dir, python_bin_path
        )
        _exec_cmd(
        ...
        for deps in filter(
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                _exec_cmd(
                    cmd, capture_output=capture_output, cwd=tmpdir, extra_env=extra_env
                )
...
def _get_or_create_virtualenv(  # noqa: D417
    ...
    try:
        ...
        if env_dir.exists():
            ...
            msg = (
                "Failed to remove %s" if env_dir.exists() else "Successfully removed %s"
            )
