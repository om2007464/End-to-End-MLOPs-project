...
class _WarningsController:
    ...
    def _patched_showwarning(
        self, message, category, filename, lineno, *args, **kwargs
    ):
        """
    ...
    def set_non_mlflow_warnings_disablement_state_for_current_thread(
        self, disabled=True
    ):
...
@contextmanager
def set_non_mlflow_warnings_behavior_for_current_thread(
    disable_warnings, reroute_warnings
):
    """
        ...
        try:
            with _SetMlflowEventsAndWarningsBehaviorGlobally._lock:
                if self._disable_event_logs:
                    if (
                        _SetMlflowEventsAndWarningsBehaviorGlobally._disable_event_logs_count
                        <= 0
                    ):
                ...
                if self._disable_warnings:
                    if (
                        _SetMlflowEventsAndWarningsBehaviorGlobally._disable_warnings_count
                        <= 0
                    ):
                ...
                if self._reroute_warnings:
                    if (
                        _SetMlflowEventsAndWarningsBehaviorGlobally._reroute_warnings_count
                        <= 0
                    ):
