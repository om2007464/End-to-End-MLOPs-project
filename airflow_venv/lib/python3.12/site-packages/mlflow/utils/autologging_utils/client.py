...
class MlflowAutologgingQueueingClient:
    ...
    def log_params(
        self, run_id: Union[str, PendingRunId], params: Dict[str, Any]
    ) -> None:
        """
    ...
    def _flush_pending_operations(self, pending_operations):
        ...
        if pending_operations.create_run:
            ...
            num_additional_tags_to_include_during_creation = (
                MAX_ENTITIES_PER_BATCH - len(create_run_tags)
            )
        ...
        assert not isinstance(
            run_id, PendingRunId
        ), "Run ID cannot be pending for logging"
        ...
        for datasets_batch in chunk_list(
            ...
            operation_results.append(
                self._try_operation(
                    self._client.log_inputs, run_id=run_id, datasets=datasets_batch
                )
            )
...
class _PendingRunOperations:
    ...
    def enqueue(
        ...
        if set_terminated:
            assert (
                not self.set_terminated
            ), "Attempted to terminate the same run multiple times"
