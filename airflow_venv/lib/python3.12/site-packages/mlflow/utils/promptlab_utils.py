...
def _create_promptlab_run_impl(
    ...
    try:
        ...
        model_output_parameters = [
            Param(key=param.key, value=str(param.value))
            for param in model_output_parameters
        ]
        ...
        with tempfile.TemporaryDirectory() as local_dir:
            ...
            eval_results_json_file_path = os.path.join(
                local_dir, "eval_results_table.json"
            )
        ...
        store.update_run_info(
            run_id, RunStatus.FAILED, int(time.time() * 1000), run_name
        )
    else:
        # end time is the current number of milliseconds since the UNIX epoch.
        store.update_run_info(
            run_id, RunStatus.FINISHED, int(time.time() * 1000), run_name
        )
