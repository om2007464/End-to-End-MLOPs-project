...
def is_in_databricks_notebook():
    ...
    try:
        return (
            path.startswith("/workspace") if (path := acl_path_of_acl_root()) else False
        )
    except Exception:
        return False
...
def is_in_databricks_shared_cluster_runtime():
    ...
    return (
        ...
        and not is_in_databricks_serverless()
    )
...
def get_workspace_info_from_dbutils():
    try:
        ...
        if dbutils:
            ...
            workspace_host = (
                "https://" + browser_hostname if browser_hostname else get_webapp_url()
            )
...
def _get_workspace_url():
    try:
        if spark_session := _get_active_spark_session():
            if workspace_url := spark_session.conf.get(
    ...
    if key_prefix:
        ...
        if dbutils:
            workspace_id = dbutils.secrets.get(
                scope=profile, key=key_prefix + "-workspace-id"
            )
            workspace_host = dbutils.secrets.get(
                scope=profile, key=key_prefix + "-host"
            )
            return workspace_host, workspace_id
    return None, None
...
def get_databricks_run_url(
    tracking_uri: str, run_id: str, artifact_path=None
) -> Optional[str]:
    """
    ...
    try:
        ...
        if workspace_info is not None:
            experiment_id = (
                MlflowClient(tracking_uri).get_run(run_id).info.experiment_id
            )
...
def get_databricks_model_version_url(
    registry_uri: str, name: str, version: str
) -> Optional[str]:
...
DatabricksWorkspaceInfoType = TypeVar(
    "DatabricksWorkspaceInfo", bound="DatabricksWorkspaceInfo"
)
        ...
        if DatabricksWorkspaceInfo.WORKSPACE_HOST_ENV_VAR in os.environ:
            return DatabricksWorkspaceInfo(
                ...
                workspace_id=os.environ.get(
                    DatabricksWorkspaceInfo.WORKSPACE_ID_ENV_VAR
                ),
            )
        else:
            return None
...
def get_databricks_workspace_info_from_uri(
    tracking_uri: str,
) -> Optional[DatabricksWorkspaceInfo]:
    ...
    if dbr_version.is_client_image or dbr_major_minor_version >= (13, 2):
        ...
        class DynamicConfigProvider(DatabricksConfigProvider):
            def get_config(self):
                ...
                return DatabricksConfig.from_token(
                    host=api_url_option.get(),
                    token=api_token_option.get(),
                    insecure=ssl_trust_all,
                )
