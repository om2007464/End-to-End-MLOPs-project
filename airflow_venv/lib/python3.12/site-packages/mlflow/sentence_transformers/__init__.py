...
def save_model(
    ...
    model_config = None
    if task is not None:
        mlflow_model.metadata = _verify_task_and_update_metadata(
            task, mlflow_model.metadata
        )
...
def load_model(model_uri: str, dst_path: Optional[str] = None):
    ...
    local_model_dir = pathlib.Path(local_model_path).joinpath(
        SENTENCE_TRANSFORMERS_DATA_PATH
    )
    ...
    return sentence_transformers.SentenceTransformer(
        str(local_model_dir), **load_kwargs
    )
