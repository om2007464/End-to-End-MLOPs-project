...
def convert_messages_to_prompt(messages: List[Dict], tokenizer) -> str:
    ...
    """
    if not (
        isinstance(messages, list) and all(isinstance(msg, dict) for msg in messages)
    ):
        raise MlflowException(
...
def _get_stopping_criteria(
    stop: Optional[Union[str, List[str]]], model_name: Optional[str] = None
):
...
def _get_default_task_for_llm_inference_task(
    llm_inference_task: Optional[str],
) -> Optional[str]:
    """
