...
def _to_json(file, encoding, errors, **kwargs):
    try:
        ...
        return BoxList.from_json(
            filename=file, encoding=encoding, errors=errors, **kwargs
        )
...
def _to_toml(file, encoding, errors, **kwargs):
    if not toml_read_library:
        raise BoxError(
            f'File "{file}" is toml but no package is available to open it. Please install "tomli"'
        )
    try:
