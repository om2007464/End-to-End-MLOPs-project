...
class BaseFile:
    ...
    _INVALID_WIN_PATH_CHARS: ClassVar[str] = (
        _INVALID_PATH_CHARS + ':*?"<>|\t\n\r\x0b\x0c'
    )
    ...
    def __init__(
        ...
        self._additional_reserved_names = tuple(
            n.upper() for n in additional_reserved_names
        )
...
class AbstractSanitizer(BaseFile, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def sanitize(
        self, value: PathType, replacement_text: str = ""
    ) -> PathType:  # pragma: no cover
        pass
