...
_RE_INVALID_PATH = re.compile(
    f"[{re.escape(BaseFile._INVALID_PATH_CHARS):s}]", re.UNICODE
)
...
class FilePathSanitizer(AbstractSanitizer):
    ...
    def sanitize(self, value: PathType, replacement_text: str = "") -> PathType:
        ...
        for entry in sanitized_path.replace("\\", "/").split("/"):
            ...
            sanitized_entry = str(
                self.__fname_sanitizer.sanitize(
                    entry, replacement_text=replacement_text
                )
            )
