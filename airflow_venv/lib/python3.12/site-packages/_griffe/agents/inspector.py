    ...
    def handle_function(self, node: ObjectNode, labels: set | None = None) -> None:
        ...
        try:
            ...
            parameters = Parameters(
                *[
                    _convert_parameter(parameter, parent=self.current)
            ...
            returns = (
                ...
                if return_annotation is _empty
                    ...
                    return_annotation, parent=self.current
                )
            )
...
def _convert_parameter(
    parameter: SignatureParameter, parent: Module | Class
) -> Parameter:
    name = parameter.name
    annotation = (
        None
        if parameter.annotation is _empty
        else _convert_object_to_annotation(parameter.annotation, parent=parent)
    )
...
def _convert_object_to_annotation(
    ...
    try:
        annotation_node = compile(
            obj, mode="eval", filename="<>", flags=ast.PyCF_ONLY_AST, optimize=2
        )
    except SyntaxError:
        return obj
