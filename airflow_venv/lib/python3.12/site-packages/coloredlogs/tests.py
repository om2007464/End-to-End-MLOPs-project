...
class ColoredLogsTestCase(TestCase):
    ...
    def find_system_log(self):
        ...
        if not filename:
            ...
            self.skipTest(
                "Insufficient permissions to read system log file! (%s)" % filename
            )
        else:
            return filename
    ...
    def test_style_parsing(self):
        ...
        decoded_styles = parse_encoded_styles(
            encoded_styles, normalize_key=lambda k: k.upper()
        )
        assert sorted(decoded_styles.keys()) == sorted(
    ...
    def test_support_for_milliseconds(self):
        ...
        assert all(
            map(PATTERN_INCLUDING_MILLISECONDS.match, stream.getvalue().splitlines())
        )
    ...
    def test_html_conversion(self):
        ...
        self.assertEqual(
            expected_html,
            convert(ansi_wrap("bright yellow", color="yellow", bright=True)),
        )
        # Check conversion of text with a background color.
        expected_html = (
            '<code><span style="background-color:#DE382B">red background</span></code>'
        )
        self.assertEqual(
            expected_html, convert(ansi_wrap("red background", background="red"))
        )
        ...
        expected_html = (
            '<code><span style="color:#FFAF00">256 color mode foreground</span></code>'
        )
        self.assertEqual(
            expected_html, convert(ansi_wrap("256 color mode foreground", color=214))
        )
        ...
        self.assertEqual(
            expected_html,
            convert(ansi_wrap("256 color mode background", background=124)),
        )
        # Check that invalid 256 color mode indexes don't raise exceptions.
        expected_html = "<code>plain text expected</code>"
        self.assertEqual(
            expected_html, convert("\x1b[38;5;256mplain text expected\x1b[0m")
        )
        ...
        self.assertEqual(
            expected_html, convert(ansi_wrap("underlined text", underline=True))
        )
        ...
        self.assertEqual(
            expected_html,
            convert(ansi_wrap("strike-through text", strike_through=True)),
        )
        # Check conversion of inverse text.
        expected_html = '<code><span style="background-color:#FFC706;color:#000">inverse</span></code>'
        self.assertEqual(
            expected_html, convert(ansi_wrap("inverse", color="yellow", inverse=True))
        )
        ...
        for sample_text in (
            ...
            sample_url = (
                sample_text if "://" in sample_text else ("http://" + sample_text)
            )
    ...
    def test_auto_install(self):
        ...
        command_line = [
            sys.executable,
            "-c",
            "import logging; logging.info(%r)" % needle,
        ]
