...
class RetryOptionsBase:
    def __init__(
        ...
        | None = None,  # On which exceptions we should retry, by default on all
...
class ExponentialRetry(RetryOptionsBase):
    def __init__(
        ...
        | None = None,  # On which exceptions we should retry
...
class JitterRetry(ExponentialRetry):
    ...
    def get_timeout(
        ...
        timeout: float = (
            super().get_timeout(attempt)
            + random.uniform(0, self._random_interval_size) ** self._factor
        )
        return timeout
