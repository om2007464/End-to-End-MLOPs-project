...
class AbstractNestedList:
    def __init__(self, lists):
        super().__init__()
        self._lists = [WordList(x) if x.__class__ is list else x for x in lists]
    ...
    def _dump(self, stream, indent="", object_ids=False):
        stream.write(
            indent + str(self) + (f" [id={id(self)}]" if object_ids else "") + "\n"
        )
        indent += "  "
...
class _BasicList(list, AbstractNestedList):
    ...
    def __str__(self):
        ...
        if len(ls) == 4:
            ls[3] = "..."
        return "{}([{}], len={})".format(
            self.__class__.__name__, ", ".join(ls), len(self)
        )
...
class NestedList(AbstractNestedList):
    def __init__(self, lists):
        ...
        if any(isinstance(x, WordList) for x in self._lists) and any(
            x.multiword for x in self._lists
        ):
            self._lists = [
                WordAsPhraseWrapper(x) if isinstance(x, WordList) else x
        ...
        if result is self and hard:
            for cls in (WordList, PhraseList):
                if all(isinstance(x, cls) for x in self._lists):
                    ...
                    # And it's simple!
                    result = cls(
                        sorted(set(itertools.chain.from_iterable(self._lists)))
                    )
...
class RandomGenerator:
    ...
    def __init__(self, config, rand=None):
        ...
        try:
            ...
            if self._check_prefix <= 0:
                raise ValueError(
                    f"expected a positive integer, got {self._check_prefix!r}"
                )
        except KeyError:
            self._check_prefix = None
        except ValueError as ex:
            raise ConfigurationError(
                f"Invalid {_CONF.FIELD.ENSURE_UNIQUE_PREFIX} value: {ex}"
            )
        # Get max slug length
        try:
            ...
            raise ConfigurationError(
                f"Invalid {_CONF.FIELD.MAX_SLUG_LENGTH} value: {ex}"
            )
        # Make sure that generate() does not go into long loop.
        # Default generator is a special case, we don't need check.
        if (
            not config["all"].get("__nocheck")
            and self._ensure_unique
            or self._check_prefix
            or self._max_slug_length
        ):
    ...
    def generate(self, pattern: Union[None, str, int] = None) -> List[str]:
        ...
        while True:
            ...
            if (
                ...
                or self._check_prefix
                and len(set(x[: self._check_prefix] for x in result)) != n
                or self._max_slug_length
    ...
    def _check_not_hanging(self):
        ...
        if self._max_slug_length:
            checks.append(
                (
                    ...
                    lambda g: sum(len(x) for x in g) + len(g) - 1
                    > self._max_slug_length,
        ...
        n = 100
        warning_threshold = (
            20  # fail probability: 0.04 for 2 attempts, 0.008 for 3 attempts, etc.
        )
        for lst_id, lst in sorted(
            self._lists.items(), key=lambda x: "" if x is None else str(x)
        ):
            context = {
                "generate": "coolname.generate({})".format(
                    "" if lst_id is None else repr(lst_id)
                )
            }
    ...
    try:
        ...
        for key, listdef in list(config.items()):
            ...
            if listdef[_CONF.FIELD.TYPE] in (_CONF.TYPE.NESTED, _CONF.TYPE.CARTESIAN):
                ...
                if sublists is None:
                    raise ValueError(
                        "Config at key {!r} has no {!r}".format(key, _CONF.FIELD.LISTS)
                    )
                if (
                    not isinstance(sublists, list)
                    or not sublists
                    or not all(_is_str(x) for x in sublists)
                ):
                    raise ValueError(
                        "Config at key {!r} has invalid {!r}".format(
                            key, _CONF.FIELD.LISTS
                        )
                    )
            ...
            # Const
                ...
                try:
                    ...
                    raise ValueError(
                        "Config at key {!r} has no {!r}".format(key, _CONF.FIELD.VALUE)
                    )
                if not _is_str(value):
                    raise ValueError(
                        "Config at key {!r} has invalid {!r}".format(
                            key, _CONF.FIELD.VALUE
                        )
                    )
            # Words
                ...
                try:
                    ...
                    raise ValueError(
                        "Config at key {!r} has no {!r}".format(key, _CONF.FIELD.WORDS)
                    )
                if not isinstance(words, list) or not words:
                    raise ValueError(
                        "Config at key {!r} has invalid {!r}".format(
                            key, _CONF.FIELD.WORDS
                        )
                    )
                ...
                if max_length is not None:
                    for word in words:
                        if len(word) > max_length:
                            raise ValueError(
                                "Config at key {!r} has invalid word {!r} "
                                "(longer than {} characters)".format(
                                    key, word, max_length
                                )
                            )
            # Phrases (sequences of one or more words)
                ...
                try:
                    ...
                    raise ValueError(
                        "Config at key {!r} has no {!r}".format(
                            key, _CONF.FIELD.PHRASES
                        )
                    )
                ...
                for phrase in phrases:
                    ...
                    if not isinstance(phrase, (tuple, list)) or not all(
                        isinstance(x, str) for x in phrase
                    ):
                        raise ValueError(
                            "Config at key {!r} has invalid {!r}: "
                            "must be all string/tuple/list".format(
                                key, _CONF.FIELD.PHRASES
                            )
                        )
                    if number_of_words is not None and len(phrase) != number_of_words:
                        raise ValueError(
                            "Config at key {!r} has invalid phrase {!r} "
                            "({} word(s) but {}={})".format(
                                key,
                                " ".join(phrase),
                                len(phrase),
                    ...
                    if (
                        ...
                        and sum(len(word) for word in phrase) > max_length
                    ):
                        raise ValueError(
                            "Config at key {!r} has invalid phrase {!r} "
                            "(longer than {} characters)".format(
                                key, " ".join(phrase), max_length
                            )
                        )
            else:
                raise ValueError(
                    "Config at key {!r} has invalid {!r}".format(key, _CONF.FIELD.TYPE)
                )
        # Check that all sublists are defined
        diff = referenced_sublists.difference(config.keys())
        if diff:
            raise ValueError(
                "Lists are referenced but not defined: {}".format(
                    ", ".join(sorted(diff)[:10])
                )
            )
...
def _create_lists(config, results, current, stack, inside_cartesian=None):
    ...
    try:
        ...
        # 2. Simple list of lists
            ...
            results[current] = NestedList(
                [
                    _create_lists(
                        config, results, x, stack, inside_cartesian=inside_cartesian
                    )
        ...
        # 3. Cartesian list of lists
            ...
            if inside_cartesian is not None:
                raise ConfigurationError(
                    ...
                    "{!r}. Nested Cartesian lists are not allowed.".format(
                        inside_cartesian, current
                    )
                )
            results[current] = CartesianList(
                [
                    _create_lists(config, results, x, stack, inside_cartesian=current)
                    for x in listdef[_CONF.FIELD.LISTS]
                ]
            )
        # 4. Scalar
...
def _create_default_generator():
    ...
    if not data_dir and not data_module:
        ...
        data_module = (
            "coolname.data"  # used when imported from egg; consumes more memory
        )
