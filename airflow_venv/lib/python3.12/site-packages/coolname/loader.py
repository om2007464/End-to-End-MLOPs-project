    ...
    for name, wordlist in wordlists.items():
        if name in config:
            raise InitializationError(
                ...
                "you should remove it from config.".format(name, _CONF.TYPE.WORDS)
            )
...
def _load_config(config_file_path):
    try:
        ...
        raise InitializationError(
            "Failed to read config from {}: {}".format(config_file_path, ex)
        )
...
_OPTIONS = [(_CONF.FIELD.MAX_LENGTH, int), (_CONF.FIELD.NUMBER_OF_WORDS, int)]
    ...
    for i, line in enumerate(stream, start=1):
        ...
        if "=" in line:
            if items:
                raise ConfigurationError(
                    ...
                    "(options must be defined before words)".format(name, i, line)
                )
            try:
        ...
        if not multiword and _WORD_REGEX.match(line):
            if max_length is not None and len(line) > max_length:
                raise ConfigurationError(
                    "Word is too long at list {!r} line {}: {!r}".format(name, i, line)
                )
            items.append(line)
            ...
            if number_of_words is not None and len(phrase) != number_of_words:
                raise ConfigurationError(
                    ...
                    "at list {!r} line {}: {!r}".format(
                        len(phrase), number_of_words, name, i, line
                    )
                )
            if max_length is not None and sum(len(x) for x in phrase) > max_length:
                raise ConfigurationError(
                    "Phrase is too long at list {!r} line {}: {!r}".format(
                        name, i, line
                    )
                )
            items.append(phrase)
            ...
            raise ConfigurationError(
                "Invalid syntax at list {!r} line {}: {!r}".format(name, i, line)
            )
    if multiword:
        # If in phrase mode, convert everything to tuples
        for i in range(0, multiword_start):
            items[i] = (items[i],)
        result = {_CONF.FIELD.TYPE: _CONF.TYPE.PHRASES, _CONF.FIELD.PHRASES: items}
        if number_of_words is not None:
            result[_CONF.FIELD.NUMBER_OF_WORDS] = number_of_words
    else:
        result = {_CONF.FIELD.TYPE: _CONF.TYPE.WORDS, _CONF.FIELD.WORDS: items}
