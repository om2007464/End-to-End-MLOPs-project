...
ESCAPE_UNESCAPED_QUOTES = functools.partial(
    QUOTE_WITH_OPTIONAL_BACKSLASHES.sub,
    r"\g<escaped_backslashes>" r"\\" r"\g<literal_quote>",
)
...
def quote(
    ...
    if is_html_string(identifier) and not isinstance(identifier, NoHtml):
        ...
        if endswith_odd_number_of_backslashes(identifier):
            warnings.warn(
                "expect syntax error scanning invalid quoted string:"
                f" {identifier!r}",
...
def a_list(label: typing.Optional[str] = None, kwargs=None, attributes=None) -> str:
    ...
    if kwargs:
        result += [
            f"{quote(k)}={quote(v)}"
            for k, v in _tools.mapping_items(kwargs)
            if v is not None
        ]
    if attributes:
