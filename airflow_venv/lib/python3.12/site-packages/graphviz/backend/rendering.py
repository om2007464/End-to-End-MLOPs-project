...
def get_format(outfile: pathlib.Path, *, format: typing.Optional[str]) -> str:
    ...
    try:
        ...
        warnings.warn(
            f"unknown outfile suffix {outfile.suffix!r}"
            f' (expected: {"." + format!r})',
        ...
        if format is not None and format.lower() != inferred_format:
            warnings.warn(
                f"expected format {inferred_format!r} from outfile"
...
def infer_format(outfile: pathlib.Path) -> str:
    ...
    if not outfile.suffix:
        raise ValueError(
            "cannot infer rendering format from outfile:"
...
def render(
    ...
    *,
    ...
    if raise_if_result_exists and overwrite_filepath:
        raise ValueError(
            "overwrite_filepath cannot be combined" " with raise_if_result_exists"
        )
    ...
    if outfile is not None:
        ...
        if (
            not overwrite_filepath
            and outfile.name == filepath.name
            and outfile.resolve() == filepath.resolve()
        ):  # noqa: E129
            raise ValueError(
        ...
        outfile_arg = (
            outfile.resolve() if outfile.parent != filepath.parent else outfile.name
        )

        # https://www.graphviz.org/doc/info/command.html#-o
        args = ["-o", outfile_arg, filepath.name]
    elif filepath is None:
        raise exceptions.RequiredArgumentError(
            "filepath: (required if outfile is not given," f" got {filepath!r})"
        )
    elif format is None:
        raise exceptions.RequiredArgumentError(
            "format: (required if outfile is not given," f" got {format!r})"
        )
    else:
        outfile = get_outfile(
            filepath, format=format, renderer=renderer, formatter=formatter
        )
        # https://www.graphviz.org/doc/info/command.html#-O
        args = ["-O", filepath.name]

    cmd = dot_command.command(
        engine, format, renderer=renderer, formatter=formatter, neato_no_op=neato_no_op
    )
