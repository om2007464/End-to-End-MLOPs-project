...
GrapheneAssetConditionEvaluationStatus = graphene.Enum.from_enum(
...
class GrapheneUnpartitionedAssetConditionEvaluationNode(graphene.ObjectType):
    ...
    def __init__(self, evaluation: AutomationConditionEvaluation):
        ...
        super().__init__(
            ...
            status=status,
            childUniqueIds=[
                child.condition_snapshot.unique_id
...
class GrapheneAssetConditionEvaluationRecord(graphene.ObjectType):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            rootUniqueId=root_evaluation.condition_snapshot.unique_id,
            evaluationNodes=[
                GrapheneAutomationConditionEvaluationNode(node)
                for node in flattened_evaluations
            ],
        )
...
def _flatten_evaluation(
    ...
    return list(
        itertools.chain([e], *(_flatten_evaluation(ce) for ce in e.child_evaluations))
    )
...
def get_expanded_label(
    ...
    child_labels = [
        f'({" ".join(get_expanded_label(c, use_label=True))})' for c in children
    ]
    if len(child_labels) == 0:
        ...
        return list(
            itertools.chain(
                *itertools.zip_longest(child_labels, [], fillvalue=node_text)
            )
        )[:-1]
