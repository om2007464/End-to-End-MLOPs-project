...
class DynamicPartitionsRequestMixin:
    ...
    def resolve_type(self, _graphene_info: ResolveInfo):
        return (
            ...
            if isinstance(
                self.get_dynamic_partitions_request(), AddDynamicPartitionsRequest
            )
            else GrapheneDynamicPartitionsRequestType.DELETE_PARTITIONS
        )


class GrapheneDynamicPartitionsRequest(
    DynamicPartitionsRequestMixin, graphene.ObjectType
):
    class Meta:
...
class GrapheneDynamicPartitionsRequestResult(
    DynamicPartitionsRequestMixin, graphene.ObjectType
):
    class Meta:
...
class GrapheneInstigationTick(graphene.ObjectType):
    ...
    dynamicPartitionsRequestResults = non_null_list(
    ...
    requestedMaterializationsForAssets = non_null_list(
    ...
    def resolve_runs(self, graphene_info: ResolveInfo):
        ...
        return [
            GrapheneRun(records_by_id[run_id])
    ...
    def resolve_requestedAssetKeys(self, _):
        return [
            ...
            for asset_key in self._tick.requested_asset_keys
        ]
...
class GrapheneDryRunInstigationTick(graphene.ObjectType):
    ...
    def __init__(
        ...
        self._selector = check.inst_param(
            selector, "selector", (ScheduleSelector, SensorSelector)
        )
        self._cursor = cursor
        super().__init__(
    ...
    def resolve_evaluationResult(self, graphene_info: ResolveInfo):
        if not graphene_info.context.has_code_location(self._selector.location_name):
            raise UserFacingGraphQLError(
                GrapheneRepositoryLocationNotFound(
                    location_name=self._selector.location_name
                )
            )
...
class GrapheneTickEvaluation(graphene.ObjectType):
    dynamicPartitionsRequests = graphene.List(
        graphene.NonNull(GrapheneDynamicPartitionsRequest)
    )
...
class GrapheneInstigationState(graphene.ObjectType):
    ...
    def resolve_hasStartPermission(self, graphene_info: ResolveInfo):
        if self._instigator_state.instigator_type == InstigatorType.SCHEDULE:
            ...
            check.invariant(
                self._instigator_state.instigator_type == InstigatorType.SENSOR
            )
            return graphene_info.context.has_permission_for_location(
    ...
    def resolve_tick(
        ...
        schedule_storage = check.not_none(
            graphene_info.context.instance.schedule_storage
        )
