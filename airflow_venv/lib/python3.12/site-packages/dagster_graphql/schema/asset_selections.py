...
class GrapheneAssetSelection(graphene.ObjectType):
    ...
    assetsOrError = graphene.NonNull(
        "dagster_graphql.schema.roots.assets.GrapheneAssetsOrError"
    )
    ...
    def _get_resolved_and_sorted_keys(
        ...
        if self._resolved_keys is None:
            ...
            self._resolved_keys = sorted(
                self._asset_selection.resolve(repo.asset_graph), key=str
            )
    ...
    def resolve_assetsOrError(self, graphene_info) -> "GrapheneAssetConnection":
        ...
        return GrapheneAssetConnection(
            nodes=self._get_assets(graphene_info), cursor=None
        )
