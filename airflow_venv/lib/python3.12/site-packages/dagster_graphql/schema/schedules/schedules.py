...
class GrapheneSchedule(graphene.ObjectType):
    ...
    def __init__(
        ...
        self._remote_schedule = check.inst_param(
            remote_schedule, "remote_schedule", RemoteSchedule
        )
        ...
        self._schedule_state = self._remote_schedule.get_current_instigator_state(
            schedule_state
        )
    ...
    def resolve_canReset(self, _graphene_info: ResolveInfo):
        return bool(
            self._stored_state
            and self._stored_state.status != InstigatorStatus.DECLARED_IN_CODE
        )
    ...
    def resolve_partition_set(self, graphene_info: ResolveInfo):
        ...
        partition_set = repository.get_partition_set(
            self._remote_schedule.partition_set_name
        )
    ...
    def resolve_futureTicks(
        ...
        future_ticks = [
            GrapheneDryRunInstigationTick(schedule_selector, tick_time)
    ...
    def resolve_potentialTickTimestamps(
        ...
        tick_times: List[float] = []
        ascending_tick_iterator = self._remote_schedule.execution_time_iterator(
            start_timestamp
        )
        ...
        for _ in range(lower_limit):
            tick_times_below_timestamp.append(
                next(descending_tick_iterator).timestamp()
            )
