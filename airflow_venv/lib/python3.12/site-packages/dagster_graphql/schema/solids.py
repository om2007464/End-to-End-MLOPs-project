...
class GrapheneInputDefinition(graphene.ObjectType):
    ...
    def __init__(
        self, represented_job: RepresentedJob, solid_def_name: str, input_def_name: str
    ):
...
class GrapheneOutputDefinition(graphene.ObjectType):
    ...
    def __init__(
        ...
        self._represented_job = check.inst_param(
            represented_job, "represented_job", RepresentedJob
        )
        check.str_param(solid_def_name, "solid_def_name")
...
class GrapheneInput(graphene.ObjectType):
    ...
    def resolve_is_dynamic_collect(self, _graphene_info):
        return self._solid_invocation_snap.input_dep_snap(
            self._input_name
        ).is_dynamic_collect
...
class GrapheneOutput(graphene.ObjectType):
    ...
    def __init__(
        self, represented_pipeline, current_dep_structure, solid_name, output_name
    ):
        self._represented_pipeline = check.inst_param(
            represented_pipeline, "represented_pipeline", RepresentedJob
        )
    ...
    def resolve_depended_by(
        self, _graphene_info: ResolveInfo
    ) -> Sequence[GrapheneInput]:
        return [
            GrapheneInput(
                self._represented_pipeline,
                self._current_dep_structure,
...
def _build_solid_handles(
    ...
    for solid_invocation in current_dep_index.node_invocations:
        solid_name, solid_def_name = (
            solid_invocation.node_name,
            solid_invocation.node_def_name,
        )
...
class ISolidDefinitionMixin:
    ...
    def resolve_metadata(self, _graphene_info):
        ...
        for key, val in self._solid_def_snap.tags.items():
            ...
            if (
                key == LEGACY_COMPUTE_KIND_TAG
                and COMPUTE_KIND_TAG not in self._solid_def_snap.tags
            ):
    ...
    def resolve_input_definitions(
        self, _graphene_info
    ) -> Sequence[GrapheneInputDefinition]:
        return [
            GrapheneInputDefinition(
    ...
    def resolve_asset_nodes(
        ...
        if isinstance(self._represented_pipeline, HistoricalJob):
            ...
            remote_nodes = [
                ...
                if (
                    (
                        remote_node.asset_node_snap.node_definition_name
                        == self.solid_def_name
                    )
                    or (
                        remote_node.asset_node_snap.graph_name
                        and remote_node.asset_node_snap.graph_name
                        == self.solid_def_name
                    )
                )
            ]

            base_deployment_context = (
                graphene_info.context.get_base_deployment_context()
            )
...
class GrapheneSolidDefinition(graphene.ObjectType, ISolidDefinitionMixin):
    ...
    def resolve_required_resources(
        ...
        return [
            GrapheneResourceRequirement(key)
...
class GrapheneSolid(graphene.ObjectType):
    ...
    def get_solid_definition(
        ...
        return build_solid_definition(
            self._represented_pipeline, self._solid_def_snap.name
        )
...
class GrapheneCompositeSolidDefinition(graphene.ObjectType, ISolidDefinitionMixin):
    ...
    def __init__(self, represented_pipeline: RepresentedJob, solid_def_name: str):
        ...
        self._solid_def_snap = represented_pipeline.get_node_def_snap(solid_def_name)
        self._comp_solid_dep_index = represented_pipeline.get_dep_structure_index(
            solid_def_name
        )
        super().__init__(
            name=solid_def_name, description=self._solid_def_snap.description
    ...
    def resolve_input_mappings(
        self, _graphene_info: ResolveInfo
    ) -> Sequence[GrapheneInputMapping]:
        assert isinstance(self._solid_def_snap, GraphDefSnap)
        return [
...
def build_solid_definition(
    ...
    if isinstance(solid_def_snap, GraphDefSnap):
        return GrapheneCompositeSolidDefinition(
            represented_pipeline, solid_def_snap.name
        )
