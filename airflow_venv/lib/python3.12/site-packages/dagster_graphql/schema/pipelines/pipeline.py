...
from dagster_graphql.schema.util import (
    ResolveInfo,
    get_compute_log_manager,
    non_null_list,
)
...
class GrapheneRun(graphene.ObjectType):
    ...
    def _get_permission_value(
        self, permission: Permissions, graphene_info: ResolveInfo
    ) -> bool:
        location_name = (
    ...
    def resolve_hasReExecutePermission(self, graphene_info: ResolveInfo):
        return self._get_permission_value(
            Permissions.LAUNCH_PIPELINE_REEXECUTION, graphene_info
        )

    def resolve_hasTerminatePermission(self, graphene_info: ResolveInfo):
        return self._get_permission_value(
            Permissions.TERMINATE_PIPELINE_EXECUTION, graphene_info
        )

    def resolve_hasDeletePermission(self, graphene_info: ResolveInfo):
        return self._get_permission_value(
            Permissions.DELETE_PIPELINE_RUN, graphene_info
        )
    ...
    def resolve_repositoryOrigin(self, _graphene_info: ResolveInfo):
        return (
            GrapheneRepositoryOrigin(
                self.dagster_run.remote_job_origin.repository_origin
            )
            if self.dagster_run.remote_job_origin
            else None
        )
    ...
    def resolve_assetCheckSelection(self, _graphene_info: ResolveInfo):
        return (
            [
                ...
                for handle in self.dagster_run.asset_check_selection
            ]
    ...
    def resolve_parentPipelineSnapshotId(self, graphene_info: ResolveInfo):
        ...
        if (
            ...
            and graphene_info.context.instance.has_job_snapshot(pipeline_snapshot_id)
        ):
            snapshot = graphene_info.context.instance.get_job_snapshot(
    ...
    def resolve_executionPlan(self, graphene_info: ResolveInfo):
        if not (
            ...
            and self.dagster_run.job_snapshot_id
        ):
            return None
    ...
    def resolve_eventConnection(
        self, graphene_info: ResolveInfo, afterCursor=None, limit=None
    ):
    ...
    def resolve_startTime(self, graphene_info: ResolveInfo):
        ...
        if (
            self._run_record.start_time is None
            and self.dagster_run.status in STARTED_STATUSES
        ):
            ...
            if self._run_stats is None or self._run_stats.start_time is None:
                self._run_stats = graphene_info.context.instance.get_run_stats(
                    self.runId
                )
    ...
    def resolve_endTime(self, graphene_info: ResolveInfo):
        if (
            self._run_record.end_time is None
            and self.dagster_run.status in COMPLETED_STATUSES
        ):
    ...
    def resolve_rootConcurrencyKeys(self, graphene_info: ResolveInfo):
        ...
        for (
            concurrency_key,
            count,
        ) in self.dagster_run.run_op_concurrency.root_key_counts.items():
...
class GrapheneIPipelineSnapshotMixin:
    ...
    def resolve_solid_handles(
        ...
        if parentHandleID == "":
            handles = {
                key: handle for key, handle in handles.items() if not handle.parent
            }
    ...
    ) -> List[GrapheneMetadataEntry]:
        represented_pipeline = self.get_represented_job()
        return list(
            iterate_metadata_entries(represented_pipeline.job_snapshot.metadata)
        )
...
class GraphenePipeline(GrapheneIPipelineSnapshotMixin, graphene.ObjectType):
    ...
    def resolve_presets(self, _graphene_info: ResolveInfo):
        return [
            ...
            for preset in sorted(
                self._remote_job.active_presets, key=lambda item: item.name
            )
        ]
    ...
    def resolve_partitionKeysOrError(
        ...
        result = graphene_info.context.get_partition_names(
            ...
            selected_asset_keys=_asset_key_input_list_to_asset_key_set(
                selected_asset_keys
            ),
            instance=graphene_info.context.instance,
        )
...
def _asset_key_input_list_to_asset_key_set(
    ...
    return (
        {key_input.to_asset_key() for key_input in asset_keys}
