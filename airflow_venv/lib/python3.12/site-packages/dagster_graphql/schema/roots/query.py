...
from dagster_graphql.schema.roots.pipeline import (
...
class GrapheneQuery(graphene.ObjectType):
    ...
    @capture_error
    def resolve_workspaceLocationEntryOrError(
        self, graphene_info: ResolveInfo, name: str
    ):
    ...
    def resolve_pipelineSnapshotOrError(
        ...
        if activePipelineSelector:
            ...
            return get_job_snapshot_or_error_from_job_selector(
                graphene_info, job_selector
            )
        elif snapshotId:
    ...
    @capture_error
    def resolve_topLevelResourceDetailsOrError(
        self, graphene_info: ResolveInfo, resourceSelector
    ):
        return get_resource_or_error(
            graphene_info, ResourceSelector.from_graphql_input(resourceSelector)
        )

    def resolve_allTopLevelResourceDetailsOrError(
    ...
    def resolve_sensorOrError(
        ...
        return get_sensor_or_error(
            graphene_info, SensorSelector.from_graphql_input(sensorSelector)
        )
    ...
    async def resolve_pipelineRunOrError(
        self, graphene_info: ResolveInfo, runId: graphene.ID
    ):
    ...
    def resolve_assetNodes(
        ...
        if assetKeys == []:
            ...
            resolved_asset_keys = set(
                AssetKey.from_graphql_input(asset_key_input)
        ...
        if group is not None:
            ...
            repo_loc_entry = graphene_info.context.get_location_entry(
                repo_sel.location_name
            )
        ...
        results = [
            ...
            or remote_node.key in check.not_none(resolved_asset_keys)
        ]
    ...
    @capture_error
    def resolve_partitionBackfillOrError(
        self, graphene_info: ResolveInfo, backfillId: str
    ):
    ...
    def resolve_permissions(self, graphene_info: ResolveInfo):
        ...
        return [
            GraphenePermission(permission, value)
