...
def create_execution_params_and_launch_pipeline_exec(
    graphene_info, execution_params_dict
):
...
class GrapheneLaunchBackfillMutation(graphene.Mutation):
    ...
    def mutate(
        self, graphene_info: ResolveInfo, backfillParams: GrapheneLaunchBackfillParams
    ):
...
def create_execution_params_and_launch_pipeline_reexec(
    graphene_info, execution_params_dict
):
    execution_params = create_execution_params(graphene_info, execution_params_dict)
    assert_permission_for_location(
...
class GrapheneAssetWipeMutation(graphene.Mutation):
    ...
    class Arguments:
        assetPartitionRanges = graphene.Argument(
            non_null_list(GraphenePartitionsByAssetSelector)
        )
    ...
    def mutate(
        ...
        normalized_ranges = [
            AssetPartitionWipeRange(
                ...
                PartitionKeyRange(
                    start=ap.partitions.range.start, end=ap.partitions.range.end
                )
                if ap.partitions
                else None,
            )
...
class GrapheneReportRunlessAssetEventsMutation(graphene.Mutation):
    ...
    class Arguments:
        eventParams = graphene.Argument(
            graphene.NonNull(GrapheneReportRunlessAssetEventsParams)
        )
    ...
    def mutate(
        self,
        graphene_info: ResolveInfo,
        eventParams: GrapheneReportRunlessAssetEventsParams,
    ):
...
class GrapheneDeleteConcurrencyLimitMutation(graphene.Mutation):
    ...
    def mutate(self, graphene_info, concurrencyKey: str):
        graphene_info.context.instance.event_log_storage.delete_concurrency_limit(
            concurrencyKey
        )
        return True
...
class GrapheneFreeConcurrencySlotsForRunMutation(graphene.Mutation):
    ...
    def mutate(self, graphene_info, runId: str):
        graphene_info.context.instance.event_log_storage.free_concurrency_slots_for_run(
            runId
        )
        return True
