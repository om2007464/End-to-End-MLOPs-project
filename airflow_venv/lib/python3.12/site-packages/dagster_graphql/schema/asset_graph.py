...
GrapheneAssetChangedReason = graphene.Enum.from_enum(
    AssetDefinitionChangeType, name="ChangeReason"
)
...
class GrapheneAssetNode(graphene.ObjectType):
    ...
    latestRunForPartition = graphene.Field(
        GrapheneRun, partition=graphene.NonNull(graphene.String)
    )
    ...
    def __init__(
        ...
        *,
        ...
        self._remote_node = check.inst_param(
            remote_node, "remote_node", RemoteAssetNode
        )
    ...
    def get_node_definition_snap(
        ...
        if self._node_definition_snap is None:
            ...
            self._node_definition_snap = graphene_info.context.get_node_def(
                selector, node_key
            )
    ...
    def get_partition_keys(
        ...
        if not self._dynamic_partitions_loader:
            check.failed(
                "dynamic_partitions_loader must be provided to get partition keys"
            )
        ...
        if partitions_snap:
            if isinstance(
                ...
                if (
                    start_idx
                    and end_idx
                    and isinstance(partitions_snap, TimeWindowPartitionsSnap)
                ):
                    ...
                    return (
                        partitions_snap.get_partitions_definition().get_partition_keys(
    ...
    def is_multipartitioned(self) -> bool:
        ...
        return partitions_snap is not None and isinstance(
        ...
        if isinstance(node_def_snap, GraphDefSnap):
            ...
            constituent_resource_key_sets = [
                self.get_required_resource_keys_rec(
                    graphene_info, job.get_node_def_snap(name)
                )
                for name in constituent_node_names
            ]
            return [
                key
                for res_key_set in constituent_resource_key_sets
                for key in res_key_set
            ]
        else:
    ...
    def resolve_assetObservations(
        ...
        if (
            ...
            latest_observation_event = (
                record.asset_entry.last_observation if record else None
            )
    ...
    def resolve_changedReasons(
        ...
        if self.asset_graph_differ is None:
            ...
            return []
        return self.asset_graph_differ.get_changes_for_asset(
            self._asset_node_snap.asset_key
        )
    ...
    def resolve_staleStatus(
        ...
        return self.stale_status_loader.get_status(
    ...
    def resolve_staleStatusByPartition(
        ...
        if partitions is None:
            ...
            self._validate_partitions_existence()
        return [
            self.stale_status_loader.get_status(
                self._asset_node_snap.asset_key, partition
            )
    ...
    def resolve_dataVersionByPartition(
        ...
        return [
            None if version == NULL_DATA_VERSION else version.value
            for version in data_versions
        ]

    def resolve_dependedBy(
        self, graphene_info: ResolveInfo
    ) -> List[GrapheneAssetDependency]:
    ...
    def resolve_dependencies(
        self, graphene_info: ResolveInfo
    ) -> Sequence[GrapheneAssetDependency]:
    ...
    def resolve_autoMaterializePolicy(
        ...
        if self._asset_node_snap.auto_materialize_policy:
            return GrapheneAutoMaterializePolicy(
                self._asset_node_snap.auto_materialize_policy
            )
        return None
    ...
    def resolve_targetingInstigators(
        ...
        if isinstance(self._remote_node, RemoteWorkspaceAssetNode):
            ...
            schedules = [
                ...
                if (schedule := graphene_info.context.get_schedule(schedule_handle))
                is not None
            ]
            sensors = [
                sensor
                for sensor_handle in self._remote_node.get_targeting_sensor_handles()
                if (sensor := graphene_info.context.get_sensor(sensor_handle))
                is not None
            ]
    ...
    def resolve_latestMaterializationByPartition(
        ...
        ordered_materializations = [
            latest_materialization_by_partition.get(partition)
            for partition in partitions
        ]
    ...
    def resolve_latestRunForPartition(
        ...
        planned_info = (
            graphene_info.context.instance.get_latest_planned_materialization_info(
                asset_key=self._asset_node_snap.asset_key, partition=partition
            )
        )
        if not planned_info:
            return None
        run_record = graphene_info.context.instance.get_run_record_by_id(
            planned_info.run_id
        )
    ...
    def resolve_partitionStats(
        ...
        if partitions_snap:
            ...
            failed_and_not_in_progress_subset = (
                failed_partition_subset - in_progress_subset
            )
    ...
    def resolve_tags(
        self, _graphene_info: ResolveInfo
    ) -> Sequence[GrapheneDefinitionTag]:
        return [
    ...
    def resolve_assetChecksOrError(
        ...
        return GrapheneAssetChecks(
            checks=[
                GrapheneAssetCheck(remote_check_node)
