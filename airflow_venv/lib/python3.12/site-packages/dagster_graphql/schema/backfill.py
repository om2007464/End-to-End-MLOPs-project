...
class GrapheneBulkActionStatus(graphene.Enum):
    ...
    def to_dagster_run_status(self) -> GrapheneRunStatus:
        ...
        if (
            self.args[0] == GrapheneBulkActionStatus.REQUESTED.value
        ):  # pyright: ignore[reportAttributeAccessIssue]
            return GrapheneRunStatus.STARTED  # pyright: ignore[reportReturnType]
        if (
            self.args[0] == GrapheneBulkActionStatus.COMPLETED.value
        ):  # pyright: ignore[reportAttributeAccessIssue]
            return GrapheneRunStatus.SUCCESS  # pyright: ignore[reportReturnType]
        if (
            self.args[0] == GrapheneBulkActionStatus.COMPLETED_SUCCESS.value
        ):  # pyright: ignore[reportAttributeAccessIssue]
            return GrapheneRunStatus.SUCCESS  # pyright: ignore[reportReturnType]
        if (
            self.args[0] == GrapheneBulkActionStatus.COMPLETED_FAILED.value
        ):  # pyright: ignore[reportAttributeAccessIssue]
            return GrapheneRunStatus.FAILURE  # pyright: ignore[reportReturnType]
        if (
            self.args[0] == GrapheneBulkActionStatus.FAILED.value
        ):  # pyright: ignore[reportAttributeAccessIssue]
            return GrapheneRunStatus.FAILURE  # pyright: ignore[reportReturnType]
        if (
            self.args[0] == GrapheneBulkActionStatus.CANCELED.value
        ):  # pyright: ignore[reportAttributeAccessIssue]
            return GrapheneRunStatus.CANCELED  # pyright: ignore[reportReturnType]
        if (
            self.args[0] == GrapheneBulkActionStatus.CANCELING.value
        ):  # pyright: ignore[reportAttributeAccessIssue]
...
class GrapheneAssetBackfillTargetPartitions(graphene.ObjectType):
    ...
    ranges = graphene.List(
        graphene.NonNull(
            "dagster_graphql.schema.partition_sets.GraphenePartitionKeyRange"
        )
    )
...
class GrapheneAssetPartitions(graphene.ObjectType):
    ...
    def __init__(
        self, asset_key: AssetKey, partitions_subset: Optional[PartitionsSubset]
    ):
        ...
        super().__init__(
            assetKey=GrapheneAssetKey(path=asset_key.path), partitions=partitions
        )
...
class GrapheneAssetPartitionRange(graphene.ObjectType):
    ...
    def __init__(
        self, asset_key: AssetKey, partition_range: Optional[PartitionKeyRange]
    ):
        super().__init__(
...
class GrapheneAssetBackfillData(graphene.ObjectType):
    ...
    def __init__(self, backfill_job: PartitionBackfill):
        ...
        check.invariant(
            self._backfill_job.is_asset_backfill, "Must be an asset backfill"
        )
    ...
    def resolve_rootTargetedPartitions(self, graphene_info: ResolveInfo):
        try:
            root_partitions_subset = (
                self._backfill_job.get_target_root_partitions_subset(
        ...
        for asset_status in status_per_asset:
            if isinstance(asset_status, PartitionedAssetBackfillStatus):
                ...
                asset_partition_status_counts.append(
                    GrapheneUnpartitionedAssetStatus(
                        assetKey=asset_status.asset_key,
                        inProgress=asset_status.backfill_status
                        is AssetBackfillStatus.IN_PROGRESS,
                        materialized=asset_status.backfill_status
                        is AssetBackfillStatus.MATERIALIZED,
                        failed=asset_status.backfill_status
                        is AssetBackfillStatus.FAILED,
                    )
                )
...
class GraphenePartitionBackfill(graphene.ObjectType):
    ...
    partitionsTargetedForAssetKey = graphene.Field(
        "dagster_graphql.schema.backfill.GrapheneAssetBackfillTargetPartitions",
        asset_key=graphene.Argument(
            "dagster_graphql.schema.inputs.GrapheneAssetKeyInput"
        ),
    )
    ...
    logEvents = graphene.Field(
        graphene.NonNull(
            "dagster_graphql.schema.instigation.GrapheneInstigationEventConnection"
        ),
    ...
    def __init__(self, backfill_job: PartitionBackfill):
        self._backfill_job = check.inst_param(
            backfill_job, "backfill_job", PartitionBackfill
        )
    ...
    def _get_partition_run_data(
        ...
        if (
            ...
            self._partition_run_data = (
                self._get_partition_run_data_for_ranged_job_backfill(
            ...
            self._partition_run_data = (
                graphene_info.context.instance.run_storage.get_run_partition_data(
                    runs_filter=RunsFilter(
                        tags=DagsterRun.tags_for_backfill_id(
    ...
    def resolve_unfinishedRuns(
        ...
        return [
            GrapheneRun(record)
            for record in records
            if not record.dagster_run.is_finished
        ]
    ...
    def resolve_cancelableRuns(
        ...
        return [
            GrapheneRun(record)
            for record in records
            if record.dagster_run.is_cancelable
        ]
    ...
    def resolve_partitionSet(
        self, graphene_info: ResolveInfo
    ) -> Optional["GraphenePartitionSet"]:
    ...
    def resolve_hasCancelPermission(self, graphene_info: ResolveInfo) -> bool:
        ...
        if self._backfill_job.partition_set_origin is None:
            return graphene_info.context.has_permission(
                Permissions.CANCEL_PARTITION_BACKFILL
            )
    ...
    def resolve_logEvents(
        self, graphene_info: ResolveInfo, cursor: Optional[str] = None
    ):
...
class GrapheneBackfillPolicy(graphene.ObjectType):
    ...
    def __init__(self, backfill_policy: BackfillPolicy):
        self._backfill_policy = check.inst_param(
