...
class GrapheneClaimedConcurrencySlot(graphene.ObjectType):
    ...
    def __init__(self, claimed_slot_info: ClaimedSlotInfo):
        super().__init__(
            runId=claimed_slot_info.run_id, stepKey=claimed_slot_info.step_key
        )
...
class GrapheneInstance(graphene.ObjectType):
    ...
    def resolve_info(self, graphene_info: ResolveInfo):
        return (
            self._instance.info_str()
            if graphene_info.context.show_instance_config
            else None
        )
    ...
    def resolve_runQueueConfig(self, _graphene_info: ResolveInfo):
        ...
        if isinstance(self._instance.run_coordinator, QueuedRunCoordinator):
            return GrapheneRunQueueConfig(
                self._instance.run_coordinator.get_run_queue_config()
            )
        else:
            return None
