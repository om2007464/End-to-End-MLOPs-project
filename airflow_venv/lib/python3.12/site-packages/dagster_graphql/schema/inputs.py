...
class GrapheneRunsFilter(graphene.InputObjectType):
    ...
    def to_selector(self):
        ...
        updated_before = (
            datetime_from_timestamp(self.updatedBefore) if self.updatedBefore else None
        )
        updated_after = (
            datetime_from_timestamp(self.updatedAfter) if self.updatedAfter else None
        )
        created_before = (
            datetime_from_timestamp(self.createdBefore) if self.createdBefore else None
        )
        created_after = (
            datetime_from_timestamp(self.createdAfter) if self.createdAfter else None
        )
...
class GraphenePartitionsSelector(graphene.InputObjectType):
    ...
    ranges = graphene.InputField(
        graphene.List(graphene.NonNull(GraphenePartitionRangeSelector))
    )
...
class GrapheneLaunchBackfillParams(graphene.InputObjectType):
    ...
    assetSelection = graphene.InputField(
...
class GrapheneResourceSelector(graphene.InputObjectType):
    ...
    class Meta:
        description = """This type represents the fields necessary to identify a top-level resource."""
...
class GrapheneInstigationSelector(graphene.InputObjectType):
    class Meta:
        ...
        description = """This type represents the fields necessary to identify a schedule or sensor."""
...
class GrapheneBulkActionsFilter(graphene.InputObjectType):
    ...
    def to_selector(self):
        statuses = (
            [BulkActionStatus[status.value] for status in self.statuses]
            if self.statuses
            else None
        )
        created_before = (
