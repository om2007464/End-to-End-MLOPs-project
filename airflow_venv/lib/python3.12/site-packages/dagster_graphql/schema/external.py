...
GrapheneLocationStateChangeEventType = graphene.Enum.from_enum(
    LocationStateChangeEventType
)
...
class GrapheneRepositoryLocation(graphene.ObjectType):
    ...
    dagsterLibraryVersions = graphene.List(
        graphene.NonNull(GrapheneDagsterLibraryVersion)
    )
    ...
    def resolve_repositories(self, graphene_info: ResolveInfo):
        return [
            ...
            for repository in self.get_location(graphene_info)
            .get_repositories()
            .values()
        ]
    ...
    def resolve_server_id(self, graphene_info: ResolveInfo):
        ...
        return (
            location.server_id if isinstance(location, GrpcServerCodeLocation) else None
        )
...
class GrapheneWorkspaceLocationStatusEntry(graphene.ObjectType):
    ...
    def resolve_permissions(self, graphene_info):
        permissions = graphene_info.context.permissions_for_location(
...
class GrapheneWorkspaceLocationEntry(graphene.ObjectType):
    ...
    def __init__(self, location_entry: CodeLocationEntry):
        self._location_entry = check.inst_param(
            location_entry, "location_entry", CodeLocationEntry
        )
    ...
    def resolve_featureFlags(self, graphene_info):
        ...
        return [
            GrapheneFeatureFlag(
                name=feature_flag_name.value, enabled=feature_flag_enabled
            )
...
class GrapheneRepository(graphene.ObjectType):
    ...
    usedSolid = graphene.Field(
        GrapheneUsedSolid, name=graphene.NonNull(graphene.String)
    )
    ...
    def resolve_sensors(
        self, graphene_info: ResolveInfo, sensorType: Optional[SensorType] = None
    ):
        batch_loader = self.get_batch_loader(graphene_info)
        repository = self.get_repository(graphene_info)
        return [
            ...
            for sensor in sorted(
                repository.get_sensors(), key=lambda sensor: sensor.name
            )
    ...
    def resolve_assetGroups(self, graphene_info: ResolveInfo):
        ...
        for asset_node_snap in self.get_repository(
            graphene_info
        ).get_asset_node_snaps():
    ...
    def resolve_allTopLevelResourceDetails(
        self, graphene_info
    ) -> List[GrapheneResourceDetails]:
        return [
