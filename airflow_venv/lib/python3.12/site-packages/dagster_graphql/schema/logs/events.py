...
class AssetEventMixin:
    ...
    runOrError = graphene.NonNull(
        "dagster_graphql.schema.pipelines.pipeline.GrapheneRunOrError"
    )
    ...
    def resolve_tags(self, _graphene_info):
        if self._metadata.tags is None:
            ...
            return [
                GrapheneEventTag(key=key, value=value)
                for key, value in self._metadata.tags.items()
            ]
...
class GrapheneMaterializationEvent(graphene.ObjectType, AssetEventMixin):
    ...
    def __init__(self, event: EventLogEntry, assetLineage=None):
        self._asset_lineage = check.opt_list_param(
            assetLineage, "assetLineage", AssetLineageInfo
        )

        dagster_event = check.not_none(event.dagster_event)
        materialization = dagster_event.step_materialization_data.materialization
        super().__init__(
            **_construct_asset_event_metadata_params(event, materialization)
        )
...
class GrapheneAssetMaterializationPlannedEvent(graphene.ObjectType):
    ...
    def resolve_assetKey(self, _graphene_info: ResolveInfo):
        return (
            self._event.get_dagster_event().asset_materialization_planned_data.asset_key
        )
...
class GrapheneRunStepStats(graphene.ObjectType):
    ...
    def __init__(self, stats):
        ...
        super().__init__(
            ...
            attempts=[
                GrapheneRunMarker(
                    startTime=attempt.start_time, endTime=attempt.end_time
                )
