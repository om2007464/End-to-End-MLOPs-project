...
def get_sensors_or_error(
    ...
    location = graphene_info.context.get_code_location(
        repository_selector.location_name
    )
    repository = location.get_repository(repository_selector.repository_name)
    batch_loader = RepositoryScopedBatchLoader(
        graphene_info.context.instance, repository
    )
...
def get_sensor_or_error(
    graphene_info: ResolveInfo, selector: SensorSelector
) -> "GrapheneSensor":
...
def start_sensor(
    graphene_info: ResolveInfo, sensor_selector: SensorSelector
) -> "GrapheneSensor":
    from dagster_graphql.schema.errors import GrapheneSensorNotFoundError
    from dagster_graphql.schema.sensors import GrapheneSensor
    ...
    if not sensor:
        raise UserFacingGraphQLError(
            GrapheneSensorNotFoundError(sensor_selector.sensor_name)
        )
