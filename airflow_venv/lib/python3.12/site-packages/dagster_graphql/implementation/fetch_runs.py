...
def get_assets_latest_info(
    ...
    asset_nodes = {
        asset_key: graphene_info.context.asset_graph.get(asset_key)
    ...
    latest_planned_run_ids_by_asset = {
        k: v
        for k, v in {
            asset_record.asset_entry.asset_key: _get_latest_planned_run_id(
                instance, asset_record
            )
            for asset_record in asset_records
        }.items()
        if v
        ...
        latest_infos.append(
            GrapheneAssetLatestInfo(
                ...
                latestRun=(
                    GrapheneRun(
                        run_records_by_run_id[
                            latest_planned_run_ids_by_asset[asset_key]
                        ]
                    )
                    ...
                    and latest_planned_run_ids_by_asset[asset_key]
                    in run_records_by_run_id
                    else None
                ),
            )
        )
    ...
    def from_string(serialized: Optional[str]):
        ...
        if len(parts) != 3:
            raise DagsterInvariantViolationError(
                f"Invalid cursor for querying runs: {serialized}"
            )
...
def get_runs_feed_entries(
    ...
    created_before_cursor = (
        datetime_from_timestamp(runs_feed_cursor.timestamp)
        if runs_feed_cursor.timestamp
        else None
    )
    ...
    if filters:
        ...
        with disable_dagster_warnings():
            ...
            run_filters = _replace_created_before_with_cursor(
                run_filters, created_before_cursor
            )
        backfill_filters = (
            _bulk_action_filters_from_run_filters(run_filters)
...
def get_runs_feed_count(
    ...
    if should_fetch_backfills:
        ...
        backfills_count = graphene_info.context.instance.get_backfills_count(
            backfill_filters
        )
    else:
        backfills_count = 0
