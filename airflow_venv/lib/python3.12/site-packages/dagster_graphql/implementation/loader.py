...
class RepositoryScopedBatchLoader:
    ...
    def _get(
        self, data_type: RepositoryDataType, key: str, limit: int
    ) -> Sequence[Any]:
    ...
    def _fetch(self, data_type: RepositoryDataType, limit: int) -> None:
        ...
        if data_type == RepositoryDataType.SCHEDULE_STATES:
            ...
            if self._instance.supports_batch_tick_queries:
                ...
                ticks_by_selector = self._instance.get_batch_ticks(
                    selector_ids, limit=limit
                )
                for schedule in self._repository.get_schedules():
            ...
            if self._instance.supports_batch_tick_queries:
                selector_ids = [
                    schedule.selector_id for schedule in self._repository.get_sensors()
                ]
                ticks_by_selector = self._instance.get_batch_ticks(
                    selector_ids, limit=limit
                )
            ...
            check.failed(
                f"Unknown data type for {self.__class__.__name__}: {data_type}"
            )
    ...
    def get_sensor_ticks(
        self, origin_id: str, selector_id: str, limit: int
    ) -> Sequence[Any]:
        check.invariant(
            any(
