    ...
    def decorator(fn: GrapheneResolverFn) -> GrapheneResolverFn:
        ...
        def _fn(self, graphene_info, *args: P.args, **kwargs: P.kwargs):
            ...
            if not graphene_info.context.was_permission_checked(permission):
                raise Exception(
                    f"Permission {permission} was never checked during the request"
                )
...
def has_permission_for_asset_graph(
    ...
    if asset_keys:
        selectors = [
            asset_graph.get_repository_handle(asset_key) for asset_key in asset_keys
        ]
    else:
...
def assert_permission_for_asset_graph(
    ...
    if not has_permission_for_asset_graph(
        graphene_info, asset_graph, asset_selection, permission
    ):
        raise UserFacingGraphQLError(GrapheneUnauthorizedError())
    ...
    for asset_key, partition_subset in partition_subset_by_asset_key.items():
        ...
        invalid_keys = set(partition_subset.get_partition_keys()) - set(
            partitions_def.get_partition_keys(
                backfill_datetime, dynamic_partitions_store
            )
        )

        if invalid_keys:
            raise UserFacingGraphQLError(
                GraphenePartitionKeysNotFoundError(invalid_keys)
            )
...
def pipeline_selector_from_graphql(data: Mapping[str, Any]) -> JobSubsetSelector:
    asset_selection = cast(
        Optional[Iterable[Dict[str, List[str]]]], data.get("assetSelection")
    )
    ...
    return JobSubsetSelector(
        ...
        asset_check_selection=(
            [
                AssetCheckKey.from_graphql_input(asset_check)
