...
def get_additional_required_keys(
    ...
    required_asset_keys = {
        ...
        if asset_node.asset_node_snap.execution_set_identifier
...
def get_assets_for_run(
    graphene_info: "ResolveInfo", run: DagsterRun
) -> Sequence["GrapheneAsset"]:
    from dagster_graphql.schema.pipelines.pipeline import GrapheneAsset
...
def get_partition_subsets(
    ...
    Optional[PartitionsSubset], Optional[PartitionsSubset], Optional[PartitionsSubset]
]:
    ...
    if instance.can_read_asset_status_cache() and is_cacheable_partition_type(
        partitions_def
    ):
        ...
        materialized_subset = (
            updated_cache_value.deserialize_materialized_partition_subsets(
                partitions_def
            )
        ...
        in_progress_subset = (
            updated_cache_value.deserialize_in_progress_partition_subsets(
                partitions_def
            )
            if updated_cache_value
            else partitions_def.empty_subset()
...
def get_2d_run_length_encoded_partitions(
    ...
    dim2_materialized_partition_subset_by_dim1: Dict[
        str, PartitionsSubset
    ] = defaultdict(lambda: secondary_dim.partitions_def.empty_subset())
    for partition_key in materialized_partitions_subset.get_partition_keys():
        ...
        dim2_materialized_partition_subset_by_dim1[
            ...
            [multipartition_key.keys_by_dimension[secondary_dim.name]]
        )
    ...
    dim2_in_progress_partition_subset_by_dim1: Dict[
        str, PartitionsSubset
    ] = defaultdict(lambda: secondary_dim.partitions_def.empty_subset())
    ...
    unevaluated_idx = 0
    range_start_idx = (
        0  # pointer to first dim1 partition with same dim2 materialization status
    )
    ...
    if (
        ...
        return GrapheneMultiPartitionStatuses(
            ranges=[], primaryDimensionName=primary_dim.name
        )
    ...
    while unevaluated_idx <= len(dim1_keys):
        if (
            ...
            if (
                ...
                > 0
                or len(dim2_failed_partition_subset_by_dim1[dim1_keys[range_start_idx]])
                > 0
                or len(
                    dim2_in_progress_partition_subset_by_dim1[
                        dim1_keys[range_start_idx]
