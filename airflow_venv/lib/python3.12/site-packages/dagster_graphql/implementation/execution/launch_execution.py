...
from dagster_graphql.implementation.execution.run_lifecycle import (
    create_valid_pipeline_run,
)
...
def launch_pipeline_reexecution(
    ...
    return _launch_pipeline_execution(
        graphene_info, execution_params, is_reexecuted=True
    )
...
def launch_reexecution_from_parent_run(
    ...
    parent_run = check.not_none(
        instance.get_run_by_id(parent_run_id),
        f"Could not find parent run with id: {parent_run_id}",
    )
