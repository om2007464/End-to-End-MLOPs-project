...
from dagster._core.execution.asset_backfill import (
    create_asset_backfill_data_from_asset_partitions,
)
    ...
    for asset_key in asset_backfill_data.get_targeted_asset_keys_topological_order(
        ...
        if asset_graph.get(asset_key).partitions_def:
            ...
            asset_partitions.append(
                GrapheneAssetPartitions(
                    asset_key=asset_key, partitions_subset=partitions_subset
                )
            )
        else:
            asset_partitions.append(
...
def create_and_launch_partition_backfill(
    ...
    if backfill_params.get("selector") is not None:  # job backfill
        ...
        selected_assets = list(
            {selector.asset_key for selector in partitions_by_assets}
        )
...
def retry_partition_backfill(
    ...
    if backfill.is_asset_backfill:
        ...
        if from_failure:
            ...
            assets_to_request = (
                assets_to_request - asset_backfill_data.materialized_subset
            )
            ...
            tags={
                **backfill.tags,
                ...
                ROOT_BACKFILL_ID_TAG: backfill.tags.get(
                    ROOT_BACKFILL_ID_TAG, backfill.backfill_id
                ),
            },
            backfill_timestamp=get_current_timestamp(),
