...
def terminate_pipeline_execution(
    ...
    if location_name:
        ...
        if not graphene_info.context.has_permission(
...
def get_chunk_size() -> int:
    return int(
        os.getenv(
            ...
            os.getenv("DAGIT_EVENT_LOAD_CHUNK_SIZE", "1000"),
        )
    )
...
def wipe_assets(
    ...
    for apr in asset_partition_ranges:
        if apr.partition_range is None:
            ...
            if (
                apr.asset_key
                not in graphene_info.context.asset_graph.asset_node_snaps_by_key
            ):
                return GrapheneAssetNotFoundError(asset_key=apr.asset_key)

            node = graphene_info.context.asset_graph.asset_node_snaps_by_key[
                apr.asset_key
            ]
    ...
    result_ranges = [
        GrapheneAssetPartitionRange(
            asset_key=apr.asset_key, partition_range=apr.partition_range
        )
        for apr in asset_partition_ranges
    ]
    ...
    if partition_keys is not None:
        for partition_key in partition_keys:
            instance.report_runless_asset_event(
                create_asset_event(
                    event_type, asset_key, partition_key, description, tags
                )
            )
    else:
        instance.report_runless_asset_event(
