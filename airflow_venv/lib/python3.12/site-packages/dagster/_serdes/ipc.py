...
def read_unary_response(
    ...
    messages = list(
        ipc_read_event_stream(output_file, timeout=timeout, ipc_process=ipc_process)
    )
...
class IPCErrorMessage(
    ...
    def __new__(
        cls, serializable_error_info: SerializableErrorInfo, message: Optional[str]
    ):
...
class FileBasedWriteStream:
    ...
    def send_error(
        self, exc_info: ExceptionInfo, message: Optional[str] = None
    ) -> None:
...
def _send_error(
    file_path: str, exc_info: ExceptionInfo, message: Optional[str]
) -> None:
    return _send(
        file_path,
        IPCErrorMessage(
            serializable_error_info=serializable_error_info_from_exc_info(exc_info),
            message=message,
        ),
    )
...
def _process_line(
    file_pointer: TextIOWrapper, sleep_interval: float = 0.1
) -> Optional[NamedTuple]:
    while True:
