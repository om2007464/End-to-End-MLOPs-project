...
@contextmanager
def postgres_instance_for_test(
    dunder_file, container_name, overrides=None, conn_args=None
):
...
class TestPostgresInstance:
    ...
    def docker_service_up_or_skip(
        ...
        try:
            ...
            warnings.warn(
                f"Error launching Dockerized Postgres: {ex}",
                RuntimeWarning,
                stacklevel=3,
            )
...
class PostgresDockerError(Exception):
    def __init__(self, message, subprocess_error):
        super(PostgresDockerError, self).__init__(
