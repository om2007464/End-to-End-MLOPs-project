...
class TestScheduleStorage:
    ...
    def test_add_multiple_schedules(self, storage):
        ...
        schedule = self.build_schedule(
            "my_schedule", "* * * * *", status=InstigatorStatus.RUNNING
        )
    ...
    def test_get_schedule_state(self, storage):
        ...
        schedule = storage.get_instigator_state(
    ...
    def test_delete_schedule_state(self, storage):
        ...
        schedule = self.build_schedule("my_schedule", "* * * * *")
        storage.add_instigator_state(schedule)
        storage.delete_instigator_state(
            schedule.instigator_origin_id, schedule.selector_id
        )
    ...
    def test_update_tick_to_success(self, storage):
        ...
        with freeze_time(freeze_datetime):
            updated_tick = tick.with_status(TickStatus.SUCCESS).with_run_info(
                run_id="1234"
            )
    ...
    def test_update_tick_to_failure(self, storage):
        ...
        assert tick.run_ids == []
        assert tick.error == SerializableErrorInfo(
            message="Error", stack=[], cls_name="TestError"
        )
    ...
    def test_get_instigator_state(self, storage):
        ...
        state = self.build_sensor("my_sensor")
        storage.add_instigator_state(state)
        state = storage.get_instigator_state(
            state.instigator_origin_id, state.selector_id
        )
    ...
    def test_ticks_filtered(self, storage):
        storage.create_tick(
            self.build_sensor_tick(time.time(), status=TickStatus.STARTED)
        )
        storage.create_tick(
            self.build_sensor_tick(time.time(), status=TickStatus.SUCCESS)
        )
        storage.create_tick(
            self.build_sensor_tick(time.time(), status=TickStatus.SKIPPED)
        )
        storage.create_tick(
            self.build_sensor_tick(
                time.time(),
                ...
                error=SerializableErrorInfo(
                    message="foobar", stack=[], cls_name=None, cause=None
                ),
            )
        )
        ...
        started = storage.get_ticks(
            "my_sensor", "my_sensor", statuses=[TickStatus.STARTED]
        )
        assert len(started) == 1

        successes = storage.get_ticks(
            "my_sensor", "my_sensor", statuses=[TickStatus.SUCCESS]
        )
        assert len(successes) == 1

        skips = storage.get_ticks(
            "my_sensor", "my_sensor", statuses=[TickStatus.SKIPPED]
        )
        assert len(skips) == 1

        failures = storage.get_ticks(
            "my_sensor", "my_sensor", statuses=[TickStatus.FAILURE]
        )
    ...
    def test_ticks_batched(self, storage):
        ...
        _a = storage.create_tick(
            self.build_sensor_tick(
                time.time(), status=TickStatus.SUCCESS, name="sensor_one"
            )
        )
        ...
        _c = storage.create_tick(
            self.build_sensor_tick(
                time.time(), status=TickStatus.SUCCESS, name="sensor_two"
            )
        )
    ...
    def test_auto_materialize_asset_evaluations(self, storage) -> None:
        ...
        for _ in range(2):  # test idempotency
            ...
            assert len(res) == 1
            assert res[0].get_evaluation_with_run_ids().evaluation.key == AssetKey(
                "asset_one"
            )
            ...
            res = storage.get_auto_materialize_evaluations_for_evaluation_id(
                evaluation_id=10
            )
            ...
            assert res[1].get_evaluation_with_run_ids().evaluation.key == AssetKey(
                "asset_two"
            )
        ...
        res = storage.get_auto_materialize_asset_evaluations(
            key=AssetKey("asset_one"), limit=1
        )
        assert len(res) == 1
        assert res[0].evaluation_id == 11
        ...
        eval_asset_three = AutomationConditionEvaluation(
            ...
            end_timestamp=1,
            true_subset=SerializableEntitySubset(
                key=AssetKey("asset_three"), value=True
            ),
            candidate_subset=SerializableEntitySubset(
                key=AssetKey("asset_three"), value=True
            ),
        ...
        res = storage.get_auto_materialize_asset_evaluations(
            key=AssetKey("asset_three"), limit=100
        )

        assert len(res) == 1
        assert res[0].evaluation_id == 11
        assert (
            res[0].get_evaluation_with_run_ids().evaluation
            == eval_asset_three.evaluation
        )
    ...
    def test_automation_condition_evaluations_check_key(self, storage) -> None:
        ...
        res = storage.get_auto_materialize_asset_evaluations(
            key=check_key.asset_key, limit=100
        )
