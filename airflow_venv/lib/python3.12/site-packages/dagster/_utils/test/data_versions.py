...
def get_mats_from_result(
    ...
    for asset_def in assets:
        node_str = (
            asset_def.node_def.name if asset_def.node_def else asset_def.key.path[-1]
        )
...
def get_upstream_version_from_mat_provenance(
    ...
    assert mat.tags
    return mat.tags[
        f"{INPUT_DATA_VERSION_TAG_PREFIX}/{upstream_asset_key.to_user_string()}"
    ]
...
def assert_data_version(
    mat: AssetMaterialization, version: Union[str, DataVersion]
) -> None:
...
def assert_different_versions(
    mat1: AssetMaterialization, mat2: AssetMaterialization
) -> None:
    assert mat1.tags
