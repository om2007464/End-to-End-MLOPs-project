...
def _apply_fold(date: datetime.datetime) -> datetime.datetime:
    ...
    if (
        date.fold == 0
        and date.hour in DAYLIGHT_SAVINGS_HOURS
        and datetime_ambiguous(date)
    ):
...
def _find_hourly_schedule_time(
    ...
    if ascending:
        ...
        if len(minutes) == 1 and (
            already_on_boundary
            or (
                date.minute == minutes[0] and date.second == 0 and date.microsecond == 0
            )
        ):
            # switch to utc so that timedelta behaves as expected instead of doing walltime math
            new_date = date.astimezone(datetime.timezone.utc) + datetime.timedelta(
                hours=1
            )
        ...
        new_timestamp = (
            new_timestamp
            + (SECONDS_PER_MINUTE - new_timestamp % SECONDS_PER_MINUTE)
            % SECONDS_PER_MINUTE
        )
    ...
    return datetime.datetime.fromtimestamp(
        check.not_none(final_timestamp), tz=date.tzinfo
    )
...
def _find_daily_schedule_time(
    ...
    if not already_on_boundary or (
        date.hour != hour
        or date.minute != minute
        or date.second != 0
        or date.microsecond != 0
    ):
...
def _find_weekly_schedule_time(
    ...
    if not already_on_boundary:
        ...
        if day_of_week != current_day_of_week:
            if ascending:
                new_time = new_time + relativedelta(
                    days=(day_of_week - current_day_of_week) % 7
                )
            else:
                new_time = new_time - relativedelta(
                    days=(current_day_of_week - day_of_week) % 7
                )
    else:
        new_time = date
    ...
    try:
        for i, cron_part in enumerate(cron_string.lower().split()):
            ...
            while len(expr_parts) > 0:
                ...
                t = re.sub(
                    r"^\*(\/.+)$",
                    r"%d-%d\1" % (CRON_RANGES[i][0], CRON_RANGES[i][1]),
                    str(expr),
                )
                ...
                if not m:
                    ...
                    t = re.sub(
                        r"^(.+)\/(.+)$", r"\1-%d/\2" % (CRON_RANGES[i][1]), str(expr)
                    )
                    m = CRON_STEP_SEARCH_REGEX.search(t)
                if m:
...
def _croniter_string_iterator(
    ...
    start_datetime = datetime.datetime.fromtimestamp(
        start_timestamp, get_timezone(timezone_str)
    )
    reverse_cron = _timezone_aware_cron_iter(
        cron_string, start_datetime, ascending=not ascending
    )
    next_date = None
    check.invariant(start_offset <= 0)
    ...
    forward_cron = _timezone_aware_cron_iter(
        cron_string, next_date, ascending=ascending
    )
    while True:
...
def reverse_cron_string_iterator(
    ...
    yield from cron_string_iterator(
        end_timestamp, cron_string, execution_timezone, ascending=False
    )
...
def schedule_execution_time_iterator(
    ...
    check.invariant(
        ...
        desc=f"{cron_schedule} must be a valid cron schedule",
    )
    ...
    if isinstance(cron_schedule, str):
        yield from (
            ...
            if ascending
            else reverse_cron_string_iterator(
