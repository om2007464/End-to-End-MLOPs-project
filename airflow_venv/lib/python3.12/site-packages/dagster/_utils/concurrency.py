...
class ConcurrencyKeyInfo:
    ...
    def pending_step_count(self) -> int:
        ...
        return len(
            [step for step in self.pending_steps if step.assigned_timestamp is None]
        )
    ...
    def pending_run_ids(self) -> Set[str]:
        ...
        return set(
            [
                ...
                if step.assigned_timestamp is None
            ]
        )
    ...
    def assigned_step_count(self) -> int:
        return len(
            [step for step in self.pending_steps if step.assigned_timestamp is not None]
        )
    ...
    def assigned_run_ids(self) -> Set[str]:
        return set(
            [
                step.run_id
                for step in self.pending_steps
                if step.assigned_timestamp is not None
            ]
        )
