...
def convert_dagster_submodule_name(
    name: str, mode: Literal["private", "public"]
) -> str:
...
def make_hashable(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return tuple(
            sorted((key, make_hashable(value)) for key, value in value.items())
        )
...
def start_termination_thread(
    ...
    check.inst_param(
        should_stop_event, "should_stop_event", ttype=type(multiprocessing.Event())
    )
...
class EventGenerationManager(Generic[T_GeneratedContext]):
    ...
    def __init__(
        ...
        self.object_cls: Type[T_GeneratedContext] = check.class_param(
...
def normalize_to_repository(
    definitions_or_repository: Optional[
        Union["Definitions", "RepositoryDefinition"]
    ] = ...,
...
def normalize_to_repository(
    ...
    if (definitions_or_repository and repository) or (
        ...
        check.failed(
            "Exactly one of `definitions` or `repository_def` must be provided."
        )
...
def tail_file(
    path_or_fd: Union[str, int], should_stop: Callable[[], bool]
) -> Iterator[str]:
    ...
    **kwargs,
    ...
    with FileLock(target_file_path.with_suffix(".concurrent-update-lock")).acquire(
        ...
        if (
            ...
            and target_file_path.lstat().st_mtime > start_mtime
        ):
            return
        update_fn(**kwargs)
        return
