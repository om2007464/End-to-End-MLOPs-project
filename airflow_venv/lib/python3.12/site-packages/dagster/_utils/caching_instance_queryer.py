...
class CachingInstanceQueryer(DynamicPartitionsStore):
    ...
    def __init__(
        ...
        self._asset_partitions_cache: Dict[
            Optional[int], Dict[AssetKey, Set[str]]
        ] = defaultdict(dict)
        ...
        self._evaluation_time = (
            evaluation_time if evaluation_time else get_current_datetime()
        )
    ...
    def get_materialized_asset_subset(
        ...
        if partitions_def:
            ...
            if cache_value is None:
                ...
                value = cache_value.deserialize_materialized_partition_subsets(
                    partitions_def
                )
        else:
    ...
    def get_in_progress_asset_subset(
        ...
        if partitions_def:
            ...
            if cache_value is None:
                ...
                value = cache_value.deserialize_in_progress_partition_subsets(
                    partitions_def
                )
        else:
            ...
            if not planned_materialization_info:
                ...
                dagster_run = self.instance.get_run_by_id(
                    planned_materialization_info.run_id
                )
                value = (
                    dagster_run is not None
                    and dagster_run.status in IN_PROGRESS_RUN_STATUSES
                )

        return SerializableEntitySubset(key=asset_key, value=value)

    @cached_method
    def get_failed_asset_subset(
        self, *, asset_key: AssetKey
    ) -> SerializableEntitySubset[AssetKey]:
        ...
        if partitions_def:
            ...
            if not planned_materialization_info:
                ...
                value = (
                    dagster_run is not None
                    and dagster_run.status == DagsterRunStatus.FAILURE
                )
    ...
    def _get_latest_materialization_or_observation_storage_ids_by_asset_partition(
        ...
        latest_storage_ids = {
            asset_partition: latest_record.storage_id
    ...
    def asset_partition_has_materialization_or_observation(
        ...
        asset_key = asset_partition.asset_key
        if (
            self.asset_graph.has(asset_key)
            and self.asset_graph.get(asset_key).is_materializable
        ):
            ...
            if (
                ...
                return False
        return (
            self.get_latest_materialization_or_observation_storage_id(asset_partition)
            or 0
        ) > (after_cursor or 0)
    ...
    def _get_planned_materializations_for_run_from_events(
        ...
        return set(
            cast(AssetKey, record.asset_key) for record in materializations_planned
        )

    def get_planned_materializations_for_run(
        self, run_id: str
    ) -> AbstractSet[AssetKey]:
    ...
    @cached_method
    def get_current_materializations_for_run(
        self, *, run_id: str
    ) -> AbstractSet[AssetKey]:
    ...
    def get_active_backfill_datas(self) -> Sequence["AssetBackfillData"]:
        ...
        for backfill in active_backfills:
            try:
                backfill_datas.append(
                    backfill.get_asset_backfill_data(self.asset_graph)
                )
    ...
    def get_active_backfill_in_progress_asset_graph_subset(self) -> AssetGraphSubset:
        ...
        for data in self.get_active_backfill_datas():
            in_progress_subset = (
                ...
                - data.failed_and_downstream_subset
            )
            ...
            if parent_partitions_def is None:
                ...
                for child_partition_key in (
                    ...
                    if not (
                        ...
                        and get_time_partition_key(
                            child_asset.partitions_def, child_partition_key
                        )
                ...
                for child_partition in child_partitions:
                    ...
                    child_asset_partition = AssetKeyPartitionKey(
                        child_asset_key, child_partition
                    )
                    if (
                        ...
                        or child_partition
                        not in self.get_failed_or_in_progress_subset(
                            asset_key=child_asset_key
                        )
                    ):
                        child_asset_partitions_with_updated_parents.add(
    ...
    def _asset_partition_versions_updated_after_cursor(
        ...
        updated_asset_partitions = {
            ...
            and self._asset_partition_versions_updated_after_cursor_cache[ap]
            <= after_cursor
        }
    ...
    def get_asset_partitions_updated_after_cursor(
        ...
        last_storage_id_by_asset_partition = self._get_latest_materialization_or_observation_storage_ids_by_asset_partition(
            asset_key=asset_key
        )
        ...
        if asset_partitions is None:
            ...
            for asset_partition in asset_partitions:
                latest_storage_id = last_storage_id_by_asset_partition.get(
                    asset_partition
                )
                if latest_storage_id is not None and latest_storage_id > (
                    after_cursor or 0
                ):
    ...
    def get_asset_subset_updated_after_time(
        ...
        method = (
            ...
            if self._event_type_for_key(asset_key)
        ...
        first_event_after_time = next(
            iter(
                method(
                    AssetRecordsFilter(
                        asset_key=asset_key, after_timestamp=after_time.timestamp()
                    ),
                    limit=1,
                    ascending=True,
                ).records
    ...
    def get_parent_asset_partitions_updated_after_child(
        ...
        *,
        ...
        """
        parent_asset_partitions_by_key: Dict[
        ...
        ) in parent_asset_partitions_by_key.items():
    ...
    def get_outdated_ancestors(
        ...
        for parent in set(parent_asset_partitions) - updated_parents:
            ...
            root_unreconciled_ancestors.update(
                self.get_outdated_ancestors(asset_partition=parent)
            )
