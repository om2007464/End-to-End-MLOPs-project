...
class TagConcurrencyLimitsCounter:
    ...
    def __init__(
        ...
        check.opt_list_param(
            tag_concurrency_limits, "tag_concurrency_limits", of_type=dict
        )
    ...
    def is_blocked(self, item: Union["DagsterRun", "ExecutionStep"]) -> bool:
        ...
        for key, value in item.tags.items():
            if (
                key in self._key_limits
                and self._key_counts[key] >= self._key_limits[key]
            ):
                return True
            ...
            if (
                ...
                and self._key_value_counts[tag_tuple]
                >= self._key_value_limits[tag_tuple]
            ):
                return True

            if (
                key in self._unique_value_limits
                and self._unique_value_counts[tag_tuple]
                >= self._unique_value_limits[key]
            ):
                return True
...
def get_boolean_tag_value(
    tag_value: Optional[str], default_value: bool = False
) -> bool:
...
VALID_TAG_KEY_REGEX = re.compile(
    r"^([A-Za-z0-9_.-]{1,63}/|"
    + VALID_NESTED_NAMESPACES_TAG_KEYS
    + r")?[A-Za-z0-9_.-]{1,63}$"
)
