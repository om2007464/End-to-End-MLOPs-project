    ...
    def remove_implicit_resolver(cls, tag):
        ...
        for ch, mappings in cls.yaml_implicit_resolvers.items():
            cls.yaml_implicit_resolvers[ch] = [
                (existing_tag, regexp)
                for existing_tag, regexp in mappings
                if existing_tag != tag
            ]
    ...
    class SourcePositionLoader(yaml.SafeLoader, _CanRemoveImplicitResolver):
        def construct_object(
            ...
            source_position = SourcePosition(
                filename=filename,
                start=LineCol(
                    line=node.start_mark.line + 1, col=node.start_mark.column + 1
                ),
            ...
            if isinstance(value, dict):
                ...
                for k, v in cast(
                    Mapping[ValueAndSourcePositionTree, ValueAndSourcePositionTree],
                    value,
                ).items():
            ...
            if isinstance(value, list):
                ...
                for i, v in enumerate(
                    cast(Sequence[ValueAndSourcePositionTree], value)
                ):
