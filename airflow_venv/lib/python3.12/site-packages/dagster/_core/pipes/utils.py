...
class PipesFileMessageReader(PipesMessageReader):
    ...
    def _reader_thread(
        self, handler: "PipesMessageHandler", is_resource_complete: Event
    ) -> None:
        try:
...
class PipesThreadedMessageReader(PipesMessageReader):
    ...
    def _messages_thread(
        ...
        try:
            ...
            while True:
                ...
                if (
                    not can_read_messages
                ):  # this branch will be executed until we can read messages
    ...
    def _log_unstartable_warning(
        self, handler: PipesMessageHandler, params: PipesParams
    ) -> None:
...
class PipesChunkedLogReader(PipesLogReader):
    ...
    def start(self, params: PipesParams, is_session_closed: Event) -> None:
        self.thread = Thread(
            target=self._reader_thread, args=(params, is_session_closed)
        )
        self.thread.start()
...
def _join_thread(thread: Thread, thread_name: str) -> None:
    ...
    if thread.is_alive():
        raise DagsterPipesExecutionError(
            f"Timed out waiting for {thread_name} thread to finish."
        )
...
def extract_message_or_forward_to_stdout(handler: "PipesMessageHandler", log_line: str):
    extract_message_or_forward_to_file(
        handler=handler, log_line=log_line, file=sys.stdout
    )
...
def open_pipes_session(
    ...
    if context.has_assets_def:
        context.set_requires_typed_event_stream(
            error_message=_FAIL_TO_YIELD_ERROR_MESSAGE
        )
