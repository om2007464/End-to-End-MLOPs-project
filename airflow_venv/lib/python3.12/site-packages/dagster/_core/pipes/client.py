    ...
    def get_results(
        ...
        *,
        ...
        return self._session.get_results(
            implicit_materializations=implicit_materializations
        )
...
def materialize_result_from_pipes_results(
    ...
    mat_results: List[MaterializeResult] = [
        mat_result
        for mat_result in all_results
        if isinstance(mat_result, MaterializeResult)
    ]
    ...
    if len(mat_results) == 0:
        raise DagsterPipesError(
            "No materialization results received from external process."
        )
