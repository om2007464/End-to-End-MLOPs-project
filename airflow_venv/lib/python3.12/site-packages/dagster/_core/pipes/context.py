...
class PipesMessageHandler:
    ...
    def _resolve_metadata(
        ...
        return {
            k: self._resolve_metadata_value(v["raw_value"], v["type"])
    ...
    def handle_message(self, message: PipesMessage) -> None:
        ...
        if method == "opened":
            ...
            raise DagsterPipesExecutionError(
                f"Unknown message method: {message['method']}"
            )
    ...
    def _handle_report_asset_materialization(
        ...
        resolved_data_version = (
            None if data_version is None else DataVersion(data_version)
        )
...
def build_external_execution_context_data(
    ...
    provenance_by_asset_key = (
        {
            _convert_asset_key(key): _convert_data_provenance(
                context.get_asset_provenance(key)
            )
            for key in context.selected_asset_keys
        }
        if context.has_assets_def
    ...
    partition_key_range = (
        context.partition_key_range if context.has_partition_key else None
    )
    ...
    return PipesContextData(
        ...
        partition_key_range=(
            _convert_partition_key_range(partition_key_range)
        ...
        job_name=None
        if isinstance(context, BaseDirectExecutionContext)
