...
def get_log_record_event_batch_metadata(
    record: logging.LogRecord,
) -> "DagsterEventBatchMetadata":
    return cast(
        "DagsterEventBatchMetadata",
        getattr(record, LOG_RECORD_EVENT_BATCH_METADATA_ATTR),
    )
...
def construct_log_record_message(metadata: DagsterLogRecordMetadata) -> str:
    ...
    if metadata["dagster_event"] is not None:
        ...
        event_type_str = EVENT_TYPE_TO_DISPLAY_STRING.get(
...
def _error_str_for_event(event: "DagsterEvent") -> Optional[str]:
    ...
    if data:
        ...
        if error:
            error_display_string = getattr(
                data, "error_display_string", error.to_string()
            )
...
class DagsterLogHandler(logging.Handler):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        return (
            self._local_thread_context.should_capture
            and not has_log_record_metadata(record)
        )
...
class DagsterLogManager(logging.Logger):
    ...
    def create(
        ...
        handlers = check.opt_sequence_param(
            handlers, "handlers", of_type=logging.Handler
        )
