...
from dagster._core.execution.plan.plan import (
    ExecutionPlan,
    StepHandleTypes,
    StepHandleUnion,
)
...
def create_execution_plan_snapshot_id(
...
class ExecutionPlanSnapshotErrorData(
    NamedTuple(
        "_ExecutionPlanSnapshotErrorData", [("error", Optional[SerializableErrorInfo])]
    )
):
...
class ExecutionStepSnap(
    ...
    def __new__(
        ...
        return super(ExecutionStepSnap, cls).__new__(
            ...
            tags=check.opt_nullable_mapping_param(
                tags, "tags", key_type=str, value_type=str
            ),
            step_handle=check.opt_inst_param(
                step_handle, "step_handle", StepHandleTypes
            ),
        )
...
class ExecutionStepInputSnap(
    ...
    def upstream_step_keys(self):
        return [
            output_handle.step_key for output_handle in self.upstream_output_handles
        ]
...
def _snapshot_from_step_input(step_input):
    ...
    if isinstance(step_input, (UnresolvedMappedStepInput, UnresolvedCollectStepInput)):
        upstream_output_handles = (
            step_input.get_step_output_handle_deps_with_placeholders()
        )
    else:
...
def _snapshot_from_execution_step(execution_step: IExecutionStep) -> ExecutionStepSnap:
    ...
    return ExecutionStepSnap(
        ...
        inputs=sorted(
            list(map(_snapshot_from_step_input, execution_step.step_inputs)),
            key=lambda si: si.name,
        ),
        outputs=sorted(
