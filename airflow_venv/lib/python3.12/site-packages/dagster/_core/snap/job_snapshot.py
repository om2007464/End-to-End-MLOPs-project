...
class JobSnap(IHaveNew):
    ...
    def from_job_def(cls, job_def: JobDefinition) -> "JobSnap":
        ...
        return JobSnap(
            ...
            tags=job_def.tags,
            run_tags=job_def.run_tags
            if job_def.has_separately_defined_run_tags
            else None,
            metadata=job_def.metadata,
            config_schema_snapshot=build_config_schema_snapshot(job_def),
            dagster_type_namespace_snapshot=build_dagster_type_namespace_snapshot(
                job_def
            ),
            node_defs_snapshot=build_node_defs_snapshot(job_def),
            dep_structure_snapshot=build_dep_structure_snapshot_from_graph_def(
                job_def.graph
            ),
