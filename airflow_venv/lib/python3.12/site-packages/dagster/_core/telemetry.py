...
def telemetry_wrapper(
    target_fn: Optional[T_Callable] = None,
    *,
...
def _telemetry_wrapper(
    ...
    metadata = check.opt_mapping_param(
        metadata, "metadata", key_type=str, value_type=str
    )
...
class TelemetryEntry(
    ...
    def __new__(
        ...
        run_storage_id = check.opt_str_param(
            run_storage_id, "run_storage_id", default=""
        )
...
def _get_telemetry_instance_id() -> Optional[str]:
    telemetry_id_path = os.path.join(
        get_or_create_dir_from_dagster_home(TELEMETRY_STR), "id.yaml"
    )
...
def get_stats_from_remote_repo(remote_repo: "RemoteRepository") -> Mapping[str, str]:
    ...
    for asset in asset_node_snaps:
        ...
        if asset.auto_materialize_policy is not None:
            if (
                asset.auto_materialize_policy.policy_type
    ...
    num_asset_reconciliation_sensors_in_repo = sum(
        1
        for sensor in remote_repo.get_sensors()
        if sensor.name == "asset_reconciliation_sensor"
    )
    ...
    return {
        **get_resource_stats(remote_resources=list(remote_resources)),
        ...
        "num_multi_partitioned_assets_in_repo": str(
            num_multi_partitioned_assets_in_repo
        ),
        ...
        "num_observable_source_assets_in_repo": str(
            num_observable_source_assets_in_repo
        ),
        "num_dbt_assets_in_repo": str(num_dbt_assets_in_repo),
        "num_assets_with_code_versions_in_repo": str(
            num_assets_with_code_versions_in_repo
        ),
        "num_asset_reconciliation_sensors_in_repo": str(
            num_asset_reconciliation_sensors_in_repo
        ),
...
def get_resource_stats(
    remote_resources: Sequence["RemoteResource"],
) -> Mapping[str, Any]:
...
def log_repo_stats(
    ...
    def _get_num_dynamic_partitioned_assets(
        asset_defs: Sequence[AssetsDefinition],
    ) -> int:
        return sum(
            1
    ...
    if _get_instance_telemetry_enabled(instance):
        ...
        if isinstance(job, ReconstructableJob):
            ...
            num_dynamic_partitioned_assets_in_repo = (
                _get_num_dynamic_partitioned_assets(all_assets)
            )
...
def log_action(
    ...
    (
        dagster_telemetry_enabled,
        instance_id,
        run_storage_id,
    ) = _get_instance_telemetry_info(instance)
...
def log_dagster_event(
    event: DagsterEvent, job_context: PlanOrchestrationContext
) -> None:
