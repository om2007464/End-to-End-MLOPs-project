...
class FileCodePointer(
    ...
    def describe(self) -> str:
        if self.working_directory:
            return (
                f"{self.python_file}::{self.fn_name} -- [dir {self.working_directory}]"
            )
        else:
            return f"{self.python_file}::{self.fn_name}"


def _load_target_from_module(
    module: ModuleType, fn_name: str, error_suffix: str
) -> object:
...
class CustomPointer(
    ...
    def __new__(
        ...
        check.inst_param(
            reconstructor_pointer, "reconstructor_pointer", ModuleCodePointer
        )
    ...
    def load_target(self) -> object:
        ...
        return reconstructor(
            *self.reconstructable_args,
            **{key: value for key, value in self.reconstructable_kwargs},
        )
