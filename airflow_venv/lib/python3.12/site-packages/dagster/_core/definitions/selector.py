...
class JobSubsetSelector(IHaveNew):
    ...
    def __new__(
        ...
        asset_check_selection = (
            frozenset(asset_check_selection)
    ...
    def is_subset_selection(self) -> bool:
        return bool(
            self.op_selection or self.asset_selection or self.asset_check_selection
        )

    def with_op_selection(
        self, op_selection: Optional[Sequence[str]]
    ) -> "JobSubsetSelector":
        check.invariant(
...
class PartitionsSelector(IHaveNew):
    ...
    def to_graphql_input(self):
        return {
            "ranges": [
                partition_range.to_graphql_input() for partition_range in self.ranges
            ]
        }
    ...
    def from_graphql_input(graphql_data: Mapping[str, Any]) -> "PartitionsSelector":
        ...
        if "range" in graphql_data:
            return PartitionsSelector(
                ranges=[
                    PartitionRangeSelector.from_graphql_input(graphql_data["range"])
                ]
            )
...
class PartitionsByAssetSelector:
    ...
    def to_graphql_input(self):
        return {
            ...
            "partitions": self.partitions.to_graphql_input()
            if self.partitions
            else None,
        }
    ...
    def from_graphql_input(graphql_data) -> "PartitionsByAssetSelector":
        ...
        return PartitionsByAssetSelector(
            ...
            partitions=PartitionsSelector.from_graphql_input(partitions)
            if partitions
            else None,
        )
