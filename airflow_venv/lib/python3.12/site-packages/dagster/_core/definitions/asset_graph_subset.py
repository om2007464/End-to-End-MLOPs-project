    ...
    def before_pack(self, value: NamedTuple) -> NamedTuple:
        ...
        for field_name, field_value in value._asdict().items():
            if isinstance(field_value, Mapping) and all(
                ...
                subsets_by_key = {
                    k: v.to_serializable_subset() for k, v in field_value.items()
                }
                ...
                if all(isinstance(k, AssetKey) for k in subsets_by_key.keys()):
                    replaced_value_by_field_name[
                        field_name
                    ] = SerializableNonScalarKeyMapping(subsets_by_key)
...
class AssetGraphSubset(NamedTuple):
    ...
    def get_partitions_subset(
        ...
        if asset_graph:
            ...
            if partitions_def is None:
                check.failed(
                    "Can only call get_partitions_subset on a partitioned asset"
                )
    ...
    def __contains__(self, asset: Union[AssetKey, AssetKeyPartitionKey]) -> bool:
        ...
        if isinstance(asset, AssetKey):
            ...
            partitions_subset = self.partitions_subsets_by_asset_key.get(
                asset.asset_key
            )
            return (
                partitions_subset is not None
                and asset.partition_key in partitions_subset
            )
    ...
    def _oper(
        self, other: "AssetGraphSubset", oper: Callable[..., Any]
    ) -> "AssetGraphSubset":
        ...
        for asset_key in other.asset_keys:
            if asset_key in other.non_partitioned_asset_keys:
                ...
                if subset is None and other_subset is not None and oper == operator.or_:
                    ...
                    subset is not None
                    and other_subset is None
                    and oper == operator.and_
                ):
    ...
    def __eq__(self, other) -> bool:
        return (
            ...
            and self.partitions_subsets_by_asset_key
            == other.partitions_subsets_by_asset_key
    ...
    def from_asset_partition_set(
        ...
        return AssetGraphSubset(
            partitions_subsets_by_asset_key={
                asset_key: (
                    cast(
                        PartitionsDefinition, asset_graph.get(asset_key).partitions_def
                    )
                    .empty_subset()
    ...
    def can_deserialize(
        ...
        for key, value in serialized_dict["partitions_subsets_by_asset_key"].items():
            ...
            if not partitions_def.can_deserialize_subset(
                value,
                serialized_partitions_def_unique_id=serializable_partitions_ids.get(
                    key
                ),
    ...
    def from_storage_dict(
        ...
        for key, value in serialized_dict["partitions_subsets_by_asset_key"].items():
            ...
            partitions_subsets_by_asset_key[
                asset_key
            ] = partitions_def.deserialize_subset(value)
