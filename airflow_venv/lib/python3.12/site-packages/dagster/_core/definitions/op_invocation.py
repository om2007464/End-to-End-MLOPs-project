...
def _separate_args_and_kwargs(
    ...
    adjusted_kwargs = {
        ...
        if k not in resources_from_args_and_kwargs and k != "config"
    }
...
def direct_invocation_result(
    ...
    **kwargs,
    ...
    if compute_fn.has_context_arg():
        ...
        if len(args) > 0:
            if args[0] is not None and not isinstance(
                args[0], BaseDirectExecutionContext
            ):
                raise DagsterInvalidInvocationError(
    ...
    extracted = _separate_args_and_kwargs(
        compute_fn, args, kwargs, resource_arg_mapping
    )
    ...
    try:
        ...
        result = invoke_compute_fn(
            ...
            config_arg_cls=(
                compute_fn.get_config_arg().annotation
...
def _resolve_inputs(
    ...
    positional_inputs = cast(
        "DecoratedOpFunction", op_def.compute_fn
    ).positional_inputs()
    ...
    if (
        unassigned_kwargs
        and cast("DecoratedOpFunction", op_def.compute_fn).has_var_kwargs()
    ):
...
def _key_for_result(
    result: AssetResult, context: "BaseDirectExecutionContext"
) -> AssetKey:
...
def _output_name_for_result_obj(
    ...
    if isinstance(event, AssetResult):
        ...
        return (
            context.per_invocation_properties.assets_def.get_output_name_for_asset_key(
                asset_key
            )
        )
        ...
        async def to_gen(async_gen):
            ...
            try:
                ...
                async for event in async_gen:
                    yield _handle_gen_event(
                        event, op_def, context, output_defs, outputs_seen
                    )
            except Exception:
                context.unbind()
                raise
            ...
            for output_def in op_def.output_defs:
                if (
                    ...
                    is_observable_asset = (
                        assets_def is not None and assets_def.is_observable
                    )
...
def _type_check_function_output(
    ...
    output_defs_by_name = {
        output_def.name: output_def for output_def in op_def.output_defs
    }
    op_context = _get_op_context(context)
    for event in validate_and_coerce_op_result_to_iterator(
        result, op_context, op_def.output_defs
    ):
