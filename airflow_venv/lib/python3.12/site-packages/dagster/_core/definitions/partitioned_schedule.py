    ...
    **Example:**
    ...
    if isinstance(job, UnresolvedAssetJobDefinition) and job.partitions_def is None:
        ...
        if isinstance(partitions_def, StaticPartitionsDefinition):
            ...
            time_partitions_def = check.not_none(
...
def _get_schedule_evaluation_fn(
    ...
    [ScheduleEvaluationContext], Union[SkipReason, RunRequest, RunRequestIterator]
]:
...
def _check_valid_schedule_partitions_def(
    ...
    if not has_one_dimension_time_window_partitioning(
        partitions_def
    ) and not isinstance(partitions_def, StaticPartitionsDefinition):
