...
class ResolvedAssetDependencies:
    ...
    def __init__(
        ...
        self._deps_by_assets_def_id = resolve_assets_def_deps(
            assets_defs, source_assets
        )

    def get_resolved_upstream_asset_keys(
        self, assets_def: AssetsDefinition, asset_key: AssetKey
    ) -> AbstractSet[AssetKey]:
        resolved_keys_by_unresolved_key = self._deps_by_assets_def_id.get(
            id(assets_def), {}
        )
        return {
            resolved_keys_by_unresolved_key.get(
                unresolved_dep.asset_key, unresolved_dep.asset_key
            )
    ...
    for asset_key in asset_keys:
        ...
        *asset_key_prefix, asset_key_name = asset_key.path
        ...
        is_similar_prefix_same_name = (
            asset_key_name == target_asset_key_name and is_similar_prefix
        )
        ...
        is_off_by_one_prefix_component_same_name = (
            ...
            and len(
                set(asset_key.path).symmetric_difference(set(target_asset_key.path))
            )
            == 1
        ...
        no_prefix_but_is_match_with_prefix = (
            len(target_asset_key.path) == 1
            and asset_key.path[-1] == target_asset_key.path[-1]
        )
...
def resolve_assets_def_deps(
    ...
    asset_keys_by_group_and_name: Dict[Tuple[str, str], List[AssetKey]] = defaultdict(
        list
    )
        ...
        for input_name, upstream_key in assets_def.keys_by_input_name.items():
            ...
            if upstream_key in all_asset_keys:
                ...
                if not warned:
                    ...
                    warned = True
            elif not assets_def.node_def.input_def_named(
                input_name
            ).dagster_type.is_nothing:
                msg = (
                ...
                similar_names = resolve_similar_asset_names(
                    ...
                    list(
                        itertools.chain.from_iterable(
                            asset_def.keys for asset_def in assets_defs
                        )
                    ),
                )
                if similar_names:
