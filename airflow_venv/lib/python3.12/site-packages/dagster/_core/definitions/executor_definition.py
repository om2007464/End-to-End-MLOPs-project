...
ExecutorRequirementsFunction: TypeAlias = Callable[
    [ExecutorConfig], Sequence[ExecutorRequirement]
]
...
class ExecutorDefinition(NamedConfigurableDefinition):
    ...
    def configured(
        ...
        return self.copy_for_configured(
            name or self.name, description, new_config_schema
        )
...
def _core_multiprocess_executor_creation(
    ...
    return MultiprocessExecutor(
        ...
        explicit_forkserver_preload=check.opt_list_elem(
            start_cfg, "preload_modules", of_type=str
        ),
    )
...
def check_cross_process_constraints(init_context: "InitExecutorContext") -> None:
    ...
    requirements_lst = init_context.executor_def.get_requirements(
...
def _get_default_executor_requirements(
    ...
    return (
        ...
        if "multiprocess" in executor_config
        else []
    )
