...
class RemoteAssetNode(BaseAssetNode, ABC):
    ...
    def is_partitioned(self) -> bool:
        return (
            self.resolve_to_singular_repo_scoped_node().asset_node_snap.partitions
            is not None
        )
    ...
    def freshness_policy(self) -> Optional[FreshnessPolicy]:
        ...
        return (
            self.resolve_to_singular_repo_scoped_node().asset_node_snap.freshness_policy
        )
...
class RemoteWorkspaceAssetNode(RemoteAssetNode):
    ...
    def is_materializable(self) -> bool:
        return any(
            info.asset_node.is_materializable for info in self.repo_scoped_asset_infos
        )

    @cached_property
    def is_observable(self) -> bool:
        return any(
            info.asset_node.is_observable for info in self.repo_scoped_asset_infos
        )
    ...
    def is_executable(self) -> bool:
        return any(
            node.asset_node.is_executable for node in self.repo_scoped_asset_infos
        )
    ...
    def auto_observe_interval_minutes(self) -> Optional[float]:
        return (
            self._observable_node_snap.auto_observe_interval_minutes
    ...
    def backfill_policy(self) -> Optional[BackfillPolicy]:
        return (
            self._materializable_node_snap.backfill_policy
            if self.is_materializable
            else None
        )
...
class RemoteAssetGraph(
    BaseAssetGraph[TRemoteAssetNode], ABC, Generic[TRemoteAssetNode]
):
    @property
    @abstractmethod
    ...
    def get_execution_set_asset_and_check_keys(
        ...
        if isinstance(entity_key, AssetKey):
            ...
            return self.remote_asset_check_nodes_by_key[
                entity_key
    ...
    def asset_checks(self) -> Sequence["AssetCheckNodeSnap"]:
        return [
            node.asset_check for node in self.remote_asset_check_nodes_by_key.values()
        ]

    @cached_property
    def _asset_check_nodes_by_asset_key(
        self,
    ) -> Mapping[AssetKey, Sequence[RemoteAssetCheckNode]]:
        by_asset_key = {}
    ...
    def get_materialization_asset_keys_for_job(
        self, job_name: str
    ) -> Sequence[AssetKey]:
    ...
    def build(cls, repo: RemoteRepository):
        ...
        for asset_snap in repo.get_asset_node_snaps():
            ...
            assets_by_key[key] = RemoteRepositoryAssetNode(
                ...
                execution_set_entity_keys=execution_sets_by_id[id]
                if id is not None
                else {key},
...
class RemoteWorkspaceAssetGraph(RemoteAssetGraph[RemoteWorkspaceAssetNode]):
    ...
    def get_repository_handle(self, key: EntityKey) -> RepositoryHandle:
        if isinstance(key, AssetKey):
            return (
                self.get(key).resolve_to_singular_repo_scoped_node().repository_handle
            )
        else:
    ...
    def split_entity_keys_by_repository(
        ...
        for key in keys:
            ...
            keys_by_repo[(repo_handle.location_name, repo_handle.repository_name)].add(
                key
            )
    ...
    def build(cls, workspace: WorkspaceSnapshot):
        ...
        asset_infos_by_key: Dict[
            AssetKey, List[RepositoryScopedAssetInfo]
        ] = defaultdict(list)
        asset_checks_by_key: Dict[AssetCheckKey, RemoteAssetCheckNode] = {}
        for repo in repos:
...
def _warn_on_duplicate_nodes(
    ...
    _warn_on_duplicates_within_subset(
        materializable_duplicates, AssetExecutionType.MATERIALIZATION
    )
    _warn_on_duplicates_within_subset(
        observable_duplicates, AssetExecutionType.OBSERVATION
    )
