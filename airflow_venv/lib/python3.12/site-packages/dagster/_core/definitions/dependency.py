...
DependencyMapping: TypeAlias = Mapping[
    T_DependencyKey, Mapping[str, "IDependencyDefinition"]
]
...
class NodeInvocation(
    ...
    def __new__(
        ...
        return super().__new__(
            ...
            hook_defs=check.opt_set_param(
                hook_defs, "hook_defs", of_type=HookDefinition
            ),
            retry_policy=check.opt_inst_param(
...
class Node(ABC):
    ...
    def __init__(
        ...
        self._hook_defs = check.opt_set_param(
            hook_defs, "hook_defs", of_type=HookDefinition
        )
        self._retry_policy = check.opt_inst_param(
            retry_policy, "retry_policy", RetryPolicy
        )
    ...
    def container_mapped_fan_in_input(
        self, input_name: str, fan_in_index: int
    ) -> InputMapping:
        mapping = self.graph_definition.input_mapping_for_pointer(
            FanInInputPointer(self.name, input_name, fan_in_index)
        )
...
class GraphNode(Node):
    ...
    def __init__(
        ...
        super().__init__(
            name, definition, graph_definition, tags, hook_defs, retry_policy
        )
        ...
        for requirement in self.definition.get_resource_requirements(
            ...
            if (
                ...
                and outer_container.dependency_structure.has_deps(
                    NodeInput(
                        self, self.definition.input_def_named(requirement.input_name)
                    )
                )
...
class NodeInput(
    NamedTuple("_NodeInput", [("node", Node), ("input_def", InputDefinition)])
):
...
class NodeOutput(
    NamedTuple("_NodeOutput", [("node", Node), ("output_def", OutputDefinition)])
):
    ...
    def __eq__(self, other: Any) -> bool:
        return (
            ...
            and self.output_def.name == other.output_def.name
        )
...
class DependencyDefinition(
    NamedTuple(
        ...
        [("node", str), ("output", str), ("description", Optional[str])],
    ),
...
class MultiDependencyDefinition(
    ...
    def get_node_dependencies(self) -> Sequence[DependencyDefinition]:
        ...
        return [
            dep for dep in self.dependencies if isinstance(dep, DependencyDefinition)
        ]
...
class DynamicCollectDependencyDefinition(
    NamedTuple(
        "_DynamicCollectDependencyDefinition",
        [("node_name", str), ("output_name", str)],
    ),
    IDependencyDefinition,
):
    def get_node_dependencies(self) -> Sequence[DependencyDefinition]:
...
def _create_handle_dict(
    ...
    for node_name, input_dict in dep_dict.items():
        ...
        for input_name, dep_def in input_dict.items():
            if isinstance(
                dep_def,
                (MultiDependencyDefinition, BlockingAssetChecksDependencyDefinition),
            ):
                ...
                for inner_dep in dep_def.get_dependencies_and_mappings():
                    if isinstance(inner_dep, DependencyDefinition):
                        handles.append(
                            node_dict[inner_dep.node].get_output(inner_dep.output)
                        )
...
class DependencyStructure:
    ...
    def __init__(
        ...
        for node_input, (
            dep_type,
            node_output_or_list,
        ) in self._input_to_output_map.items():
            ...
            for node_output in node_output_list:
                self._node_output_index[node_output.node.name][node_output].append(
                    node_input
                )

    def _validate_and_set_fan_out(
        self, node_input: NodeInput, node_output: NodeOutput
    ) -> None:
        """Helper function for populating _dynamic_fan_out_index."""
        if not node_input.node.definition.input_supports_dynamic_output_dep(
            node_input.input_name
        ):
            raise DagsterInvalidDefinitionError(
    ...
    def get_dependency_definition(
        self, node_input: NodeInput
    ) -> Optional[IDependencyDefinition]:
    ...
    def get_deps_list(self, node_input: NodeInput) -> Sequence[NodeOutput]:
        ...
        if dep_type == DependencyType.DIRECT:
            ...
            return [
                handle for handle in handle_or_list if isinstance(handle, NodeOutput)
            ]
        else:
            check.failed(f"Unexpected dep type {dep_type}")
    ...
    def get_upstream_dynamic_output_for_node(
        self, node_name: str
    ) -> Optional[NodeOutput]:
