...
def is_reserved_asset_job_name(name: str) -> bool:
    ...
    return name == IMPLICIT_ASSET_JOB_NAME or name.startswith(
...
def build_asset_job(
    ...
    resource_defs = merge_dicts(
        {DEFAULT_IO_MANAGER_KEY: default_job_io_manager}, resource_defs
    )
...
def get_asset_graph_for_job(
    ...
    other_keys = {
        *(
            k
            for ad in executable_assets_defs
            for k in ad.node_keys_by_input_name.values()
        ),
        *(
            k
            for ad in executable_assets_defs
            for k in ad.node_keys_by_output_name.values()
        ),
    } - selected_keys
    ...
    unexecutable_assets_defs = [
        create_unexecutable_external_asset_from_assets_def(ad)
            ...
            excluded_assets.add(
                asset.subset_for(
                    ...
                    selected_asset_check_keys=(
                        asset.check_keys - subset_asset.check_keys
                    ),
                )
            )
...
def _infer_and_validate_common_partitions_def(
    ...
    for key in asset_keys:
        ...
        if partitions_def is not None:
            if (
                required_partitions_def is not None
                and partitions_def != required_partitions_def
            ):
                raise DagsterInvalidDefinitionError(
...
def build_node_deps(
    ...
    assets_defs = sorted(
        asset_graph.assets_defs, key=lambda ad: (sorted((ak for ak in ad.keys)))
    )
        ...
        for input_name, upstream_asset_key in inputs_map.items():
            ...
            if upstream_asset_key not in {ck.asset_key for ck in assets_def.check_keys}:
                blocking_asset_check_output_handles = (
                    blocking_asset_check_output_handles_by_asset_key.get(
                        upstream_asset_key
                    )
                )
                asset_check_deps = [
            ...
            if upstream_asset_key in node_alias_and_output_by_asset_key:
                ...
                ) = node_alias_and_output_by_asset_key[upstream_asset_key]
...
def _attempt_resolve_node_cycles(asset_graph: AssetGraph) -> AssetGraph:
    ...
    def _dfs(key: AssetKey, cur_color: int):
        ...
        cur_node_asset_keys = (
            node.assets_def.keys if node.is_materializable else node.child_keys
        )
    ...
    for assets_def, color_mapping in color_mapping_by_assets_defs.items():
        if (
            assets_def.is_external
            or len(color_mapping) == 1
            or not assets_def.can_subset
        ):
