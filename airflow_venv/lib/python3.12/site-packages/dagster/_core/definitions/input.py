...
def _check_default_value(
    input_name: str, dagster_type: DagsterType, default_value: T
) -> T:
    from dagster._core.types.dagster_type import BuiltinScalarDagsterType
...
class InputDefinition:
    ...
    def __init__(
        ...
        asset_key: Optional[
            Union[AssetKey, Callable[["InputContext"], AssetKey]]
        ] = None,
        asset_partitions: Optional[
            Union[Set[str], Callable[["InputContext"], Set[str]]]
        ] = None,
        ...
        self._default_value = _check_default_value(
            self._name, self._dagster_type, default_value
        )

        self._input_manager_key = check.opt_str_param(
        ...
        if callable(asset_partitions):
            ...
            _asset_partitions = check.set_param(
    ...
    def default_value(self) -> Any:
        check.invariant(
            self.has_default_value, "Can only fetch default_value if has_default_value"
        )
...
class InputPointer(
    NamedTuple("_InputPointer", [("node_name", str), ("input_name", str)])
):
...
class FanInInputPointer(
    NamedTuple(
        "_FanInInputPointer",
        [("node_name", str), ("input_name", str), ("fan_in_index", int)],
    )
):
...
class InputMapping(NamedTuple):
    ...
    def describe(self) -> str:
        idx = (
            self.maps_to.fan_in_index
            if isinstance(self.maps_to, FanInInputPointer)
            else ""
        )
...
class In(
    NamedTuple(
        ...
        [
            ...
            (
                "asset_key",
                PublicAttr[
                    Optional[Union[AssetKey, Callable[["InputContext"], AssetKey]]]
                ],
            ),
            (
                "asset_partitions",
                PublicAttr[
                    Optional[Union[Set[str], Callable[["InputContext"], Set[str]]]]
                ],
            ),
    ...
    def __new__(
        ...
        return super(In, cls).__new__(
            ...
            asset_key=check.opt_inst_param(
                asset_key, "asset_key", (AssetKey, FunctionType)
            ),
            asset_partitions=asset_partitions,
            input_manager_key=check.opt_str_param(
                input_manager_key, "input_manager_key"
            ),
        )
    ...
    def to_definition(self, name: str) -> InputDefinition:
        dagster_type = (
            self.dagster_type if self.dagster_type is not NoValueSentinel else None
        )
        return InputDefinition(
            name=name,
            dagster_type=dagster_type,
