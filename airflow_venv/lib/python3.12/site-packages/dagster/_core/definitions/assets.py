...
class AssetsDefinition(ResourceAddable, IHasInternalInit):
    ...
    def __init__(
        ...
        *,
        ...
        # TODO: FOU-243
        auto_materialize_policies_by_key: Optional[
        ...
        automation_conditions_by_key = (
            {
                ...
                for k, v in auto_materialize_policies_by_key.items()
            }
        ...
        if node_def is None:
            ...
            check.invariant(
                not can_subset, "node_def is None, so can_subset must be False"
            )
            ...
            selected_asset_keys, selected_asset_check_keys = _resolve_selections(
                ...
                all_check_keys={
                    spec.key for spec in (check_specs_by_output_name or {}).values()
                },
            ...
            if partition_mappings:
                _validate_partition_mappings(
                    ...
                    input_asset_keys=set(
                        computation_not_none.keys_by_input_name.values()
                    ),
        ...
        for spec in resolved_specs:
            ...
            if self._computation is not None:
                (
                    output_def,
                    _,
                ) = self._computation.full_node_def.resolve_output_to_origin(
            ...
            description = (
                spec.description or output_def_description or node_def_description
            )
    ...
    def __call__(self, *args: object, **kwargs: object) -> object:
        ...
        if (
            self._computation
            and isinstance(self._computation.node_def, GraphDefinition)
    ...
    def from_graph(
        ...
        *,
        ...
        metadata_by_output_name: Optional[
            Mapping[str, Optional[ArbitraryMetadataMapping]]
        ] = None,
        tags_by_output_name: Optional[Mapping[str, Optional[Mapping[str, str]]]] = None,
        freshness_policies_by_output_name: Optional[
            Mapping[str, Optional[FreshnessPolicy]]
        ] = None,
    ...
    def _from_node(
        ...
        *,
        ...
        specs = _asset_specs_from_attr_key_params(
            ...
            group_names_by_key=group_names_by_key,
            freshness_policies_by_key=_output_dict_to_asset_dict(
            ...
            code_versions_by_key=_output_dict_to_asset_dict(
    ...
    def group_names_by_key(self) -> Mapping[AssetKey, str]:
        ...
        """
        return {
            key: check.not_none(spec.group_name)
    ...
    def node_def(self) -> NodeDefinition:
        ...
        return check.not_none(
            self._computation, "This AssetsDefinition has no node_def"
        ).node_def
    ...
    def asset_deps(self) -> Mapping[AssetKey, AbstractSet[AssetKey]]:
        ...
        """
        return {
            key: {dep.asset_key for dep in spec.deps}
            for key, spec in self._specs_by_key.items()
        }
    ...
    def check_specs_by_output_name(self) -> Mapping[str, AssetCheckSpec]:
        return {
            ...
            or spec.key in self._computation.selected_asset_check_keys
        }
    ...
    def keys_by_input_name(self) -> Mapping[str, AssetKey]:
        upstream_keys = {
            *(
                ...
                for dep in self._specs_by_key[key].deps
            ),
            *(
                spec.asset_key
                for spec in self.check_specs
                if spec.asset_key not in self.keys
            ),
        }
    ...
    def is_auto_created_stub(self) -> bool:
        return (
            self._get_external_asset_metadata_value(
                SYSTEM_METADATA_KEY_AUTO_CREATED_STUB_ASSET
            )
            is not None
        )
    ...
    def get_partition_mapping_for_dep(
        self, dep_key: AssetKey
    ) -> Optional[PartitionMapping]:
    ...
    def has_output_for_asset_key(self, key: AssetKey) -> bool:
        return (
            self._computation is not None
            and key in self._computation.output_names_by_key
        )
    ...
    def with_attributes(
        ...
        *,
        ...
        replaced_attributes = dict(
            ...
            selected_asset_keys={
                output_asset_key_replacements.get(key, key) for key in self.keys
            },
            backfill_policy=backfill_policy
            if backfill_policy
            else self.backfill_policy,
    ...
    def map_asset_specs(
        self, fn: Callable[[AssetSpec], AssetSpec]
    ) -> "AssetsDefinition":
        mapped_specs = []
        for spec in self.specs:
            ...
            if (
                ...
                mapped_spec.deps is not spec.deps
                and mapped_spec.deps != spec.deps
            ):
                raise DagsterInvalidDefinitionError(
    ...
    def get_io_manager_key_for_asset_key(self, key: AssetKey) -> str:
        if self._computation is None:
            ...
            if SYSTEM_METADATA_KEY_IO_MANAGER_KEY in self._specs_by_key[key].metadata:
                return self._specs_by_key[key].metadata[
                    SYSTEM_METADATA_KEY_IO_MANAGER_KEY
                ]
            check.invariant(
                SYSTEM_METADATA_KEY_IO_MANAGER_KEY
                not in self._specs_by_key[key].metadata
            )
    ...
    def __str__(self):
        if len(self.keys) == 1:
            ...
            asset_keys = ", ".join(
                sorted(([asset_key.to_string() for asset_key in self.keys]))
            )
            return f"AssetsDefinition with keys {asset_keys}"

    @cached_property
    def unique_id(self) -> str:
        return unique_id_from_asset_and_check_keys(
            itertools.chain(self.keys, self.check_keys)
        )
    ...
    def get_attributes_dict(self) -> Dict[str, Any]:
        return dict(
            ...
            specs=self.specs,
            is_subset=self.is_subset,
            execution_type=self._computation.execution_type
            if self._computation
            else None,
        )
...
def _validate_graph_def(
    ...
    forward_edges, _ = create_adjacency_lists(
        graph_def.nodes, graph_def.dependency_structure
    )
    leaf_nodes = {
        node_name
    ...
    mapped_output_nodes = {
        output_mapping.maps_from.node_name
    ...
    if selected_asset_keys is None and selected_asset_check_keys is None:
        ...
        if selected_asset_check_keys is None:
            ...
            resolved_selected_asset_check_keys = {
                ...
                if key.asset_key in resolved_selected_asset_keys
            }
        else:
...
def _asset_specs_from_attr_key_params(
    ...
    for key in all_asset_keys:
        if deps_by_asset_key:
            dep_objs = [
                AssetDep(
                    asset=key, partition_mapping=(partition_mappings or {}).get(key)
                )
        ...
        with disable_dagster_warnings():
            result.append(
                AssetSpec.dagster_internal_init(
                    ...
                    automation_condition=validated_automation_conditions_by_key.get(
                        key
                    ),
    ...
    for input_key in input_keys:
        if input_key in output_keys_set:
            if input_key in partition_mappings:
                ...
                time_window_partition_mapping = (
                    get_self_dep_time_window_partition_mapping(
...
def get_self_dep_time_window_partition_mapping(
    ...
    if isinstance(partition_mapping, TimeWindowPartitionMapping):
        ...
        time_partition_mapping = (
            partition_mapping.downstream_mappings_by_upstream_dimension.get(
...
def _resolve_partitions_def(
    ...
    if specs:
        ...
        if len(asset_keys_by_partitions_def) > 1:
            (
                partition_1_asset_keys,
                partition_2_asset_keys,
                *_,
