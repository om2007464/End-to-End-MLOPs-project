...
def _check_node_defs_arg(
    ...
    for node_def in _node_defs:
        if isinstance(node_def, NodeDefinition):
            ...
            raise DagsterInvalidDefinitionError(
                f"Invalid item in node list: {node_def!r}"
            )
...
class GraphDefinition(NodeDefinition):
    ...
    def __init__(
        ...
        *,
        ...
        node_input_source_assets: Optional[
        ...
        **kwargs: Any,
        ...
        self._input_mappings = check.opt_sequence_param(
        ...
        self._composition_fn = check.opt_callable_param(
    ...
    def default_value_for_input(self, input_name: str) -> object:
        ...
        return mapped_node.definition.default_value_for_input(
    ...
    def config_schema(self) -> Optional[IDefinitionConfigSchema]:
        return (
            self.config_mapping.config_schema
            if self.config_mapping is not None
            else None
        )
    ...
    def input_supports_dynamic_output_dep(self, input_name: str) -> bool:
        ...
        return self.node_named(
            target_node
        ).definition.input_supports_dynamic_output_dep(mapping.maps_to.input_name)
    ...
    def copy_for_configured(
        ...
        return self.copy(
            ...
            description=check.opt_str_param(
                description, "description", default=self.description
            ),
    ...
    def get_resource_requirements(
        ...
        for node in self.node_dict.values():
            yield from node.get_resource_requirements(
                outer_container=self, asset_layer=asset_layer
            )
    ...
    def resolve_output_to_destinations(
        ...
        for mapping in self.output_mappings:
            ...
            output_def = output_node.definition.output_def_named(
                output_pointer.output_name
            )
    ...
    def get_op_handles(self, parent: NodeHandle) -> AbstractSet[NodeHandle]:
        return {
            ...
            for op_handle in node.definition.get_op_handles(
    ...
    def get_op_input_output_handle_pairs(
        ...
        for node in self.nodes:
            ...
            if isinstance(node.definition, GraphDefinition):
                result.update(
                    node.definition.get_op_input_output_handle_pairs(node_handle)
                )

            for (
                node_input,
                upstream_outputs,
            ) in self.dependency_structure.input_to_upstream_outputs_for_node(
                node.name
            ).items():
                op_input_handles = (
                    node_input.node.definition.resolve_input_to_destinations(
                        NodeInputHandle(
                            node_handle=node_handle,
                            input_name=node_input.input_def.name,
                        )
                    )
                )
                for op_input_handle in op_input_handles:
                    for upstream_node_output in upstream_outputs:
                        (
                            ...
                            NodeHandle(
                                upstream_node_output.node.name, parent=outer_handle
                            ),
                        )
                        origin_output_handle = NodeOutputHandle(
                            node_handle=origin_node_handle,
                            output_name=origin_output_def.name,
                        )
...
class SubselectedGraphDefinition(GraphDefinition):
    ...
    def get_top_level_omitted_nodes(self) -> Sequence[Node]:
        return [
            node
            for node in self.parent_graph_def.nodes
            if not self.has_node_named(node.name)
        ]
...
def _validate_in_mappings(
    ...
    target_input_types_by_graph_input_name: Dict[str, Set[DagsterType]] = defaultdict(
        set
    )
    ...
    for mapping in input_mappings:
        ...
        if mapping.maps_to_fan_in:
            ...
            mapping_keys.add(
                f"{maps_to.node_name}.{maps_to.input_name}.{maps_to.fan_in_index}"
            )
        ...
        if dependency_structure.has_fan_in_deps(node_input):
            for idx, dep in enumerate(dependency_structure.get_fan_in_deps(node_input)):
                if dep is MappedInputPlaceholder:
                    mapping_str = (
                        f"{node_input.node_name}.{node_input.input_name}.{idx}"
                    )
    ...
    for graph_input_name, graph_input_def in input_defs_by_name.items():
        if graph_input_def.dagster_type.kind == DagsterTypeKind.ANY:
            target_input_types = target_input_types_by_graph_input_name[
                graph_input_name
            ]
            if len(target_input_types) == 1:
                input_defs_by_name[
                    graph_input_name
                ] = graph_input_def.with_dagster_type(next(iter(target_input_types)))
