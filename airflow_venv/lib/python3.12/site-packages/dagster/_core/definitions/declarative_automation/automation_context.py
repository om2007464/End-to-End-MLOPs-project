...
class AutomationContext(Generic[T_EntityKey]):
    ...
    @staticmethod
    def create(
        key: EntityKey, evaluator: "AutomationConditionEvaluator"
    ) -> "AutomationContext":
        asset_graph_view = evaluator.asset_graph_view
        condition = check.not_none(
            evaluator.asset_graph.get(key).automation_condition
            or evaluator.default_condition
        )
        condition_unqiue_id = condition.get_node_unique_id(
    ...
    def cursor(self) -> Optional[str]:
        ...
        return (
            self._node_cursor.get_structured_cursor(as_type=str)
    ...
    def previous_candidate_subset(self) -> Optional[EntitySubset[T_EntityKey]]:
        ...
        """
        candidate_subset = (
            self._node_cursor.candidate_subset if self._node_cursor else None
        )
    ...
    def get_structured_cursor(
        ...
        return (
            self._node_cursor.get_structured_cursor(as_type=as_type)
            if self._node_cursor
            else None
        )
