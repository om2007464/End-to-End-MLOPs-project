...
@whitelist_for_serdes(
    serializer=EntitySubsetSerializer, storage_field_names={"key": "asset_key"}
)
class ValidAssetSubset(SerializableEntitySubset[AssetKey]):
    ...
    def _oper(
        self, other: "ValidAssetSubset", oper: Callable[..., Any]
    ) -> "ValidAssetSubset":
    ...
    def _is_compatible_with_subset(self, other: "SerializableEntitySubset") -> bool:
        if isinstance(
            other.value, (BaseTimeWindowPartitionsSubset, AllPartitionsSubset)
        ):
    ...
    def all(
        ...
        if partitions_def is None:
            ...
            return ValidAssetSubset(
                key=asset_key,
                value=AllPartitionsSubset(
                    partitions_def, dynamic_partitions_store, current_time
                ),
            )
