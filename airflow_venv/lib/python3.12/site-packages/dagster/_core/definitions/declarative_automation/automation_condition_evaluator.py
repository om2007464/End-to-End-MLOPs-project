...
class AutomationConditionEvaluator:
    def __init__(
        ...
        *,
        ...
        self.legacy_auto_materialize_run_tags: Mapping[
            str, str
        ...
        self.legacy_expected_data_time_by_key: Dict[
            AssetKey, Optional[datetime.datetime]
        ] = {}
    ...
    def evaluated_asset_keys_and_parents(self) -> AbstractSet[AssetKey]:
        ...
        return {
            parent
            for ek in asset_keys
            for parent in self.asset_graph.get(ek).parent_keys
        } | asset_keys
    ...
    def asset_records_to_prefetch(self) -> Sequence[AssetKey]:
        return [
            key
            for key in self.evaluated_asset_keys_and_parents
            if self.asset_graph.has(key)
        ]
    ...
    async def async_evaluate(
        ...
        async def _evaluate_entity_async(entity_key: EntityKey, offset: int):
            ...
            if result.true_subset.is_partitioned:
                requested_str = ",".join(
                    result.true_subset.expensively_compute_partition_keys()
                )
            else:
    ...
    async def evaluate_entity(self, key: EntityKey) -> None:
        ...
        result = await AutomationContext.create(
            key=key, evaluator=self
        ).evaluate_async()
        ...
        if isinstance(key, AssetKey):
            self.legacy_expected_data_time_by_key[
                key
