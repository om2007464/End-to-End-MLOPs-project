...
StructuredCursor = Union[
    str, SerializableEntitySubset, Sequence[SerializableEntitySubset]
]
...
class AutomationConditionEvaluation(Generic[T_EntityKey]):
    ...
    def with_run_ids(
        self, run_ids: AbstractSet[str]
    ) -> "AutomationConditionEvaluationWithRunIds":
        return AutomationConditionEvaluationWithRunIds(
            evaluation=self, run_ids=frozenset(run_ids)
        )
...
class AutomationConditionCursor(Generic[T_EntityKey]):
    ...
    def backcompat_from_evaluation_state(
        ...
        return AutomationConditionCursor(
            ...
            effective_timestamp=evaluation_state.previous_tick_evaluation_timestamp
            or 0,
            last_event_id=evaluation_state.max_storage_id,
            node_cursors_by_unique_id=_get_node_cursors(
                evaluation_state.previous_evaluation
            ),
            result_value_hash="",
        )
    ...
    def from_result(
        ...
        def _gather_node_cursors(r: "AutomationResult"):
            node_cursors = (
                {r.condition_unique_id: r.node_cursor} if r.node_cursor else {}
            )
