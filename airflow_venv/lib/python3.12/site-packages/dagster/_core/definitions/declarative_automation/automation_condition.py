...
class AutomationCondition(ABC, Generic[T_EntityKey]):
    ...
    def get_snapshot(
        ...
        return AutomationConditionSnapshot(
            node_snapshot=node_snapshot, children=children
        )

    def get_node_unique_id(
        self, *, parent_unique_id: Optional[str], index: Optional[int]
    ) -> str:
    ...
    def get_unique_id(
        self,
        *,
        parent_node_unique_id: Optional[str] = None,
        ...
        return non_secure_md5_hash_str(
            "".join([node_unique_id, *child_unique_ids]).encode()
        )
    ...
    def has_rule_condition(self) -> bool:
        from dagster._core.definitions.declarative_automation.legacy import (
            RuleCondition,
        )
    ...
    def any_deps_in_progress() -> "DepsAutomationCondition":
        ...
        ).with_label("any_deps_in_progress")
    ...
    def all_deps_blocking_checks_passed() -> "DepsAutomationCondition":
        ...
        return AutomationCondition.all_deps_match(
            AutomationCondition.all_checks_match(
                AutomationCondition.check_passed()
                | AutomationCondition.will_be_requested(),
                blocking_only=True,
...
class AutomationResult(Generic[T_EntityKey]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._subsets_with_metadata = check.opt_sequence_param(
            kwargs.get("subsets_with_metadata"),
        ...
        invalid_hidden_params = set(kwargs.keys()) - {
            "subsets_with_metadata",
            "structured_cursor",
        }
        check.param_invariant(
            not invalid_hidden_params,
            "kwargs",
            f"Invalid hidden params: {invalid_hidden_params}",
        )
        check.param_invariant(
    ...
    def value_hash(self) -> str:
        ...
        components: Sequence[str] = [
            ...
            *(
                _compute_subset_with_metadata_value_str(swm)
                for swm in self._subsets_with_metadata
            ),
    ...
    def serializable_evaluation(self) -> AutomationConditionEvaluation:
        return AutomationConditionEvaluation(
            condition_snapshot=self.condition.get_node_snapshot(
                self.condition_unique_id
            ),
            true_subset=self.get_serializable_subset(),
            candidate_subset=get_serializable_candidate_subset(
            ...
            child_evaluations=[
                child_result.serializable_evaluation
                for child_result in self._child_results
            ],
        )

    def set_internal_serializable_subset_override(
        self, override: SerializableEntitySubset
    ) -> None:
    ...
    def get_child_node_cursors(self) -> Mapping[str, AutomationConditionNodeCursor]:
        node_cursors = (
            {self.condition_unique_id: self.node_cursor} if self.node_cursor else {}
        )
        for child_result in self._child_results:
    ...
    def get_serializable_subset(self) -> SerializableEntitySubset:
        return (
            self.true_subset.convert_to_serializable_subset()
            or self._serializable_subset_override
        )
...
def _compute_subset_with_metadata_value_str(
    subset_with_metadata: AssetSubsetWithMetadata,
):
