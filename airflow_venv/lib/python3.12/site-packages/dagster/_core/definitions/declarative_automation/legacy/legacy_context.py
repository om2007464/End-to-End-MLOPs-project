...
class LegacyRuleEvaluationContext:
    ...
    def create(asset_key: AssetKey, evaluator: "AutomationConditionEvaluator"):
        instance_queryer = (
            evaluator.asset_graph_view.get_inner_queryer_for_back_compat()
        )

        cursor = evaluator.cursor.get_previous_condition_cursor(asset_key)
        condition = check.not_none(
            evaluator.asset_graph.get(asset_key).automation_condition
        )
    ...
    @property
    @root_property
    def _previous_tick_discarded_subset(
        self,
    ) -> Optional[SerializableEntitySubset[AssetKey]]:
        ...
        unique_id = not_discard_condition.get_node_unique_id(
            parent_unique_id=unique_id, index=2
        )
        ...
        unique_id = discard_condition.get_node_unique_id(
            parent_unique_id=unique_id, index=0
        )
        ...
        discard_cursor = (
            self.cursor.node_cursors_by_unique_id.get(unique_id)
            if self.cursor
            else None
        )
    ...
    def materialized_requested_or_discarded_since_previous_tick_subset(
        ...
        return (
            ...
            | self.previous_tick_requested_subset
        )
    ...
    def candidates_not_evaluated_on_previous_tick_subset(self) -> ValidAssetSubset:
        ...
        # were *not* evaluated on the previous tick
            ...
            self.node_cursor.candidate_subset, HistoricalAllPartitionsSubsetSentinel
        ):
            return self.empty_subset()
        return self.candidate_subset - self.node_cursor.candidate_subset

    def materializable_in_same_run(
        self, child_key: AssetKey, parent_key: AssetKey
    ) -> bool:
    ...
    def get_parents_that_will_not_be_materialized_on_current_tick(
        ...
        return {
            ...
            or not self.materializable_in_same_run(
                asset_partition.asset_key, parent.asset_key
            )
        }

    def will_update_asset_partition(
        self, asset_partition: AssetKeyPartitionKey
    ) -> bool:
    ...
    def add_evaluation_data_from_previous_tick(
        ...
        for (
            frozen_metadata,
            asset_partitions,
        ) in asset_partitions_by_frozen_metadata.items():
        ...
        for elt in self.previous_subsets_with_metadata:
            carry_forward_subset = (
                ValidAssetSubset.coerce_from_subset(elt.subset, self.partitions_def)
                - ignore_subset
            )
