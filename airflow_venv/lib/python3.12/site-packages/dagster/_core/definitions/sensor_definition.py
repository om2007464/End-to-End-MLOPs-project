...
class SensorEvaluationContext:
    ...
    def caching_dynamic_partitions_loader(self):
        return (
            CachingDynamicPartitionsLoader(self.instance) if self.instance_ref else None
        )
...
RawSensorEvaluationFunction: TypeAlias = Callable[
...
def get_context_param_name(fn: Callable[..., Any]) -> Optional[str]:
    ...
    return next(
        (
            ...
            if param.name not in resource_params
        ),
        None,
    )
...
class SensorDefinition(IHasInternalInit):
    ...
    def with_updated_jobs(
        self, new_jobs: Sequence[ExecutableDefinition]
    ) -> "SensorDefinition":
    ...
    def __init__(
        ...
        *,
        ...
        if evaluation_fn is None:
            raise DagsterInvalidDefinitionError(
                "Must provide evaluation_fn to SensorDefinition."
            )
        ...
        self._asset_selection = (
            AssetSelection.from_coercible(asset_selection)
            if asset_selection is not None
            else None
        )
        validate_resource_annotated_function(self._raw_fn)
        resource_arg_names: Set[str] = {
            arg.name for arg in get_resource_args(self._raw_fn)
        }
    ...
    def __call__(self, *args, **kwargs) -> RawSensorEvaluationFunctionReturn:
        ...
        context_param = (
            {context_param_name_if_present: context}
            if context_param_name_if_present
            else {}
        )
    ...
    def jobs(self) -> List[ExecutableDefinition]:
        ...
        if not targets:
            raise DagsterInvalidDefinitionError(
                "No job was provided to SensorDefinition."
            )
    ...
    def evaluate_tick(
        ...
        if not result or result == [None]:
            ...
            item = check.inst(
                result[0], (SkipReason, RunRequest, DagsterRunReaction, SensorResult)
            )
            ...
            if isinstance(item, SensorResult):
                ...
                skip_message = (
                    ...
                    if item.skip_reason
                        ...
                        None
                        if run_requests
                        else "Sensor function returned an empty result"
                    )
                )
                ...
                if context.cursor_updated and item.cursor:
                    ...
                    updated_cursor = (
                        item.cursor
                    )  # overwrite value set from context above
                ...
                skip_message = (
                    item.skip_message if isinstance(item, SkipReason) else None
                )
        ...
        (
            run_requests_for_backfill_daemon,
            run_requests_for_single_runs,
        ) = split_run_requests(run_requests)
    ...
    def resolve_run_requests(
        ...
        if asset_selection is not None:
            run_requests = [
                *_run_requests_with_base_asset_jobs(
                    run_requests, context, asset_selection
                )
            ]
        ...
        for run_request in run_requests:
            ...
            if run_request.partition_key and not run_request.has_resolved_partition():
                if run_request.asset_selection:
                    ...
                    partitions_defs = {
                        asset_graph.get(k).partitions_def
                        for k in run_request.asset_selection
                    }
                    defined_partitions_defs = {
                        pd for pd in partitions_defs if pd is not None
                    }
                    check.invariant(
                ...
                selected_job = _get_repo_job_by_name(
                    context,
                    run_request.job_name if run_request.job_name else target_names[0],
                )
    ...
    def has_anonymous_job(self) -> bool:
        return bool(
            self._target
            and self._target.job_name.startswith(ANONYMOUS_ASSET_JOB_PREFIX)
        )
...
class SensorExecutionData(
    NamedTuple(
        ...
        [
            ...
            (
                "asset_events",
                Sequence[
                    Union[AssetMaterialization, AssetObservation, AssetCheckEvaluation]
                ],
            ),
            (
                "automation_condition_evaluations",
    ...
    def __new__(
        ...
        automation_condition_evaluations: Optional[
            Sequence[AutomationConditionEvaluation]
        ] = None,
    ):
        ...
        check.opt_sequence_param(
            dagster_run_reactions, "dagster_run_reactions", DagsterRunReaction
        )
...
def get_sensor_context_from_args_or_kwargs(
    ...
    if len(args) > 0:
        ...
        context = check.opt_inst(
            kwargs.get(context_param_name or "context"), context_type
        )
...
def _run_requests_with_base_asset_jobs(
    ...
    for run_request in run_requests:
        ...
        if run_request.asset_check_keys is not None:
            ...
            asset_check_keys = KeysAssetSelection(
                selected_keys=list(asset_keys)
            ).resolve_checks(asset_graph)
