...
class AssetLayer(NamedTuple):
    ...
    def from_graph_and_assets_node_mapping(
        ...
        node_output_handles_by_asset_check_key: Mapping[
            AssetCheckKey, NodeOutputHandle
        ] = {}
        check_names_by_asset_key_by_node_handle: Dict[
            NodeHandle, Dict[AssetKey, Set[str]]
        ] = {}
        ...
        for node_handle, assets_def in assets_defs_by_outer_node_handle.items():
            ...
            for (
                input_name,
                input_asset_key,
            ) in assets_def.node_keys_by_input_name.items():
                ...
                node_input_handles = full_node_def.resolve_input_to_destinations(
                    input_handle
                )
            ...
            for output_name, asset_key in assets_def.node_keys_by_output_name.items():
                ...
                (
                    inner_output_def,
                    inner_node_handle,
                ) = full_node_def.resolve_output_to_origin(
            ...
            if len(assets_def.check_specs_by_output_name) > 0:
                ...
                for (
                    output_name,
                    check_spec,
                ) in assets_def.check_specs_by_output_name.items():
                    (
                        inner_output_def,
                        inner_node_handle,
                    ...
                    check_names_by_asset_key_by_node_handle[node_handle][
                        check_spec.asset_key
                    ].add(check_spec.name)
                    check_key_by_output[node_output_handle] = check_spec.key

                assets_defs_by_check_key.update(
                    {k: assets_def for k in assets_def.check_keys}
                )

            if computation is not None:
                for op_handle in computation.node_def.get_op_handles(
                    parent=node_handle
                ):
    ...
    def asset_key_for_input(
        self, node_handle: NodeHandle, input_name: str
    ) -> Optional[AssetKey]:
        return self.asset_keys_by_node_input_handle.get(
            NodeInputHandle(node_handle=node_handle, input_name=input_name)
        )

    def input_for_asset_key(
        self, node_handle: NodeHandle, key: AssetKey
    ) -> Optional[str]:
        return next(
            (
    ...
    def downstream_dep_assets_and_checks(
        ...
        return {
            ...
            ).entity_keys_by_dep_op_output_handle[
    ...
    def upstream_dep_op_handles(self, asset_key: AssetKey) -> AbstractSet[NodeHandle]:
        computation = check.not_none(
            self.asset_graph.get(asset_key).assets_def.computation
        )
        op_handles_in_assets_def = computation.dep_op_handles_by_entity_key[asset_key]
        outer_node_handle = NodeHandle(
            self.outer_node_names_by_asset_key[asset_key], parent=None
        )
        return {
            outer_node_handle.with_child(op_handle)
            for op_handle in op_handles_in_assets_def
        }
