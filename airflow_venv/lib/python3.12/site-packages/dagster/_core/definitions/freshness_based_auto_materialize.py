    ...
    if freshness_policy.cron_schedule:
        ...
        return TimeWindow(
            ...
            start=effective_data_time + 0.9 * freshness_policy.maximum_lag_delta,
                ...
                effective_data_time + freshness_policy.maximum_lag_delta, current_time
            ),
        )
...
def get_execution_period_and_evaluation_data_for_policies(
    ...
    for period, policy in sorted(
        (
            (
                get_execution_period_for_policy(
                    policy, effective_data_time, current_time
                ),
                policy,
            )
    ...
    if not contains_local and not contains_downstream:
        ...
        evaluation_data = TextRuleEvaluationData(
            "Required by downstream asset's policy"
        )
        ...
        for parent_key in asset_graph.get(asset_key).parent_keys:
            ...
            if isinstance(
                ...
                if parent_repo != asset_graph.get_repository_handle(asset_key):
                    return context.data_time_resolver.get_current_data_time(
            ...
            parent_expected_data_time = context.expected_data_time_mapping.get(
                parent_key
            ) or context.data_time_resolver.get_current_data_time(
                parent_key, current_time
            )
...
def freshness_evaluation_results_for_asset_key(
    ...
    current_data_time = context.data_time_resolver.get_current_data_time(
        asset_key, current_time
    )
    ...
    (
        ...
        policies=context.asset_graph.get_downstream_freshness_policies(
            asset_key=asset_key
        ),
    ...
    if (
        ...
        return (
            ...
            [
                AssetSubsetWithMetadata(
                    subset=all_subset, metadata=evaluation_data.metadata
                )
            ],
        )
    else:
