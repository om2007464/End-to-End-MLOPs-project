...
class SkipReason(
    NamedTuple("_SkipReason", [("skip_message", PublicAttr[Optional[str]])])
):
...
class RunRequest(IHaveNew, LegacyNamedTupleMixin):
    ...
    def has_resolved_partition(self) -> bool:
        ...
        # partitioning
        return (
            self.tags.get(PARTITION_NAME_TAG) is not None
    ...
    def partition_key_range(self) -> Optional[PartitionKeyRange]:
        if (
            ...
            return PartitionKeyRange(
                self.tags[ASSET_PARTITION_RANGE_START_TAG],
                self.tags[ASSET_PARTITION_RANGE_END_TAG],
            )
        else:
            return None
...
class DynamicPartitionsStoreAfterRequests(DynamicPartitionsStore):
    ...
    def from_requests(
        ...
        added_partition_keys_by_partitions_def_name: Dict[str, Set[str]] = defaultdict(
            set
        )
        deleted_partition_keys_by_partitions_def_name: Dict[
            str, Set[str]
        ] = defaultdict(set)
        ...
        for req in dynamic_partitions_requests:
            ...
            if isinstance(req, AddDynamicPartitionsRequest):
                added_partition_keys_by_partitions_def_name[name].update(
                    set(req.partition_keys)
                )
            elif isinstance(req, DeleteDynamicPartitionsRequest):
                deleted_partition_keys_by_partitions_def_name[name].update(
                    set(req.partition_keys)
                )
            else:
    ...
    def get_dynamic_partitions(self, partitions_def_name: str) -> Sequence[str]:
        partition_keys = set(
            self.wrapped_dynamic_partitions_store.get_dynamic_partitions(
                partitions_def_name
            )
        )
    ...
    def has_dynamic_partition(
        ...
        return (
            partition_key
            not in self.deleted_partition_keys_by_partitions_def_name.get(
                partitions_def_name, set()
            )
            and (
                partition_key
                in self.added_partition_keys_by_partitions_def_name.get(
                    partitions_def_name, set()
                )
    ...
    NamedTuple(
        ...
        [
            ...
            (
                "asset_events",
                List[
                    Union[AssetObservation, AssetMaterialization, AssetCheckEvaluation]
                ],
            ),
            (
    ...
    def __new__(
        ...
        **kwargs,
        ...
        skip_reason = check.opt_inst_param(
            skip_reason, "skip_reason", (SkipReason, str)
        )
        ...
        return super(SensorResult, cls).__new__(
            ...
            run_requests=check.opt_sequence_param(
                run_requests, "run_requests", RunRequest
            ),
