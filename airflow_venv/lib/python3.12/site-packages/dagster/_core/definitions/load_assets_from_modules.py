...
def assets_from_modules(
    ...
    for module in modules:
        for asset in find_objects_in_module_of_types(
            ...
            asset = cast(
                Union[AssetsDefinition, SourceAsset, CacheableAssetsDefinition], asset
            )
...
def prefix_assets(
    ...
    """
    asset_keys = {
        asset_key for assets_def in assets_defs for asset_key in assets_def.keys
    }
    check_target_keys = {
    ...
    for assets_def in assets_defs:
        output_asset_key_replacements = {
            ...
            for asset_key in (
                assets_def.keys
                | {check_key.asset_key for check_key in assets_def.check_keys}
            )
        }
    ...
    if source_key_prefix:
        result_source_assets = [
            source_asset.with_attributes(
                key=AssetKey([*source_key_prefix, *source_asset.key.path])
            )
...
def assets_with_attributes(
    ...
    if key_prefix:
        ...
        cacheable_assets = [
            cached_asset.with_prefix_for_all(key_prefix)
