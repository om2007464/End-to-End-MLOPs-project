...
class OpDefinition(NodeDefinition, IHasInternalInit):
    ...
    def __init__(
        ...
        config_schema: Optional[
            Union[UserConfigSchema, IDefinitionConfigSchema]
        ] = None,
        ...
        super(OpDefinition, self).__init__(
            name=name,
            input_defs=check.sequence_param(
                resolved_input_defs, "input_defs", InputDefinition
            ),
            output_defs=check.sequence_param(
    ...
    def outs(self) -> Mapping[str, Out]:
        ...
        return {
            output_def.name: Out.from_definition(output_def)
...
def _resolve_output_defs_from_outs(
    ...
    if outs is None:
        return [
            OutputDefinition.create_from_inferred(
                inferred_output_props, default_code_version
            )
        ]
    ...
    if len(outs) == 1:
        ...
        only_out = outs[name]
        return [
            only_out.to_definition(annotation, name, description, default_code_version)
        ]
