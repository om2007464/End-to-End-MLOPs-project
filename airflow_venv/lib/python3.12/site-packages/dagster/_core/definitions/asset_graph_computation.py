...
class AssetGraphComputation(IHaveNew):
    ...
    def entity_keys_by_op_output_handle(
        ...
        for output_name, key in itertools.chain(
            ...
            output_def, node_handle = self.full_node_def.resolve_output_to_origin(
                output_name, None
            )
            result[
                NodeOutputHandle(node_handle=node_handle, output_name=output_def.name)
            ] = key
    ...
    def subset_for(
        ...
        if selected_asset_check_keys is None:
            ...
            asset_check_subselection = {
                key
                for key in self.selected_asset_check_keys
                if key.asset_key in asset_subselection
            }
        else:
            asset_check_subselection = (
                selected_asset_check_keys & self.selected_asset_check_keys
            )
    ...
    def entity_keys_by_dep_op_output_handle(
        ...
        for op_output_handle in reverse_toposorted_op_outputs_handles:
            asset_key_or_check_key = entity_keys_by_op_output_handle.get(
                op_output_handle
            )
            ...
            if asset_key_or_check_key:
                ...
                child_op_output_handles = op_output_graph.downstream.get(
                    op_output_handle, set()
                )
...
class OpOutputHandleGraph:
    ...
    def from_graph(graph_def: "GraphDefinition") -> "OpOutputHandleGraph":
        ...
        op_output_handles_by_op_handle: Dict[
            NodeHandle, Set[NodeOutputHandle]
        ] = defaultdict(set)
        for op_output_handle in op_output_handles:
            op_output_handles_by_op_handle[op_output_handle.node_handle].add(
                op_output_handle
            )
