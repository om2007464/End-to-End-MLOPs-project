...
from dagster._core.definitions.backfill_policy import (
    BackfillPolicy,
...
class JobDefinition(IHasInternalInit):
    ...
    def __init__(
        ...
        *,
        ...
        _subset_selection_data: Optional[
            Union[OpSelectionData, AssetSelectionData]
        ] = None,
        ...
        self._name = (
            check_valid_name(check.str_param(name, "name")) if name else graph_def.name
        )
        self._executor_def = check.opt_inst_param(
        ...
        input_values = check.opt_mapping_param(
            input_values, "input_values", key_type=str
        )
        ...
        if partitions_def:
            ...
            if isinstance(config, ConfigMapping):
                ...
                if asset_layer:
                    for (
                        asset_key
                    ) in asset_layer.asset_keys_by_node_output_handle.values():
    ...
    def run_config_schema(self) -> "RunConfigSchema":
        if self._run_config_schema is None:
            self._run_config_schema = _create_run_config_schema(
                self, self.required_resource_keys
            )
    ...
    def partitions_def(self) -> Optional[PartitionsDefinition]:
        ...
        return (
            ...
            else self.partitioned_config.partitions_def
        )

    @cached_property
    def backfill_policy(self) -> BackfillPolicy:
        executable_nodes = {
            self.asset_layer.get(k) for k in self.asset_layer.executable_asset_keys
        }
        backfill_policies = {
            n.backfill_policy for n in executable_nodes if n.is_partitioned
        }
    ...
    def _get_required_resource_keys(
        self, validate_requirements: bool = False
    ) -> AbstractSet[str]:
        ...
        required_keys = {
            req.key for req in requirements if isinstance(req, ResourceKeyRequirement)
        }
        if validate_requirements:
    ...
    def get_all_hooks_for_handle(
        self, handle: NodeHandle
    ) -> AbstractSet[HookDefinition]:
    ...
    def execute_in_process(
        ...
        run_config = check.opt_mapping_param(
            convert_config_input(run_config), "run_config"
        )
        ...
        ephemeral_job = JobDefinition.dagster_internal_init(
            ...
            resource_defs={
                **_swap_default_io_man(bound_resource_defs, self),
                **resource_defs,
            },
        ...
        if partition_key:
            ...
            if not run_config and self.partitioned_config:
                run_config = self.partitioned_config.get_run_config_for_partition_key(
                    partition_key
                )
        ...
        if self.partitions_def:
            ...
            if selected_asset_keys:
                ...
                resolved_selected_asset_keys = [
                    key
                    for key in self.asset_layer.asset_keys_by_node_output_handle.values()
                ]
    ...
    def get_tags_for_partition_key(
        ...
        if self._partitioned_config is not None:
            return self._partitioned_config.get_tags_for_partition_key(
    ...
    def get_run_config_for_partition_key(self, partition_key: str) -> Mapping[str, Any]:
        if self._partitioned_config:
            return self._partitioned_config.get_run_config_for_partition_key(
                partition_key
            )
        else:
            return {}
    ...
    def _get_job_def_for_op_selection(
        self, op_selection: Iterable[str]
    ) -> "JobDefinition":
        try:
            sub_graph = get_graph_subset(
                self.graph, op_selection, selected_outputs_by_op_handle={}
            )
    ...
    def _copy(self, **kwargs: Any) -> "JobDefinition":
        ...
        resolved_kwargs = {
            **base_kwargs,
            **kwargs,
        }  # base kwargs overwritten for conflicts
    ...
    def with_top_level_resources(
        ...
        resource_defs = check.mapping_param(
            resource_defs, "resource_defs", key_type=str
        )
    ...
    def with_logger_defs(
        self, logger_defs: Mapping[str, LoggerDefinition]
    ) -> "JobDefinition":
    ...
    def op_selection(self) -> Optional[AbstractSet[str]]:
        return (
            set(self.op_selection_data.op_selection) if self.op_selection_data else None
        )
    ...
    def resolved_op_selection(self) -> Optional[AbstractSet[str]]:
        return (
            self.op_selection_data.resolved_op_selection
            if self.op_selection_data
            else None
        )
...
def _swap_default_io_man(
    resources: Mapping[str, ResourceDefinition], job: JobDefinition
):
    ...
    if module_name and attribute_name:
        ...
        try:
            ...
            with build_resources(
                {"io_manager": attr}, instance=init_context.instance
            ) as resources:
...
def default_job_io_manager_with_fs_io_manager_schema(
    ...
    base_dir = init_context.resource_config.get(
        "base_dir",
        init_context.instance.storage_directory() if init_context.instance else None,
    )
...
def _infer_asset_layer_from_source_asset_deps(
    job_graph_def: GraphDefinition,
) -> AssetLayer:
...
def _build_all_node_defs(
    node_defs: Sequence[NodeDefinition],
) -> Mapping[str, NodeDefinition]:
...
def _create_run_config_schema(
    ...
    if job_def.is_subset:
        if isinstance(
            job_def.graph, SubselectedGraphDefinition
        ):  # op selection provided
            ...
            ignored_nodes = [
                ...
                if not job_def.has_node_named(node.name)
            ]
    else:
        ignored_nodes = []
