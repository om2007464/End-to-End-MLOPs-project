...
class PersistedTimeWindow(
    NamedTuple(
        "_PersistedTimeWindow",
        [("start", TimestampWithTimezone), ("end", TimestampWithTimezone)],
    )
):
    ...
    def subtract(self, other: "PersistedTimeWindow") -> Sequence["PersistedTimeWindow"]:
        ...
        if (
            end_timestamp <= other_start_timestamp
        ...
        if other_start_timestamp > start_timestamp:
            windows.append(
                PersistedTimeWindow(
                    start=self._asdict()["start"], end=other._asdict()["start"]
                ),
            )
        ...
        if other_end_timestamp < end_timestamp:
            windows.append(
                PersistedTimeWindow(
                    start=other._asdict()["end"], end=self._asdict()["end"]
                )
            )
...
class TimeWindowPartitionsDefinition(PartitionsDefinition, IHaveNew):
    ...
    def start(self) -> datetime:
        ...
        return datetime_from_timestamp(
            start_timestamp_with_timezone.timestamp,
            start_timestamp_with_timezone.timezone,
        )
    ...
    def get_num_partitions_in_window(self, time_window: TimeWindow) -> int:
        ...
        if fixed_minute_interval:
            minutes_in_window = (
                time_window.end.timestamp() - time_window.start.timestamp()
            ) / 60
    ...
    def get_num_partitions(
        ...
        return self.get_num_partitions_in_window(
            TimeWindow(
                start=first_partition_window.start, end=last_partition_window.end
            )
        )
    ...
    def get_partition_keys_between_indexes(
        ...
        for idx, time_window in enumerate(
            self._iterate_time_windows(self.start.timestamp())
        ):
    ...
    def __str__(self) -> str:
        schedule_str = (
            self.schedule_type.value.capitalize()
            if self.schedule_type
            else self.cron_schedule
        )
        ...
        for partition_key in sorted_pks:
            ...
            if (
                ...
                cur_windows_iterator = iter(
                    self._iterate_time_windows(
                        dst_safe_strptime(
                            partition_key, self.timezone, self.fmt
                        ).timestamp()
                    )
                )
    ...
    def start_time_for_partition_key(self, partition_key: str) -> datetime:
        ...
        return next(
            iter(self._iterate_time_windows(partition_key_dt.timestamp()))
        ).start
    ...
    def get_next_partition_key(
        ...
        if start_time.timestamp() >= last_partition_window.end.timestamp():
            ...
            return dst_safe_strftime(
                start_time, self.timezone, self.fmt, self.cron_schedule
            )
    ...
    def get_prev_partition_window(
        ...
        if respect_bounds:
            ...
            if (
                ...
                or prev_window.start.timestamp()
                < first_partition_window.start.timestamp()
            ):
                return None
    ...
    def _get_first_partition_window(
        ...
        if self.end_offset == 0:
            ...
            return (
                time_window
                if time_window.end.timestamp()
                <= curr_window_plus_offset.start.timestamp()
                else None
            )
        else:
            # end offset < 0
    ...
    def _get_last_partition_window(
        ...
        if (
            self._get_first_partition_window(current_timestamp=current_timestamp)
            is None
        ):
            return None
        ...
        if self.end_offset == 0:
            ...
            last_partition_key = super().get_last_partition_key(
                datetime.fromtimestamp(
                    current_timestamp, tz=get_timezone(self.timezone)
                )
            )
            return (
    ...
    def get_last_partition_key(
        ...
        return dst_safe_strftime(
            last_window.start, self.timezone, self.fmt, self.cron_schedule
        )
    ...
    @functools.lru_cache(maxsize=5)
    def get_partition_keys_in_time_window(
    ...
    def get_partition_key_range_for_time_window(
        ...
        start_partition_key = self.get_partition_key_for_timestamp(
            time_window.start.timestamp()
        )
        end_partition_key = self.get_partition_key_for_timestamp(
            check.not_none(
                self.get_prev_partition_window(time_window.end)
            ).start.timestamp()
        )
    ...
    def minute_offset(self) -> int:
        ...
        """
        match = re.fullmatch(
            r"(\d+) (\d+|\*) (\d+|\*) (\d+|\*) (\d+|\*)", self.cron_schedule
        )
        if match is None:
    ...
    def get_cron_schedule(
        ...
        if schedule_type == ScheduleType.HOURLY:
            ...
            hour_of_day = cast(
                int,
                check.opt_int_param(
                    hour_of_day, "hour_of_day", default=self.hour_offset
                ),
            )
        ...
        if schedule_type == ScheduleType.MONTHLY:
            ...
            day_offset = check.opt_int_param(
                day_of_month, "day_of_month", default=default
            )
        elif schedule_type == ScheduleType.WEEKLY:
            default = self.day_offset or 0
            day_offset = check.opt_int_param(
                day_of_week, "day_of_week", default=default
            )
        else:
            day_offset = 0
    ...
    def get_partition_key_for_timestamp(
        self, timestamp: float, end_closed: bool = False
    ) -> str:
        """Args:
        ...
        if end_closed or prev_next.timestamp() > timestamp:
            ...
            return dst_safe_strftime(
                prev_next, self.timezone, self.fmt, self.cron_schedule
            )
    ...
    def equal_except_for_start_or_end(
        self, other: "TimeWindowPartitionsDefinition"
    ) -> bool:
...
def daily_partitioned_config(
    ...
    tags_for_partition_fn: Optional[
        Callable[[datetime, datetime], Mapping[str, str]]
    ] = None,
) -> Callable[
    ...
    def inner(
        ...
        return PartitionedConfig(
            run_config_for_partition_key_fn=wrap_time_window_run_config_fn(
                fn, partitions_def
            ),
...
class BaseTimeWindowPartitionsSubset(PartitionsSubset):
    ...
    def _get_partition_time_windows_not_in_subset(
        ...
        if len(self.included_time_windows) == 0:
            return [
                PersistedTimeWindow.from_public_time_window(
                    TimeWindow(first_tw.start, last_tw.end),
        ...
        for i in range(len(self.included_time_windows) - 1):
            ...
            if self.included_time_windows[i].end.timestamp() < last_tw_end_timestamp:
                if (
                    self.included_time_windows[i + 1].start.timestamp()
                    <= last_tw_end_timestamp
                ):
                    time_windows.append(
                        PersistedTimeWindow.from_public_time_window(
                            TimeWindow(
    ...
    def _add_partitions_to_time_windows(
        ...
        for window in sorted(time_windows, key=lambda tw: tw.start.timestamp()):
            ...
            for i in reversed(range(len(result_windows))):
                ...
                lt_end_of_range = (
                    window_start_timestamp < included_window.end.timestamp()
                )
                gte_start_of_range = (
                    window_start_timestamp >= included_window.start.timestamp()
                )
                ...
                if not lt_end_of_range:
                    merge_with_range = (
                        included_window.end.timestamp() == window_start_timestamp
                    )
                    merge_with_later_range = i + 1 < len(result_windows) and (
                        window.end.timestamp()
                        == result_windows[i + 1].start.timestamp()
                    )
                ...
                if (
                    result_windows
                    and window_start_timestamp == result_windows[0].start.timestamp()
                ):
                    ...
                    result_windows
                    and window.end.timestamp() == result_windows[0].start.timestamp()
                ):
                    result_windows[0] = PersistedTimeWindow.from_public_time_window(
                        TimeWindow(window.start, included_window.end),
                        self.partitions_def.timezone,
    ...
    def from_serialized(
        ...
        if not isinstance(partitions_def, TimeWindowPartitionsDefinition):
            check.failed(
                "Partitions definition must be a TimeWindowPartitionsDefinition"
            )
    ...
    def can_deserialize(
        ...
        if (
            ...
            and serialized_partitions_def_class_name
            != partitions_def.__class__.__name__
        ):
            return False
...
class PartitionKeysTimeWindowPartitionsSubset(BaseTimeWindowPartitionsSubset):
    ...
    def __or__(self, other: "PartitionsSubset") -> "PartitionsSubset":
        ...
        return _attempt_coerce_to_time_window_subset(
            self
        ) | _attempt_coerce_to_time_window_subset(other)
...
class TimeWindowPartitionsSubsetSerializer(NamedTupleSerializer):
    ...
    def before_pack(
        self, value: "TimeWindowPartitionsSubset"
...
class TimeWindowPartitionsSubset(
    ...
    def __new__(
        ...
        return super(TimeWindowPartitionsSubset, cls).__new__(
            ...
            included_time_windows=check.sequence_param(
                included_time_windows,
                "included_time_windows",
                of_type=PersistedTimeWindow,
            ),
        )

    @staticmethod
    def from_all_partitions_subset(
        subset: AllPartitionsSubset,
    ) -> "TimeWindowPartitionsSubset":
    ...
    def __sub__(self, other: "PartitionsSubset") -> "PartitionsSubset":
        ...
        time_windows = sorted(
            self.included_time_windows, key=lambda tw: tw.start.timestamp()
        )
        ...
        while (next_time_window_index_to_process < len(time_windows)) and (
            ...
            time_windows[
                next_time_window_index_to_process : next_time_window_index_to_process
                + 1
            ...
            if len(subtracted_time_windows) == 0:
                ...
                if (
                    updated_time_window.end.timestamp()
                    <= other_time_window.start.timestamp()
                ):
                    # Current subtractor is too early to intersect, can advance
                    next_time_window_index_to_process += 1
                elif (
                    other_time_window.end.timestamp()
                    <= updated_time_window.start.timestamp()
                ):
...
class PartitionTimeWindowStatus:
    ...
    def __repr__(self):
        return (
            f"({self.time_window.start} - {self.time_window.end}): {self.status.value}"
        )
...
def _flatten(
    ...
    high_pri_time_windows = sorted(
        high_pri_time_windows, key=lambda t: t.time_window.start
    )
    low_pri_time_windows = sorted(
        low_pri_time_windows, key=lambda t: t.time_window.start
    )
    ...
    while True:
        ...
        if (
            low_pri_tw.time_window.start.timestamp()
            < high_pri_tw.time_window.start.timestamp()
        ):
            if (
                low_pri_tw.time_window.end.timestamp()
                <= high_pri_tw.time_window.start.timestamp()
            ):
                ...
                if (
                    low_pri_tw.time_window.end.timestamp()
                    > high_pri_tw.time_window.end.timestamp()
                ):
                    ...
                    low_pri_time_windows[low_pri_idx] = PartitionTimeWindowStatus(
                        TimeWindow(
                            high_pri_tw.time_window.end, low_pri_tw.time_window.end
                        ),
                        low_pri_tw.status,
                    )
                    high_pri_idx += 1
...
def get_time_partition_key(
    ...
    if partitions_def is None or partition_key is None:
        ...
        return partitions_def.get_partition_key_from_str(
            partition_key
        ).keys_by_dimension[partitions_def.time_window_dimension.name]
    else:
        check.failed(
            f"Cannot get time partition from non-time partitions def {partitions_def}"
        )


def _attempt_coerce_to_time_window_subset(
