...
class IdentityPartitionMapping(
    PartitionMapping, NamedTuple("_IdentityPartitionMapping", [])
):
    ...
    def get_upstream_mapped_partitions_result_for_partitions(
        ...
        downstream_partition_keys = set(
            downstream_partitions_subset.get_partition_keys()
        )
...
class AllPartitionMapping(PartitionMapping, NamedTuple("_AllPartitionMapping", [])):
    ...
    def description(self) -> str:
        return (
            "Each downstream partition depends on all partitions of the upstream asset."
        )
...
class LastPartitionMapping(PartitionMapping, NamedTuple("_LastPartitionMapping", [])):
    ...
    def get_downstream_partitions_for_partitions(
        ...
        if (
            ...
            and last_upstream_partition in upstream_partitions_subset
        ):
            return downstream_partitions_def.subset_with_all_partitions(
                current_time=current_time,
                dynamic_partitions_store=dynamic_partitions_store,
...
class BaseMultiPartitionMapping(ABC):
    ...
    def _get_dependency_partitions_subset(
        ...
        b_dimension_partitions_def_by_name: Dict[
            Optional[str], PartitionsDefinition
        ] = (
            {
        ...
        if a_upstream_of_b:
            ...
            for a_dim_name, keys in a_partition_keys_by_dimension.items():
                if a_dim_name in a_dim_to_dependency_b_dim:
                    ...
                    for key in keys:
                        mapped_partitions_result = partition_mapping.get_upstream_mapped_partitions_result_for_partitions(
                            a_dimension_partitions_def.empty_subset().with_partition_keys(
                                [key]
                            ),
                            a_dimension_partitions_def,
                        ...
                        required_but_nonexistent_upstream_partitions.update(
                            set(
                                mapped_partitions_result.required_but_nonexistent_partition_keys
                            )
                        )
        ...
        mapped_b_dim_names = [
            mapping[0] for mapping in a_dim_to_dependency_b_dim.values()
        ]
                ...
                *[
                ...
                b_partition_keys.add(
                    MultiPartitionKey(
                        {
                            cast(
                                str, (mapped_b_dim_names + unmapped_b_dim_names)[i]
                            ): key
        ...
        mapped_subset = b_partitions_def.empty_subset().with_partition_keys(
            b_partition_keys
        )
        if a_upstream_of_b:
...
class MultiToSingleDimensionPartitionMapping(
    ...
    def get_dimension_dependencies(
        ...
        if not infer_mapping_result.can_infer:
            check.invariant(
                isinstance(infer_mapping_result.inference_failure_reason, str)
            )
...
class MultiPartitionMapping(
    ...
    def __new__(
        cls,
        downstream_mappings_by_upstream_dimension: Mapping[
            str, DimensionPartitionMapping
        ],
    ):
    ...
    def _check_all_dimensions_accounted_for(
        ...
        upstream_dimension_names = {
            dim.name
            for dim in cast(
                MultiPartitionsDefinition, upstream_partitions_def
            ).partitions_defs
        }
        dimension_names = {
            dim.name
...
class StaticPartitionMapping(
    ...
    def __init__(
        ...
        for (
            ...
            self._mapping[upstream_key] = (
                {downstream_keys}
                if isinstance(downstream_keys, str)
                else set(downstream_keys)
            )
    ...
    def _check_downstream(
        self, *, downstream_partitions_def: StaticPartitionsDefinition
    ):
...
class InferSingleToMultiDimensionDepsResult(
    ...
    def __new__(
        ...
        if not can_infer and inference_failure_reason is None:
            check.failed(
                "inference_failure_reason must be provided if can_infer is False"
            )
...
def _get_infer_single_to_multi_dimension_deps_result(
    ...
    upstream_is_multipartitioned = isinstance(
    ...
    multipartitions_def = cast(
        MultiPartitionsDefinition, next(iter(multipartitions_defs))
    )
    ...
    if len(matching_dimension_defs) == 1:
        return InferSingleToMultiDimensionDepsResult(
            ...
            dimension_dependency=DimensionDependency(
                ...
                upstream_dimension_name=(
                    matching_dimension_defs[0].name
                    if upstream_is_multipartitioned
                    else None
                ),
                downstream_dimension_name=(
                    matching_dimension_defs[0].name
                    if not upstream_is_multipartitioned
                    else None
                ),
            ),
        )
...
def infer_partition_mapping(
    ...
    if partition_mapping is not None:
        ...
        ) and isinstance(downstream_partitions_def, TimeWindowPartitionsDefinition):
