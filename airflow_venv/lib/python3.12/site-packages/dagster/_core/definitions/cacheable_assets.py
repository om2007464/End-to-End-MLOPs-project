...
class AssetsDefinitionCacheableData(
    NamedTuple(
        ...
        [
            ...
            (
                ...
                Optional[Mapping[str, FreshnessPolicy]],
            ),
            (
    ...
    def __new__(
        ...
        extra_metadata = check.opt_nullable_mapping_param(
...
class CacheableAssetsDefinition(ResourceAddable, ABC):
    ...
    def with_prefix_for_all(
        ...
        return PrefixOrGroupWrappedCacheableAssetsDefinition(
            self, prefix_for_all_assets=prefix
        )
    ...
    def transformed_assets_def(self, assets_def: AssetsDefinition) -> AssetsDefinition:
        ...
        if isinstance(self._auto_materialize_policy, dict):
            automation_condition = {
                ...
                for k, v in self._auto_materialize_policy.items()
            }
        elif isinstance(self._auto_materialize_policy, AutoMaterializePolicy):
            automation_condition = (
                self._auto_materialize_policy.to_automation_condition()
            )
        else:
            ...
            _map_to_hashable(
                {
                    k: (
                        compute_fields_hash(
                            {"root": v.config_schema.as_field()}, v.description
                        ),
