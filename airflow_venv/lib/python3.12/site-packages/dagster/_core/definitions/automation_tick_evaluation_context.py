...
class AutomationTickEvaluationContext:
    def __init__(
        ...
        resolved_entity_keys = {
            ...
            or asset_graph.get(entity_key).automation_condition is not None
        }
    ...
    def _legacy_build_auto_observe_run_requests(self) -> Sequence[RunRequest]:
        ...
        for repository_asset_keys in self.asset_graph.split_entity_keys_by_repository(
            ...
            partitions_def_and_asset_key_groups.extend(
    ...
    def _build_run_requests(
        self, entity_subsets: Sequence[EntitySubset]
    ) -> Sequence[RunRequest]:
    ...
    def _get_updated_cursor(
        ...
        observe_run_requests: Iterable[RunRequest],
    ) -> AssetDaemonCursor:
...
def _get_mapping_from_asset_partitions(
    ...
    for asset_partition in asset_partitions:
        mapping[
            asset_graph.get(asset_partition.asset_key).partitions_def,
...
def _build_run_requests_from_partitions_def_mapping(
    ...
    for (partitions_def, partition_key), entity_keys in mapping.items():
        ...
        for entity_keys_for_repo in asset_graph.split_entity_keys_by_repository(
            entity_keys
        ):
            asset_check_keys = [
            ...
            run_requests.append(
                ...
                RunRequest(
                    asset_selection=[
                        k for k in entity_keys_for_repo if isinstance(k, AssetKey)
                    ],
    ...
    return sorted(
        run_requests, key=lambda x: x.partition_key if x.partition_key else ""
    )
...
def build_run_requests_with_backfill_policies(
    ...
    for asset_partition in asset_partitions:
        if asset_partition.partition_key:
            asset_partition_keys[asset_partition.asset_key].add(
                asset_partition.partition_key
            )
    ...
    for (
        ...
        assets_to_reconcile_by_partitions_def_partition_keys_backfill_policy.items()
    ):
...
def _build_run_requests_for_partition_key_range(
    ...
    while partition_chunk_start_index <= partition_range_end_index:
        partition_chunk_end_index = (
            partition_chunk_start_index + max_partitions_per_run - 1
        )
...
def _build_run_request_for_partition_key_range(
    ...
    partition_key = (
        partition_range_start if partition_range_start == partition_range_end else None
    )
    return RunRequest(
        asset_selection=asset_keys, partition_key=partition_key, tags=tags
    )
