...
def assets_to_keys(
    ...
    return [
        asset_key for asset in assets for asset_key in asset_to_keys_iterable(asset)
    ]
...
def retrieve_timestamp_from_record(asset_record: EventLogRecord) -> Optional[float]:
    ...
    return (
        get_timestamp_out_of_metadata_if_set(
            check.not_none(asset_record.asset_event).metadata,
...
def get_timestamp_out_of_metadata_if_set(
    metadata: Mapping, key: str
) -> Optional[float]:
...
def get_description_for_freshness_check_result(
    ...
    update_time_delta_str = (
        seconds_in_words(current_timestamp - update_timestamp)
        if update_timestamp
        else None
    )
...
def freshness_multi_asset_check(
    params_metadata: JsonMetadataValue, asset_keys: Sequence[AssetKey]
):
