...
def build_time_partition_freshness_checks(
    *,
    ...
    check.invariant(
        deadline_cron is None or is_valid_cron_string(deadline_cron),
        "Invalid cron string.",
    )
...
def _build_freshness_multi_check(
    ...
    def the_check(context: AssetCheckExecutionContext) -> Iterable[AssetCheckResult]:
        for check_key in context.selected_asset_check_keys:
            ...
            current_time_in_freshness_tz = datetime_from_timestamp(
                current_timestamp, tz=timezone
            )
            ...
            last_completed_time_window = check.not_none(
                partitions_def.get_last_partition_window(
                    current_time=deadline_in_partitions_def_tz
                )
            )
            ...
            metadata: Dict[str, MetadataValue] = {
                ...
                LATEST_CRON_TICK_METADATA_KEY: TimestampMetadataValue(
                    deadline.timestamp()
                ),
            }
            ...
            if not passed and latest_record_any_partition is not None:
                ...
                metadata[LAST_UPDATED_TIMESTAMP_METADATA_KEY] = TimestampMetadataValue(
                    check.not_none(
                        retrieve_timestamp_from_record(latest_record_any_partition)
                    )
                )
            elif passed:
...
def _construct_description(
    ...
    return f"Asset is overdue. We expected partition {partition_key} to have arrived, and it has not."
