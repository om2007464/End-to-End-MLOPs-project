...
class TableSchemaDiff(BaseModel):
    ...
    def from_table_schemas(
        ...
        old_columns_by_key = {
            column.name: column for column in old_table_schema.columns
        }
        new_columns_by_key = {
            column.name: column for column in new_table_schema.columns
        }

        added_columns = [
            column
            for column in new_table_schema.columns
            if column.name not in old_columns_by_key
        ]
        removed_columns = [
            column
            for column in old_table_schema.columns
            if column.name not in new_columns_by_key
        ]
