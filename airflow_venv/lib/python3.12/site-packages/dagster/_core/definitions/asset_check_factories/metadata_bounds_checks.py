...
def build_metadata_bounds_checks(
    *,
    ...
    check.sequence_param(
        assets, "assets", of_type=(AssetKey, str, AssetsDefinition, SourceAsset)
    )
    ...
    if min_value is not None and max_value is not None and min_value > max_value:
        raise DagsterInvalidDefinitionError(
            "`min_value` may not be greater than `max_value`"
        )

    asset_keys = assets_to_keys(assets)

    description = (
        f"Checks that the latest materialization has metadata value `{metadata_key}`"
    )
    if min_value is not None:
        description += (
            f" greater than {'or equal to ' if not exclusive_min else ''}{min_value}"
        )
    ...
    def _result_for_check_key(
        ...
        if min_value is not None and (
            value <= min_value if exclusive_min else value < min_value
        ):
            return (
                False,
        ...
        if max_value is not None and (
            value >= max_value if exclusive_max else value > max_value
        ):
            return (
                False,
