...
class _Op:
    def __init__(
        ...
        self.ins = check.opt_nullable_mapping_param(
            ins, "ins", key_type=str, value_type=In
        )
        self.out = out
...
class DecoratedOpFunction(NamedTuple):
    ...
    def validate_malformed_config(self) -> None:
        ...
        for param in get_function_params(self.decorated_fn):
            if (
                safe_is_subclass(param.annotation, Config)
                and param.name in positional_inputs
            ):
...
def resolve_checked_op_fn_inputs(
    ...
    inferred_props = {
        ...
        for inferred in infer_input_props(
            compute_fn.decorated_fn, compute_fn.has_context_arg()
        )
    }
    input_defs = []
    for input_def in explicit_input_defs:
        if input_def.name in inferred_props:
            ...
            input_defs.append(
                input_def.combine_with_inferred(inferred_props[input_def.name])
            )
        else:
