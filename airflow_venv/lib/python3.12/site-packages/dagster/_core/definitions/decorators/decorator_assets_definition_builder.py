...
def get_function_params_without_context_or_config_or_resources(
    ...
    is_context_provided = len(params) > 0 and params[
        0
    ].name in get_valid_name_permutations("context")
...
def build_named_ins(
    ...
    for input_name in all_input_names:
        ...
        asset_key = asset_key or AssetKey(
            list(filter(None, [*(key_prefix or []), input_name]))
        )
...
def build_named_outs(
    asset_outs: Mapping[str, AssetOut]
) -> Mapping[AssetKey, "NamedOut"]:
    ...
    for output_name, asset_out in asset_outs.items():
        ...
        named_outs_by_asset_key[asset_key] = NamedOut(
            output_name.replace("-", "_"), out
        )
...
def build_subsettable_named_ins(
    ...
    potential_deps = (
        set().union(*internal_upstream_deps).difference(set(asset_ins.keys()))
    )
    return {
...
class DecoratorAssetsDefinitionBuilder:
    ...
    def for_multi_asset(
        *, fn: Callable[..., Any], args: DecoratorAssetsDefinitionBuilderArgs
        ...
        if args.asset_out_map and args.specs:
            raise DagsterInvalidDefinitionError(
                "Must specify only outs or specs but not both."
            )
        ...
        if args.specs:
            check.invariant(
                args.decorator_name == "@multi_asset",
        ...
        return (
            DecoratorAssetsDefinitionBuilder.from_asset_outs_in_asset_centric_decorator(
                fn=fn,
                op_name=op_name,
                asset_in_map=args.asset_in_map,
    ...
    def from_multi_asset_specs(
        *,
        ...
        check.param_invariant(
            passed_args.specs, "passed_args", "Must use specs in this codepath"
        )
        ...
        for asset_spec in asset_specs:
            ...
            named_outs_by_asset_key[asset_spec.key] = NamedOut(
                ...
                Out(
                    ...
                    io_manager_key=asset_spec.metadata.get(
                        SYSTEM_METADATA_KEY_IO_MANAGER_KEY
                    ),
                ),
            )
        ...
        for spec in asset_specs:
            for dep in spec.deps:
                ...
                if (
                    dep.asset_key in named_outs_by_asset_key
        ...
        remaining_upstream_keys = {
            key for key in upstream_keys if key not in loaded_upstreams
        }
        named_ins_by_asset_key = build_named_ins(
            fn, asset_in_map, deps=remaining_upstream_keys
        )
    ...
    def from_asset_outs_in_asset_centric_decorator(
        *,
        ...
        named_ins_by_asset_key = build_named_ins(
            ...
            asset_in_map,
            deps=(
                {dep.asset_key for dep in upstream_asset_deps}
        ...
        internal_deps = {
            keys_by_output_name[name]: asset_deps[name] for name in asset_deps
        }
    ...
    def overlapping_output_names(self) -> Set[str]:
        return set(self.outs_by_output_name.keys()) & set(
    ...
    def ins_by_input_names(self) -> Mapping[str, In]:
        return {
            in_name: in_obj for in_name, in_obj in self.named_ins_by_asset_key.values()
        }
    ...
    def create_op_definition(self) -> OpDefinition:
        return _Op(
            ...
            tags={
                **(
                    {COMPUTE_KIND_TAG: self.args.compute_kind}
                    if self.args.compute_kind
                    else {}
                ),
...
def create_check_specs_by_output_name(
    ...
    checks_by_output_name = {
        ...
        for spec in check.opt_sequence_param(
            check_specs, "check_specs", of_type=AssetCheckSpec
        )
    }
