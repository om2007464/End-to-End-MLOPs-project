...
class ResourceRelatedState(NamedTuple):
    ...
    def resolved_io_manager_key(self) -> str:
        if self.io_manager_def:
            ...
            return (
                self.io_manager_key if self.io_manager_key else DEFAULT_IO_MANAGER_KEY
            )
    ...
    def asset_resource_defs(self) -> Mapping[str, ResourceDefinition]:
        ...
        if (
            io_manager_key in self.resources
            and self.resources[io_manager_key] != io_manager_def
        ):
            raise DagsterInvalidDefinitionError(
        ...
        return wrap_resources_for_execution(
            {**self.resources, **{io_manager_key: io_manager_def}}
        )
...
def create_assets_def_from_fn_and_decorator_args(
    ...
    with disable_dagster_warnings():
        builder_args = DecoratorAssetsDefinitionBuilderArgs(
            ...
            check_specs_by_output_name=create_check_specs_by_output_name(
                args.check_specs
            ),
            ...
            asset_out_map={
                DEFAULT_OUTPUT: AssetOut(
                    ...
                    dagster_type=args.dagster_type
                    if args.dagster_type
...
def multi_asset(
    ...
    **kwargs: Any,
    ...
    args = DecoratorAssetsDefinitionBuilderArgs(
        ...
        asset_out_map=check.opt_mapping_param(
            outs, "outs", key_type=str, value_type=AssetOut
        ),
        upstream_asset_deps=_deps_and_non_argument_deps_to_asset_deps(
            deps=deps,
            non_argument_deps=_validate_hidden_non_argument_dep_param(
        ...
        asset_in_map=check.opt_mapping_param(
            ins, "ins", key_type=str, value_type=AssetIn
        ),
    ...
    def inner(fn: Callable[..., Any]) -> AssetsDefinition:
        ...
        return AssetsDefinition.from_graph(
            ...
            keys_by_output_name={
                ...
                for asset_key, (output_name, _) in named_outs.items()
            },
            partitions_def=partitions_def,
            partition_mappings=partition_mappings if partition_mappings else None,
            group_name=group_name,
