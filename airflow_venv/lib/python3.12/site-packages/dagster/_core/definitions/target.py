...
class AutomationTarget(
    ...
    def from_coercible(
        ...
        automation_name: Optional[
            str
        ] = None,  # only needed if we are generating an anonymous job
    ) -> Self:
        if isinstance(coercible, (JobDefinition, UnresolvedAssetJobDefinition)):
            ...
            if isinstance(coercible, AssetsDefinition):
                ...
                assets_defs = (
                    [
                        x
                        for x in coercible
                        if isinstance(x, (AssetsDefinition, SourceAsset))
                    ]
            ...
            return cls(
                resolvable_to_job=define_asset_job(
                    name=job_name, selection=asset_selection
                ),
    ...
    def has_job_def(self) -> bool:
        return isinstance(
            self.resolvable_to_job, (JobDefinition, UnresolvedAssetJobDefinition)
        )
