...
SYSTEM_METADATA_KEY_AUTO_OBSERVE_INTERVAL_MINUTES = (
    "dagster/auto_observe_interval_minutes"
)
...
def validate_kind_tags(kinds: Optional[AbstractSet[str]]) -> None:
    if kinds is not None and len(kinds) > 3:
        raise DagsterInvalidDefinitionError(
            "Assets can have at most three kinds currently."
        )
...
class AssetSpec(
    ...
    def __new__(
        ...
        **kwargs,
        ...
        kind_tags = {
            tag_key
            for tag_key in tags_with_kinds.keys()
            if tag_key.startswith(KIND_PREFIX)
        }
    ...
    def with_io_manager_key(self, io_manager_key: str) -> "AssetSpec":
        ...
        return self._replace(
            metadata={
                **self.metadata,
                SYSTEM_METADATA_KEY_IO_MANAGER_KEY: io_manager_key,
            }
        )
...
def replace_attributes(
    ...
    *,
    ...
    return spec.dagster_internal_init(
        ...
        partitions_def=partitions_def
        if partitions_def is not ...
        else spec.partitions_def,
    )
