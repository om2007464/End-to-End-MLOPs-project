...
class RunStatusSensorContext:
    ...
    def __init__(
        ...
        self._dagster_event = check.inst_param(
            dagster_event, "dagster_event", DagsterEvent
        )
        self._instance = check.inst_param(instance, "instance", DagsterInstance)
        self._logger: Optional[logging.Logger] = logger or (
            context.log if context else None
        )
...
class RunFailureSensorContext(RunStatusSensorContext):
    ...
    def get_step_failure_events(self) -> Sequence[DagsterEvent]:
        ...
        return [
            cast(DagsterEvent, record.event_log_entry.dagster_event)
...
def run_failure_sensor(
    ...
    def inner(
        ...
        def _run_failure_sensor(*args, **kwargs) -> Any:
            args_modified = [
                arg.for_run_failure()
                if isinstance(arg, RunStatusSensorContext)
                else arg
                for arg in args
            ]
            kwargs_modified = {
...
class RunStatusSensorDefinition(SensorDefinition):
    ...
    def __init__(
        ...
        resource_arg_names: Set[str] = {
            arg.name for arg in get_resource_args(run_status_sensor_fn)
        }
        ...
        monitored_jobs = [
            job.to_repository_selector()
            if isinstance(job, CodeLocationSelector)
            else job
        ...
        other_repo_jobs = (
            [x for x in monitored_jobs if isinstance(x, JobSelector)]
            if monitored_jobs
            else []
        )
        ...
        def _wrapped_fn(
            ...
            if (
                ...
                and context.instance.event_log_storage.is_run_sharded
            ):
            ...
            run_ids_to_fetch = list(
                set(
                    event_record.event_log_entry.run_id
            ...
            for event_record in event_records:
                ...
                if (
                    ...
                    if monitored_jobs:
                        if dagster_run.job_name in map(
                            lambda x: x.name, current_repo_jobs
                        ):
                            job_match = True
                    else:
                        job_match = True
                ...
                if (
                    ...
                    remote_repository_origin = (
                        dagster_run.remote_job_origin.repository_origin
                    )
                ...
                try:
                    with RunStatusSensorContext(
                        ...
                        context_param_name = get_context_param_name(
