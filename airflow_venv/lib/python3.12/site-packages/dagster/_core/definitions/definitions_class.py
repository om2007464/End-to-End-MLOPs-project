...
def _io_manager_needs_replacement(
    job: JobDefinition, resource_defs: Mapping[str, Any]
) -> bool:
...
def _jobs_which_will_have_io_manager_replaced(
    ...
    return [
        ...
        and _io_manager_needs_replacement(job, resource_defs)
    ]
...
def _attach_resources_to_jobs_and_instigator_jobs(
    ...
    jobs = [
        ...
        *[
            ...
            and isinstance(
                target.job_def, (JobDefinition, UnresolvedAssetJobDefinition)
            )
        ],
    ]
    # Dedupe
    ...
    unsatisfied_jobs = [
        ...
        if isinstance(job, JobDefinition)
        and (
            job.is_missing_required_resources()
...
def _create_repository_using_definitions_args(
    ...
    (
        ...
        jobs, schedules, sensors, resource_defs
    )
    ...
    def merge(*def_sets: "Definitions") -> "Definitions":
        ...
        for i, def_set in enumerate(def_sets):
            ...
            for resource_key, resource_value in (def_set.resources or {}).items():
                if (
                    resource_key in resources
                    and resources[resource_key] is not resource_value
                ):
