    ...
    **Warning:**
    ...
    def __new__(
        ...
        check.invariant(
            ...
            or max_materializations_per_minute > 0,
        ...
        return super(AutoMaterializePolicy, cls).__new__(
            cls,
            rules=frozenset(
                check.set_param(rules, "rules", of_type=AutoMaterializeRule)
            ),
    ...
    def skip_rules(self) -> AbstractSet["AutoMaterializeRule"]:
        ...
        return {
            rule
            for rule in self.rules
            if rule.decision_type == AutoMaterializeDecisionType.SKIP
        }
    ...
    def from_automation_condition(
        ...
        return AutoMaterializePolicy(
            ...
            asset_condition=automation_condition,
        )
    ...
    @public
    def without_rules(
        self, *rules_to_remove: "AutoMaterializeRule"
    ) -> "AutoMaterializePolicy":
    ...
    @public
    def with_rules(
        self, *rules_to_add: "AutoMaterializeRule"
    ) -> "AutoMaterializePolicy":
    ...
    def to_automation_condition(self) -> "AutomationCondition":
        ...
        materialize_condition = OrAutomationCondition(
            operands=[
                ...
                for rule in sorted(
                    self.materialize_rules, key=lambda rule: rule.description
                )
            ]
        )
