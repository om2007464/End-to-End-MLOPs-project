...
class AssetKey(IHaveNew):
    ...
    parts: Sequence[
        str
    ]  # with path available as a property defined below still returning a list.
...
class AssetCheckKey(NamedTuple):
    ...
    def from_db_string(db_string: str) -> Optional["AssetCheckKey"]:
        try:
            ...
            if isinstance(values, dict) and values.keys() == {
                "asset_key",
                "check_name",
            }:
                return AssetCheckKey(
                    asset_key=check.not_none(
                        AssetKey.from_db_string(values["asset_key"])
                    ),
    ...
    def to_db_string(self) -> str:
        return seven.json.dumps(
            {"asset_key": self.asset_key.to_string(), "check_name": self.name}
        )
...
def entity_key_from_db_string(db_string: str) -> EntityKey:
    ...
    return (
        check_key if check_key else check.not_none(AssetKey.from_db_string(db_string))
    )
