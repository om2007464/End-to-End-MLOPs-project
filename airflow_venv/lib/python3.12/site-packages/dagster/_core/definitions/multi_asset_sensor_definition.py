...
class MultiAssetSensorContextCursor:
    ...
    def __init__(
        self, cursor: Optional[str], context: "MultiAssetSensorEvaluationContext"
    ):
        loaded_cursor = json.loads(cursor) if cursor else {}
        loaded_cursor = loaded_cursor if isinstance(loaded_cursor, dict) else {}
        self._cursor_component_by_asset_key: Dict[
            str, MultiAssetSensorAssetCursorComponent
        ] = {}

        # The initial latest consumed event ID at the beginning of the tick
        self.initial_latest_consumed_event_ids_by_asset_key: Dict[
            str, Optional[int]
        ] = {}
        ...
        check.dict_param(
            self._cursor_component_by_asset_key, "unpacked_cursor", key_type=str
        )
        self._context = context

    def get_cursor_for_asset(
...
class MultiAssetSensorEvaluationContext(SensorEvaluationContext):
    ...
    def __init__(
        ...
        self._partitions_def_by_asset_key: Dict[
        ...
        for asset_key in self._monitored_asset_keys:
            assets_def = (
                asset_graph.get(asset_key).assets_def
        ...
        for asset_key in self._monitored_asset_keys:
            ...
            if unconsumed_event_ids:
                ...
                self._initial_unconsumed_events_by_id.update(
                    {
                        event_record.storage_id: event_record
    ...
    @public
    def get_trailing_unconsumed_events(
        self, asset_key: AssetKey
    ) -> Sequence["EventLogRecord"]:
    ...
    def _get_partitions_after_cursor(self, asset_key: AssetKey) -> Sequence[str]:
        ...
        if not isinstance(partitions_def, PartitionsDefinition):
            raise DagsterInvalidInvocationError(
                f"No partitions defined for asset key {asset_key}"
            )
    ...
    def latest_materialization_records_by_key(
        ...
        if asset_keys is None:
            ...
            asset_keys = check.opt_sequence_param(
    ...
    def materialization_records_for_key(
        ...
        if not limit:
            deprecation_warning(
                "Calling materialization_records_for_key without a limit", "1.8"
            )
            records = []
            has_more = True
            cursor = None
            while has_more:
                result = self.instance.fetch_materializations(
                    AssetRecordsFilter(
                        asset_key=asset_key,
                        after_storage_id=self._get_cursor(
                            asset_key
                        ).latest_consumed_event_id,
                    ),
                    ascending=True,
    ...
    def latest_materialization_records_by_partition_and_asset(
        ...
        if not partitions_defs or not all(
            x == partitions_defs[0] for x in partitions_defs
        ):
            raise DagsterInvalidInvocationError(
        ...
        for asset_key in self._monitored_asset_keys:
            materialization_by_partition = (
                self.latest_materialization_records_by_partition(asset_key)
            )
            for partition, materialization in materialization_by_partition.items():
                asset_and_materialization_tuple_by_partition[partition][
                    asset_key
                ] = materialization
    ...
    def get_cursor_partition(self, asset_key: Optional[AssetKey]) -> Optional[str]:
        ...
        if asset_key:
            ...
            self._monitored_asset_keys is not None
            and len(self._monitored_asset_keys) == 1
        ):
    ...
    def all_partitions_materialized(
        ...
        if partitions is not None:
            ...
            if len(partitions) == 0:
                raise DagsterInvalidInvocationError(
                    "Must provide at least one partition in list"
                )
    ...
    def get_downstream_partition_keys(
        ...
        to_asset = self._get_asset(
            to_asset_key, fn_name="get_downstream_partition_keys"
        )
        from_asset = self._get_asset(
            from_asset_key, fn_name="get_downstream_partition_keys"
        )
        ...
        downstream_partition_key_subset = (
            partition_mapping.get_downstream_partitions_for_partitions(
                from_asset.partitions_def.empty_subset().with_partition_keys(
                    [partition_key]
                ),
...
class MultiAssetSensorCursorAdvances:
    ...
    def add_advanced_records(
        ...
        for asset_key, materialization in materialization_records_by_key.items():
            if materialization:
                self._advanced_record_ids_by_key[asset_key].add(
    ...
    def get_asset_cursor_with_advances(
        ...
        advanced_records: Set[int] = self._advanced_record_ids_by_key.get(
            asset_key, set()
        )
        ...
        latest_consumed_event_id_at_tick_start = (
            initial_asset_cursor.latest_consumed_event_id
        )
        ...
        if not self.advance_all_cursors_called:
            ...
            if greatest_consumed_event_id_in_tick > (
                ...
                unconsumed_events = list(
                    context.get_trailing_unconsumed_events(asset_key)
                ) + list(materialization_events)
            else:
                unconsumed_events = []
            ...
            if (
                ...
                and latest_consumed_partition_in_tick
                in latest_unconsumed_record_by_partition
            ):
                latest_unconsumed_record_by_partition.pop(
                    latest_consumed_partition_in_tick
                )

            if (
                len(latest_unconsumed_record_by_partition.keys())
                >= MAX_NUM_UNCONSUMED_EVENTS
            ):
                raise DagsterInvariantViolationError(
                    f"""
...
def build_multi_asset_sensor_context(
    *,
    ...
    check.bool_param(
        cursor_from_latest_materializations, "cursor_from_latest_materializations"
    )
            ...
            asset_keys = cast(
                ...
                list(
                    monitored_assets.resolve(
                        list(set(repository_def.asset_graph.assets_defs))
                    )
                ),
            )
        else:
...
class MultiAssetSensorDefinition(SensorDefinition):
    ...
    def __call__(self, *args, **kwargs) -> AssetMaterializationFunctionReturn:
        ...
        context_param = (
            {context_param_name: context} if context_param_name and context else {}
        )
