...
class BaseAssetGraph(ABC, Generic[T_AssetNode]):
    ...
    def materializable_asset_keys(self) -> AbstractSet[AssetKey]:
        return {
            key
            for key, node in self._asset_nodes_by_key.items()
    ...
    def observable_asset_keys(self) -> AbstractSet[AssetKey]:
        return {
            key for key, node in self._asset_nodes_by_key.items() if node.is_observable
        }

    @cached_property
    def external_asset_keys(self) -> AbstractSet[AssetKey]:
        return {
            key for key, node in self._asset_nodes_by_key.items() if node.is_external
        }

    @cached_property
    def executable_asset_keys(self) -> AbstractSet[AssetKey]:
        return {
            key for key, node in self._asset_nodes_by_key.items() if node.is_executable
        }
    ...
    def toposorted_entity_keys_by_level(self) -> Sequence[Sequence[EntityKey]]:
        ...
        """
        sort_key = (
            lambda e: (e, None) if isinstance(e, AssetKey) else (e.asset_key, e.name)
        )
    ...
    def asset_keys_for_partitions_def(
        ...
        return {
            ...
            if node.partitions_def == partitions_def
        }
    ...
    def all_partitions_defs(self) -> Sequence[PartitionsDefinition]:
        return sorted(
            set(
                node.partitions_def for node in self.asset_nodes if node.partitions_def
            ),
            key=repr,
        )
    ...
    def get_partitions_in_range(
        ...
        partition_keys_in_range = check.not_none(
            partition_def
        ).get_partition_keys_in_range(partition_key_range, dynamic_partitions_store)
        return [
    ...
    def get_child_partition_keys_of_parent(
        ...
        child_partitions_subset = (
            partition_mapping.get_downstream_partitions_for_partitions(
                parent_partitions_def.empty_subset().with_partition_keys(
    ...
    def get_downstream_automation_conditions(
        ...
        for child_key in asset.child_keys:
            ...
            for c, aks in self.get_downstream_automation_conditions(
                asset_key=child_key
            ).items():
    ...
    def bfs_filter_subsets(
        ...
        while len(queue) > 0:
            ...
            if condition_fn(asset_key, partitions_subset):
                result |= AssetGraphSubset(
                    non_partitioned_asset_keys={asset_key}
                    if partitions_subset is None
                    else set(),
                    partitions_subsets_by_asset_key=(
                        {asset_key: partitions_subset}
    ...
    def bfs_filter_asset_partitions(
        ...
        queue = ToposortedPriorityQueue(
            self, all_nodes, include_full_execution_set=True
        )
...
def sort_key_for_asset_partition(
    ...
    time_partition_key = get_time_partition_key(
        partitions_def, asset_partition.partition_key
    )
...
class ToposortedPriorityQueue:
    ...
    def _queue_item(
        ...
        if self._include_full_execution_set:
            execution_set_keys = self._asset_graph.get(
                asset_key
        ...
        level = max(
            self._toposort_level_by_asset_key[asset_key]
