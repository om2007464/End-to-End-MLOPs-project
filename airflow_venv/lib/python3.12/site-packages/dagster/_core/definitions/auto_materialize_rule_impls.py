...
class MaterializeOnRequiredForFreshnessRule(
    ...
    def evaluate_for_asset(self, context: "AutomationContext") -> "AutomationResult":
        ...
        return AutomationResult(
            context, true_subset, subsets_with_metadata=subsets_with_metadata
        )
...
class MaterializeOnCronRule(
    ...
    def get_new_candidate_asset_partitions(
        ...
        if isinstance(partitions_def, MultiPartitionsDefinition):
            ...
            return {
                AssetKeyPartitionKey(
                    context.legacy_context.asset_key, time_partition_key
                )
                for time_partition_key in missed_time_partition_keys
            }
    ...
    def evaluate_for_asset(self, context: "AutomationContext") -> "AutomationResult":
        ...
        new_asset_partitions = self.get_new_candidate_asset_partitions(
            context, missed_ticks
        )
...
class AutoMaterializeAssetPartitionsFilter(
    ...
    def passes(
        ...
        for asset_partition in asset_partitions:
            if context.legacy_context.will_update_asset_partition(asset_partition):
                ...
                if latest_storage_id is not None:
                    storage_ids_to_fetch_by_key[asset_partition.asset_key].append(
                        latest_storage_id
                    )

        asset_partitions_by_latest_run_id: Dict[
            str, Set[AssetKeyPartitionKey]
        ] = defaultdict(set)
        ...
        if len(asset_partitions_by_latest_run_id) > 0:
            run_step = int(
                os.getenv("DAGSTER_ASSET_DAEMON_RUN_TAGS_RUN_FETCH_LIMIT", "1000")
            )
                ...
                run_ids_with_required_tags.update(
                    {
                        run.run_id
                        for run in runs
                        if required_tag_items <= run.tags.items()
                    }
                )
...
class MaterializeOnParentUpdatedRule(
    ...
    def description(self) -> str:
        ...
        if self.updated_parent_filter is not None:
            return (
                f"{base} and matches filter '{self.updated_parent_filter.description}'"
            )
        else:
            return base
    ...
    def evaluate_for_asset(self, context: "AutomationContext") -> "AutomationResult":
        ...
        for updated_or_will_update_parent in filtered_updated_and_will_update_parents:
            for child in asset_partitions_by_updated_parents.get(
            ...
            asset_partitions_by_evaluation_data[
                ParentUpdatedRuleEvaluationData(
                    updated_asset_keys=frozenset(
                        updated_parent_assets_by_asset_partition.get(
                            asset_partition, []
                        )
                    ),
                    will_update_asset_keys=frozenset(
                        will_update_parent_assets_by_asset_partition.get(
                            asset_partition, []
                        )
                    ),
                ).frozen_metadata
...
class MaterializeOnMissingRule(
    AutoMaterializeRule, NamedTuple("_MaterializeOnMissingRule", [])
):
    @property
    def decision_type(self) -> AutoMaterializeDecisionType:
        return AutoMaterializeDecisionType.MATERIALIZE
    ...
    def get_handled_subset(
        ...
        previous_handled_subset = (
            context.legacy_context.node_cursor.get_structured_cursor(
                SerializableEntitySubset
            )
        ...
        if (
            ...
            or not previous_handled_subset.is_compatible_with_partitions_def(
                context.partitions_def
            )
        ):
            previous_handled_subset = (
    ...
    def evaluate_for_asset(self, context: "AutomationContext") -> "AutomationResult":
        ...
        if (
            ...
            if isinstance(
                context.legacy_context.partitions_def, TimeWindowPartitionsDefinition
            ):
                last_partition_key = (
                ...
                if last_partition_key != previous_last_partition_key:
                    asset_partitions_to_evaluate |= {
                        AssetKeyPartitionKey(
                            context.legacy_context.asset_key, last_partition_key
                        )
                    }
...
class SkipOnParentOutdatedRule(
    AutoMaterializeRule, NamedTuple("_SkipOnParentOutdatedRule", [])
):
    @property
    def decision_type(self) -> AutoMaterializeDecisionType:
...
@whitelist_for_serdes
class SkipOnParentMissingRule(
    AutoMaterializeRule, NamedTuple("_SkipOnParentMissingRule", [])
):
    @property
    def decision_type(self) -> AutoMaterializeDecisionType:
        return AutoMaterializeDecisionType.SKIP
        ...
        for candidate in subset_to_evaluate.asset_partitions:
            ...
            for (
                ...
                if not (
                    ...
                    and context.legacy_context.asset_graph.get(
                        parent.asset_key
                    ).is_executable
                ):
                    continue
...
class SkipOnNotAllParentsUpdatedRule(
    AutoMaterializeRule,
    NamedTuple(
        "_SkipOnNotAllParentsUpdatedRule",
        [("require_update_for_all_parent_partitions", bool)],
    ),
):
        ...
        for candidate in subset_to_evaluate.asset_partitions:
            ...
            if self.require_update_for_all_parent_partitions:
                ...
                non_updated_parent_keys = {
                    parent.asset_key
                    for parent in parent_partitions - updated_parent_partitions
                }
            else:
...
class SkipOnNotAllParentsUpdatedSinceCronRule(
    ...
    def get_parent_subsets_updated_since_cron_by_key(
        ...
        for parent_asset_key in context.legacy_context.asset_graph.get(
            ...
            updated_subsets_by_key[
                parent_asset_key
            ] = self.get_parent_subset_updated_since_cron(
    ...
    def parent_updated_since_cron(
        ...
        if isinstance(parent_partitions_def, TimeWindowPartitionsDefinition):
            ...
            required_parent_partitions = (
                parent_partitions_def.get_partition_keys_in_time_window(
        ...
        # been materialized since the previous cron tick
            ...
            if parent_partitions_def is None:
                ...
                parent_subset = context.legacy_context.asset_graph.get_parent_partition_keys_for_child(
...
class DiscardOnMaxMaterializationsExceededRule(
    AutoMaterializeRule,
    NamedTuple("_DiscardOnMaxMaterializationsExceededRule", [("limit", int)]),
):
    @property
    def decision_type(self) -> AutoMaterializeDecisionType:
    ...
    def evaluate_for_asset(self, context: "AutomationContext") -> "AutomationResult":
        ...
        rate_limited_asset_partitions = set(
            sorted(
                ...
                key=lambda x: sort_key_for_asset_partition(
                    context.legacy_context.asset_graph, x
                ),
            )[self.limit :]
        )
...
class SkipOnRunInProgressRule(
    AutoMaterializeRule, NamedTuple("_SkipOnRunInProgressRule", [])
):
    @property
    def decision_type(self) -> AutoMaterializeDecisionType:
        return AutoMaterializeDecisionType.SKIP
