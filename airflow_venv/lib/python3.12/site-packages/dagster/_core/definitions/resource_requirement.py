...
class ResourceKeyRequirement(ResourceRequirement, ABC):
    ...
    def is_satisfied(self, resource_defs: Mapping[str, "ResourceDefinition"]) -> bool:
        return self.key in resource_defs and isinstance(
...
class ExternalAssetIOManagerRequirement(ResourceKeyRequirement):
    ...
    def describe_requirement(self) -> str:
        ...
        return (
            f"io manager with key '{self.key}' required by {external_asset_descriptor}"
        )
...
class SourceAssetIOManagerRequirement(ResourceKeyRequirement):
    ...
    def describe_requirement(self) -> str:
        source_asset_descriptor = (
            f"SourceAsset with key {self.asset_key}"
            if self.asset_key
            else "SourceAsset"
        )
...
class ResourceDependencyRequirement(ResourceKeyRequirement):
    ...
    def describe_requirement(self) -> str:
        source_descriptor = (
            f" by resource with key '{self.source_key}'" if self.source_key else ""
        )
...
def get_resource_key_conflicts(
    ...
    overlapping_keys = set(resource_defs.keys()).intersection(
        set(other_resource_defs.keys())
    )
    overlapping_keys = {
        key for key in overlapping_keys if key != DEFAULT_IO_MANAGER_KEY
    }
...
def merge_resource_defs(
    ...
    overlapping_keys = get_resource_key_conflicts(
        old_resource_defs, resource_defs_to_merge_in
    )
    if overlapping_keys:
