...
class CachingDataTimeResolver:
    ...
    def _calculate_data_time_partitioned(
        ...
        partition_subset = partitions_def.empty_subset().with_partition_keys(
            partition_key
            for partition_key in self._instance_queryer.get_materialized_partitions(
                asset_key
            )
    ...
    def _upstream_records_by_key(
        ...
        for parent_key in self.asset_graph.get(asset_key).parent_keys:
            if not (
                self.asset_graph.has(parent_key)
                and self.asset_graph.get(parent_key).is_executable
            ):
                continue
            ...
            if before_cursor is not None:
                parent_record = self._instance_queryer.get_latest_materialization_or_observation_record(
            ...
            for upstream_key, data_time in self._calculate_data_time_by_key(
                ...
                if data_time is None:
                    ...
                    data_time_by_key[upstream_key] = (
                        min(cur_data_time, data_time)
                        if cur_data_time is not None
                        else None
                    )
    ...
    def _calculate_data_time_by_key(
        ...
        *,
        ...
        if record_id is None:
            return {
                key: None
                for key in self.asset_graph.get_materializable_roots(asset_key)
            }
    ...
    def _get_in_progress_data_time_in_run(
        ...
        """
        planned_keys = self._instance_queryer.get_planned_materializations_for_run(
            run_id=run_id
        )
    ...
    def get_in_progress_data_time(
        ...
        for run_id in self._get_in_progress_run_ids(current_time=current_time):
            if not self._instance_queryer.is_asset_planned_for_run(
                run_id=run_id, asset=asset_key
            ):
                continue
    ...
    def get_ignored_failure_data_time(
        ...
        for this asset.
        """
        current_data_time = self.get_current_data_time(
            asset_key, current_time=current_time
        )
        ...
        latest_run_record = self._instance_queryer._get_run_record_by_id(
            run_id=run_id
        )  # noqa: SLF001
        ...
        run_failure_time = datetime_from_timestamp(
            ...
            or latest_run_record.create_timestamp.timestamp(),
        )
    ...
    def get_current_data_time(
        ...
        latest_record = (
            self.instance_queryer.get_latest_materialization_or_observation_record(
        ...
        data_times = set(
            self.get_data_time_by_key_for_record(latest_record, current_time).values()
        )
    ...
    def get_minutes_overdue(
        ...
        if asset.is_observable:
            current_data_time = self._get_source_data_time(
                asset_key, current_time=evaluation_time
            )
        else:
            current_data_time = self.get_current_data_time(
                asset_key, current_time=evaluation_time
