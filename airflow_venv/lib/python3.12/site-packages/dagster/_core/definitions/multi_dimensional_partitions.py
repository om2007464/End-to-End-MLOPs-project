...
class MultiPartitionsDefinition(PartitionsDefinition[MultiPartitionKey]):
    ...
    def get_partition_keys_in_range(
        ...
        start: MultiPartitionKey = self.get_partition_key_from_str(
            partition_key_range.start
        )
        end: MultiPartitionKey = self.get_partition_key_from_str(
    ...
    def time_window_partitions_def(self) -> TimeWindowPartitionsDefinition:
        check.invariant(
            self.has_time_window_dimension, "Must have time window dimension"
        )
        return cast(
            TimeWindowPartitionsDefinition,
            check.inst(
