...
def def_config_field(
    ...
    return Field(
        Shape(
            {"config": configurable_def.config_field}
            if configurable_def.has_config_field
            else {}
        ),
        is_required=is_required,
...
def define_execution_field(
    executor_defs: Sequence[ExecutorDefinition], description: str
) -> Field:
    ...
    if default_in_process:
        return Field(
            selector,
            default_value={in_process_executor.name: {}},
            description=description,
        )
    ...
    if len(executor_defs) == 1 and executor_defs[0] == execute_in_process_executor:
        return Field(
            Permissive(), is_required=False, default_value={}, description=description
        )

    return Field(selector, description=description)


def define_single_execution_field(
    executor_def: ExecutorDefinition, description: str
) -> Field:
...
def define_run_config_schema_type(
    ...
    if creation_data.graph_def.has_config_mapping:
        config_schema = cast(
            IDefinitionConfigSchema, creation_data.graph_def.config_schema
        )
        nodes_field = Field(
...
def selector_for_named_defs(named_defs) -> Selector:
    return Selector(
        {named_def.name: def_config_field(named_def) for named_def in named_defs}
    )
...
def input_has_upstream(
    ...
    return dependency_structure.has_deps(input_handle) or node.container_maps_input(
        input_name
    )
...
def get_type_loader_input_field(
    node: Node, input_name: str, input_def: InputDefinition
) -> Field:
...
def get_output_manager_output_field(
    ...
    if not isinstance(
        resource_defs[output_def.io_manager_key], IOutputManagerDefinition
    ):
        raise DagsterInvalidDefinitionError(
            f'Output "{output_def.name}" for {node.describe_node()} requires io_manager_key '
...
def node_config_field(
    fields: Mapping[str, Optional[Field]], ignored: bool
) -> Optional[Field]:
...
def define_node_shape(
    ...
    """
    ignored_nodes = check.opt_sequence_param(
        ignored_nodes, "ignored_nodes", of_type=Node
    )
...
def construct_config_type_dictionary(
    ...
    all_types = list(
        _gather_all_config_types(node_defs, run_config_schema_type)
    ) + list(_gather_all_schemas(node_defs))
...
def convert_config_input(
    inp: Union[CoercibleToRunConfig, T]
