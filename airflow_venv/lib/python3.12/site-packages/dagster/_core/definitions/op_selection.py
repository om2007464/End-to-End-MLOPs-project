...
def _node_path_lists_to_tree(paths: Sequence[Tuple[str, ...]]) -> OpSelectionNode:
    ...
    for k, group in itertools.groupby(paths, lambda x: x[0]):
        child = _node_path_lists_to_tree([x[1:] for x in group if len(x) > 1])._replace(
            name=k
        )
...
def _validate_selection_tree(
    selection_tree: OpSelectionNode, graph_def: GraphDefinition
) -> None:
...
def _get_graph_subset(
    ...
    for node in graph.nodes_in_topological_order:
        ...
        def is_output_selected(node_output: NodeOutput) -> bool:
            ...
            output_def, op_handle = graph.node_named(
                ...
                NodeHandle(node_output.node_name, parent_handle),
            )
            return (
                op_handle not in selected_outputs_by_op_handle
                or output_def.name
                in selected_outputs_by_op_handle[check.not_none(op_handle)]
            )
        ...
        for node_input in node.inputs():
            if graph.dependency_structure.has_direct_dep(node_input):
                ...
                node_output = graph.dependency_structure.get_dynamic_fan_in_dep(
                    node_input
                )
                if is_output_selected(node_output):
                    node_deps[
    ...
    subgraph_input_mappings = [
        imap
        for imap in graph.input_mappings
        if imap.maps_to.node_name in subgraph_nodes.keys()
    ]
    subgraph_output_mappings = [
        omap
        for omap in graph.output_mappings
        if omap.maps_from.node_name in subgraph_nodes.keys()
    ]
