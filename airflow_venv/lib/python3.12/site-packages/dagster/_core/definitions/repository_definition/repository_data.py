...
from dagster._core.definitions.repository_definition.caching_index import (
    CacheingDefinitionIndex,
)
    ...
    def get_job(self, job_name: str) -> JobDefinition:
        ...
        if match is None:
            raise DagsterInvariantViolationError(
                f"Could not find job {job_name} in repository"
            )
        return match
...
class CachingRepositoryData(RepositoryData):
    ...
    def __init__(
        ...
        schedules: Mapping[
            str, Union[ScheduleDefinition, Resolvable[ScheduleDefinition]]
        ],
        ...
        check.mapping_param(
            jobs, "jobs", key_type=str, value_type=(JobDefinition, FunctionType)
        )
        check.mapping_param(
            schedules,
            "schedules",
            key_type=str,
            value_type=(ScheduleDefinition, FunctionType),
        )
        check.mapping_param(
            sensors,
            "sensors",
            key_type=str,
            value_type=(SensorDefinition, FunctionType),
        )
        check.mapping_param(
            source_assets_by_key,
            "source_assets_by_key",
            key_type=AssetKey,
            value_type=SourceAsset,
        )
        check.mapping_param(
            assets_defs_by_key,
            "assets_defs_by_key",
    ...
    def _resolve_partitioned_asset_schedule_lambda(
        self,
        unresolved_partitioned_asset_schedule: "UnresolvedPartitionedAssetScheduleDefinition",
