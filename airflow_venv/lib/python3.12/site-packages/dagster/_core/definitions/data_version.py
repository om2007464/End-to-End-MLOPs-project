...
class DataVersionsByPartition(
    NamedTuple(
        "_DataVersionsByPartition",
        [("data_versions_by_partition", Mapping[str, DataVersion])],
    )
):
    def __new__(
        cls,
...
class DataProvenance(
    ...
    @deprecated(
        ...
        additional_warn_text="Use `input_data_versions` instead.",
    )
...
def read_input_data_version_from_tags(
    ...
    value = tags.get(
        ...
        get_input_data_version_tag(input_key, prefix=_OLD_INPUT_DATA_VERSION_TAG_PREFIX)
    )
...
class StaleCause(
    ...
    def __new__(
        ...
        return super().__new__(
            ...
            category,
            reason,
            AssetKeyPartitionKey(dependency)
            if isinstance(dependency, AssetKey)
            else dependency,
            children,
        )
    ...
    def sort_key(self) -> str:
        if not hasattr(self, "_sort_key"):
            self._sort_key = (
                f"{self.key}/{self.dependency}" if self.dependency else str(self.key)
            )
...
class CachingStaleStatusResolver:
    ...
    def get_current_data_version(
        ...
        return self._get_current_data_version(
            key=AssetKeyPartitionKey(key, partition_key)
        )
    ...
    def _get_stale_causes(self, key: "AssetKeyPartitionKey") -> Sequence[StaleCause]:
        ...
        if asset.is_partitioned and not key.partition_key:
            ...
            if current_version == NULL_DATA_VERSION:
                ...
                return sorted(
                    self._get_stale_causes_materialized(key=key),
                    key=lambda cause: cause.sort_key,
                )

    def _is_dep_updated(
        self, provenance: DataProvenance, dep_key: "AssetKeyPartitionKey"
    ) -> bool:
        ...
        if dep_key.partition_key is None:
            ...
            return (
                provenance.input_data_versions[dep_key.asset_key]
            ...
            if updated_record:
                ...
                updated_version = extract_data_version_from_entry(
                    updated_record.event_log_entry
                )
                return previous_version != updated_version
            else:
                return False

    def _get_stale_causes_materialized(
        ...
        for dep_key in sorted(partition_deps):
            ...
            # provide a stale reason here.
                ...
                if dep_materialization is None:
                    ...
                    yield StaleCause(
                        key, StaleCauseCategory.DATA, "has a new input", dep_key
                    )
    ...
    @cached_method
    def _is_current_data_version_user_provided(
        self, *, key: "AssetKeyPartitionKey"
    ) -> bool:
    ...
    def _is_volatile(self, *, key: "AssetKey") -> bool:
        ...
        if asset.is_external:
            ...
            return len(deps) == 0 or any(
                self._is_volatile(key=dep_key) for dep_key in deps
            )
    ...
    def _exceeds_self_partition_limit(self, asset_key: "AssetKey") -> bool:
        return (
            ...
            ).get_num_partitions(dynamic_partitions_store=self._instance)
