...
class TimeWindowPartitionMapping(
    ...
    def description(self) -> str:
        description_str = "Maps a downstream partition to any upstream partition with an overlapping time window."
    ...
    def _merge_time_windows(
        self, time_windows: Sequence[TimeWindow]
    ) -> Sequence[TimeWindow]:
        ...
        for window in sorted_time_windows[1:]:
            ...
            if window.start.timestamp() <= last_window.end.timestamp():
                merged_time_windows[-1] = TimeWindow(
                    last_window.start,
                    max(last_window.end, window.end, key=lambda d: d.timestamp()),
                )
            else:
    ...
    def _map_partitions(
        ...
        if not isinstance(from_partitions_subset, BaseTimeWindowPartitionsSubset):
            check.failed(
                "from_partitions_subset must be a BaseTimeWindowPartitionsSubset"
            )
        ...
        for from_partition_time_window in from_partitions_subset.included_time_windows:
            ...
            if (
                ...
                to_end_dt = to_partitions_def.end_time_for_partition_key(
                    to_end_partition_key
                )
            ...
            if offsetted_to_start_dt.timestamp() < offsetted_to_end_dt.timestamp():
                time_windows.append(
                    TimeWindow(offsetted_to_start_dt, offsetted_to_end_dt)
                )
        ...
        for time_window in time_windows:
            if (
                ...
                window_end = min(
                    time_window.end, last_window.end, key=lambda d: d.timestamp()
                )
    ...
    def _do_cheap_partition_mapping_if_possible(
        ...
        if (
            ...
            and (
                from_partitions_def.start.timestamp()
            ...
            and (
                ...
                or (
                    ...
                    and to_partitions_def.end.timestamp()
                    >= from_partitions_def.end.timestamp()
                )
            )
        ):
            return UpstreamPartitionsResult(
...
def _offsetted_datetime_with_bounds(
    ...
    if bounds_window is not None:
        if offset < 0:
            offsetted_dt = max(
                bounds_window.start, offsetted_dt, key=lambda d: d.timestamp()
            )

        if offset > 0:
            offsetted_dt = min(
                bounds_window.end, offsetted_dt, key=lambda d: d.timestamp()
            )
