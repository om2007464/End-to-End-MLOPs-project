...
class ReconstructableRepository(
    ...
    def get_definition(self) -> "RepositoryDefinition":
        return reconstruct_repository_def_from_pointer(
            self.pointer, self.repository_load_data
        )
...
class ReconstructableJob(
    ...
    def __new__(
        ...
        return super(ReconstructableJob, cls).__new__(
            cls,
            repository=check.inst_param(
                repository, "repository", ReconstructableRepository
            ),
            job_name=check.str_param(job_name, "job_name"),
            op_selection=check.opt_nullable_set_param(
    ...
    def with_repository_load_data(
        ...
        return self._replace(
            repository=self.repository.with_repository_load_data(metadata)
        )
    ...
    def get_definition(self) -> "JobDefinition":
        return check.not_none(
            self.get_repository_definition()
        ).get_maybe_subset_job_def(
            self.job_name,
            self.op_selection,
    ...
    def for_module(module: str, fn_name: str) -> "ReconstructableJob":
        return bootstrap_standalone_recon_job(
            ModuleCodePointer(module, fn_name, os.getcwd())
        )
...
def build_reconstructable_job(
    ...
    pointer = CustomPointer(
        reconstructor_pointer, _reconstructable_args, _reconstructable_kwargs
    )
...
def load_def_in_module(
    ...
    return def_from_pointer(
        CodePointer.from_module(module_name, attribute, working_directory)
    )
...
def load_def_in_python_file(
    ...
    return def_from_pointer(
        CodePointer.from_python_file(python_file, attribute, working_directory)
    )
