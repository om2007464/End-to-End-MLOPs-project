...
def _not_invoked_warning(
    ...
    warning_message = "While in {context} context '{name}', received an uninvoked {node_type} '{node_name}'.\n"
    if node.given_alias:
...
def assert_in_composition(name: str, node_def: NodeDefinition) -> None:
    if len(_composition_stack) < 1:
        ...
        correction = f"Invoking {node_label}s is only valid in a function decorated with @job or @graph."
        raise DagsterInvariantViolationError(
...
class PendingNodeInvocation(Generic[T_NodeDefinition]):
    ...
    def _process_argument_node(
        ...
        if isinstance(output_node, SourceAsset):
            input_bindings[input_name] = create_external_asset_from_source_asset(
                output_node
            )
            ...
            output_node,
            (AssetsDefinition, InvokedNodeOutputHandle, InputMappingNode, DynamicFanIn),
        ):
            ...
            for idx, fanned_in_node in enumerate(output_node):
                if isinstance(
                    fanned_in_node, (InvokedNodeOutputHandle, InputMappingNode)
                ):
    ...
    def to_job(
        ...
        config: Optional[
            Union[ConfigMapping, Mapping[str, Any], "PartitionedConfig"]
        ] = None,
...
class InvokedNodeDynamicOutputWrapper:
    ...
    def map(
        ...
        "InvokedNodeDynamicOutputWrapper",
        Tuple["InvokedNodeDynamicOutputWrapper", ...],
        None,
    ]:
        check.is_callable(fn)
        result = fn(
            InvokedNodeOutputHandle(self.node_name, self.output_name, self.node_type)
        )
    ...
    if isinstance(output, tuple) and all(
        ...
        for i, output_name in enumerate(output_def_dict.keys()):
            ...
            output_mapping_dict[output_name] = output_def_dict[
                output_name
            ].mapping_from(handle.node_name, handle.output_name)
...
def do_composition(
    ...
    for defn in actual_input_defs:
        mappings = [
            ...
            if mapping.graph_input_name == defn.name
        ]
