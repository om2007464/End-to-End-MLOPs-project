...
class AssetGraph(BaseAssetGraph[AssetNode]):
    ...
    def normalize_assets(
        ...
        assets_defs = [
            create_external_asset_from_source_asset(a)
        ...
        input_asset_key_replacements = [
            {
                ...
                if (
                    normalized_key := resolved_deps.get_resolved_asset_key_for_input(
                        ad, input_name
                    )
                )
                != raw_key
    ...
    def from_assets(
        ...
        check_keys_by_asset_key: DefaultDict[
            AssetKey, Set[AssetCheckKey]
        ] = defaultdict(set)
        for ad in assets_defs:
    ...
    def unpartitioned_assets_def_asset_check_keys(self) -> AbstractSet[AssetCheckKey]:
        ...
        return {
            k
            for k in self.asset_check_keys
            if self.assets_def_for_key(k).partitions_def is None
        }
