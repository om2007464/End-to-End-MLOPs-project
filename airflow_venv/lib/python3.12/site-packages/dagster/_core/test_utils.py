...
def poll_for_step_start(
    instance: DagsterInstance, run_id: str, timeout: float = 30, message=None
):
    poll_for_event(
        instance, run_id, event_type="STEP_START", message=message, timeout=timeout
    )
...
def mock_workspace_from_repos(
    repos: Sequence[RepositoryDefinition],
) -> WorkspaceSnapshot:
    remote_repos = {}
    for repo in repos:
    ...
    type(mock_entry).code_location = unittest.mock.PropertyMock(
