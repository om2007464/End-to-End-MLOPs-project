...
class CodeLocation(AbstractContextManager):
    ...
    def get_job(self, selector: JobSubsetSelector) -> RemoteJob:
        ...
        if not selector.is_subset_selection:
            return self.get_repository(selector.repository_name).get_full_job(
                selector.job_name
            )
    ...
    def get_repository_python_origin(
        ...
        if repository_name not in self.repository_code_pointer_dict:
            raise DagsterInvariantViolationError(
                f"Unable to find repository {repository_name}."
            )
...
class InProcessCodeLocation(CodeLocation):
    def __init__(self, origin: InProcessCodeLocationOrigin, instance: DagsterInstance):
        ...
        self._repository_code_pointer_dict = (
            self._loaded_repositories.code_pointers_by_repo_name
        )
...
class GrpcServerCodeLocation(CodeLocation):
    def __init__(
        ...
        try:
            ...
            self._server_id = (
                server_id if server_id else sync_get_server_id(self.client)
            )
            ...
            self._dagster_library_versions = (
                list_repositories_response.dagster_library_versions
            )
    ...
    def get_current_runs(self) -> Sequence[str]:
        return deserialize_value(
            self.client.get_current_runs(), GetCurrentRunsResult
        ).current_runs
    ...
    def get_execution_plan(
        ...
        asset_selection = (
            frozenset(
                check.opt_set_param(remote_job.asset_selection, "asset_selection")
            )
        ...
        asset_check_selection = (
            frozenset(
                check.opt_set_param(
                    remote_job.asset_check_selection, "asset_check_selection"
                )
            )
    ...
    def get_subset_remote_job_result(
        ...
        if subset.job_data_snap:
            full_job = self.get_repository(selector.repository_name).get_full_job(
                selector.job_name
            )
            subset = copy(
                subset,
                job_data_snap=copy(
                    subset.job_data_snap, parent_job=full_job.job_snapshot
                ),
            )
    ...
    def get_partition_names_from_repo(
        ...
        return sync_get_external_partition_names_grpc(
            self.client, repository_handle, job_name
        )
    ...
    def get_notebook_data(self, notebook_path: str) -> bytes:
        check.str_param(notebook_path, "notebook_path")
        return sync_get_streaming_external_notebook_data_grpc(
            self.client, notebook_path
        )
