    ...
    def from_def(
        ...
        if defer_snapshots:
            ...
            job_datas = sorted(
                list(
                    map(
                        lambda job: JobDataSnap.from_job_def(
                            job, include_parent_snapshot=True
                        ),
                        jobs,
                    )
                ),
        ...
        for resource_key, nested_resources in nested_resource_map.items():
            for attribute, nested_resource in nested_resources.items():
                if nested_resource.type == NestedResourceType.TOP_LEVEL:
                    inverted_nested_resources_map[nested_resource.name][
                        resource_key
                    ] = attribute
...
class JobDataSnap:
    ...
    @classmethod
    def from_job_def(
        cls, job_def: JobDefinition, include_parent_snapshot: bool
    ) -> Self:
        return cls(
            name=job_def.name,
            job=job_def.get_job_snapshot(),
            parent_job=job_def.get_parent_job_snapshot()
...
@whitelist_for_serdes(
    storage_name="ExternalJobRef", old_fields={"is_legacy_pipeline": False}
)
@record
class JobRefSnap:
    name: str
    ...
    def from_def(
        ...
        if schedule_def.has_anonymous_job:
            ...
            serializable_asset_selection = (
                job_def.selection.to_serializable_asset_selection(
...
class SensorSnap(IHaveNew):
    ...
    @classmethod
    def from_def(
        cls, sensor_def: SensorDefinition, repository_def: RepositoryDefinition
    ) -> Self:
        ...
        if self.cron_schedule is not None:
            return TimeWindowPartitionsDefinition(
                ...
                end_offset=self.end_offset,
                    ...
                    datetime_from_timestamp(self.end, tz=self.timezone)
                    if self.end
                    else None
                ),
            )
...
@whitelist_for_serdes(
    ...
    storage_field_names={
        "partition_dimensions": "external_partition_dimension_definitions"
    },
)
@record
...
class ResourceSnap(IHaveNew):
        ...
        return super().__new__(
            ...
            asset_keys_using=list(
                check.opt_sequence_param(
                    asset_keys_using, "asset_keys_using", of_type=AssetKey
                )
            ),
            job_ops_using=list(
                check.opt_sequence_param(
            ...
            schedules_using=list(
                check.opt_sequence_param(
                    schedules_using, "schedules_using", of_type=str
                )
            ),
            sensors_using=list(
    ...
    def from_def(
        ...
        while (
            ...
            unconfigured_config_schema = (
                unconfigured_config_schema.parent_def.config_schema
            )
        ...
        if type(resource_type_def) in (ResourceDefinition, IOManagerDefinition):
            ...
            module_name = check.not_none(
                inspect.getmodule(original_resource_fn)
            ).__name__
...
class BackcompatTeamOwnerFieldDeserializer(FieldSerializer):
    ...
    def unpack(self, __unpacked_value, whitelist_map, context):
        return (
            [
                owner
                if (is_valid_email(owner) or owner.startswith("team:"))
                else f"team:{owner}"
...
def _get_resource_job_usage(job_defs: Sequence[JobDefinition]) -> ResourceJobUsageMap:
    ...
    for job_def in job_defs:
        ...
        for solid in job_def.nodes_in_topological_order:
            resource_usage += [
                use for use in _get_resource_usage_from_node(job_def, solid)
            ]
...
def asset_check_node_snaps_from_repo(
    ...
    for check_key, job_names in job_names_by_check_key.items():
        ...
        asset_check_node_snaps.append(
            AssetCheckNodeSnap(
                ...
                execution_set_identifier=repo.asset_graph.get_execution_set_identifier(
                    check_key
                ),
                job_names=job_names,
    ...
    return sorted(
        asset_check_node_snaps, key=lambda check: (check.asset_key, check.name)
    )
...
def asset_node_snaps_from_repo(repo: RepositoryDefinition) -> Sequence[AssetNodeSnap]:
    ...
    primary_node_pairs_by_asset_key: Dict[
        AssetKey, Tuple[NodeOutputHandle, JobDefinition]
    ] = {}
    ...
    for job_def in repo.get_all_jobs():
        ...
        for node_output_handle, asset_key in asset_keys_by_node_output.items():
            if asset_key not in asset_layer.asset_keys_for_node(
    ...
    for key in sorted(asset_graph.get_all_asset_keys()):
        ...
        if key in primary_node_pairs_by_asset_key:
            ...
            graph_name = (
                root_node_handle.name
                if root_node_handle != output_handle.node_handle
                else None
            )
        ...
        (
            automation_condition,
            automation_condition_snapshot,
        ) = resolve_automation_condition_args(asset_node.automation_condition)
        asset_node_snaps.append(
            AssetNodeSnap(
                ...
                child_edges=[
                    AssetChildEdgeSnap(child_asset_key=k)
...
def _get_nested_resources_map(
    ...
    for resource_name, resource_def in resource_datas.items():
        out_map[resource_name] = _get_nested_resources(
            resource_def, top_level_resources
        )
    return out_map
