...
from dagster._core.definitions.utils import (
    get_default_automation_condition_sensor_selection,
)
...
class RemoteRepository:
    ...
    def __init__(
        ...
        self.repository_snap = check.inst_param(
            repository_snap, "repository_snap", RepositorySnap
        )
        ...
        for asset_check_node_snap in repository_snap.asset_check_nodes or []:
            for job_name in asset_check_node_snap.job_names:
                self._asset_check_jobs.setdefault(job_name, []).append(
                    asset_check_node_snap
                )
    ...
    def _partition_sets(self) -> Dict[str, "RemotePartitionSet"]:
        return {
            partition_set_snap.name: RemotePartitionSet(
                partition_set_snap, self._handle
            )
    ...
    def get_full_job(self, job_name: str) -> "RemoteJob":
        ...
        check.invariant(
            self.has_job(job_name), f'No remote job named "{job_name}" found'
        )
    ...
    def get_asset_node_snaps(
        self, job_name: Optional[str] = None
    ) -> Sequence[AssetNodeSnap]:
        return (
    ...
    def _get_partitions_def_for_job(
        ...
        for asset_node in asset_nodes:
            if (
                selected_asset_keys is not None
                and asset_node.asset_key not in selected_asset_keys
            ):
                continue

            if asset_node.partitions is not None:
                unique_partitions_defs.add(
                    asset_node.partitions.get_partitions_definition()
                )
    ...
    def get_sensors_targeting(self, asset_key: AssetKey) -> AbstractSet["RemoteSensor"]:
        ...
        for job_name in asset_snap.job_names:
            if (
                ...
                and job_name in self.sensors_by_job_name
            ):
                sensors.update(self.sensors_by_job_name[job_name])

        return sensors

    def get_schedules_targeting(
        self, asset_key: AssetKey
    ) -> AbstractSet["RemoteSchedule"]:
        asset_snap = self.get_asset_node_snap(asset_key)
        if not asset_snap:
            return _empty_set
        ...
        for job_name in asset_snap.job_names:
            if (
                job_name != IMPLICIT_ASSET_JOB_NAME
                and job_name in self.schedules_by_job_name
            ):
    ...
    def __init__(
        ...
        if job_data_snap:
            self._active_preset_dict = {
                ap.name: ap for ap in job_data_snap.active_presets
            }
            self._name = job_data_snap.name

        elif job_ref_snap:
            self._active_preset_dict = {
                ap.name: ap for ap in job_ref_snap.active_presets
            }
    ...
    def topological_steps(self):
        if self._topological_steps is None:
            self._topological_steps = [
                step
                for step_level in self.topological_step_levels()
...
class RemoteResource:
    ...
    def __init__(self, resource_snap: ResourceSnap, handle: RepositoryHandle):
        self._resource_snap = check.inst_param(
            resource_snap, "resource_snap", ResourceSnap
        )
...
class RemoteSchedule:
    def __init__(self, schedule_snap: ScheduleSnap, handle: RepositoryHandle):
        self._schedule_snap = check.inst_param(
            schedule_snap, "schedule_snap", ScheduleSnap
        )
        self._handle = InstigatorHandle(
