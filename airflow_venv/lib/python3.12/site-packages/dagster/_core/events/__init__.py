...
def _assert_type(
    ...
    _expected_type = (
        [expected_type]
        if isinstance(expected_type, DagsterEventType)
        else expected_type
    )
    check.invariant(
...
def _validate_event_specific_data(
    ...
    if event_type == DagsterEventType.STEP_OUTPUT:
        ...
        check.inst_param(
            event_specific_data, "event_specific_data", StepMaterializationData
        )
    elif event_type == DagsterEventType.STEP_EXPECTATION_RESULT:
        check.inst_param(
            event_specific_data, "event_specific_data", StepExpectationResultData
        )
        ...
        check.inst_param(
            event_specific_data, "event_specific_data", AssetCheckEvaluationPlanned
        )
    elif event_type == DagsterEventType.ASSET_CHECK_EVALUATION:
        check.inst_param(
            event_specific_data, "event_specific_data", AssetCheckEvaluation
        )
...
class DagsterEvent(
    ...
    def job_canceled_data(self) -> "JobCanceledData":
        _assert_type(
            "job_canceled_data", DagsterEventType.RUN_CANCELED, self.event_type
        )
    ...
    def step_concurrency_blocked(
        ...
        return DagsterEvent.from_step(
            ...
            message=message,
            event_specific_data=EngineEventData(
                metadata={"concurrency_key": concurrency_key}
            ),
        )
    ...
    def job_failure(
        ...
        if isinstance(job_context_or_name, IPlanContext):
            ...
            event = DagsterEvent(
                ...
                event_specific_data=JobFailureData(
                    error_info, failure_reason=failure_reason
                ),
    ...
    def job_canceled(
        ...
        return DagsterEvent.from_job(
            ...
            job_context,
            message=message
            or f'Execution of run for "{job_context.job_name}" canceled.',
...
class JobFailureData(
    ...
    def __new__(
        ...
        return super(JobFailureData, cls).__new__(
            cls,
            error=check.opt_inst_param(error, "error", SerializableErrorInfo),
            failure_reason=check.opt_inst_param(
                failure_reason, "failure_reason", RunFailureReason
            ),
...
def _handle_back_compat(
    ...
    if event_type_value == "STEP_MATERIALIZATION":
        assert (
            event_specific_data is not None
        ), "STEP_MATERIALIZATION event must have specific data"
