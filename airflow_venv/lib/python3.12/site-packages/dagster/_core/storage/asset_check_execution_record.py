...
class AssetCheckExecutionRecord(
    ...
    def __new__(
        ...
        if status == AssetCheckExecutionRecordStatus.PLANNED:
            check.invariant(
                event is None
                or event_type == DagsterEventType.ASSET_CHECK_EVALUATION_PLANNED,
    ...
    def _blocking_batch_load(
        ...
        records_by_key = (
            context.instance.event_log_storage.get_latest_asset_check_execution_by_key(
                list(keys)
            )
        )
    ...
    async def targets_latest_materialization(
        self, loading_context: LoadingContext
    ) -> bool:
        ...
        latest_materialization = (
            asset_record.asset_entry.last_materialization_record
            if asset_record
            else None
        )
