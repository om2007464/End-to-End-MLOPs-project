...
def get_currently_upgrading_instance() -> DagsterInstance:
    ...
    check.invariant(
        _UPGRADING_INSTANCE is not None, "currently upgrading instance not set"
    )
...
def create_0_10_0_schedule_tables() -> None:
    ...
    if not has_table("job_ticks"):
        ...
        op.create_index(
            "idx_job_tick_status",
            "job_ticks",
            ["job_origin_id", "status"],
            unique=False,
        )
        op.create_index(
            "idx_job_tick_timestamp",
            "job_ticks",
            ["job_origin_id", "timestamp"],
            unique=False,
        )
...
def create_bulk_actions_table() -> None:
    ...
    if not has_table("bulk_actions"):
        ...
        op.create_index(
            "idx_bulk_actions_status", "bulk_actions", ["status"], unique=False
        )
...
def extract_asset_keys_idx_columns() -> None:
    ...
    op.add_column(
        "asset_keys", db.Column("last_materialization_timestamp", db.types.TIMESTAMP)
    )
    ...
    if not has_table("instigators"):
        op.create_table(
            ...
            db.Column(
                "create_timestamp",
                db.DateTime,
                server_default=get_sql_current_timestamp(),
            ),
            db.Column(
                "update_timestamp",
                db.DateTime,
...
def create_tick_selector_index() -> None:
    ...
    op.create_index(
        "idx_tick_selector_timestamp",
        "job_ticks",
        ["selector_id", "timestamp"],
        unique=False,
    )
...
def drop_id_based_event_indices() -> None:
    ...
    if not has_index("event_logs", "idx_asset_key"):
        ...
        if has_index("event_logs", "idx_events_by_asset"):
            op.drop_index(
                "idx_events_by_asset", "event_logs", postgresql_concurrently=True
            )
