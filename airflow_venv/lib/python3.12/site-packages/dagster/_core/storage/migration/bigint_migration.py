...
def _has_integer_id_column(inspector, table):
    type_by_col_name = {
        col["name"]: col["type"] for col in inspector.get_columns(table.name)
    }
...
def _convert_id_from_int_to_bigint(conn, table, should_autoincrement):
    ...
    if dialect == "postgresql":
        ...
        if should_autoincrement:
            statement = db.text(
                f"ALTER TABLE {table.name} MODIFY COLUMN id BIGINT AUTO_INCREMENT"
            )
        else:
...
def _remove_asset_event_tags_foreign_key(conn, inspector, print_fn):
    for fk in inspector.get_foreign_keys("asset_event_tags"):
        ...
        conn.execute(
            db.text(f"ALTER TABLE asset_event_tags DROP CONSTRAINT {fk['name']}")
        )
...
def run_bigint_migration(
    instance: DagsterInstance, print_fn: Callable[..., Any] = print
):
    if isinstance(instance.event_log_storage, SqliteEventLogStorage):
        print_fn(
            "Sqlite does not support bigint types, no need to migrate event log storage."
        )
    ...
    if isinstance(instance.run_storage, SqliteRunStorage):
        print_fn(
            "Sqlite does not support bigint types, no need to migrate run storage."
        )
    ...
    if isinstance(instance.schedule_storage, SqliteScheduleStorage):
        print_fn(
            "Sqlite does not support bigint types, no need to migrate schedule storage."
        )
