...
class SqlRunStorage(RunStorage):
    ...
    def handle_run_event(self, run_id: str, event: DagsterEvent) -> None:
        ...
        if event.event_type == DagsterEventType.PIPELINE_FAILURE and isinstance(
            ...
            if failure_reason and failure_reason != RunFailureReason.UNKNOWN:
                self.add_run_tags(
                    run_id, {RUN_FAILURE_REASON_TAG: failure_reason.value}
                )
    ...
    def _add_cursor_limit_to_query(
        ...
        if cursor:
            cursor_query = db_select([RunsTable.c.id]).where(
    ...
    def _add_filters_to_query(
        ...
        if filters.updated_after:
            query = query.where(
                ...
                > filters.updated_after.replace(tzinfo=None)
            )

        if filters.updated_before:
            query = query.where(
                RunsTable.c.update_timestamp
                < filters.updated_before.replace(tzinfo=None)
            )

        if filters.created_after:
            query = query.where(
                RunsTable.c.create_timestamp
                > filters.created_after.replace(tzinfo=None)
            )

        if filters.created_before:
            query = query.where(
                RunsTable.c.create_timestamp
                < filters.created_before.replace(tzinfo=None)
            )
    ...
    def get_runs(
        ...
        query = self._runs_query(
            filters, cursor, limit, bucket_by=bucket_by, ascending=ascending
        )
    ...
    def get_run_ids(
        ...
        query = self._runs_query(
            filters=filters, cursor=cursor, limit=limit, columns=["run_id"]
        )
        rows = self.fetchall(query)
    ...
    def get_run_records(
        ...
        return [
            RunRecord(
                ...
                start_time=(
                    check.opt_inst(row["start_time"], float)
    ...
    def get_run_group(self, run_id: str) -> Tuple[str, Sequence[DagsterRun]]:
        ...
        # get run group
        run_group_query = db_select(
            [RunsTable.c.run_body, RunsTable.c.status]
        ).select_from(
            root_to_run.join(
                RunsTable,
    ...
    def add_job_snapshot(
        self, job_snapshot: JobSnap, snapshot_id: Optional[str] = None
    ) -> str:
    ...
    def _add_backfill_filters_to_table(
        ...
        if (
            filters
            and filters.tags
            and self.has_built_index(BACKFILL_JOB_NAME_AND_TAGS)
        ):
            for i, (key, value) in enumerate(filters.tags.items()):
                backfill_tags_alias = db.alias(
                    BackfillTagsTable, f"backfill_tags_filter{i}"
                )
    ...
    def _backfills_query(self, filters: Optional[BulkActionsFilter] = None):
        ...
        if filters and filters.job_name:
            if self.has_built_index(BACKFILL_JOB_NAME_AND_TAGS):
                ...
                backfills_with_job_name_query = db_select(
                    [RunTagsTable.c.value]
                ).select_from(runs_in_backfill_with_job_name)
                query = query.where(
                    BulkActionsTable.c.key.in_(
                        db_subquery(backfills_with_job_name_query)
                    )
                )
        if filters and filters.statuses:
            query = query.where(
                BulkActionsTable.c.status.in_(
    ...
    def _apply_backfill_tags_filter_to_results(
        self,
        backfills: Sequence[PartitionBackfill],
    ...
    def get_backfills(
        ...
        query = self._add_cursor_limit_to_backfills_query(
        ...
        if (
            filters
            and filters.tags
            and not self.has_built_index(BACKFILL_JOB_NAME_AND_TAGS)
        ):
    ...
    def get_backfills_count(self, filters: Optional[BulkActionsFilter] = None) -> int:
        ...
        if filters and filters.tags:
            ...
            return len(
                self._apply_backfill_tags_filter_to_results(
                    backfill_candidates, filters.tags
                )
            )
    ...
    def get_backfill(self, backfill_id: str) -> Optional[PartitionBackfill]:
        ...
        query = db_select([BulkActionsTable.c.body]).where(
