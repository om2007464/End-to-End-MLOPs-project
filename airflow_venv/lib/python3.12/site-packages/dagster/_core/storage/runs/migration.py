...
def add_run_stats(run_storage: RunStorage, run_id: str) -> None:
    ...
    instance = check.inst_param(
        run_storage._instance, "instance", DagsterInstance
    )  # noqa: SLF001
...
def migrate_run_repo_tags(
    run_storage: RunStorage, print_fn: Optional[PrintFn] = None
) -> None:
    from dagster._core.storage.runs.sql_run_storage import SqlRunStorage
    ...
    base_query = (
        ...
        .select_from(
            RunsTable.join(
                subquery, RunsTable.c.run_id == subquery.c.tags_run_id, isouter=True
            )
        )
    ...
    for backfill in chunked_backfill_iterator(storage, print_fn):
        ...
        if backfill.job_name is not None:
            add_backfill_job_name(
                run_storage=storage,
                backfill_id=backfill.backfill_id,
                job_name=backfill.job_name,
            )


def add_backfill_tags(
    run_storage: RunStorage, backfill_id: str, tags: Mapping[str, str]
):
    from dagster._core.storage.runs.sql_run_storage import SqlRunStorage
