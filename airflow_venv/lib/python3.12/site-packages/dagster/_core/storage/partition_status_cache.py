...
class AssetStatusCacheValue(
    ...
    def _blocking_batch_load(
        cls,
        keys: Iterable[Tuple[AssetKey, PartitionsDefinition]],
        context: LoadingContext,
    ) -> Iterable[Optional["AssetStatusCacheValue"]]:
        return context.instance.event_log_storage.get_asset_status_cache_values(
            dict(keys), context
        )
    ...
    def deserialize_materialized_partition_subsets(
        ...
        return partitions_def.deserialize_subset(
            self.serialized_materialized_partition_subset
        )
    ...
    def deserialize_failed_partition_subsets(
        ...
        return partitions_def.deserialize_subset(
            self.serialized_failed_partition_subset
        )
    ...
    def deserialize_in_progress_partition_subsets(
        ...
        return partitions_def.deserialize_subset(
            self.serialized_in_progress_partition_subset
        )
    ...
    for event_tags in instance.get_event_tags_for_asset(asset_key):
        ...
        if all(
            ...
            materialized_keys.append(
                MultiPartitionKey(
                    {
                        dimension_names[0]: event_partition_keys_by_dimension[
                            dimension_names[0]
                        ],
                        dimension_names[1]: event_partition_keys_by_dimension[
                            dimension_names[1]
                        ],
                    }
                )
...
def get_last_planned_storage_id(
    ...
    if (
        instance.event_log_storage.asset_records_have_last_planned_materialization_storage_id
    ):
        return (
...
def _build_status_cache(
    ...
    if stored_cache_value:
        ...
        if failed_subset and new_partitions:
            failed_subset = (
                failed_subset
                - partitions_def.empty_subset().with_partition_keys(new_partitions)
            )
...
def build_failed_and_in_progress_partition_subset(
    ...
    if last_planned_materialization_storage_id and (
        not after_storage_id
        or last_planned_materialization_storage_id > after_storage_id
    ):
    ...
    if incomplete_materializations:
        to_fetch = list(
            set([run_id for run_id, _event_id in incomplete_materializations.values()])
        )
        finished_runs = {}
        unfinished_runs = {}
    ...
    return (
        ...
        (
            partitions_def.empty_subset().with_partition_keys(
                get_validated_partition_keys(
                    instance, partitions_def, in_progress_partitions
                )
            )
