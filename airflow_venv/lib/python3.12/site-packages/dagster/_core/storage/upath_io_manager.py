...
class UPathIOManager(IOManager):
    ...
    def _with_extension(self, path: "UPath") -> "UPath":
        return (
            path.with_suffix(path.suffix + self.extension) if self.extension else path
        )
    ...
    def get_loading_input_partition_log_message(
        self, path: "UPath", partition_key: str
    ) -> str:
    ...
    def _load_partition_from_path(
        ...
        try:
            context.log.debug(
                self.get_loading_input_partition_log_message(path, partition_key)
            )
            obj = self.load_from_path(context=context, path=path)
            return obj
