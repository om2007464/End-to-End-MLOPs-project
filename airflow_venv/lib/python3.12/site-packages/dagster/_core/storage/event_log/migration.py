...
SECONDARY_INDEX_ASSET_KEY = (
    "asset_key_table"  # builds the asset key table from the event log
)
ASSET_KEY_INDEX_COLS = (
    "asset_key_index_columns"  # extracts index columns from the asset_keys table
)
...
def migrate_event_log_data(instance=None):
    ...
    for run in instance.get_runs():
        for record in event_log_storage.get_records_for_run(run.run_id).records:
            event_log_storage.update_event_log_record(
                record.storage_id, record.event_log_entry
            )
    ...
    with event_log_storage.index_connection() as conn:
        ...
        for row in results:
            ...
            if asset_details_str:
                ...
                wipe_timestamp = (
                    asset_details.last_wipe_timestamp if asset_details else None
                )

            if last_materialization_str:
                event_or_materialization = deserialize_value(
                    last_materialization_str, NamedTuple
                )
            ...
            if not event:
                ...
                conn.execute(
                    ...
                    .values(
                        ...
                        wipe_timestamp=(
                            datetime_from_timestamp(wipe_timestamp)
                            if wipe_timestamp
                            else None
                        ),
                    )
