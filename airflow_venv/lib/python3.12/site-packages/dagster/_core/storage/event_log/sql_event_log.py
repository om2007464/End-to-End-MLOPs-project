...
class SqlEventLogStorage(EventLogStorage):
    ...
    def has_asset_key_col(self, column_name: str) -> bool:
        with self.index_connection() as conn:
            column_names = [
                x.get("name") for x in db.inspect(conn).get_columns(AssetKeyTable.name)
            ]
    ...
    def store_asset_event(self, event: EventLogEntry, event_id: int):
        ...
        values = self._get_asset_entry_values(
            event, event_id, self.has_asset_key_index_cols()
        )
    ...
    def _get_asset_entry_values(
        ...
        if dagster_event.is_step_materialization:
            ...
            if has_asset_key_index_cols:
                entry_values.update(
                    {
                        "last_materialization_timestamp": datetime_from_timestamp(
                            event.timestamp
                        ),
                    }
                )
    ...
    def add_asset_event_tags(
        ...
        current_tags_list = self.get_event_tags_for_asset(
            asset_key, filter_event_id=event_id
        )
    ...
    def _tags_for_asset_event(self, event: EventLogEntry) -> Mapping[str, str]:
        ...
        if event.dagster_event and event.dagster_event.asset_key:
            if event.dagster_event.is_step_materialization:
                tags = (
                    event.get_dagster_event().step_materialization_data.materialization.tags
                    or {}
                )
            elif event.dagster_event.is_asset_observation:
                tags = (
                    event.get_dagster_event().asset_observation_data.asset_observation.tags
                )
    ...
    def get_records_for_run(
        ...
        check.invariant(
            not of_type or isinstance(of_type, (DagsterEventType, frozenset, set))
        )
        ...
        dagster_event_types = (
            ...
            if isinstance(of_type, DagsterEventType)
                ...
                of_type, "dagster_event_type", of_type=DagsterEventType
            )
        )
            ...
            query = query.where(
                SqlEventLogStorageTable.c.dagster_event_type.in_(
                    [
                        ...
                        for dagster_event_type in dagster_event_types
                    ]
                )
            )
        ...
        if cursor is not None:
            ...
            if cursor_obj.is_offset_cursor():
                ...
                if ascending:
                    query = query.where(
                        SqlEventLogStorageTable.c.id > cursor_obj.storage_id()
                    )
                else:
                    query = query.where(
                        SqlEventLogStorageTable.c.id < cursor_obj.storage_id()
    ...
    def get_stats_for_run(self, run_id: str) -> DagsterRunStatsSnapshot:
        ...
        try:
            ...
            return DagsterRunStatsSnapshot(
                ...
                enqueued_time=(
                    utc_datetime_from_naive(enqueued_time).timestamp()
                    if enqueued_time
                    else None
                ),
                launch_time=(
                    utc_datetime_from_naive(launch_time).timestamp()
                    if launch_time
                    else None
                ),
                start_time=(
                    utc_datetime_from_naive(start_time).timestamp()
                    if start_time
                    else None
                ),
                end_time=(
                    utc_datetime_from_naive(end_time).timestamp() if end_time else None
                ),
            )
        except (seven.JSONDecodeError, DeserializationError) as err:
            raise DagsterEventLogInvalidForRun(run_id=run_id) from err
    ...
    def get_step_stats_for_run(
        ...
        try:
            records = deserialize_values(
                (json_str for (json_str,) in results), EventLogEntry
            )
    ...
    def _apply_filter_to_query(
        ...
        if event_records_filter.asset_partitions:
            query = query.where(
                SqlEventLogStorageTable.c.partition.in_(
        ...
        if apply_cursor_filters:
            ...
            if event_records_filter.before_cursor is not None:
                before_cursor_id = (
                    ...
                    if isinstance(
                        event_records_filter.before_cursor, RunShardedEventsCursor
                    )
        ...
        if event_records_filter.storage_ids:
            query = query.where(
                SqlEventLogStorageTable.c.id.in_(event_records_filter.storage_ids)
            )
    ...
    def _apply_tags_table_joins(
        ...
        event_id_col = (
            table.c.id if table == SqlEventLogStorageTable else table.c.event_id
        )
        i = 0
    ...
    def _get_event_records(
        ...
        check.inst_param(
            event_records_filter, "event_records_filter", EventRecordsFilter
        )
        ...
        for row_id, json_str in results:
            try:
                ...
                if not isinstance(event_record, EventLogEntry):
                    logging.warning(
                        "Could not resolve event record as EventLogEntry for id `%s`.",
                        row_id,
                    )
                    continue
    ...
    def _get_event_records_result(
        ...
        if records:
            new_cursor = EventLogCursor.from_storage_id(
                records[-1].storage_id
            ).to_string()
        elif cursor:
            new_cursor = cursor
        else:
    ...
    def fetch_materializations(
        ...
        if isinstance(records_filter, AssetRecordsFilter):
            ...
            before_cursor, after_cursor = EventRecordsFilter.get_cursor_params(
                cursor, ascending
            )
        ...
        return self._get_event_records_result(
            event_records_filter, limit, cursor, ascending
        )
    ...
    def get_maximum_record_id(self) -> Optional[int]:
        with self.index_connection() as conn:
            result = conn.execute(
                db_select([db.func.max(SqlEventLogStorageTable.c.id)])
            ).fetchone()
    ...
    def _get_latest_materialization_records(
        ...
        backcompat_query = db_select(
            ...
            latest_event_subquery.join(
                ...
                db.and_(
                    SqlEventLogStorageTable.c.asset_key
                    == latest_event_subquery.c.asset_key,
        ...
        for row in event_rows:
            ...
            if asset_key:
                results[asset_key] = EventLogRecord(
                    ...
                    event_log_entry=deserialize_value(
                        cast(str, row["event"]), EventLogEntry
                    ),
                )
        return results
    ...
    def get_asset_check_summary_records(
        ...
        for asset_check_key in asset_check_keys:
            execution_record = self.get_asset_check_execution_history(
                asset_check_key, limit=1
            )
            states[asset_check_key] = AssetCheckSummaryRecord(
                asset_check_key=asset_check_key,
                last_check_execution_record=execution_record[0]
                if execution_record
                else None,
    ...
    def get_latest_materialization_events(
        ...
        rows = self._fetch_asset_rows(asset_keys=asset_keys)
        return {
            asset_key: event_log_record.event_log_entry
            if event_log_record is not None
            else None
    ...
    def _fetch_asset_rows(
        ...
        is_partial_query = (
            asset_keys is not None or bool(prefix) or bool(limit) or bool(cursor)
        )
    ...
    def _fetch_raw_asset_rows(
        ...
        query = self._apply_asset_filter_to_query(
            query, asset_keys, prefix, limit, cursor
        )
        ...
        for row in rows:
            ...
            if isinstance(
                materialization_or_event_or_record, (EventLogRecord, EventLogEntry)
            ):
                if isinstance(materialization_or_event_or_record, EventLogRecord):
                    event_timestamp = (
                        materialization_or_event_or_record.event_log_entry.timestamp
                    )
                else:
    ...
    def _fetch_backcompat_materialization_times(
        ...
        return {
            AssetKey.from_db_string(row["asset_key"]): row["timestamp"]
            for row in backcompat_rows
        }  # type: ignore
    ...
    def _can_mark_assets_as_migrated(self, rows):
        ...
        for row in rows:
            ...
            if _get_from_row(row, "asset_details") and not _get_from_row(
                row, "wipe_timestamp"
            ):
                return False
    ...
    def _get_assets_details(
        ...
        with self.index_connection() as conn:
            ...
            return [
                asset_key_to_details.get(asset_key.to_string(), None)
    ...
    def get_event_tags_for_asset(
        ...
        if not filter_tags:
            ...
            table = self._apply_tags_table_joins(
                AssetEventTagsTable, filter_tags, asset_key
            )
            tags_query = db_select(
                [
                    AssetEventTagsTable.c.key,
        ...
        if filter_event_id is not None:
            tags_query = tags_query.where(
                AssetEventTagsTable.c.event_id == filter_event_id
            )
    ...
    def get_materialized_partitions(
        ...
        query = self._add_assets_wipe_filter_to_query(
            query, assets_details, [asset_key]
        )
    ...
    def _latest_event_ids_by_partition_subquery(
        ...
        if asset_partitions is not None:
            query = query.where(
                SqlEventLogStorageTable.c.partition.in_(asset_partitions)
            )
    ...
    def get_latest_storage_id_by_partition(
        ...
        latest_event_ids_by_partition_subquery = (
            self._latest_event_ids_by_partition_subquery(
                asset_key,
                [event_type],
    ...
    def get_latest_tags_by_partition(
        ...
        if not tag_keys or len(tag_keys) != len(
            self.get_asset_tags_to_index(set(tag_keys))
        ):
            check.failed(
        ...
        for row in rows:
            latest_tags_by_partition[cast(str, row[0])][cast(str, row[1])] = cast(
                str, row[2]
            )
    ...
    def get_latest_asset_partition_materialization_attempts_without_materializations(
        ...
        with self.index_connection() as conn:
            materialization_planned_rows = db_fetch_mappings(
                conn, materialization_planned_events
            )
        ...
        for mat_row in materialization_rows:
            ...
            if mat_partition not in materialization_planned_rows_by_partition:
                continue
            _, planned_event_id = materialization_planned_rows_by_partition[
                mat_partition
            ]
    ...
    def _reconcile_concurrency_limits_from_slots(self) -> None:
        ...
        if not self._has_rows(ConcurrencySlotsTable) or self._has_rows(
            ConcurrencyLimitsTable
        ):
            return
    ...
    def set_concurrency_slots(self, concurrency_key: str, num: int) -> None:
        ...
        if num < 0:
            raise DagsterInvalidInvocationError(
                "Cannot have a negative number of slots."
            )
    ...
    def _allocate_concurrency_slots(
        self, conn, concurrency_key: str, num: int
    ) -> List[str]:
        keys_to_assign = []
        ...
        if existing > num:
            ...
            rows = conn.execute(
                ...
                .order_by(
                    db_case(
                        [(ConcurrencySlotsTable.c.run_id.is_(None), 1)], else_=0
                    ).desc(),
    ...
    def check_concurrency_claim(
        ...
        with self.index_connection() as conn:
            ...
            assigned_timestamp = (
                cast(datetime, pending_row[0]) if pending_row[0] else None
            )
            create_timestamp = (
                cast(datetime, pending_row[2]) if pending_row[2] else None
            )
                    ...
                    PendingStepsTable.insert().values(
                        [
                            dict(
                                ...
                                assigned_timestamp=db.func.now()
                                if should_assign
                                else None,
                            )
                        ]
    ...
    def get_concurrency_info(self, concurrency_key: str) -> ConcurrencyKeyInfo:
        ...
        with self.index_connection() as conn:
            ...
            return ConcurrencyKeyInfo(
                concurrency_key=concurrency_key,
                slot_count=len(
                    [slot_row for slot_row in slot_rows if not slot_row["deleted"]]
                ),
                claimed_slots=[
                    ClaimedSlotInfo(
                        run_id=slot_row["run_id"], step_key=slot_row["step_key"]
                    )
                ...
                pending_steps=[
                    PendingStepInfo(
                        ...
                        enqueued_timestamp=utc_datetime_from_naive(
                            row["create_timestamp"]
                        ),
                        assigned_timestamp=utc_datetime_from_naive(
    ...
    def get_concurrency_run_ids(self) -> Set[str]:
        with self.index_connection() as conn:
            rows = conn.execute(
                db_select([PendingStepsTable.c.run_id]).distinct()
            ).fetchall()
    ...
    def _event_insert_timestamp(self, event):
        # Postgres requires a datetime that is in UTC but has no timezone info
        return datetime.fromtimestamp(event.timestamp, timezone.utc).replace(
            tzinfo=None
        )
    ...
    def _update_asset_check_evaluation(
        self, event: EventLogEntry, event_id: Optional[int]
    ) -> None:
        evaluation = cast(
            AssetCheckEvaluation,
    ...
    def get_asset_check_execution_history(
        ...
        return [
            AssetCheckExecutionRecord.from_db_row(row, key=check_key) for row in rows
        ]
    ...
    def get_latest_asset_check_execution_by_key(
        ...
        latest_ids_subquery = db_subquery(
            ...
            .where(
                db.and_(
                    ...
                    AssetCheckExecutionsTable.c.check_name.in_(
                        [key.name for key in check_keys]
                    ),
                )
            )
            .group_by(
        ...
        for row in rows:
            check_key = AssetCheckKey(
                asset_key=check.not_none(
                    AssetKey.from_db_string(cast(str, row["asset_key"]))
                ),
                name=cast(str, row["check_name"]),
            )
            results[check_key] = AssetCheckExecutionRecord.from_db_row(
                row, key=check_key
            )
        return results
    ...
    def _get_partition_data_versions(
        ...
        data_version_subquery = db_subquery(
            data_version_subquery, "data_version_subquery"
        )
