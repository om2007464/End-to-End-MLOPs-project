...
class SqliteEventLogStorage(SqlEventLogStorage, ConfigurableClass):
    ...
    def store_event(self, event: EventLogEntry) -> None:
        ...
        if (
            ...
            and event.dagster_event_type in EVENT_TYPE_TO_PIPELINE_RUN_STATUS
        ):
    ...
    def get_event_records(
        ...
        is_asset_query = (
            event_records_filter and event_records_filter.event_type in ASSET_EVENTS
        )
        if is_asset_query:
...
class SqliteEventLogStorageWatchdog(PatternMatchingEventHandler):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._cursor = cursor
        super(SqliteEventLogStorageWatchdog, self).__init__(
            patterns=[self._log_path], **kwargs
        )

    def _process_log(self) -> None:
        connection = self._event_log_storage.get_records_for_run(
            self._run_id, self._cursor
        )
        ...
        for record in connection.records:
            ...
            try:
                ...
                logging.exception(
                    "Exception in callback for event watch on run %s.", self._run_id
                )
