...
class SqlPollingEventWatcher:
    ...
    def __init__(self, event_log_storage: EventLogStorage):
        ...
        self._run_id_to_watcher_dict: MutableMapping[
            str, SqlPollingRunIdEventWatcherThread
        ] = {}
        ...
        with self._dict_lock:
            if run_id not in self._run_id_to_watcher_dict:
                self._run_id_to_watcher_dict[
                    run_id
                ] = SqlPollingRunIdEventWatcherThread(self._event_log_storage, run_id)
...
class SqlPollingRunIdEventWatcherThread(threading.Thread):
    ...
    def add_callback(
        self, cursor: Optional[str], callback: Callable[[EventLogEntry, str], None]
    ):
                ...
                with self._callback_fn_list_lock:
                    for callback_with_cursor in self._callback_fn_list:
                        if (
                            ...
                            or EventLogCursor.parse(
                                callback_with_cursor.cursor
                            ).storage_id()
