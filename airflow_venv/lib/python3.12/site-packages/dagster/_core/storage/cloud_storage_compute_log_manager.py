...
class CloudStorageComputeLogManager(ComputeLogManager[T_DagsterInstance]):
    ...
    def log_data_for_type(
        ...
        if self.has_local_file(log_key, io_type):
            ...
            return self.local_manager.read_path(
                local_path, offset=offset, max_bytes=max_bytes
            )
    ...
    def get_log_data(
        ...
        return CapturedLogData(
            ...
            stdout=stdout,
            stderr=stderr,
            cursor=self.local_manager.build_cursor(
                new_stdout_offset, new_stderr_offset
            ),
        )
    ...
    def get_log_metadata(self, log_key: Sequence[str]) -> CapturedLogMetadata:
        return CapturedLogMetadata(
            ...
            stdout_download_url=self.download_url_for_type(
                log_key, ComputeIOType.STDOUT
            ),
            stderr_download_url=self.download_url_for_type(
    ...
    def _should_download(self, log_key: Sequence[str], io_type: ComputeIOType):
        return not self.has_local_file(
            log_key, io_type
        ) and self.cloud_storage_has_logs(log_key, io_type)
