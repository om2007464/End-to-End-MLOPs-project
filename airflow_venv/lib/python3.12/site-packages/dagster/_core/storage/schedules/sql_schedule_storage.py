...
class SqlScheduleStorage(ScheduleStorage):
    ...
    def all_instigator_state(
        ...
        if self.has_instigators_table() and self.has_built_index(
            SCHEDULE_JOBS_SELECTOR_ID
        ):
            query = db_select([InstigatorsTable.c.instigator_body]).select_from(
                InstigatorsTable
            )
            ...
            if instigator_type:
                query = query.where(
                    InstigatorsTable.c.instigator_type == instigator_type.value
                )
            if instigator_statuses:
                query = query.where(
                    InstigatorsTable.c.status.in_(
            ...
            if repository_origin_id:
                query = query.where(
                    JobTable.c.repository_origin_id == repository_origin_id
                )
            if instigator_type:
                query = query.where(JobTable.c.job_type == instigator_type.value)
            if instigator_statuses:
                query = query.where(
                    JobTable.c.status.in_(
                        [status.value for status in instigator_statuses]
                    )
                )
    ...
    def get_instigator_state(
        ...
        rows = self.execute(query)
        return (
            self._deserialize_rows(rows[:1], InstigatorState)[0] if len(rows) else None
        )
    ...
    def _add_or_update_instigators_table(
        self, conn: Connection, state: InstigatorState
    ) -> None:
    ...
    def _add_filter_limit(
        ...
        if before:
            query = query.where(
                JobTickTable.c.timestamp < datetime_from_timestamp(before)
            )
        if after:
            query = query.where(
                JobTickTable.c.timestamp > datetime_from_timestamp(after)
            )
        if limit:
    ...
    def supports_batch_queries(self) -> bool:
        return self.has_instigators_table() and self.has_built_index(
            SCHEDULE_TICKS_SELECTOR_ID
        )
    ...
    def get_ticks(
        ...
        rows = self.execute(query)
        return list(
            map(lambda r: InstigatorTick(r[0], deserialize_value(r[1], TickData)), rows)
        )
        ...
        if tick_statuses:
            query = query.where(
                JobTickTable.c.status.in_(
                    [tick_status.value for tick_status in tick_statuses]
                )
            )
    ...
    def get_auto_materialize_asset_evaluations(
        ...
        with self.connect() as conn:
            query = (
                ...
                .where(
                    AssetDaemonAssetEvaluationsTable.c.asset_key == key.to_db_string()
                )
            ...
            if cursor is not None:
                query = query.where(
                    AssetDaemonAssetEvaluationsTable.c.evaluation_id < cursor
                )

            rows = db_fetch_mappings(conn, query)
            return [
                AutoMaterializeAssetEvaluationRecord.from_db_row(row) for row in rows
            ]
