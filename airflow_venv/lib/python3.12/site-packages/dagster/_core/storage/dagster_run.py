    ...
    def before_unpack(self, context, unpacked_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "selector" in unpacked_dict:
            ...
            if solids_to_execute is None:
                solids_to_execute = (
                    frozenset(selector_subset) if selector_subset else None
                )
...
class DagsterRun(
    ...
    @staticmethod
    def tags_for_tick_id(
        tick_id: str, has_evaluations: bool = False
    ) -> Mapping[str, str]:
        if has_evaluations:
            automation_tags = {
                AUTOMATION_CONDITION_TAG: "true",
                ASSET_EVALUATION_ID_TAG: tick_id,
            }
        else:
            automation_tags = {}
...
class RunRecord(
    ...
    def _blocking_batch_load(
        ...
        records = context.instance.get_run_records(
            RunsFilter(run_ids=list(result_map.keys()))
        )
...
class ExecutionSelector(
    NamedTuple(
        "_ExecutionSelector", [("name", str), ("solid_subset", Optional[Sequence[str]])]
    )
):
