...
def ensure_resource_deps_satisfiable(
    resource_deps: Mapping[str, AbstractSet[str]]
) -> None:
    ...
    try:
        ...
        for level in toposort(resource_dependencies):
            for resource_name in level:
                ...
                resource_fn = cast(
                    Callable[[InitResourceContext], Any], resource_def.resource_fn
                )
                ...
                for event in manager.generate_setup_events():
                    if event:
                        yield event
                initialized_resource = check.inst(
                    manager.get_object(), InitializedResource
                )
                ...
                contains_generator = (
                    contains_generator or initialized_resource.is_generator
                )
...
def single_resource_event_generator(
    ...
    try:
        msg_fn = (
            lambda: f"Error executing resource_fn on ResourceDefinition {resource_name}"
        )
    ...
    with user_code_error_boundary(
        ...
        try:
            ...
            check.failed(
                f"Resource generator {resource_name} yielded more than one item."
            )
...
def get_required_resource_keys_to_init(
    ...
    return frozenset(
        get_transitive_required_resource_keys(resource_keys, job_def.resource_defs)
    )
...
def get_required_resource_keys_for_step(
    ...
    for step_input in execution_step.step_inputs:
        ...
        resource_keys = resource_keys.union(
            input_def.dagster_type.required_resource_keys
        )
        ...
        if isinstance(step_input, StepInput):
            ...
            step_input, (UnresolvedMappedStepInput, UnresolvedCollectStepInput)
        ):
    ...
    for step_output in execution_step.step_outputs:
        ...
        resource_keys = resource_keys.union(
            output_def.dagster_type.required_resource_keys
        )
