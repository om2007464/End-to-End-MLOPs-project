...
def _get_implicit_job_name_for_assets(
    ...
    return next(
        (
            ...
            if job_name.startswith(IMPLICIT_ASSET_JOB_NAME)
        ),
        None,
    )
...
def _create_asset_run(
    ...
    for i in range(EXECUTION_PLAN_CREATION_RETRIES + 1):
        ...
        check_for_debug_crash(
            debug_crash_flags, f"EXECUTION_PLAN_CREATED_{run_request_index}"
        )
        ...
        if not should_retry:
            execution_plan_entity_keys = _get_execution_plan_entity_keys(
                check.not_none(
                    execution_data
                ).remote_execution_plan.execution_plan_snapshot
            )
...
def submit_asset_run(
    ...
    """
    check.invariant(
        not run_request.run_config, "Asset run requests have no custom run config"
    )
    ...
    if existing_run:
        if existing_run.status != DagsterRunStatus.NOT_STARTED:
            ...
            check_for_debug_crash(
                debug_crash_flags, f"RUN_SUBMITTED_{run_request_index}"
            )
    ...
    instance.submit_run(
        run_to_submit.run_id, workspace_process_context.create_request_context()
    )
