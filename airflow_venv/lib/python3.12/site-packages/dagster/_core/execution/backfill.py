...
class BulkActionStatus(Enum):
    REQUESTED = "REQUESTED"
    COMPLETED = (
        "COMPLETED"  # deprecated. Use COMPLETED_SUCCESS or COMPLETED_FAILED instead
    )
...
class PartitionBackfill(
    ...
    def __new__(
        ...
        return super(PartitionBackfill, cls).__new__(
            ...
            backfill_timestamp=check.float_param(
    ...
    def selector_id(self):
        return (
            self.partition_set_origin.get_selector_id()
            if self.partition_set_origin
            else None
        )
    ...
    def is_asset_backfill(self) -> bool:
        return (
            self.serialized_asset_backfill_data is not None
            or self.asset_backfill_data is not None
        )
    ...
    def get_num_partitions(
        ...
        if self.is_asset_backfill:
            ...
            if self.partition_names is None:
                check.failed(
                    "Non-asset backfills should have a non-null partition_names field"
                )
