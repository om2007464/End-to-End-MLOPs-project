...
def _subdivide_partition_key_range(
    ...
    if max_range_size is None:
        ...
        chunks = [
            keys[i : i + max_range_size] for i in range(0, len(keys), max_range_size)
        ]
...
def _get_partitions_chunk(
    ...
    initial_checkpoint = (
        partition_names.index(checkpoint) + 1
        if checkpoint and checkpoint in partition_names
        else 0
    )
    ...
    if backfill_policy and backfill_policy.max_partitions_per_run != 1:
        ...
        next_checkpoint = (
            ranges_to_launch[-1].end if len(ranges_to_launch) > 0 else checkpoint
        )
...
def submit_backfill_runs(
    ...
    run_config_by_key_or_range: Mapping[
        Union[str, PartitionKeyRange], Mapping[str, Any]
    ]
    if isinstance(partition_names_or_ranges[0], PartitionKeyRange):
...
def create_backfill_run(
    ...
    if not backfill_job.from_failure and not backfill_job.reexecution_steps:
        ...
        last_run = _fetch_last_run(
            instance, remote_partition_set, partition_key_or_range
        )
    ...
    return instance.create_run(
        ...
        asset_selection=(
            frozenset(backfill_job.asset_selection)
            if backfill_job.asset_selection
            else None
        ),
