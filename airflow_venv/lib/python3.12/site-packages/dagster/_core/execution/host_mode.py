...
def _get_host_mode_executor(
    ...
    execution_config_type = Field(
        selector_for_named_defs(executor_defs),
        default_value={executor_defs[0].name: {}},
    ).config_type
    ...
    executor_defs_by_name = {
        executor_def.name: executor_def for executor_def in executor_defs
    }
...
def host_mode_execution_context_event_generator(
    ...
    executor_defs = check.list_param(
        executor_defs, "executor_defs", of_type=ExecutorDefinition
    )
    ...
    try:
        executor = _get_host_mode_executor(
            pipeline, run_config, executor_defs, instance
        )
...
def execute_run_host_mode(
    ...
    executor_defs = (
        executor_defs if executor_defs is not None else [multi_or_in_process_executor]
    )
