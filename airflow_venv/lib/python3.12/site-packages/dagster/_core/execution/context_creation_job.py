...
def initialize_console_manager(
    ...
    return DagsterLogManager.create(
        loggers=loggers, dagster_run=dagster_run, instance=instance
    )
...
class PlanOrchestrationContextManager(
    ExecutionContextManager[PlanOrchestrationContext]
):
    def __init__(
        self,
    ...
    try:
        ...
        execution_context = PlanOrchestrationContext(
            plan_data=create_plan_data(
                context_creation_data, raise_on_error, executor.retries
            ),
            log_manager=log_manager,
            executor=executor,
            output_capture=output_capture,
...
def _validate_plan_with_context(
    job_context: IPlanContext, execution_plan: ExecutionPlan
) -> None:
...
def create_executor(context_creation_data: ContextCreationData) -> "Executor":
    check.inst_param(
        context_creation_data, "context_creation_data", ContextCreationData
    )
    ...
    creation_fn = check.not_none(
        context_creation_data.executor_def.executor_creation_fn
    )
...
def create_context_free_log_manager(
    ...
    return DagsterLogManager.create(
        loggers=loggers, instance=instance, dagster_run=dagster_run
    )
