                ...
                def gen_fail_restarted_run_worker():
                    ...
                    yield instance.report_run_failed(
                        ...
                        job_failure_data=JobFailureData(
                            error=None,
                            failure_reason=RunFailureReason.RUN_WORKER_RESTART,
                        ),
                    )
...
def execute_job(
    ...
    if reexecution_options:
        if run_config is None:
            run = check.not_none(
                instance.get_run_by_id(reexecution_options.parent_run_id)
            )
...
def _reexecute_job(
    ...
    with ephemeral_instance_if_missing(instance) as execute_instance:
        ...
        parent_dagster_run = execute_instance.get_run_by_id(
            reexecution_options.parent_run_id
        )
...
def execute_plan_iterator(
    ...
    check.inst_param(dagster_run, "dagster_run", DagsterRun)
    check.inst_param(instance, "instance", DagsterInstance)
    retry_mode = check.opt_inst_param(
        retry_mode, "retry_mode", RetryMode, RetryMode.DISABLED
    )
...
def _check_execute_job_args(
    ...
    job_def = (
        job_arg if isinstance(job_arg, JobDefinition) else job_arg.get_definition()
    )
...
def _job_with_repository_load_data(
    ...
    if isinstance(job_arg, ReconstructableJob):
        ...
        return (
            job_arg.with_repository_load_data(repository_load_data),
            repository_load_data,
        )
    return job_arg, None
