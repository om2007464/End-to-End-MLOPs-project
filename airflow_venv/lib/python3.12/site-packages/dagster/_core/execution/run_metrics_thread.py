...
def _get_container_metrics(
    ...
    if cpu_quota_us and cpu_quota_us > 0 and cpu_period_us and cpu_period_us > 0:
        ...
        cpu_limit_ms = (1000.0 * check.not_none(cpu_quota_us)) / check.not_none(
            cpu_period_us
        )
    ...
    if (
        previous_cpu_usage_ms and cpu_usage_ms and cpu_usage_ms >= previous_cpu_usage_ms
    ) and (
    ...
    cpu_percent = None
    if (cpu_limit_ms and cpu_limit_ms > 0) and (
        cpu_usage_rate_ms and cpu_usage_rate_ms > 0
    ):
        cpu_percent = (
            100.0 * check.not_none(cpu_usage_rate_ms) / check.not_none(cpu_limit_ms)
        )
    ...
    if (
        cgroup_memory_limit
        and 0 < cgroup_memory_limit < UNCONSTRAINED_CGROUP_MEMORY_LIMIT
    ) and (memory_usage and memory_usage >= 0):
        memory_limit = cgroup_memory_limit
        memory_percent = (
            100.0 * check.not_none(memory_usage) / check.not_none(memory_limit)
        )
...
def _get_python_runtime_metrics() -> Dict[str, float]:
    ...
    for index, gen_dict in enumerate(gc_stats):
        gen_metrics = {
            f"python.runtime.gc_gen_{index}.{key}": value
...
def _report_run_metrics(
    ...
    for metric, value in metrics.items():
        ...
        try:
            ...
            logging.warning(
                f"Failed to convert metric value to float: {metric}={value}, skipping"
            )
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if container_metrics_enabled:
                ...
                previous_measurement_timestamp = container_metrics.get(
            ...
            if logger:
                logger.error(
                    "Exception during capture of metrics, will cease capturing"
                )
