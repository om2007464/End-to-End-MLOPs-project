...
class UnpartitionedAssetBackfillStatus(
    ...
    def __new__(
        cls, asset_key: AssetKey, asset_backfill_status: Optional[AssetBackfillStatus]
    ):
...
class AssetBackfillData(NamedTuple):
    ...
    def replace_requested_subset(
        self, requested_subset: AssetGraphSubset
    ) -> "AssetBackfillData":
        return self._replace(requested_subset=requested_subset)

    def with_latest_storage_id(
        self, latest_storage_id: Optional[int]
    ) -> "AssetBackfillData":
        return self._replace(
            latest_storage_id=latest_storage_id,
        )

    def with_requested_runs_for_target_roots(
        self, requested_runs_for_target_roots: bool
    ):
        return self._replace(
            requested_runs_for_target_roots=requested_runs_for_target_roots
        )
    ...
    def get_target_root_asset_partitions(
        ...
        root_subset = self.target_subset.filter_asset_keys(
            assets_with_no_parents_in_target_subset
        )

        # Partitions in root_subset and their downstreams within the target subset
        root_and_downstream_partitions = _get_self_and_downstream_targeted_subset(
            root_subset
        )
        ...
        while (
            ...
            root_and_downstream_partitions = (
                root_and_downstream_partitions
                | _get_self_and_downstream_targeted_subset(
                    unreachable_target_root_subset
                )
            )
    ...
    def get_target_root_partitions_subset(
        ...
        target_partitioned_asset_keys = {
            asset_key
            for asset_key in self.target_subset.partitions_subsets_by_asset_key
        }
        ...
        if root_partitioned_asset_keys:
            return self.target_subset.get_partitions_subset(
                next(iter(root_partitioned_asset_keys))
            )
    ...
    def get_num_partitions(self) -> Optional[int]:
        ...
        asset_partition_nums = {
            len(subset)
            for subset in self.target_subset.partitions_subsets_by_asset_key.values()
        }
    ...
    def get_targeted_asset_keys_topological_order(
        ...
        return [
            k
            for k in asset_graph.toposorted_asset_keys
            if k in self.target_subset.asset_keys
        ]
        ...
        def _get_status_for_asset_key(
            ...
            if asset_graph.get(asset_key).is_partitioned:
                ...
                return PartitionedAssetBackfillStatus(
                    asset_key,
                    len(
                        self.target_subset.get_partitions_subset(asset_key, asset_graph)
                    ),
                    {
                        AssetBackfillStatus.MATERIALIZED: len(materialized_subset),
                        AssetBackfillStatus.FAILED: len(
                            failed_subset - materialized_subset
                        ),
                ...
                failed = bool(
                    asset_key
                    in self.failed_and_downstream_subset.non_partitioned_asset_keys
                )
                materialized = bool(
                    asset_key in self.materialized_subset.non_partitioned_asset_keys
                )
                in_progress = bool(
                    asset_key in self.requested_subset.non_partitioned_asset_keys
                )

                if failed:
                    return UnpartitionedAssetBackfillStatus(
                        asset_key, AssetBackfillStatus.FAILED
                    )
                if materialized:
                    return UnpartitionedAssetBackfillStatus(
    ...
    @classmethod
    def is_valid_serialization(
        cls, serialized: str, asset_graph: BaseAssetGraph
    ) -> bool:
    ...
    def from_serialized(
        ...
        return cls(
            ...
            requested_runs_for_target_roots=storage_dict[
                "requested_runs_for_target_roots"
            ],
    ...
    def from_partitions_by_assets(
        ...
        check.sequence_param(
            partitions_by_assets, "partitions_by_asset", PartitionsByAssetSelector
        )
        ...
        return cls.empty(
            target_subset, backfill_start_timestamp, dynamic_partitions_store
        )
    ...
    def from_asset_partitions(
        ...
        if all_partitions:
            ...
            target_subset = AssetGraphSubset(
                non_partitioned_asset_keys=set(asset_selection)
                - partitioned_asset_keys,
            )
    ...
    def from_asset_graph_subset(
        ...
        return cls.empty(
            asset_graph_subset, backfill_start_timestamp, dynamic_partitions_store
        )
...
def _check_target_partitions_subset_is_valid(
    ...
    if target_partitions_subset:  # Asset was partitioned at storage time
        ...
        if isinstance(
            ...
            removed_partitions_subset = (
                target_partitions_subset - existent_partitions_subset
            )
    ...
    try:
        ...
        if (
            ...
            raise DagsterAssetBackfillDataLoadError(
                f"{ex}. {unloadable_locations_error}"
            )
    ...
    if backfill.status == BulkActionStatus.REQUESTED:
        if backfill.submitting_run_requests:
            ...
            updated_backfill = (
                ...
                .with_submitting_run_requests(
                    result.run_requests, result.reserved_run_ids
                )
        ...
        updated_backfill_in_progress = (
            updated_backfill_data.requested_subset
            - updated_backfill_data.materialized_subset
        )
        ...
        new_requested_partitions = (
            updated_backfill_in_progress - previous_backfill_in_progress
        )
        logger.info(
        ...
        if not instance.run_coordinator:
            check.failed(
                "The instance must have a run coordinator in order to cancel runs"
            )
        ...
        if runs_to_cancel_in_iteration:
            ...
            waiting_for_runs_to_finish_after_cancelation = (
                len(run_waiting_to_cancel) > 0
            )
    ...
    for subset in asset_subsets:
        if subset.is_partitioned:
            ...
            partition_ranges_str = _partition_subset_str(
                subset.subset_value, partitions_def
            )
            return_str += (
                f"- {subset.key.to_user_string()}: {{{partition_ranges_str}}}\n"
            )
        else:
...
def execute_asset_backfill_iteration_inner(
    ...
    if request_roots:
        ...
        target_roots = asset_backfill_data.get_target_root_asset_partitions(
            instance_queryer
        )
    ...
    if all_assets_have_backfill_policies:
        ...
        if not all(
            backfill_policy is None for backfill_policy in asset_backfill_policies
        ):
    ...
    updated_asset_backfill_data = AssetBackfillData(
        ...
        or asset_backfill_data.latest_storage_id,
        requested_runs_for_target_roots=asset_backfill_data.requested_runs_for_target_roots
        or request_roots,
        materialized_subset=updated_materialized_subset,
        failed_and_downstream_subset=failed_and_downstream_subset,
...
def can_run_with_parent(
    ...
    candidate_target_subset = target_subset.get_asset_subset(
        candidate.asset_key, asset_graph
    )
...
def should_backfill_atomic_asset_partitions_unit(
    ...
    for candidate in candidates_unit:
        ...
        asset_partitions_to_request_map: Dict[
            AssetKey, Set[Optional[str]]
        ] = defaultdict(set)
