...
class LocalExternalStepLauncher(StepLauncher):
    ...
    def launch_step(
        ...
        step_run_ref_file_path = os.path.join(
            step_run_dir, PICKLED_STEP_RUN_REF_FILE_NAME
        )
        ...
        all_events = cast(
            Sequence[EventLogEntry], deserialize_value(pickle.loads(events_data))
        )
...
def external_instance_from_step_run_ref(
    ...
    event_listener_fn: Optional[Callable[[EventLogEntry], object]] = None,
) -> DagsterInstance:
...
def step_run_ref_to_step_context(
    ...
    execution_step = cast(
        "ExecutionStep", execution_plan.get_step_by_key(step_run_ref.step_key)
    )
