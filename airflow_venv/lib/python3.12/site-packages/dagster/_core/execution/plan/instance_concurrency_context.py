...
class InstanceConcurrencyContext:
    ...
    def __exit__(
        ...
        for step_key in self._pending_claims:
            self._instance.event_log_storage.free_concurrency_slot_for_step(
                self._run_id, step_key
            )
    ...
    def interval_to_next_pending_claim_check(self) -> float:
        ...
        now = time.time()
        return min(
            [0, *[ready_at - now for ready_at in self._pending_timeouts.values()]]
        )
...
def _calculate_timeout_interval(
    sleep_interval: Optional[float], pending_claim_count: int
) -> float:
