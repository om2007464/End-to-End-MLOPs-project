        ...
        step_outputs.append(
            StepOutput(
                ...
                properties=StepOutputProperties(
                    ...
                    if asset_node
                    and asset_node.key in asset_layer.asset_keys_for_node(handle)
                    else None,
                    is_asset_partitioned=bool(asset_node.partitions_def)
                    if asset_node
                    else False,
                    asset_check_key=asset_layer.asset_check_key_for_output(
                        handle, name
                    ),
                ),
            )
...
def execute_core_compute(
    ...
    for step_output in _yield_compute_results(
        ...
        if isinstance(step_output, (DynamicOutput, Output)):
            ...
            asset_key = (
                ...
                or step_context.job_def.asset_layer.asset_key_for_node(
                    step_context.node_handle
                )
            )
            emitted_result_names.add(
                step_context.job_def.asset_layer.node_output_handle_for_asset(
                    asset_key
                ).output_name
            )
            # Check results embedded in MaterializeResult are counted
            for check_result in step_output.check_results or []:
                handle = check_result.to_asset_check_evaluation(
                    step_context
                ).asset_check_key
                output_name = (
            ...
            if step_output.asset_key and step_output.check_name:
                ...
                handle = step_output.to_asset_check_evaluation(
                    step_context
                ).asset_check_key
            output_name = (
                step_context.job_def.asset_layer.get_output_name_for_asset_check(handle)
            )
            emitted_result_names.add(output_name)
