...
class FromLoadableAsset(StepInputSource):
    ...
    def load_input_object(
        ...
        op_config = step_context.resolved_run_config.ops.get(
            str(step_context.node_handle)
        )
    ...
    def required_resource_keys(
        ...
        input_asset_key = job_def.asset_layer.asset_key_for_input(
            op_handle, op_input_name
        )
...
@whitelist_for_serdes(
    storage_name="FromRootInputManager",
    storage_field_names={"node_handle": "solid_handle"},
)
@record
class FromInputManager(StepInputSource):
    ...
    def load_input_object(
        ...
        check.invariant(
            step_context.node_handle == self.node_handle
...
class FromStepOutput(StepInputSource, IHaveNew):
    ...
    def get_load_context(
        ...
        solid_config = step_context.resolved_run_config.ops.get(
            str(step_context.node_handle)
        )
...
class FromConfig(StepInputSource):
    ...
    def load_input_object(
        ...
        with user_code_error_boundary(
            ...
            dagster_type = self.get_associated_input_def(
                step_context.job_def
            ).dagster_type
    ...
    def required_resource_keys(
        ...
        return (
            dagster_type.loader.required_resource_keys()
...
class FromDefaultValue(StepInputSource):
    ...
    def _load_value(self, pipeline_def: JobDefinition):
        return pipeline_def.get_node(
            self.node_handle
        ).definition.default_value_for_input(self.input_name)
...
class FromMultipleSources(MultiStepInputSource, IHaveNew):
    ...
    def load_input_object(
        ...
        for inner_source in self.sources:
            ...
            for event_or_input_value in inner_source.load_input_object(
...
class FromMultipleSourcesLoadSingleSource(MultiStepInputSource, IHaveNew):
    ...
    def __new__(
        cls, sources: Sequence[StepInputSource], source_to_load_from: StepInputSource
    ):
        check.sequence_param(sources, "sources", StepInputSource)
        for source in sources:
            check.invariant(
...
class FromUnresolvedStepOutput(IHaveNew):
    ...
    def get_step_output_handle_dep_with_placeholder(self) -> StepOutputHandle:
        return (
            self.unresolved_step_output_handle.get_step_output_handle_with_placeholder()
        )
...
class UnresolvedMappedStepInput:
    ...
    def get_step_output_handle_deps_with_placeholders(
        self,
    ) -> Sequence[StepOutputHandle]:
