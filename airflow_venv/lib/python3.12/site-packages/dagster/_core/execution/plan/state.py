...
class StepOutputVersionSerializer(FieldSerializer):
    def pack(
        self,
        value: Mapping[StepOutputHandle, str],
...
@whitelist_for_serdes(
    field_serializers={"step_output_versions": StepOutputVersionSerializer}
)
class KnownExecutionState(
    ...
    def __new__(
        ...
        dynamic_mappings: Optional[
            Mapping[str, Mapping[str, Optional[Sequence[str]]]]
        ] = None,
...
def _derive_state_of_past_run(
    ...
    dynamic_outputs = cast(
        Dict[str, Dict[str, Optional[List[str]]]], observed_dynamic_outputs
    )
    ...
    for step_snap in execution_plan.topological_steps():
        ...
        if (
            parent_run.step_keys_to_execute
            and step_snap.key not in parent_run.step_keys_to_execute
        ):
            continue
        ...
        for output in step_snap.outputs:
            if output.properties.is_dynamic:
                if (
                    step_key in dynamic_outputs
                    and output.name in dynamic_outputs[step_key]
                ):
                    continue
        ...
        if _in_tracking_dict(step_handle, failed_steps_in_parent_run_logs):
            _copy_from_tracking_dict(
                to_retry, failed_steps_in_parent_run_logs, step_handle
            )
        ...
        if _in_tracking_dict(step_handle, interrupted_steps_in_parent_run_logs):
            _copy_from_tracking_dict(
                to_retry, interrupted_steps_in_parent_run_logs, step_handle
            )
    ...
    steps_to_retry = [
        step_handle.to_key()
        for step_set in to_retry.values()
        for step_handle in step_set
    ]
