...
class ActiveExecution:
    ...
    def __init__(
        ...
        self._step_outputs: Set[StepOutputHandle] = set(
            self._plan.known_state.ready_outputs
        )
        ...
        self._gathering_dynamic_outputs: Dict[
            str, Mapping[str, Optional[List[str]]]
        ] = {}
        # then on resolution move to _completed
        self._completed_dynamic_outputs: Dict[
            str, Mapping[str, Optional[Sequence[str]]]
        ] = (
            dict(self._plan.known_state.dynamic_mappings)
            if self._plan.known_state
            else {}
        )
    ...
    def _pending_state_str(self) -> str:
        ...
        return "{pending_str}{in_flight_str}{action_str}{retry_str}{claim_str}".format(
            ...
            action_str=f"\nSteps pending action: {pending_action}"
            if pending_action
            else "",
            retry_str=(
    ...
    def _should_skip_step(
        self, step_key: str, successful_or_skipped_steps: Set[str]
    ) -> bool:
    ...
    def get_next_step(self) -> Optional[ExecutionStep]:
        check.invariant(
            not self.is_complete, "Can not call get_next_step when is_complete is True"
        )
    ...
    def get_steps_to_execute(
        ...
        for step in steps:
            ...
            if run_scoped_concurrency_limits_counter:
                run_scoped_concurrency_limits_counter.update_counters_with_launched_item(
                    step
                )
            ...
            for step in steps_to_abandon:
                ...
                for step_input in step.step_inputs:
                    failed_inputs.extend(
                        self._failed.intersection(step_input.dependency_keys)
                    )
    ...
    def mark_up_for_retry(self, step_key: str, at_time: Optional[float] = None) -> None:
        ...
        if self._retry_mode.enabled:
            if at_time:
                ...
                self._pending[step_key] = self._plan.get_executable_step_deps()[
                    step_key
                ]
    ...
    def handle_event(self, dagster_event: DagsterEvent) -> None:
        ...
        if dagster_event.is_step_failure:
            ...
            event_specific_data = cast(
                StepOutputData, dagster_event.event_specific_data
            )
    ...
    def _prep_for_dynamic_outputs(self, step: ExecutionStep):
        dyn_outputs = [
            step_out for step_out in step.step_outputs if step_out.is_dynamic
        ]
        if dyn_outputs:
            self._gathering_dynamic_outputs[step.key] = {
                out.name: [] for out in dyn_outputs
            }
    ...
    def _skip_for_dynamic_outputs(self, step: ExecutionStep):
        ...
        if dyn_outputs:
            ...
            self._gathering_dynamic_outputs[step.key] = {
                out.name: None for out in dyn_outputs
            }
    ...
    def _resolve_any_dynamic_outputs(self, step_key: str) -> None:
        if step_key in self._gathering_dynamic_outputs:
            ...
            for output_name, mappings in self._gathering_dynamic_outputs[
                step_key
            ].items():
    ...
    def concurrency_event_iterator(
        ...
        for step_key in pending_claims:
            ...
            if (
                ...
                or time.time() - last_messaged_timestamp
                > CONCURRENCY_CLAIM_MESSAGE_INTERVAL
            ):
                step = self.get_step_by_key(step_key)
