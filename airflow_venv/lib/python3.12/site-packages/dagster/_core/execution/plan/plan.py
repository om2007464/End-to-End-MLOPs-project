...
class _PlanBuilder:
    ...
    def __init__(
        ...
        self.known_state = check.inst_param(
        ...
        for node in nodes:
            ...
            for name, output_def in node.definition.output_dict.items():
                ...
                (
                    resolved_output_def,
                    resolved_handle,
                ) = node.definition.resolve_output_to_origin(output_def.name, handle)
...
def get_step_input_source(
    ...
    if node.container_maps_input(input_name):
        ...
        parent_inputs = {
            step_input.name: step_input for step_input in parent_step_inputs
        }
...
def _step_input_source_from_multi_dep_def(
    ...
    for idx, handle_or_placeholder in enumerate(deps):
        if isinstance(handle_or_placeholder, NodeOutput):
            ...
            parent_name = node.container_mapped_fan_in_input(
                input_name, idx
            ).graph_input_name
            parent_inputs = {
                step_input.name: step_input for step_input in parent_step_inputs
...
def _step_input_source_from_blocking_asset_checks_dep_def(
    ...
    if source_to_load_from is None:
        asset_key_for_input = asset_layer.asset_key_for_input(
            node_handle, input_handle.input_name
        )
        if asset_key_for_input:
            source_to_load_from = FromLoadableAsset(
                node_handle=node_handle, input_name=input_name
            )
            sources.append(source_to_load_from)
        else:
            check.failed(
                "Unexpected: no sources to load from and no asset key to load from"
            )
...
class ExecutionPlan(
    ...
    def get_all_step_deps(self) -> Mapping[str, Set[str]]:
        ...
        for step in self.step_dict.values():
            if isinstance(step, ExecutionStep):
                ...
                step, (UnresolvedMappedExecutionStep, UnresolvedCollectExecutionStep)
            ):
    ...
    def get_steps_to_execute_in_topo_order(self) -> Sequence[ExecutionStep]:
        return [
            step
            for step_level in self.get_steps_to_execute_by_level()
    ...
    def build_subset_plan(
        ...
        if len(step_output_versions) > 0:
            known_state = self.known_state._replace(
...
def should_skip_step(
    execution_plan: ExecutionPlan, instance: DagsterInstance, run_id: str
) -> bool:
    ...
    for event_record in all_logs:
        if (
            ...
            and event_record.dagster_event.is_successful_output
        ):
        ...
        for step_input in step.step_inputs:
            for step_output_handle in step_input.get_step_output_handle_dependencies():
                io_manager_key = _get_manager_key(
                    step_dict_by_key, step_output_handle, job_def
                )
...
def _get_step_output(
    step_dict_by_key, step_output_handle: StepOutputHandle
) -> StepOutput:
    check.inst_param(step_output_handle, "step_output_handle", StepOutputHandle)
