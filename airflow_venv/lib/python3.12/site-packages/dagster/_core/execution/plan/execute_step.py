...
def _step_output_error_checked_user_event_sequence(
    ...
    for user_event in user_event_sequence:
        ...
        if isinstance(output, Output):
            ...
            if asset_key is not None and asset_key in asset_layer.asset_keys_for_node(
                node_handle
            ):
...
def do_type_check(
    context: TypeCheckContext, dagster_type: DagsterType, value: Any
) -> TypeCheck:
...
def _dagster_event_for_asset_event(
    ...
    if isinstance(asset_event, AssetMaterialization):
        return DagsterEvent.asset_materialization(
            step_context, asset_event, batch_metadata
        )
    else:  # observation
