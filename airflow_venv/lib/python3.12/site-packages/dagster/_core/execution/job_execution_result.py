...
class JobExecutionResult(ExecutionResult):
    ...
    def output_for_node(self, node_str: str, output_name: str = DEFAULT_OUTPUT) -> Any:
        ...
        Returns:
            Any: The value of the retrieved output.
        """
        return super(JobExecutionResult, self).output_for_node(
    ...
    def _get_output_for_handle(self, handle: NodeHandle, output_name: str) -> Any:
        ...
        for compute_step_event in self.compute_events_for_handle(handle):
            if (
                ...
                dagster_type = (
                    self.job_def.get_node(handle)
                    .output_def_named(output_name)
                    .dagster_type
                )
                value = self._get_value(
                    self._context.for_step(step), output, dagster_type
                )
                check.invariant(
    ...
    def _get_value(
        ...
        manager_key = context.execution_plan.get_manager_key(
            step_output_handle, self.job_def
        )
        res = manager.load_input(
            context.for_input_manager(
                ...
                resource_config=context.resolved_run_config.resources[
                    manager_key
                ].config,
