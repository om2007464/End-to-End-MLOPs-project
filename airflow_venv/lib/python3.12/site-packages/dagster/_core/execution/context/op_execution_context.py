...
class OpExecutionContext(AbstractComputeExecutionContext):
    ...
    @deprecated(
        breaking_version="2.0",
        additional_warn_text="Use `partition_time_window` instead.",
    )
    @public
    ...
    def asset_partitions_def_for_output(
        ...
        result = self._step_execution_context.job_def.asset_layer.get(
            asset_key
        ).partitions_def
        if result is None:
            raise DagsterInvariantViolationError(
    ...
    def get() -> "OpExecutionContext":
        ...
        if ctx is None:
            raise DagsterInvariantViolationError(
                "No current OpExecutionContext in scope."
            )
