    ...
    def add_input_metadata(
        ...
        if self.has_asset_key:
            ...
            if self._step_context:
                self._events.append(
                    DagsterEvent.asset_observation(self._step_context, observation)
                )
...
def build_input_context(
    ...
    upstream_output = check.opt_inst_param(
        upstream_output, "upstream_output", OutputContext
    )
    ...
    step_context = check.opt_inst_param(
        step_context, "step_context", StepExecutionContext
    )
    ...
    if asset_partitions_def and asset_partition_key_range:
        asset_partitions_subset = (
            asset_partitions_def.empty_subset().with_partition_key_range(
