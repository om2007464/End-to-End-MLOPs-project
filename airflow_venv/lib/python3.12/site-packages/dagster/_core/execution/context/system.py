...
class StepExecutionContext(PlanExecutionContext, IStepContext):
    ...
    def get_output_context(
        ...
        return get_output_context(
            ...
            step_context=self,
            resources=None,
            version=self.execution_plan.get_version_for_step_output_handle(
                step_output_handle
            ),
    ...
    def for_input_manager(
        ...
        if source_handle and artificial_output_context:
            check.failed(
                "Cannot specify both source_handle and artificial_output_context."
            )
        ...
        asset_partitions_def = (
            self.job_def.asset_layer.get(asset_key).partitions_def
            if asset_key
            else None
        )
        return InputContext(
    ...
    def is_in_graph_asset(self) -> bool:
        ...
        return (
            self.is_op_in_graph
            and self.job_def.asset_layer.assets_defs_by_node_handle.get(
                self.node_handle
            )
            is not None
        )
    ...
    def is_asset_check_step(self) -> bool:
        ...
        return any(
            self.job_def.asset_layer.asset_check_key_for_output(
    ...
    def input_asset_records(
        self,
    ) -> Optional[Mapping[AssetKey, Optional["InputAssetVersionInfo"]]]:
    ...
    def maybe_fetch_and_get_input_asset_version_info(
        ...
        return self._data_version_cache.maybe_fetch_and_get_input_asset_version_info(
            key
        )
    ...
    def asset_partitions_def(self) -> Optional[PartitionsDefinition]:
        ...
        asset_layer = self.job_def.asset_layer
        assets_def = (
            asset_layer.assets_def_for_node(self.node_handle) if asset_layer else None
        )
    ...
    def partition_key_range(self) -> PartitionKeyRange:
        ...
        if any(
            ...
            return PartitionKeyRange(
                partition_key_range_start, tags[ASSET_PARTITION_RANGE_END_TAG]
            )
    ...
    def asset_partitions_subset_for_input(
        ...
        if upstream_asset_key is not None:
            upstream_asset_partitions_def = asset_layer.get(
                upstream_asset_key
            ).partitions_def
    ...
    def partition_time_window(self) -> TimeWindow:
        ...
        if not has_one_dimension_time_window_partitioning(
        ...
        if self.has_partition_key:
            return cast(
                Union[MultiPartitionsDefinition, TimeWindowPartitionsDefinition],
                partitions_def,
