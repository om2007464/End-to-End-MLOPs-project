...
class OutputContext:
    ...
    def get_asset_identifier(self) -> Sequence[str]:
        ...
        if self.asset_key is not None:
            ...
            check.failed(
                "Can't get asset output identifier for an output with no asset key"
            )
    ...
    def log_event(self, event: Union[AssetObservation, AssetMaterialization]) -> None:
        ...
        if isinstance(event, (AssetMaterialization)):
            if self._step_context:
                self._events.append(
                    DagsterEvent.asset_materialization(self._step_context, event)
                )
...
def get_output_context(
    ...
    output_def = job_def.get_node(step_output.node_handle).output_def_named(
        step_output.name
    )
    ...
    if asset_key is not None:
        definition_metadata = (
            job_def.asset_layer.get(asset_key).metadata or output_def.metadata
        )
    else:
...
def build_output_context(
    ...
    resource_config = check.opt_mapping_param(
        resource_config, "resource_config", key_type=str
    )
