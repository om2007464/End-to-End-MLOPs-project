...
class DirectOpExecutionContext(OpExecutionContext, BaseDirectExecutionContext):
    ...
    def __init__(
        ...
        self._instance = self._exit_stack.enter_context(
            ephemeral_instance_if_missing(instance)
        )
    ...
    def _check_bound_to_invocation(
        self, fn_name: str, fn_type: str
    ) -> PerInvocationProperties:
    ...
    def bind(
        ...
        tags = (
            merge_dicts(op_def.tags, invocation_tags)
            if invocation_tags
            else op_def.tags
        )
        ...
        if self._op_config and config_from_args:
            raise DagsterInvalidInvocationError(
                "Cannot provide config in both context and kwargs"
            )
    ...
    def partition_key_range(self) -> PartitionKeyRange:
        ...
        if self._partition_key_range:
            ...
            check.failed(
                "Tried to access partition_key range for a non-partitioned run"
            )
    ...
    def get_step_execution_context(self) -> StepExecutionContext:
        raise DagsterInvalidPropertyError(
            _property_msg("get_step_execution_context", "method")
        )
    ...
    def observe_output(
        ...
        if mapping_key:
            ...
            cast(Set[str], self._execution_properties.seen_outputs[output_name]).add(
                mapping_key
            )
        else:
        ...
        **Examples:**
        ...
        if output_name in self._execution_properties.output_metadata:
            if (
                not mapping_key
                or mapping_key
                in self._execution_properties.output_metadata[output_name]
            ):
                raise DagsterInvariantViolationError(
    ...
    def set_requires_typed_event_stream(self, *, error_message: Optional[str]) -> None:
        self._check_bound_to_invocation(
            fn_name="set_requires_typed_event_stream", fn_type="method"
        )
...
class DirectAssetExecutionContext(AssetExecutionContext, BaseDirectExecutionContext):
    ...
    def bind(
        ...
        if (
            self._op_execution_context._per_invocation_properties is not None
        ):  # noqa: SLF001
            raise DagsterInvalidInvocationError(
    ...
    def observe_output(
        ...
        self.op_execution_context.observe_output(
