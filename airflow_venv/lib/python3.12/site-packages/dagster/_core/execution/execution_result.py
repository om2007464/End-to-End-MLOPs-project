...
class ExecutionResult(ABC):
    ...
    def output_value(self, output_name: str = DEFAULT_OUTPUT) -> object:
        ...
        if (
            not graph_def.has_output(output_name)
            and len(graph_def.output_mappings) == 0
        ):
        ...
        (
            origin_output_def,
            origin_handle,
        ) = mapped_node.definition.resolve_output_to_origin(
            output_mapping.maps_from.output_name,
            NodeHandle(mapped_node.name, None),
        )

        # Get output from origin node
        return self._get_output_for_handle(
            check.not_none(origin_handle), origin_output_def.name
        )

    def output_for_node(
        self, node_str: str, output_name: str = DEFAULT_OUTPUT
    ) -> object:
    ...
    def filter_events(
        self, event_filter: Callable[[DagsterEvent], bool]
    ...
    def get_job_failure_event(self) -> DagsterEvent:
        ...
        if len(events) == 0:
            raise DagsterError(
                "No event of type DagsterEventType.PIPELINE_FAILURE found."
            )
    ...
    def get_job_success_event(self) -> DagsterEvent:
        ...
        if len(events) == 0:
            raise DagsterError(
                "No event of type DagsterEventType.PIPELINE_SUCCESS found."
            )
    ...
    def expectation_results_for_node(
        self, node_str: str
        ...
        return [
            cast(
                StepExpectationResultData, event.event_specific_data
            ).expectation_result
