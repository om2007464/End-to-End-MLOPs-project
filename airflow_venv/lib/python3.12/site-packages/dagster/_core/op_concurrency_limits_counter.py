...
class GlobalOpConcurrencyLimitsCounter:
    def __init__(
        ...
        self._started_run_concurrency_keys_allotted_seconds = int(
            os.getenv(
                "DAGSTER_OP_CONCURRENCY_KEYS_ALLOTTED_FOR_STARTED_RUN_SECONDS", "5"
            )
        )
    ...
    def _fetch_concurrency_info(
        self, instance: DagsterInstance, queued_runs: Sequence[DagsterRun]
    ):
        ...
        for run in queued_runs:
            if run.run_op_concurrency:
                all_run_concurrency_keys.update(
        ...
        for key in all_run_concurrency_keys:
            ...
            self._concurrency_info_by_key[
                key
            ] = instance.event_log_storage.get_concurrency_info(key)
    ...
    def is_blocked(self, run: DagsterRun) -> bool:
        ...
        # should dequeue
        if (
            not run.run_op_concurrency
            or run.run_op_concurrency.has_unconstrained_root_nodes
        ):
    ...
    def get_blocked_run_debug_info(self, run: DagsterRun) -> Mapping:
        ...
        for concurrency_key in run.run_op_concurrency.root_key_counts.keys():
            ...
            log_info[concurrency_key] = {
                ...
                "launched_count": self._launched_concurrency_key_counts[
                    concurrency_key
                ],
                "in_progress_count": self._in_progress_concurrency_key_counts[
                    concurrency_key
                ],
            }
        return log_info
