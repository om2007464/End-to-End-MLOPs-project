...
class SchedulerDebugInfo(
    ...
    def __new__(
        ...
        return super(SchedulerDebugInfo, cls).__new__(
            ...
            scheduler_config_info=check.str_param(
                scheduler_config_info, "scheduler_config_info"
            ),
...
class Scheduler(abc.ABC):
    ...
    def start_schedule(
        ...
        if not stored_state:
            ...
            started_state = stored_state.with_status(
                InstigatorStatus.RUNNING
            ).with_data(new_instigator_data)
    ...
    def stop_schedule(
        ...
        if not stored_state:
            ...
            stopped_state = stored_state.with_status(
                InstigatorStatus.STOPPED
            ).with_data(
                ScheduleInstigatorData(
...
class DagsterDaemonScheduler(Scheduler, ConfigurableClass):
    ...
    def __init__(
        ...
        self.max_tick_retries = check.opt_int_param(
            max_tick_retries, "max_tick_retries", 0
        )
    ...
    def _get_or_create_logs_directory(
        ...
        logs_directory = os.path.join(
            instance.schedules_directory(), "logs", schedule_origin_id
        )
