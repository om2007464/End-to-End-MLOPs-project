...
class DynamicPartitionsRequestResult(
    ...
    def __new__(
        ...
        if not xor(added_partitions is None, deleted_partitions is None):
            check.failed(
                "Exactly one of added_partitions and deleted_partitions must be provided"
            )
    ...
    def __new__(
        ...
        return super(SensorInstigatorData, cls).__new__(
            ...
            check.opt_float_param(
                last_tick_start_timestamp, "last_tick_start_timestamp"
            ),
            check.opt_float_param(
            ...
            check.opt_float_param(
                last_tick_success_timestamp, "last_tick_success_timestamp"
            ),
        )

    def with_sensor_start_timestamp(
...
class ScheduleInstigatorData(
    ...
    def __new__(
        ...
        if not isinstance(cron_schedule, str):
            cron_schedule = check.sequence_param(
                cron_schedule, "cron_schedule", of_type=str
            )
...
class InstigatorTick(
    ...
    def with_status(self, status: TickStatus, **kwargs: Any):
        ...
        end_timestamp = (
            get_current_timestamp() if status != TickStatus.STARTED else None
        )
    ...
    def with_run_requests(
        ...
        return self._replace(
            tick_data=self.tick_data.with_run_requests(run_requests, **kwargs)
        )
    ...
    def requested_assets_and_partitions(self) -> Mapping[AssetKey, AbstractSet[str]]:
        ...
        for run_request in self.tick_data.run_requests or []:
            if run_request.requires_backfill_daemon():
                ...
                ) in asset_graph_subset.iterate_asset_partitions():
                    if asset_key_partition_key.asset_key not in partitions_by_asset_key:
                        partitions_by_asset_key[
                            asset_key_partition_key.asset_key
                        ] = set()
                ...
                for asset_key in run_request.asset_selection or []:
                    ...
                    if run_request.partition_key:
                        partitions_by_asset_key[asset_key].add(
...
class TickData(
    NamedTuple(
        ...
        [
            ...
            ),  # run requests created by the tick
    ...
    def __new__(
        ...
        return super(TickData, cls).__new__(
            ...
            origin_run_ids=check.opt_sequence_param(
                origin_run_ids, "origin_run_ids", of_type=str
            ),
    ...
    def with_reason(self, skip_reason: Optional[str]) -> "TickData":
        return TickData(
            **merge_dicts(
                self._asdict(),
                {"skip_reason": check.opt_str_param(skip_reason, "skip_reason")},
            )
        )
    ...
    def with_cursor(self, cursor: Optional[str]) -> "TickData":
        return TickData(
            **merge_dicts(
                self._asdict(), {"cursor": check.opt_str_param(cursor, "cursor")}
            )
        )
...
def _validate_tick_args(
    ...
    if status == TickStatus.SUCCESS:
        ...
        check.invariant(
            error is None, desc="Tick status is SUCCESS, but error was provided"
        )
    elif status == TickStatus.FAILURE:
        check.inst_param(error, "error", SerializableErrorInfo)
    else:
        check.invariant(
            error is None, "Tick status was not FAILURE but error was provided"
        )
...
class AutoMaterializeAssetEvaluationRecord(Generic[T_EntityKey]):
    ...
    def get_evaluation_with_run_ids(
        self,
    ) -> AutomationConditionEvaluationWithRunIds[T_EntityKey]:
