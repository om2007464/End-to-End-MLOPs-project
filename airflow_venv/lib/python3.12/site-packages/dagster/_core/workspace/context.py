...
class BaseWorkspaceRequestContext(LoadingContext):
    ...
    def get_full_job(
        self, selector: Union[JobSubsetSelector, JobSelector]
    ) -> RemoteJob:
        return (
    ...
    def get_execution_plan(
        ...
        return self.get_code_location(
            remote_job.handle.location_name
        ).get_execution_plan(
    ...
    def get_dagster_type(
        ...
        job = self.get_full_job(job_selector)
        return job.job_snapshot.dagster_type_namespace_snapshot.get_dagster_type_snap(
            type_key
        )

    def get_dagster_library_versions(
        self, location_name: str
...
class WorkspaceRequestContext(BaseWorkspaceRequestContext):
    ...
    def permissions_for_location(
        ...
        if location_name in self._read_only_locations:
            return get_location_scoped_user_permissions(
                self._read_only_locations[location_name]
            )
...
class WorkspaceProcessContext(IWorkspaceProcessContext):
    ...
    def __init__(
        ...
        self._workspace_snapshot: WorkspaceSnapshot = WorkspaceSnapshot(
    ...
    def _load_location(
        self, origin: CodeLocationOrigin, reload: bool
    ) -> CodeLocationEntry:
        location_name = origin.location_name
        location = None
        error = None
    ...
    def has_code_location(self, location_name: str) -> bool:
        ...
        with self._lock:
            return (
                ...
                and self._workspace_snapshot.code_location_entries[
                    location_name
                ].code_location
                is not None
            )
    ...
    def reload_code_location(self, name: str) -> None:
        ...
        with self._lock:
            ...
            # is referencing it
            self._workspace_snapshot = self._workspace_snapshot.with_code_location(
                name, new_entry
            )

    def shutdown_code_location(self, name: str) -> None:
        with self._lock:
            self._workspace_snapshot.code_location_entries[
                name
            ].origin.shutdown_server()
    ...
    def _update_workspace(self, new_locations: Dict[str, CodeLocationEntry]):
        ...
        with self._lock:
            ...
            self._workspace_snapshot = WorkspaceSnapshot(
                code_location_entries=new_locations
            )
