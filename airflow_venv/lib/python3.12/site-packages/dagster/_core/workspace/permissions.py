...
class PermissionResult(
    NamedTuple(
        "_PermissionResult", [("enabled", bool), ("disabled_reason", Optional[str])]
    )
):
    def __bool__(self):
        raise Exception(
...
def get_user_permissions(read_only: bool) -> Mapping[str, PermissionResult]:
    ...
    return {
        perm: PermissionResult(
            enabled=enabled, disabled_reason=_get_disabled_reason(enabled)
        )
        for perm, enabled in perm_map.items()
    }


def get_location_scoped_user_permissions(
    read_only: bool,
