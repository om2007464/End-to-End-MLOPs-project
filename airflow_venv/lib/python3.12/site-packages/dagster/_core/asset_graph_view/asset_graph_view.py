...
from dagster._core.asset_graph_view.entity_subset import (
    EntitySubset,
    _ValidatedEntitySubsetValue,
)
...
class AssetGraphView(LoadingContext):
    ...
    def for_test(
        ...
        return AssetGraphView(
            temporal_context=TemporalContext(
                ...
                or instance.event_log_storage.get_maximum_record_id(),
            ),
            instance=instance,
    ...
    def get_subset_from_serializable_subset(
        ...
        if self.asset_graph.has(
            key
        ) and serializable_subset.is_compatible_with_partitions_def(
    ...
    def compute_latest_time_window_subset(
        ...
        if isinstance(partitions_def, TimeWindowPartitionsDefinition):
            return self._build_time_partition_subset(
                asset_key, partitions_def, time_window
            )
    ...
    async def compute_subset_with_status(
        ...
        resolved_status = (
            ...
            if latest_record
            and await latest_record.targets_latest_materialization(self)
            else None
        )
    ...
    async def _compute_run_in_progress_asset_subset(
        self, key: AssetKey
    ) -> EntitySubset[AssetKey]:
    ...
    async def _compute_execution_failed_asset_subset(
        self, key: AssetKey
    ) -> EntitySubset[AssetKey]:
        from dagster._core.storage.partition_status_cache import AssetStatusCacheValue
    ...
    async def _compute_missing_asset_subset(
        ...
        if self.asset_graph.get(key).is_materializable:
            ...
            partitions_def = self._get_partitions_def(key)
            if partitions_def:
                cache_value = await AssetStatusCacheValue.gen(
                    self, (key, partitions_def)
                )
                materialized_subset = (
            ...
            missing_asset_partitions = {
                ...
                if not self._queryer.asset_partition_has_materialization_or_observation(
                    ap
                )
            }
    ...
    async def _expensively_filter_entity_subset(
        self,
        subset: EntitySubset,
        filter_fn: Callable[[Optional[str]], Awaitable[bool]],
    ) -> EntitySubset:
        if subset.is_partitioned:
            return subset.compute_intersection_with_partition_keys(
                {
                    pk
                    for pk in subset.expensively_compute_partition_keys()
                    if await filter_fn(pk)
                }
            )
        else:
            return (
    ...
    def _run_record_targets_entity(
        self, run_record: "RunRecord", target_key: EntityKey
    ) -> bool:
    ...
    async def _compute_latest_check_run_executed_with_target(
        ...
        if check_record and check_record.event:
            ...
            return bool(run_record) and self._run_record_targets_entity(
    ...
    async def _compute_updated_since_time_subset(
        ...
        if (
            ...
            or record.event.dagster_event_type
    ...
    async def compute_updated_since_temporal_context_subset(
        ...
        return await _dispatch(
            ...
            check_method=functools.partial(
                self._compute_updated_since_time_subset,
                time=temporal_context.effective_dt,
            ),
            asset_method=functools.partial(
                self._compute_updated_since_cursor_subset,
                cursor=temporal_context.last_event_id,
            ),
        )
        ...
        return self.get_full_subset(key=asset_key).compute_intersection(
            self.get_asset_subset_from_asset_partitions(
                asset_key,
                {
                    ...
                    for pk in partitions_def.get_partition_keys_in_time_window(
                        time_window
                    )
                },
            )
