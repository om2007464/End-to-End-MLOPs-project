...
class EntitySubsetSerializer(DataclassSerializer):
    ...
    def before_pack(
        self, value: "SerializableEntitySubset"
...
class SerializableEntitySubset(Generic[T_EntityKey]):
    ...
    def is_compatible_with_partitions_def(
        ...
        if self.is_partitioned:
            ...
            if isinstance(
                self.value, (BaseTimeWindowPartitionsSubset, AllPartitionsSubset)
            ):
    ...
    def __contains__(self, item: AssetKeyPartitionKey) -> bool:
        if not self.is_partitioned:
            return (
                item.asset_key == self.key
                and item.partition_key is None
                and self.bool_value
            )
        else:
            return (
                item.asset_key == self.key and item.partition_key in self.subset_value
            )
