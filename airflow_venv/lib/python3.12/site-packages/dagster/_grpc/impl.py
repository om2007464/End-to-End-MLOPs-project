...
def _instance_from_ref_for_dynamic_partitions(
    ...
    with (
        ...
        if (
            instance_ref
            and (_partitions_def_contains_dynamic_partitions_def(partitions_def))
        )
        else nullcontext()
    ) as instance:
        yield instance
...
def _run_in_subprocess(
    ...
    try:
        execute_run_args = deserialize_value(
            serialized_execute_run_args, ExecuteExternalJobArgs
        )
    ...
    try:
        for event in core_execute_run(
            recon_pipeline, dagster_run, instance, inject_env_vars=False
        ):
...
def get_external_sensor_execution(
    ...
    try:
        ...
        return SensorExecutionErrorSnap(
            error=serializable_error_info_from_exc_info(sys.exc_info())
        )


def _partitions_def_contains_dynamic_partitions_def(
    ...
    try:
        ...
        with user_code_error_boundary(
            ...
            tags = job_def.get_tags_for_partition_key(
                partition_name, selected_asset_keys=None
            )
...
def get_partition_set_execution_param_data(
    ...
    (
        ...
        partitions_def,
        partitioned_config,
    ) = _get_job_partitions_and_config_for_partition_set_name(
        repo_def, partition_set_name
    )

    try:
        with _instance_from_ref_for_dynamic_partitions(
            instance_ref, partitions_def
        ) as instance:
            with user_code_error_boundary(
                PartitionExecutionError,
                lambda: (
                ...
                partition_keys = [
                    key for key in all_partition_keys if key in partition_names
                ]
