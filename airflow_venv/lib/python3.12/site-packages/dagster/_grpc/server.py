...
def _record_utilization_metrics(logger: logging.Logger) -> None:
    with _METRICS_LOCK:
        ...
        last_cpu_measurement = _UTILIZATION_METRICS["container_utilization"][
            "cpu_usage"
        ]
...
def _set_request_count(api_name: str, value: Any) -> None:
    if api_name not in _UTILIZATION_METRICS["per_api_metrics"]:
        _UTILIZATION_METRICS["per_api_metrics"][api_name] = init_optional_typeddict(
            GrpcApiMetrics
        )
...
def retrieve_metrics():
    class _MetricsRetriever:
        def __call__(self, fn: Callable[..., Any]) -> Callable:
            ...
            def wrapper(
                self: "DagsterApiServer", request: Any, context: grpc.ServicerContext
            ):
                ...
                with _METRICS_LOCK:
                    ...
                    _set_request_count(
                        api_call, cur_request_count + 1 if cur_request_count else 1
                    )
                ...
                with _METRICS_LOCK:
                    cur_request_count = _get_request_count(api_call)
                    _set_request_count(
                        api_call, cur_request_count - 1 if cur_request_count else 0
                    )
...
class DagsterApiServer(DagsterApiServicer):
    ...
    def __init__(
        ...
        check.invariant(
            heartbeat_timeout > 0, "heartbeat_timeout must be greater than 0"
        )
        ...
        self._server_id = check.opt_str_param(
            fixed_server_id, "fixed_server_id", str(uuid.uuid4())
        )
        ...
        self._container_context = check.opt_dict_param(
    ...
    def ExecutionPlanSnapshot(
        self,
        request: api_pb2.ExecutionPlanSnapshotRequest,
        ...
        try:
            execution_plan_snapshot_or_error = get_external_execution_plan_snapshot(
                self._get_repo_for_origin(
                    execution_plan_args.job_origin.repository_origin
                ),
        ...
        return api_pb2.ExecutionPlanSnapshotReply(
            serialized_execution_plan_snapshot=serialize_value(
    ...
    def ExternalPartitionNames(
        self,
        request: api_pb2.ExternalPartitionNamesRequest,
        _context: grpc.ServicerContext,
    ...
    def ExternalPartitionSetExecutionParams(
        ...
        try:
            ...
            instance_ref = (
                args.instance_ref if args.instance_ref else self._instance_ref
            )
    ...
    def ExternalPartitionConfig(
        self,
        request: api_pb2.ExternalPartitionConfigRequest,
        _context: grpc.ServicerContext,
    ...
    def ExternalPartitionTags(
        self,
        request: api_pb2.ExternalPartitionTagsRequest,
        _context: grpc.ServicerContext,
    ) -> api_pb2.ExternalPartitionTagsReply:
        try:
            partition_args = deserialize_value(
                request.serialized_partition_args, PartitionArgs
            )

            instance_ref = (
                partition_args.instance_ref
                if partition_args.instance_ref
                else self._instance_ref
            )
    ...
    def ExternalPipelineSubsetSnapshot(
        self,
        request: api_pb2.ExternalPipelineSubsetSnapshotRequest,
        _context: grpc.ServicerContext,
    ...
    def ExternalRepository(
        ...
        serialized_external_repository_data = (
            self._get_serialized_external_repository_data(request)
        )
    ...
    def ExternalJob(
        ...
        try:
            ...
            job_def = self._get_repo_for_origin(repository_origin).get_job(
                request.job_name
            )
    ...
    def ExternalScheduleExecution(
        self,
        request: api_pb2.ExternalScheduleExecutionRequest,
        _context: grpc.ServicerContext,
    ) -> Iterable[api_pb2.StreamingChunkEvent]:
    ...
    def _external_sensor_execution(
        self, request: api_pb2.ExternalSensorExecutionRequest
    ) -> str:
        try:
            args = deserialize_value(
    ...
    def SyncExternalSensorExecution(
        self,
        request: api_pb2.ExternalSensorExecutionRequest,
        _context: grpc.ServicerContext,
            ...
            with self._execution_lock:
                if cancel_execution_request.run_id in self._executions:
                    ...
                    self._termination_times[
                        cancel_execution_request.run_id
                    ] = time.time()
                    success = True
    ...
    def CanCancelExecution(
        ...
        with self._execution_lock:
            ...
            can_cancel = (
                run_id in self._executions
                and not self._termination_events[run_id].is_set()
            )
    ...
    def StartRun(
        ...
        try:
            ...
            recon_repo = check.not_none(
                self._loaded_repositories
            ).reconstructables_by_name[
        ...
        while success is None:
            ...
            try:
                ...
                if isinstance(
                    ...
                    continue
                if isinstance(
                    dagster_event_or_ipc_error_message_or_done, IPCErrorMessage
                ):
                    success = False
        ...
        with self._execution_lock:
            return api_pb2.GetCurrentRunsReply(
                serialized_current_runs=serialize_value(
                    GetCurrentRunsResult(
                        current_runs=list(self._executions.keys()),
                        serializable_error_info=None,
                    )
                )
            )
...
def open_server_process(
    ...
    check.invariant(
        (port or socket) and not (port and socket), "Set only port or socket"
    )
    ...
    executable_path = (
        loadable_target_origin.executable_path if loadable_target_origin else None
    )
    ...
    subprocess_args = [
        ...
        *(
            ["--container-context", json.dumps(container_context)]
    ...
    try:
        wait_for_grpc_server(
            server_process, client, subprocess_args, timeout=startup_timeout
        )
    except:
