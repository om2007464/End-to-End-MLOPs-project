...
def is_callable(
    obj: T_Callable, additional_message: Optional[str] = None
) -> T_Callable:
    if not callable(obj):
        raise CheckError(
...
def literal_param(
    ...
    if obj not in values:
        raise _param_value_mismatch_exception(
            obj, values, param_name, additional_message
        )
    return obj
...
def sequence_param(
    ...
    # isinstance check against abc is costly, so try to handle common cases with cheapest check possible
    if not (
        ttype is list or ttype is tuple or isinstance(obj, collections.abc.Sequence)
    ):
        raise _param_type_mismatch_exception(
...
def iterable_param(
    ...
    # isinstance check against abc is costly, so try to handle common cases with cheapest check possible
    if not (
        ttype is list or ttype is tuple or isinstance(obj, collections.abc.Iterable)
    ):
        raise _param_type_mismatch_exception(
