    ...
    with ExitStack() as stack:
        ...
        while True:
            ...
            try:
                ...
                next_interval_time = min(
                    start_time + ERROR_INTERVAL_TIME, next_interval_time
                )
...
def launch_scheduled_runs(
                ...
                for schedule in repo.get_schedules():
                    ...
                    if schedule.get_current_instigator_state(
                        ...
                        _write_and_get_next_checkpoint_timestamp(
                            ...
                            cast(
                                ScheduleInstigatorData, schedule_state.instigator_data
                            ),
                            now_timestamp,
                        )
    ...
    for state in states_to_delete:
        location_name = (
            state.origin.repository_origin.code_location_origin.location_name
        )
...
def launch_scheduled_runs_for_schedule_iterator(
    ...
    end_datetime_utc = check.inst_param(
        end_datetime_utc, "end_datetime_utc", datetime.datetime
    )
    instance = workspace_process_context.instance

    instigator_origin_id = remote_schedule.get_remote_origin_id()
    ticks = instance.get_ticks(
        instigator_origin_id, remote_schedule.selector_id, limit=1
    )
    ...
    if len(tick_times) == 1:
        ...
        times = ", ".join(
            [time.strftime(default_date_format_string()) for time in tick_times]
        )
        logger.info(
    ...
    for schedule_time in tick_times:
        ...
        if latest_tick and latest_tick.timestamp == schedule_timestamp:
            ...
            if latest_tick.status == TickStatus.FAILURE:
                logger.info(
                    f"Retrying previously failed schedule execution at {schedule_time_str}"
                )
            else:
                logger.info(
            ...
            try:
                ...
                if isinstance(
                    ...
                    try:
                        ...
                        error_data = serializable_error_info_from_exc_info(
                            sys.exc_info()
                        )
...
def _submit_run_request(
    ...
    run = _get_existing_run_for_request(
        instance, remote_schedule, schedule_time, run_request
    )
    if run:
        ...
        code_location = _get_code_location_for_schedule(
            workspace_process_context, remote_schedule
        )
...
def _schedule_runs_at_time(
    ...
    if not schedule_execution_data.run_requests:
        if schedule_execution_data.skip_message:
            ...
            logger.info(
                f"No run requests returned for {remote_schedule.name}, skipping"
            )
    ...
    if submit_threadpool_executor:
        gen_run_request_results = submit_threadpool_executor.map(
            submit_run_request, run_requests
        )
    else:
    ...
    for run_request_result in gen_run_request_results:
        ...
        if run_request_result.existing_run:
            tick_context.add_run_info(
                run_id=run_request_result.existing_run.run_id,
            ...
            tick_context.add_run_info(
                run_id=run.run_id, run_key=run_request_result.run_key
            )
...
def _create_scheduler_run(
    ...
    tags[SCHEDULED_EXECUTION_TIME_TAG] = schedule_time.astimezone(
        datetime.timezone.utc
    ).isoformat()
    if run_request.run_key:
        tags[RUN_KEY_TAG] = run_request.run_key
...
def _write_and_get_next_checkpoint_timestamp(
    ...
    if (
        ...
        or instigator_data.last_iteration_timestamp
