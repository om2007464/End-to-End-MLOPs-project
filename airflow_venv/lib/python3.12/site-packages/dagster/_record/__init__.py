...
_RECORD_MARKER_FIELD = "__checkrepublic__"  # "I do want to release this as checkrepublic one day" - schrockn
...
def record(
    ...
    ...  # Overload for using decorator with no ().
...
def record(
    *,
    checked: bool = True,
) -> Callable[[TType], TType]:
    ...  # Overload for using decorator used with args.
...
class JitCheckedNew:
    ...
    def _build_checked_new_str(self) -> str:
        kw_args_str, set_calls_str = build_args_and_assignment_strs(
            self._field_set, self._defaults
        )
        check_calls = []
        ...
        lazy_imports_str = "\n    ".join(
            f"from {module} import {t}"
            for t, module in self._eval_ctx.lazy_imports.items()
        )
