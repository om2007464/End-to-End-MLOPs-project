...
def _get_url_for_version(version: str) -> str:
    return f"https://codeload.github.com/dagster-io/dagster/tar.gz/{_get_target_for_version(version)}"
...
def download_example_from_github(path: str, example: str, version: str):
    if example not in AVAILABLE_EXAMPLES:
        click.echo(
            click.style(
                f'Example "{example}" not available from the `dagster project` CLI. ',
                fg="red",
            )
    ...
    path_to_selected_example = (
        f"dagster-{_get_target_for_version(version)}/examples/{example}/"
    )
