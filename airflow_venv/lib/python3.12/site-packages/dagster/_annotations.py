...
def experimental(
    ...
    *,
    ...
    if __obj is None:
        ...
        setattr(
            target,
            _EXPERIMENTAL_ATTR_NAME,
            ExperimentalInfo(additional_warn_text, subject),
        )
...
def is_experimental(obj: Annotatable) -> bool:
    target = _get_annotation_target(obj)
    return hasattr(target, _EXPERIMENTAL_ATTR_NAME) and getattr(
        target, _EXPERIMENTAL_ATTR_NAME
    )
...
def copy_annotations(dest: Annotatable, src: Annotatable) -> None:
    ...
    if hasattr(src_target, _DEPRECATED_ATTR_NAME):
        setattr(
            dest_target,
            _DEPRECATED_ATTR_NAME,
            getattr(src_target, _DEPRECATED_ATTR_NAME),
        )
    ...
    if hasattr(src_target, _EXPERIMENTAL_ATTR_NAME):
        setattr(
            dest_target,
            _EXPERIMENTAL_ATTR_NAME,
            getattr(src_target, _EXPERIMENTAL_ATTR_NAME),
        )
...
def only_allow_hidden_params_in_kwargs(
    annotatable: Annotatable, kwargs: Mapping[str, Any]
) -> None:
    deprecated_params = (
    ...
    for param in kwargs:
        if param not in deprecated_params:
            raise TypeError(
                f"{annotatable.__name__} got an unexpected keyword argument '{param}'"
            )
