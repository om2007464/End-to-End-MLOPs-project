...
_INTERVAL_SECONDS = int(
    os.environ.get("DAGSTER_EVENT_LOG_CONSUMER_DAEMON_INTERVAL_SECONDS", 5)
)
_EVENT_LOG_FETCH_LIMIT = int(
    os.environ.get("DAGSTER_EVENT_LOG_CONSUMER_DAEMON_FETCH_LIMIT", 500)
)
...
class EventLogConsumerDaemon(IntervalDaemon):
    ...
    def run_iteration(self, workspace_process_context: IWorkspaceProcessContext):
        ...
        persisted_cursors = _fetch_persisted_cursors(
            instance, DAGSTER_EVENT_TYPES, self._logger
        )
        ...
        for event_type in DAGSTER_EVENT_TYPES:
            ...
            events_by_log_id_for_type = (
                instance.event_log_storage.get_logs_for_all_runs_by_log_id(
                    after_cursor=cursor,
    ...
    for event_type in event_types:
        ...
        if raw_cursor_value is None:
            ...
            try:
                ...
                logger.exception(
                    f"Invalid cursor for event_type {event_type}: {raw_cursor_value}"
                )
                raise
...
def _persist_cursors(
    instance: DagsterInstance, cursors: Mapping[DagsterEventType, int]
) -> None:
    check.inst_param(instance, "instance", DagsterInstance)
