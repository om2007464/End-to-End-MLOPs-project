...
def filter_runs_to_should_retry(
    ...
    default_retry_on_asset_or_op_failure: bool = instance.get_settings(
        "run_retries"
    ).get("retry_on_asset_or_op_failure", True)
...
def get_reexecution_strategy(
    ...
    if raw_strategy_tag not in ReexecutionStrategy.__members__:
        instance.report_engine_event(
            f"Error parsing retry strategy from tag '{RETRY_STRATEGY_TAG}: {raw_strategy_tag}'",
            run,
        )
        return None
    else:
...
def retry_run(
    ...
    code_location = workspace.get_code_location(
        origin.code_location_origin.location_name
    )
    ...
    strategy = (
        get_reexecution_strategy(failed_run, instance) or DEFAULT_REEXECUTION_POLICY
    )
    ...
    instance.report_engine_event(
        ...
        engine_event_data=EngineEventData(
            {"new run": MetadataValue.dagster_run(new_run.run_id)}
        ),
    )
    instance.report_engine_event(
