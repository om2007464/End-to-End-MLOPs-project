...
def daemon_controller_from_instance(
    ...
    with ExitStack() as stack:
        ...
        configure_loggers(
            handler="default", formatter=log_format, log_level=log_level.upper()
        )
...
class DagsterDaemonController(AbstractContextManager):
    ...
    def check_workspace_freshness(self, last_workspace_update_time: float) -> float:
        ...
        try:
            ...
            if (
                nowish - last_workspace_update_time
            ) > DEFAULT_WORKSPACE_FRESHNESS_TOLERANCE:
                self._logger.exception(
                    "Daemon controller surpassed workspace freshness tolerance."
                )
                raise
            else:
                self._logger.exception(
    ...
    def __exit__(
        ...
        for daemon_type, thread in self._daemon_threads.items():
            if thread.is_alive():
                ...
                if thread.is_alive():
                    self._logger.error(
                        "Thread for %s did not shut down gracefully.", daemon_type
                    )
...
def create_daemon_of_type(daemon_type: str, instance: DagsterInstance) -> DagsterDaemon:
    if daemon_type == SchedulerDaemon.daemon_type():
        ...
        return MonitoringDaemon(
            interval_seconds=instance.run_monitoring_poll_interval_seconds
        )
...
def debug_daemon_heartbeats(instance: DagsterInstance) -> None:
    ...
    instance.add_daemon_heartbeat(
        DaemonHeartbeat(timestamp, daemon.daemon_type(), None, None)
    )
    returned_timestamp = instance.get_daemon_heartbeats()[
        daemon.daemon_type()
    ].timestamp
    print(
