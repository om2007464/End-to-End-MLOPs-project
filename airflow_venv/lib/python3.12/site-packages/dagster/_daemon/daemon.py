...
TELEMETRY_LOGGING_INTERVAL = (
    3600 * 24
)  # Interval (in seconds) at which to log that daemon is alive
...
DaemonIterator: TypeAlias = Generator[
    Union[None, SerializableErrorInfo, SpanMarker], None, None
]
...
class DagsterDaemon(AbstractContextManager, ABC, Generic[TContext]):
    ...
    def _check_add_heartbeat(
        ...
        error_max_time = get_current_datetime() - datetime.timedelta(
...
class IntervalDaemon(DagsterDaemon[TContext], ABC):
    def __init__(
        ...
        *,
        ...
        self.interval_seconds = check.numeric_param(
    ...
    def core_loop(
        ...
        while True:
            interval = self.interval_seconds + random.uniform(
                0, self.interval_jitter_seconds
            )
...
class SensorDaemon(DagsterDaemon):
    def __init__(self, settings: Mapping[str, Any]) -> None:
        ...
        self._submit_threadpool_executor: Optional[
...
class MonitoringDaemon(IntervalDaemon):
    ...
    def run_iteration(
        ...
        yield from execute_run_monitoring_iteration(
            workspace_process_context, self._logger
        )
        yield from execute_concurrency_slots_iteration(
            workspace_process_context, self._logger
        )
