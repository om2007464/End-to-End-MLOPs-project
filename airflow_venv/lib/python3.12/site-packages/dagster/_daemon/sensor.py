...
class SensorLaunchContext(AbstractContextManager):
    ...
    def set_should_update_cursor_on_failure(
        self, should_update_cursor_on_failure: bool
    ) -> None:
    ...
    def __exit__(
        ...
        for day_offset, statuses in self._purge_settings.items():
            ...
            self._instance.purge_ticks(
                ...
                before=(
                    get_current_datetime() - datetime.timedelta(days=day_offset)
                ).timestamp(),
...
def execute_sensor_iteration(
    ...
    for sensor in sensors.values():
        ...
        sensor_debug_crash_flags = (
            debug_crash_flags.get(sensor_name) if debug_crash_flags else None
        )
        ...
        if threadpool_executor:
            if sensor_tick_futures is None:
                check.failed(
                    "sensor_tick_futures dict must be passed with threadpool_executor"
                )
...
def _submit_run_request(
    ...
    target_data: TargetSnap = check.not_none(
        remote_sensor.get_target(run_request.job_name)
    )
    ...
    code_location = _get_code_location_for_sensor(
        workspace_process_context, remote_sensor
    )
    ...
    if isinstance(run, SkippedSensorRun):
        return SubmitRunRequestResult(
            run_key=run_request.run_key, error_info=None, run=run
        )
    ...
    try:
        ...
        instance.submit_run(
            run.run_id, workspace_process_context.create_request_context()
        )
    ...
    return SubmitRunRequestResult(
        run_key=run_request.run_key, error_info=error_info, run=run
    )
...
def _evaluate_sensor(
    ...
    if not (
        ...
        or sensor_runtime_data.automation_condition_evaluations
    ):
        if sensor_runtime_data.dagster_run_reactions:
            ...
            context.logger.info(
                f"No run requests returned for {remote_sensor.name}, skipping"
            )
    ...
    for request in dynamic_partitions_requests:
        ...
        for partition_key in request.partition_keys:
            if instance.has_dynamic_partition(
                request.partitions_def_name, partition_key
            ):
            ...
            if existent_partitions:
                ...
                for partition in existent_partitions:
                    instance.delete_dynamic_partition(
                        request.partitions_def_name, partition
                    )
...
def _handle_run_requests_and_automation_condition_evaluations(
    ...
    automation_condition_evaluations: Sequence[
        AutomationConditionEvaluation[EntityKey]
    ],
    cursor: Optional[str],
    instance: DagsterInstance,
    context: SensorLaunchContext,
    ...
    evaluations = [
        evaluation.with_run_ids(set())
        for evaluation in automation_condition_evaluations
    ]
    if (
        instance.schedule_storage
        and instance.schedule_storage.supports_auto_materialize_asset_evaluations
    ...
    reserved_run_ids = [
        reserved_run_id(run_request) for run_request in raw_run_requests
    ]
...
def _submit_run_requests(
    ...
    if (
        ...
        instance.schedule_storage.add_auto_materialize_asset_evaluations(
            ...
            asset_evaluations=[
                evaluations_by_key[key] for key in updated_evaluation_keys
            ],
        )
...
def _submit_backfill_request(
    ...
    instance.add_backfill(
        PartitionBackfill.from_asset_graph_subset(
            ...
            asset_graph_subset=check.inst(
                run_request.asset_graph_subset, AssetGraphSubset
            ),
...
def is_under_min_interval(state: InstigatorState, remote_sensor: RemoteSensor) -> bool:
    ...
    if (
        not instigator_data.last_tick_start_timestamp
        and not instigator_data.last_tick_timestamp
    ):
        return False
...
def _fetch_existing_runs(
    ...
    run_keys = [
        run_request.run_key for run_request in run_requests if run_request.run_key
    ]
...
def _get_or_create_sensor_run(
    ...
    run = existing_runs_by_key.get(run_request.run_key) or instance.get_run_by_id(
        run_id
    )
