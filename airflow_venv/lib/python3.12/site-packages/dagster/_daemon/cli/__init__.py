    ...
    try:
        with capture_interrupts():
            with get_instance_for_cli(
                ...
                _daemon_run_command(
                    instance, log_level, code_server_log_level, log_format, kwargs
                )
...
def debug_heartbeat_dump_command() -> None:
    with get_instance_for_cli() as instance:
        for daemon_status in get_daemon_statuses(
            instance, instance.get_required_daemon_types()
        ):
...
@click.group(
    commands={
        "heartbeat": debug_heartbeat_command,
        "heartbeat-dump": debug_heartbeat_dump_command,
    }
)
