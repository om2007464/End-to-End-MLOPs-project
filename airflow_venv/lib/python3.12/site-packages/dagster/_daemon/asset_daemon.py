...
def _get_has_migrated(instance: DagsterInstance, migration_key: str) -> bool:
    return bool(
        instance.daemon_cursor_storage.get_cursor_values({migration_key}).get(
            migration_key
        )
    )
...
def get_auto_materialize_paused(instance: DagsterInstance) -> bool:
    return (
        instance.daemon_cursor_storage.get_cursor_values(
            {_PRE_SENSOR_ASSET_DAEMON_PAUSED_KEY}
        ).get(_PRE_SENSOR_ASSET_DAEMON_PAUSED_KEY)
        != "false"
    )
...
def get_current_evaluation_id(
    ...
    if not sensor_origin:
        ...
        instigator_state = check.not_none(
...
def asset_daemon_cursor_to_instigator_serialized_cursor(
...
class AutoMaterializeLaunchContext:
    ...
    def set_run_requests(
        ...
        self._tick = self._tick.with_run_requests(
            run_requests, reserved_run_ids=reserved_run_ids
        )
        return self._tick
...
class AssetDaemon(DagsterDaemon):
    ...
    def _run_iteration_impl(
        ...
        sensors_and_repos: Sequence[
            Tuple[Optional[RemoteSensor], Optional[RemoteRepository]]
        ] = []
        ...
        if use_auto_materialize_sensors:
            ...
            if not self._checked_migrations:
                ...
                if not get_has_migrated_sensor_names(instance):
                    ...
                    all_sensor_states = (
                        self._copy_default_auto_materialize_sensor_states(
        ...
        for sensor, repo in sensors_and_repos:
            ...
            if not sensor:
                ...
                if (
                    ...
                    and now - self._last_pre_sensor_submit_time
                    < self._pre_sensor_interval_seconds
                ):
                    continue
            ...
            if threadpool_executor:
                ...
                if (
                    selector_id in amp_tick_futures
                    and not amp_tick_futures[selector_id].done()
                ):
                    continue
    ...
    def _copy_default_auto_materialize_sensor_states(
        ...
        for instigator_state in all_sensor_states.values():
            ...
            if (
                instigator_state.origin.instigator_name
    ...
    def _process_auto_materialize_tick_generator(
        ...
        try:
            ...
            if sensor:
                ...
                eligible_keys = selection.resolve(
                    repo_asset_graph
                ) | selection.resolve_checks(repo_asset_graph)
            else:
            ...
            if not auto_materialize_entity_keys and not auto_observe_asset_keys:
                self._logger.debug(
                    f"No assets/checks that require evaluation{print_group_name}"
                )
                yield
                return
            ...
            ticks = instance.get_ticks(
                instigator_origin_id, instigator_selector_id, limit=1
            )
            ...
            if latest_tick:
                ...
                previous_cursor_written = (
                    latest_tick.automation_condition_evaluation_id
                ...
                tick = instance.create_tick(
                    TickData(
                        ...
                        instigator_type=(
                            InstigatorType.SENSOR
                            if sensor
                            else InstigatorType.AUTO_MATERIALIZE
                        ),
    ...
    def _evaluate_auto_materialize_tick(
        ...
        if is_retry:
            ...
            sensor_tags = (
                {SENSOR_NAME_TAG: sensor.name, **sensor.run_tags} if sensor else {}
            )
            ...
            evaluations_by_key = {
                evaluation.key: evaluation.with_run_ids(set())
            ...
            reserved_run_ids = [
                make_new_backfill_id()
                if rr.requires_backfill_daemon()
        ...
        for i, (run_request, reserved_run_id) in enumerate(
            zip(run_requests, reserved_run_ids)
        ):
        ...
        if schedule_storage.supports_auto_materialize_asset_evaluations:
            schedule_storage.purge_asset_evaluations(
                before=(
                    ...
                    - datetime.timedelta(days=EVALUATIONS_TTL_DAYS)
                ).timestamp(),
            )
