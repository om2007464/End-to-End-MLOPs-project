...
def _convert_pydantic_field(
    ...
    if safe_is_subclass(field_type, Config):
        ...
        if not pydantic_field.is_required() and not is_closed_python_optional_type(
            field_type
        ):
...
def _convert_pydantic_discriminated_union_field(
    ...
    if not all(issubclass(sub_field, Config) for sub_field in sub_fields):
        raise NotImplementedError(
            "Discriminated unions with non-Config types are not supported."
        )
