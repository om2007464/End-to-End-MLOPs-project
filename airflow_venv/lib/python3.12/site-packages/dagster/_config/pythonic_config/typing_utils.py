        ...
        try:
            ...
            for field in annotations:
                if isinstance(annotations[field], DagsterType):
                    raise DagsterInvalidDagsterTypeInPythonicConfigDefinitionError(
                        name, field
                    )
        ...
        for field in annotations:
            if not field.startswith("__"):
                ...
                if (
                    ...
                    annotations[field],
                    LateBoundTypesForResourceTypeChecking.get_resource_type(),
                ):
...
class TypecheckAllowPartialResourceInitParams:
    ...
    def __set__(
        self, obj: Optional[object], value: Union[Any, "PartialResource[Any]"]
    ) -> None:
        # no-op implementation (only used to affect type signature)
