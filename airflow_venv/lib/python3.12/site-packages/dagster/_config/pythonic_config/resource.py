    ...
    def get_resource_requirements(
        ...
        for inner_key, resource in self.nested_resources.items():
            if is_coercible_to_resource(resource):
                yield from coerce_to_resource(resource).get_resource_requirements(
                    inner_key
                )
            ...
            if is_coercible_to_resource(partial_resource):
                resolved_keys.add(
                    _resolve_partial_resource_to_key(
                        attr_name, partial_resource, resource_defs
                    )
                )
            else:
                check.failed(
        ...
        for resource in self.nested_resources.values():
            if is_coercible_to_resource(resource):
                resolved_keys.update(
                    coerce_to_resource(resource).get_required_resource_keys(
                        resource_defs
                    )
                )
...
def _resolve_partial_resource_to_key(
    ...
    matches = [
        ...
        if resource_def is partial_def
    ]
    check.invariant(
        len(matches) == 1,
