    ...
    if isinstance(set_value, dict) and isinstance(input_value, dict):
        for key, value in input_value.items():
            if isinstance(value, (EnvVar, IntEnvVar)):
                ...
                set_value[key] = ensure_env_vars_set_post_init(
                    set_value.get(key) or {}, value
                )
...
class Config(MakeConfigCacheable, metaclass=BaseConfigMeta):
    ...
    def __init__(self, **config_dict) -> None:
        ...
        for config_key, value in config_dict.items():
            ...
            # If the passed value matches the name of an expected Enum value, convert it to the value
                ...
                modified_data_by_config_key[config_key] = field.annotation.__members__[
                    value
                ].value
            elif (
                field
                and safe_is_subclass(field.annotation, Config)
        ...
        self.__dict__ = ensure_env_vars_set_post_init(
            self.__dict__, modified_data_by_field_key
        )
    ...
    def _get_non_default_public_field_values(self) -> Mapping[str, Any]:
        return self.__class__._get_non_default_public_field_values_cls(
            dict(self)
        )  # noqa: SLF001
