...
@instance_cli.command(
    name="info", help="List the information about the current instance."
)
def info_command():
...
@instance_cli.command(
    name="migrate", help="Automatically migrate an out of date instance."
)
@click.option(
    "--bigint-migration", is_flag=True, help="Run an optional bigint migration"
)
...
@instance_cli.command(
    name="reindex", help="Rebuild index over historical runs for performance."
)
def reindex_command():
    with get_instance_for_cli() as instance:
        home = os.environ.get("DAGSTER_HOME")
...
def concurrency_print_key(instance: DagsterInstance, key: str):
    ...
    click.echo(
        f'"{key}": {len(key_info.claimed_slots)} / {key_info.slot_count} slots occupied'
    )
