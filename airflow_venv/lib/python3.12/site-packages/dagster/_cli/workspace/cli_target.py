...
def _raise_cli_usage_error(msg: Optional[str] = None) -> Never:
    raise UsageError(
        msg
        or "Invalid set of CLI arguments for loading repository/job. See --help for details."
    )
...
def python_module_option(allow_multiple: bool) -> ClickOption:
    return click.option(
        ...
        help=(
            "Specify module "
            + (
                "or modules (flag can be used multiple times) "
                if allow_multiple
                else ""
            )
            + "where dagster definitions reside as top-level symbols/variables and load "
...
def python_target_click_options(
    allow_multiple_python_targets: bool,
...
def workspace_target_click_options() -> Sequence[ClickOption]:
    return [
        click.option(
            "--empty-workspace", is_flag=True, help="Allow an empty workspace"
        ),
        workspace_option(),
...
def job_repository_target_argument(f: T_Callable) -> T_Callable:
    ...
    return apply_click_params(
        job_workspace_target_argument(f), *repository_click_options()
    )
...
def get_repository_python_origin_from_kwargs(
    kwargs: ClickArgMapping,
) -> RepositoryPythonOrigin:
...
def get_remote_repository_from_kwargs(
    ...
    with get_code_location_from_kwargs(instance, version, kwargs) as code_location:
        provided_repo_name = check.opt_str_elem(kwargs, "repository")
        yield get_remote_repository_from_code_location(
            code_location, provided_repo_name
        )
...
@contextmanager
def get_remote_job_from_kwargs(
    instance: DagsterInstance, version: str, kwargs: ClickArgMapping
):
