...
def print_changes(
    ...
    if (
        not errors
        and not added_schedules
        and not changed_schedules
        and not removed_schedules
    ):
        if preview:
            print_fn(
                click.style("No planned changes to schedules.", fg="magenta", bold=True)
            )
    ...
    if added_schedules or changed_schedules or removed_schedules:
        print_fn(
            click.style(
                "Planned Schedule Changes:" if preview else "Changes:",
                fg="magenta",
                bold=True,
            )
        )
...
def check_repo_and_scheduler(
    repository: RemoteRepository, instance: DagsterInstance
) -> None:
    ...
    if not repository.get_schedules():
        raise click.UsageError(
            f"There are no schedules defined for repository {repository_name}."
        )
...
@schedule_cli.command(
    name="preview",
    help="Preview changes that will be performed by `dagster schedule up`.",
)
@repository_target_argument
...
@repository_target_argument
@click.option(
    "--running", help="Filter for running schedules", is_flag=True, default=False
)
@click.option(
    "--stopped", help="Filter for stopped schedules", is_flag=True, default=False
)
@click.option(
    "--name", help="Only display schedule schedule names", is_flag=True, default=False
)
...
def execute_list_command(
    running_filter, stopped_filter, name_filter, cli_args, print_fn
):
    with get_instance_for_cli() as instance:
        with get_remote_repository_from_kwargs(
            instance, version=dagster_version, kwargs=cli_args
...
def extract_schedule_name(
    schedule_name: Optional[Union[str, Sequence[str]]]
) -> Optional[str]:
    if schedule_name and not isinstance(schedule_name, str):
        if len(schedule_name) == 1:
            ...
            check.failed(
                f"Can only handle zero or one schedule args. Got {schedule_name!r}"
            )
    return None
...
def execute_logs_command(schedule_name, cli_args, print_fn, instance=None):
    with get_instance_for_cli() as instance:
        with get_remote_repository_from_kwargs(
            ...
            most_recent_log = (
                max(result_files, key=os.path.getctime) if result_files else None
            )
...
def schedule_restart_command(schedule_name, restart_all_running, **kwargs):
    ...
    return execute_restart_command(
        schedule_name, restart_all_running, kwargs, click.echo
    )
            ...
            if all_running_flag:
                for schedule_state in instance.all_instigator_state(
                    ...
                    if schedule_state.status == InstigatorStatus.RUNNING:
                        try:
                            remote_schedule = repo.get_schedule(
                ...
                print_fn(
                    f"Restarted all running schedules for repository {repository_name}"
                )
            else:
                remote_schedule = repo.get_schedule(schedule_name)
                ...
                if (
                    ...
                    and schedule_state.status != InstigatorStatus.RUNNING
                ):
                    click.UsageError(
...
@schedule_cli.command(
    name="wipe", help="Delete the schedule history and turn off all schedules."
)
