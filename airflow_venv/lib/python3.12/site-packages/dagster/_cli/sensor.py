...
def print_changes(
    ...
    if not added_sensors and not removed_sensors:
        if preview:
            print_fn(
                click.style("No planned changes to sensors.", fg="magenta", bold=True)
            )
    ...
    print_fn(
        click.style(
            "Planned Sensor Changes:" if preview else "Changes:",
            fg="magenta",
            bold=True,
        )
    )
...
def check_repo_and_scheduler(
    ...
    if not repository.get_sensors():
        raise click.UsageError(
            f"There are no sensors defined for repository {repository_name}."
        )
...
@repository_target_argument
@click.option(
    "--running", help="Filter for running sensors", is_flag=True, default=False
)
@click.option(
    "--stopped", help="Filter for stopped sensors", is_flag=True, default=False
)
@click.option(
    "--name", help="Only display sensor sensor names", is_flag=True, default=False
)
...
def sensor_preview_command(sensor_name, since, last_run_key, cursor, **kwargs):
    ...
    return execute_preview_command(
        sensor_name, since, last_run_key, cursor, kwargs, click.echo
    )
...
@click.option(
    "--delete",
    help="Delete the existing cursor value for the sensor context",
    is_flag=True,
)
@repository_target_argument
...
def execute_cursor_command(sensor_name, cli_args, print_fn):
    with get_instance_for_cli() as instance:
        with get_code_location_from_kwargs(
            ...
            if cursor_value:
                print_fn(
                    f'Set cursor state for sensor {sensor.name} to "{cursor_value}"'
                )
            else:
