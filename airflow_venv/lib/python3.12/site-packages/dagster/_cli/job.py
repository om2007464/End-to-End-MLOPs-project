...
@job_cli.command(
    name="print",
    help="Print a job.\n\n{instructions}".format(
        instructions=get_job_instructions("print")
    ),
)
...
def get_run_config_from_file_list(
    file_list: Optional[Sequence[str]],
) -> Mapping[str, object]:
    check.opt_sequence_param(file_list, "file_list", of_type=str)
    return cast(
        Mapping[str, object], load_yaml_from_glob_list(file_list) if file_list else {}
    )
...
@click.option(
    "--tags", type=click.STRING, help="JSON string of tags to use for this job run"
)
@click.option(
    "-o",
    "--op-selection",
...
def execute_execute_command(
    ...
    config = list(
        check.opt_tuple_param(
            cast(Tuple[str, ...], kwargs.get("config")), "config", of_type=str
        )
    )
...
def get_op_selection_from_args(kwargs: ClickArgMapping) -> Optional[Sequence[str]]:
    ...
    return (
        [ele.strip() for ele in op_selection_str.split(",")]
        if op_selection_str
        else None
    )
...
@click.option(
    "--run-id", type=click.STRING, help="The ID to give to the launched job run"
)
...
def _execute_backfill_command_at_location(
    ...
    remote_job = get_remote_job_from_remote_repo(
        repo, check.opt_str_elem(cli_args, "job_name")
    )
...
def gen_partition_names_from_args(
    ...
    if kwargs.get("partitions"):
        selected_args = [
            s.strip()
            for s in check.str_elem(kwargs, "partitions").split(",")
            if s.strip()
        ]
        selected_partitions = [
        ...
        if len(selected_partitions) < len(selected_args):
            ...
            unknown = [
                selected for selected in selected_args if selected not in selected_names
            ]
