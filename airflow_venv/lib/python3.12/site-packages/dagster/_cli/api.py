...
from dagster._core.execution.run_cancellation_thread import (
...
def _should_start_metrics_thread(dagster_run: DagsterRun) -> bool:
    return any(
        get_boolean_tag_value(dagster_run.tags.get(tag)) for tag in RUN_METRIC_TAGS
    )
...
def _execute_run_command_body(
    ...
    if instance.should_start_background_run_thread:
        ...
        ) = start_run_cancellation_thread(instance, run_id)
    else:
...
def _shutdown_threads(
    ...
    if instance.should_start_background_run_thread:
        cancellation_thread_shutdown_event = check.not_none(
            cancellation_thread_shutdown_event
        )
...
def get_step_stats_by_key(
    ...
    step_stats = instance.get_run_step_stats(
        dagster_run.run_id, step_keys=step_keys_to_execute
    )
...
def verify_step(
    ...
    step_stats_by_key = get_step_stats_by_key(
        instance, dagster_run, step_keys_to_execute
    )
...
@click.argument(
    "input_json", type=click.STRING, envvar="DAGSTER_EXECUTE_STEP_ARGS", required=False
)
@click.option(
...
def execute_step_command(
    input_json: Optional[str], compressed_input_json: Optional[str]
) -> None:
    with capture_interrupts():
...
@api_cli.command(
    name="grpc-health-check", help="Check the status of a dagster GRPC server"
)
@click.option(
    "--port",
    "-p",
