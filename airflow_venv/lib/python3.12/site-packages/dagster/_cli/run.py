...
@click.option(
    "--limit", help="Only list a specified number of runs", default=None, type=int
)
...
@click.option(
    "--force", "-f", is_flag=True, default=False, help="Skip prompt to delete run."
)
...
@run_cli.command(
    name="wipe",
    help="Eliminate all run history and event logs. Warning: Cannot be undone.",
)
@click.option(
    "--force",
    "-f",
...
def run_wipe_command(force: bool) -> None:
    ...
    if should_delete_run:
        ...
        raise click.ClickException(
            "Exiting without deleting all run history and event logs."
        )
...
def run_migrate_command(from_label: str, **kwargs: Any) -> None:
    ...
    with get_instance_for_cli() as instance:
        ...
        if not to_label:
            raise click.UsageError(
                "Must specify valid job targets to migrate history to."
            )
        ...
        records = instance.get_run_records(
            filters=RunsFilter(
                job_name=job_name, tags={REPOSITORY_LABEL_TAG: from_label}
            )
        )
        ...
        if not isinstance(instance.run_storage, SqlRunStorage):
            raise click.UsageError(
                "Run migration only applies to SQL-based run storage"
            )
        ...
        if should_migrate:
            for record in tqdm(records):
                instance.run_storage.replace_job_origin(
                    record.dagster_run, new_job_origin
                )
            click.echo(
                f"Migrated the run history for {job_name} from {from_label} to {to_label}."
            )
        else:
