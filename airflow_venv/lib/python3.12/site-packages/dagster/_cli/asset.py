...
@asset_cli.command(
    name="materialize", help="Execute a run to materialize a selection of assets"
)
@python_origin_target_argument
...
def execute_materialize_command(
    instance: DagsterInstance, kwargs: Mapping[str, str]
) -> None:
    ...
    if partition:
        ...
        for asset_key in asset_keys:
            backfill_policy = implicit_job_def.asset_layer.get(
                asset_key
            ).backfill_policy
            if (
...
@click.argument("key", nargs=-1)
@click.option(
    "--all", is_flag=True, help="Wipe partitions status cache of all asset keys"
)
@click.option("--noprompt", is_flag=True)
