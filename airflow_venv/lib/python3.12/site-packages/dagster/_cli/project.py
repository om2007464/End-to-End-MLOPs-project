...
def check_if_pypi_package_conflict_exists(
    project_name: str,
) -> PackageConflictCheckResult:
    ...
    if any(keyword in project_name for keyword in FLAGGED_PACKAGE_KEYWORDS):
        try:
            ...
            if res.status_code == 200:
                return PackageConflictCheckResult(
                    request_error_msg=None, conflict_exists=True
                )
...
def _check_and_error_on_package_conflicts(project_name: str) -> None:
    package_check_result: PackageConflictCheckResult = (
        check_if_pypi_package_conflict_exists(project_name)
    )
