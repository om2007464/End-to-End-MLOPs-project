...
from dagster._seven.temp_dir import (
    get_system_temp_directory as get_system_temp_directory,
)
...
def is_function_or_decorator_instance_of(
    ...
    return inspect.isfunction(target) or (
        isinstance(target, kls) and hasattr(target, "__name__")
    )


def qualname_differs(target: object) -> bool:
    return hasattr(target, "__qualname__") and getattr(
        target, "__qualname__"
