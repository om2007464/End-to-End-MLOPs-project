...
async def handle_report_asset_materialization_request(
    ...
    asset_key = _asset_key_from_request(
        ReportAssetMatParam.asset_key, request, json_body
    )
    ...
    data_version = _value_from_body_or_params(
        ReportAssetMatParam.data_version, request, json_body
    )
    if data_version is not None:
        tags[DATA_VERSION_TAG] = data_version
        tags[DATA_VERSION_IS_USER_PROVIDED_TAG] = "true"

    partition = _value_from_body_or_params(
        ReportAssetMatParam.partition, request, json_body
    )
    description = _value_from_body_or_params(
        ReportAssetMatParam.description, request, json_body
    )
...
async def handle_report_asset_check_request(
    ...
    asset_key = _asset_key_from_request(
        ReportAssetCheckEvalParam.asset_key, request, json_body
    )
    if asset_key is None:
        return JSONResponse(
            {
    ...
    passed = _value_from_body_or_params(
        ReportAssetCheckEvalParam.passed, request, json_body
    )
    check_name = _value_from_body_or_params(
        ReportAssetCheckEvalParam.check_name, request, json_body
    )
    severity = _value_from_body_or_params(
        ReportAssetCheckEvalParam.severity, request, json_body
    )
    if severity is None:
    ...
    if ReportAssetCheckEvalParam.metadata in json_body:
        ...
        try:
            metadata = json.loads(
                request.query_params[ReportAssetCheckEvalParam.metadata]
            )
        except Exception as exc:
            return JSONResponse(
                {
...
async def handle_report_asset_observation_request(
    ...
    asset_key = _asset_key_from_request(
        ReportAssetObsParam.asset_key, request, json_body
    )
    if asset_key is None:
        return JSONResponse(
            {
    ...
    partition = _value_from_body_or_params(
        ReportAssetObsParam.partition, request, json_body
    )
    description = _value_from_body_or_params(
        ReportAssetObsParam.description, request, json_body
    )

    tags = context.get_reporting_user_tags()
    data_version = _value_from_body_or_params(
        ReportAssetObsParam.data_version, request, json_body
    )
    if data_version is not None:
        tags[DATA_VERSION_TAG] = data_version
        tags[DATA_VERSION_IS_USER_PROVIDED_TAG] = "true"
