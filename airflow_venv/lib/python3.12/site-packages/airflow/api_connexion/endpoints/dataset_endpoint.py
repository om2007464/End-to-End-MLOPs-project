...
def get_datasets(
    *,
    ...
    if dag_ids:
        ...
        query = query.filter(
            (
                DatasetModel.consuming_dags.any(
                    DagScheduleDatasetReference.dag_id.in_(dags_list)
                )
            )
            | (
                DatasetModel.producing_tasks.any(
                    TaskOutletDatasetReference.dag_id.in_(dags_list)
                )
            )
        )
    if uri_pattern:
    ...
    datasets = session.scalars(
        query.options(
            subqueryload(DatasetModel.consuming_dags),
...
def create_dataset_event(session: Session = NEW_SESSION) -> APIResponse:
    ...
    if not dataset:
        raise NotFound(
            title="Dataset not found", detail=f"Dataset with uri: '{uri}' not found"
        )
