...
class DagFileProcessorProcess(LoggingMixin, MultiprocessingStartMethodMixin):
    ...
    def _run_file_processor(
        ...
        def _handle_dag_file_processing():
            ...
            dag_file_processor = DagFileProcessor(
                dag_ids=dag_ids, dag_directory=dag_directory, log=log
            )
        ...
        try:
            DAG_PROCESSOR_LOG_TARGET = conf.get_mandatory_value(
            ...
            if DAG_PROCESSOR_LOG_TARGET == "stdout":
                ...
                with redirect_stdout(
                    StreamLogWriter(log, logging.INFO)
                ), redirect_stderr(
        ...
        if conf.getboolean("scheduler", "parsing_pre_import_modules", fallback=True):
            ...
            if zipfile.is_zipfile(self.file_path):
                try:
                    ...
                    self.log.error(
                        "There was an err accessing %s, %s", self.file_path, err
                    )
            if zip_file_paths:
    ...
    def _kill_process(self) -> None:
        ...
        if self._process.is_alive() and self._process.pid:
            self.log.warning(
                "Killing DAGFileProcessorProcess (PID=%d)", self._process.pid
            )
...
class DagFileProcessor(LoggingMixin):
    ...
    @provide_session
    def manage_slas(
        cls, dag_folder, dag_id: str, session: Session = NEW_SESSION
    ) -> None:
        """
        ...
        for ti in max_tis:
            ...
            next_info = dag.next_dagrun_info(
                dag.get_run_data_interval(ti.dag_run), restricted=False
            )
            while next_info and next_info.logical_date < ts:
                next_info = dag.next_dagrun_info(
                    next_info.data_interval, restricted=False
                )
                ...
                if next_info.logical_date + task.sla < ts:
                    ...
                    Stats.incr(
                        "sla_missed", tags={"dag_id": ti.dag_id, "task_id": ti.task_id}
                    )
            if sla_misses:
                session.add_all(sla_misses)
        session.commit()
        slas: list[SlaMiss] = session.scalars(
            select(SlaMiss).where(
                ~SlaMiss.notification_sent, SlaMiss.dag_id == dag.dag_id
            )
        ).all()
        if slas:
            ...
            if emails:
                try:
                    send_email(
                        emails, f"[airflow] SLA miss on DAG={dag.dag_id}", email_content
                    )
                    email_sent = True
                    notification_sent = True
                except Exception:
                    Stats.incr(
                        "sla_email_notification_failure", tags={"dag_id": dag.dag_id}
                    )
                    cls.logger().exception(
    ...
    def update_import_errors(
        ...
        existing_import_error_files = [
            x.filename for x in session.query(ParseImportError.filename).all()
        ]
    ...
    def update_dag_warnings(cla, *, dagbag: DagBag) -> None:
        ...
        return DagFileProcessor._validate_task_pools_and_update_dag_warnings(
            pool_dict, dag_ids
        )
    ...
    def _validate_task_pools_and_update_dag_warnings(
        ...
        stored_warnings = set(
            session.query(DagWarning).filter(DagWarning.dag_id.in_(dag_ids)).all()
        )
        ...
        for request in callback_requests:
            ...
            try:
                if isinstance(request, TaskCallbackRequest):
                    cls._execute_task_callbacks(
                        dagbag, request, unit_test_mode, session=session
                    )
                    ...
                    if InternalApiConfig.get_use_internal_api():
                        ...
                        DagFileProcessor.manage_slas(
                            dagbag.dag_folder, request.dag_id, session=session
                        )
    ...
    def _execute_dag_callbacks(
        cls, dagbag: DagBag, request: DagCallbackRequest, session: Session
    ):
    ...
    def _execute_task_callbacks(
        ...
        if callback_type is TaskInstanceState.SUCCESS:
            ...
            ti.handle_failure(
                error=request.msg, test_mode=unit_test_mode, session=session
            )
            cls.logger().info("Executed callback for %s in state %s", ti, ti.state)
        session.flush()
    ...
    def process_file(
        ...
        with count_queries(session) as query_counter:
            try:
                ...
                Stats.incr(
                    "dag_file_refresh_error", 1, 1, tags={"file_path": file_path}
                )
    ...
    def save_dag_to_db(
        ...
        import_errors = DagBag._sync_to_db(
            dags=dags, processor_subdir=dag_directory, session=session
        )
        session.commit()
        ...
        if pickle_dags:
            ...
            unpaused_dags: list[DAG] = [
                dag for dag_id, dag in dags.items() if dag_id not in paused_dag_ids
            ]
