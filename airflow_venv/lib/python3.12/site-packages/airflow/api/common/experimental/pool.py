...
@deprecated(
    reason="Use Pool.get_pool() instead",
    version="2.2.4",
...
@deprecated(
    reason="Use Pool.get_pools() instead",
    version="2.2.4",
    category=RemovedInAirflow3Warning,
)
@provide_session
...
@deprecated(
    reason="Use Pool.create_pool() instead",
    version="2.2.4",
    category=RemovedInAirflow3Warning,
)
@provide_session
def create_pool(name, slots, description, session: Session = NEW_SESSION):
    ...
    if pool is None:
        pool = Pool(
            pool=name, slots=slots, description=description, include_deferred=False
        )
        session.add(pool)
    else:
        pool.slots = slots
...
@deprecated(
    reason="Use Pool.delete_pool() instead",
    version="2.2.4",
    category=RemovedInAirflow3Warning,
)
@provide_session
