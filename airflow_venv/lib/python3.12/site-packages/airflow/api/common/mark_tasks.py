...
def set_state(
    *,
    ...
    task_ids = [
        task_id if isinstance(task_id, str) else task_id[0]
        for task_id in task_id_map_index_list
    ]

    confirmed_infos = list(
        _iter_existing_dag_run_infos(dag, dag_run_ids, session=session)
    )
    ...
    sub_dag_run_ids = (
        list(
            _iter_subdag_run_ids(
                dag, session, DagRunState(state), task_ids, commit, confirmed_infos
            ),
        )
    ...
    if commit:
        ...
        if sub_dag_run_ids:
            qry_sub_dag = all_subdag_tasks_query(
                sub_dag_run_ids, session, state, confirmed_dates
            )
...
def _iter_subdag_run_ids(
    ...
    while dags:
        ...
        for task_id in task_ids:
            ...
            if (
                isinstance(current_task, SubDagOperator)
                or current_task.task_type == "SubDagOperator"
            ):
...
def get_run_ids(
    ...
    last_dagrun = dag.get_last_dagrun(
        include_externally_triggered=True, session=session
    )
