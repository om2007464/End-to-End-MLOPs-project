...
class HookLineageCollector(LoggingMixin):
    ...
    def create_dataset(
        ...
        if not dataset_factory:
            self.log.debug(
                "Unsupported scheme: %s. Please provide a valid URI to create a Dataset.",
                scheme,
            )
            return None
    ...
    def collected_datasets(self) -> HookLineage:
        ...
        return HookLineage(
            [
                DatasetLineageInfo(
                    dataset=dataset, count=self._input_counts[key], context=context
                )
                for key, (dataset, context) in self._inputs.items()
            ],
            [
                DatasetLineageInfo(
                    dataset=dataset, count=self._output_counts[key], context=context
                )
