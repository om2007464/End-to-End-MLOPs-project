    ...
    * ``[core] compress_serialized_dags``:
    ...
    def write_dag(
        ...
        if (
            ...
            and serialized_dag_db.processor_subdir
            == new_serialized_dag.processor_subdir
        ):
            log.debug(
    ...
    def get(
        ...
        # out the root dag
        root_dag_id = session.scalar(
            select(DagModel.root_dag_id).where(DagModel.dag_id == dag_id)
        )
    ...
    def get_dag_dependencies(
        ...
        if session.bind.dialect.name in ["sqlite", "mysql"]:
            query = session.execute(
                select(
                    cls.dag_id, func.json_extract(cls._data, "$.dag.dag_dependencies")
                )
            )
            iterator = (
