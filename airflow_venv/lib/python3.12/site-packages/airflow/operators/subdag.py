...
class SubDagOperator(BaseSensorOperator):
    ...
    def _validate_pool(self, session):
        if self.pool:
            ...
            if conflicts:
                ...
                pool = session.scalar(
                    select(Pool).where(Pool.slots == 1, Pool.pool == self.pool)
                )
    ...
    def _reset_dag_run_and_task_instances(
        self, dag_run: DagRun, execution_date: datetime
    ) -> None:
        """
    ...
    def pre_execute(self, context):
        ...
        if dag_run is None:
            if (
                context["data_interval_start"] is None
                or context["data_interval_end"] is None
            ):
