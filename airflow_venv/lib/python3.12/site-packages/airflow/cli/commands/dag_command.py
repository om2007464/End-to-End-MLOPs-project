...
def _run_dag_backfill(dags: list[DAG], args) -> None:
    ...
    for dag in dags:
        ...
        if args.dry_run:
            ...
            dagrun_infos = dag.iter_dagrun_infos_between(
                earliest=args.start_date, latest=args.end_date
            )
            ...
            try:
                dag.run(
                    ...
                    local=args.local,
                    donot_pickle=(
                        args.donot_pickle or conf.getboolean("core", "donot_pickle")
                    ),
...
def dag_backfill(args, dag: list[DAG] | DAG | None = None) -> None:
    ...
    if not dag:
        dags = get_dags(
            args.subdir, dag_id=args.dag_id, use_regex=args.treat_dag_id_as_regex
        )
