...
def update_permission_action(session, permission_query, action):
    permission_query.update(
        {Permission.action_id: action.id}, synchronize_session=False
    )
    session.commit()
...
def migrate_to_new_dag_permissions(db):
    ...
    old_can_dag_read_permissions = get_permission_with_action_query(
        db.session, can_dag_read_action
    )
    ...
    old_can_dag_edit_permissions = get_permission_with_action_query(
        db.session, can_dag_edit_action
    )
    ...
    if all_dags_resource:
        old_all_dags_permission = get_permission_with_resource_query(
            db.session, all_dags_resource
        )
...
def undo_migrate_to_new_dag_permissions(session):
    ...
    new_can_read_permissions = get_permission_with_action_query(
        session, can_read_action
    )
    ...
    new_can_edit_permissions = get_permission_with_action_query(
        session, can_edit_action
    )
