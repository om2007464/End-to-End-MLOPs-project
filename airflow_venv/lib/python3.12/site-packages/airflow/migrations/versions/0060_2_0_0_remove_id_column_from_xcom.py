        ...
        if "id" in xcom_columns:
            if conn.dialect.name == "mssql":
                ...
                drop_column_constraints(
                    operator=bop, column_name="id", constraint_dict=constraint_dict
                )
            bop.drop_column("id")
            ...
            if conn.dialect.name != "mssql":
                bop.create_primary_key(
                    "pk_xcom", ["dag_id", "task_id", "key", "execution_date"]
                )
...
def downgrade():
    ...
    with op.batch_alter_table("xcom") as bop:
        ...
        bop.create_index(
            "idx_xcom_dag_task_date", ["dag_id", "task_id", "key", "execution_date"]
        )
