    ...
    if "run_type" not in dag_run_columns:
        ...
        for run_type in DagRunType:
            session.query(DagRun).filter(
                DagRun.run_id.like(f"{run_type.value}__%")
            ).update({DagRun.run_type: run_type.value}, synchronize_session=False)
        ...
        with op.batch_alter_table("dag_run") as batch_op:
            batch_op.alter_column(
                "run_type",
                existing_type=run_type_col_type,
                type_=run_type_col_type,
                nullable=False,
            )
