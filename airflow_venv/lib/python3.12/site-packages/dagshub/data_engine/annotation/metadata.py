...
from dagshub_annotation_converter.ir.image.annotations.base import (
    IRAnnotationBase,
...
class MetadataAnnotations:
    ...
    @staticmethod
    def from_ls_task(
        datapoint: "Datapoint", field: str, ls_task: bytes
    ) -> "MetadataAnnotations":
    ...
    def to_ls_task(self) -> Optional[bytes]:
        ...
        task = LabelStudioTask(
            user_id=UserAPI.get_current_user(
                self.datapoint.datasource.source.repoApi.host
            ).user_id,
        )
    ...
    def get_image_dimensions(
        self, image_width: Optional[int], image_height: Optional[int]
    ) -> Tuple[int, int]:
        """
        ...
        if "width" not in self.datapoint.metadata:
            raise ValueError(
                'Image width not provided, and a "width" field was not found in the datapoint'
            )
        if "height" not in self.datapoint.metadata:
            raise ValueError(
                'Image height not provided, and a "height" field was not found in the datapoint'
            )
    ...
    def add_image_pose(
        ...
        category: str,
        points: Union[
            Sequence[Tuple[float, float]], Sequence[Tuple[float, float, Optional[bool]]]
        ],
    ...
    def add_yolo_annotation(
        ...
        if isinstance(annotation, str):
            if categories is None:
                raise ValueError(
                    "`categories` dictionary is required when importing annotations from a string"
                )
            ...
            if annotation_type == "pose":
                if pose_keypoint_dim is None:
                    raise ValueError(
                        "`pose_keypoint_dim` is required when importing pose annotations"
                    )
        ...
        log_message(
            f"Added {len(annotations)} YOLO annotation(s) to datapoint {self.datapoint.path}"
        )
        self._update_datapoint()

    @staticmethod
    def _generate_yolo_context(
        annotation_type, categories: Dict[int, str]
    ) -> YoloContext:
        cats = Categories()
