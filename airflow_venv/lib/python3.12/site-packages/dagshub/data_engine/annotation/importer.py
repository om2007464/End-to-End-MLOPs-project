...
class AnnotationsNotFoundError(Exception):
    def __init__(self, path):
        super().__init__(
            f'Annotations not found at path "{path}" in neither disk or repository.'
        )
...
class AnnotationImporter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.load_from = (
            ...
            else self.determine_load_location(ds, annotations_file)
        )
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            if self.annotations_type == "yolo":
                annotation_dict, _ = load_yolo_from_fs(
                    annotation_type=self.additional_args["yolo_type"],
                    meta_file=annotations_file,
                )
    ...
    def download_annotations(self, dest_dir: Path):
        ...
        if self.annotations_type == "cvat":
            ...
            repoApi.download(
                self.annotations_file.as_posix(), dest_dir, keep_source_prefix=True
            )
            ...
            context = YoloContext.from_yaml_file(
                meta_file, annotation_type=self.additional_args["yolo_type"]
            )
            # Download the annotation data
            assert context.path is not None
            repoApi.download(
                self.annotations_file.parent / context.path,
                dest_dir,
                keep_source_prefix=True,
            )

    @staticmethod
    def determine_load_location(
        ds: "Datasource", annotations_path: Union[str, Path]
    ) -> AnnotationLocation:
    ...
    def guess_annotation_filename_remapping(
        ...
        if len(datapoint_paths) == 0:
            raise ValueError(
                f"No datapoints found that match the annotation path {annotation_path}"
            )
        ...
        if len(datapoint_paths) > 1:
            ...
            dp_path = AnnotationImporter.get_best_fit_datapoint_path(
                annotation_path, datapoint_paths
            )
            log_message(
                f'Multiple datapoints found for annotation path "{annotation_path}". Using "{dp_path}"'
            )

        return AnnotationImporter.generate_path_map_func(annotation_path, dp_path)

    @staticmethod
    def generate_path_map_func(
        ann_path: str, dp_path: str
    ) -> Callable[[str], Optional[str]]:
        ...
        if match.a + match.size != len(
            ann_path_posix.parts
        ) or match.b + match.size != len(dp_path_posix.parts):
            raise CannotRemapPathError(ann_path, dp_path)
    ...
    def get_best_fit_datapoint_path(ann_path: str, datapoint_paths: List[str]) -> str:
        ...
        if best_match is None:
            raise ValueError(
                f"No good match found for annotation path {ann_path} in the datasource."
            )
        return best_match

    def convert_to_ls_tasks(
