...
class DagsHubDataset:
    def __init__(
        ...
        self.tensorizers = (
            ...
            if type(tensorizers) is str
            or (type(tensorizers) is list and type(tensorizers[0]) is str)
            else tensorizers
        )
    ...
    def _get_tensorizers(
        ...
        if datatypes in ["auto", "guess"]:  # guess is an easter egg argument
            ...
            type(datatypes) is list and len(datatypes) == len(self.metadata_columns) + 1
        ):
            return [
                getattr(self.tensorlib, datatype) if type(datatype) is str else datatype
