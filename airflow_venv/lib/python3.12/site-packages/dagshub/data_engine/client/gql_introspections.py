...
class IntrospectionType:
    ...
    def hash_str(self):
        return (
            ...
            + "".join([f.name for f in self.inputFields])
        )
...
class Validators:
    @staticmethod
    def query_input_validator(
        params: Dict[str, Any], query_input_introspection: TypesIntrospection
    ):
        ...
        if not all([f in introspect_query_input_fields for f in sent_fields]):
            unsupported_fields = [
                f for f in sent_fields if f not in introspect_query_input_fields
            ]
            raise ValueError(
                f"QueryInput fields are not supported: {unsupported_fields}"
            )

    @staticmethod
    def get_fields(
        datapoints_connection_introspection: TypesIntrospection,
        type_name: str,
        attr: str = "fields",
    ) -> List[str]:
        datapoints_connection_fields = [
            f for f in datapoints_connection_introspection.types if f.name == type_name
        ]
