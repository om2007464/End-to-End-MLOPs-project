...
class GqlQueries:
    ...
    def datasource() -> GqlQuery:
        q = (
            GqlQuery()
            .operation(
                "query", name="datasource", input={"$id": "ID", "$name": "String"}
            )
            .query(
                "datasource",
                input={
    ...
    def datasource_params(
        id: Optional[Union[int, str]], name: Optional[str]
    ) -> Dict[str, Any]:
        return {
            "id": id,
            "name": name,
    ...
    def datasource_query(
        include_metadata: bool, introspection: "TypesIntrospection"
    ) -> GqlQuery:
        metadata_fields = ""
        if include_metadata:
            # Filter out the unavailable fields
            queryable_fields = ["key", "value", "timeZone"]
            queryable_fields = Validators.filter_supported_fields(
                queryable_fields, "MetadataField", introspection
            )
        ...
        if include_metadata:
            fields.append(
                "selectFields { name originalName autoGenerated valueType asOf multiple tags }"
            )
        q = q.fields(
            Validators.filter_supported_fields(
                fields, "DatapointsConnection", introspection
            )
        )
