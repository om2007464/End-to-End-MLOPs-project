...
from dagshub.data_engine.client.gql_introspections import (
    GqlIntrospections,
    TypesIntrospection,
)
...
class DataClient:
    ...
    def _init_client(self):
        ...
        transport = RequestsHTTPTransport(
            url=url, auth=auth, headers=config.requests_headers
        )
    ...
    def create_datasource(self, ds: "DatasourceState") -> DatasourceResult:
        ...
        params = GqlMutations.create_datasource_params(
            name=ds.name, url=ds.path, ds_type=ds.source_type
        )
        res = self._exec(q, params)
        return dacite.from_dict(
            DatasourceResult, res["createDatasource"], config=dacite_config
        )
    ...
    def sample(
        self, datasource: "Datasource", n: Optional[int], include_metadata: bool
    ) -> QueryResult:
        """
    ...
    def _get_all(self, datasource: "Datasource", include_metadata: bool) -> QueryResult:
        ...
        with progress:
            while has_next_page:
                resp = self._datasource_query(
                    datasource, include_metadata, self.FULL_LIST_PAGE_SIZE, after
                )
                has_next_page = resp["pageInfo"]["hasNextPage"]
                after = resp["pageInfo"]["endCursor"]
                ...
                res.query_data_time = new_entries.query_data_time
                progress.update(
                    total_task, advance=len(new_entries.entries), refresh=True
                )
    ...
    def _exec(
        ...
        try:
            ...
            raise DataEngineGqlError(
                e, self.client.transport.response_headers.get("X-DagsHub-Support-Id")
            )
        return resp
    ...
    def _datasource_query(
        ...
        send_analytics_event(
            "Client_DataEngine_QueryRun", repo=datasource.source.repoApi
        )
    ...
    def update_metadata(
        self, datasource: "Datasource", entries: List["DatapointMetadataUpdateEntry"]
    ):
        """
    ...
    def delete_metadata_for_datapoint(
        self, datasource: "Datasource", entries: List["DatapointDeleteMetadataEntry"]
    ):
        """
    ...
    def delete_datapoints(
        self, datasource: "Datasource", entries: List["DatapointDeleteEntry"]
    ):
        """
    ...
    def update_metadata_fields(
        self, datasource: "Datasource", metadata_field_props: List[MetadataFieldSchema]
    ):
    ...
    def get_datasources(
        ...
        if res is None:
            return []
        return [
            dacite.from_dict(DatasourceResult, val, config=dacite_config) for val in res
        ]
    ...
    def delete_datasource(self, datasource: "Datasource"):
        ...
        params = GqlMutations.delete_datasource_params(
            datasource_id=datasource.source.id
        )
        return self._exec(q, params)

    def scan_datasource(
        self, datasource: "Datasource", options: Optional[List[ScanOption]]
    ):
        """
        ...
        params = GqlMutations.scan_datasource_params(
            datasource_id=datasource.source.id, options=options
        )
        return self._exec(q, params)
    ...
    def get_datasets(
        self, id: Optional[Union[str, int]], name: Optional[str]
    ) -> List[DatasetResult]:
        """
        ...
        return [
            dacite.from_dict(DatasetResult, val, config=dacite_config) for val in res
        ]
