...
def sanitize_voxel_filter(sess: "fo.Session", filters: List[VoxelFilterState]):
    ...
    for f in filters:
        if f.filter_field in _generated_fields:
            logger.warning(
                f"Not applying filter to field {f.filter_field} because it's a generated field"
            )
        else:
...
def apply_filters_to_datasource(
    ds: "Datasource", filters: List[VoxelFilterState]
) -> "Datasource":
    """
