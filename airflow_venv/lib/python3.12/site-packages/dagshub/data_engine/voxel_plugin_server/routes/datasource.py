...
async def update_metadata(request: Request):
    ...
    send_analytics_event(
        "Client_DataEngine_addEnrichmentsWithVoxel", repo=ds.source.repoApi
    )

    req_data = dacite.from_dict(
        UpdateMetadataRequestData, await request.json(), config=dacite_config
    )
...
async def refresh_dataset(request: Request):
    ...
    updated_dataset = ds.to_voxel51_dataset(
        name=current_dataset.name, force_download=True
    )
