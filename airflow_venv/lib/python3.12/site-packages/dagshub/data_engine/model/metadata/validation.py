...
class MultipleDataTypesUploadedError(Exception):
    def __init__(
        self,
        field_name: str,
        orig_type: MetadataFieldType,
        other_type: MetadataFieldType,
    ):
    ...
    for m in metadata:
        if m.key not in res:
            info = UploadingMetadataInfo(
                ...
                existing_metadata_in_ds=next(
                    filter(lambda f: f.name == m.key, ds.fields), None
                ),
            )
            res[m.key] = info
            ...
            if info.existing_metadata_in_ds is not None:
                ...
                if info.existing_metadata_in_ds.valueType != m.valueType:
                    raise MultipleDataTypesUploadedError(
                        m.key, info.existing_metadata_in_ds.valueType, m.valueType
                    )
            ...
            if info.field_type != m.valueType:
                raise MultipleDataTypesUploadedError(
                    m.key, info.field_type, m.valueType
                )

        if info.field_type == MetadataFieldType.STRING:
            info.longest_value = (
                info.longest_value
                if len(info.longest_value) > len(m.value)
                else m.value
            )
    return res
