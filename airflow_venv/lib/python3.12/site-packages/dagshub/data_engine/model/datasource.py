...
from dagshub.data_engine.annotation.importer import (
    AnnotationImporter,
    AnnotationType,
    AnnotationLocation,
)
...
class Datasource:
    ...
    def clear_query(self, reset_to_dataset=True):
        ...
        if (
            reset_to_dataset
            and self.assigned_dataset is not None
            and self.assigned_dataset.query is not None
        ):
    ...
    def head(
        self, size=100, load_documents=True, load_annotations=True
    ) -> "QueryResult":
        """
        ...
        send_analytics_event(
            "Client_DataEngine_DisplayTopResults", repo=self.source.repoApi
        )
        res = self._source.client.head(self, size)
        res._load_autoload_fields(
            documents=load_documents, annotations=load_annotations
        )
        return res
        ...
        for arg in args:
            if isinstance(arg, str):
                ...
                if isinstance(arg[1], bool):
                    ...
                    raise RuntimeError(
                        f"Invalid sort argument {arg}, second value must be 'asc'|'desc'|True|False"
                    )
    ...
    def _check_preprocess(self):
        ...
        if (
            ...
            or self.source.preprocessing_status
            == PreprocessingStatus.AUTO_SCAN_IN_PROGRESS
        ):
            logger.warning(
    ...
    def apply_field_changes(self, field_builders: List[MetadataFieldBuilder]):
        ...
        self.source.client.update_metadata_fields(
            self, [builder.schema for builder in field_builders]
        )
        self.source.get_from_dagshub()
    ...
    def metadata_context(self) -> ContextManager["MetadataContextManager"]:
        ...
        def func():
            self.source.get_from_dagshub()
            send_analytics_event(
                "Client_DataEngine_addEnrichments", repo=self.source.repoApi
            )
            ...
            try:
                entries = (
                    ctx.get_metadata_entries()
                    + self.implicit_update_context.get_metadata_entries()
                )
    ...
    def upload_metadata_from_file(
        self,
        file_path,
        path_column: Optional[Union[str, int]] = None,
        ingest_on_server: bool = False,
    ):
        """
        ...
        """
        send_analytics_event(
            "Client_DataEngine_addEnrichmentsWithFile", repo=self.source.repoApi
        )

        if ingest_on_server:
            datasource_name = self.source.name
            self._source.import_metadata_from_file(
                datasource_name, file_path, path_column
            )
        else:
    ...
    def upload_metadata_from_dataframe(
        ...
        self.source.get_from_dagshub()
        send_analytics_event(
            "Client_DataEngine_addEnrichmentsWithDataFrame", repo=self.source.repoApi
        )

        if ingest_on_server:
            self._remote_upload_metadata_from_dataframe(df, path_column)
        else:
            metadata = self._df_to_metadata(
                df, path_column, multivalue_fields=self._get_multivalue_fields()
            )
    ...
    def _df_to_metadata(
        ...
        if path_column is None:
            ...
            if path_column not in df.columns:
                raise RuntimeError(
                    f"Column {path_column} does not exist in the dataframe"
                )
        ...
        for _, row in df.iterrows():
            ...
            for key, val in row.items():
                ...
                if isinstance(val, list) or isinstance(
                    val, pandas._libs.tslibs.timestamps.Timestamp
                ):
                ...
                if isinstance(val, list):
                    ...
                    for sub_val in val:
                        ...
                        if value_type == MetadataFieldType.BLOB and not isinstance(
                            sub_val, bytes
                        ):
    ...
    def _upload_metadata(self, metadata_entries: List[DatapointMetadataUpdateEntry]):
        ...
        total_task = progress.add_task(
            f"Uploading metadata (batch size {upload_batch_size})...",
            total=total_entries,
        )
    ...
    def delete_metadata_from_datapoints(
        self, datapoints: List[Datapoint], fields: List[str]
    ):
        """
        ...
        for d in datapoints:
            for n in fields:
                metadata_entries.append(
                    DatapointDeleteMetadataEntry(datapointId=d.datapoint_id, key=n)
                )
    ...
    def _autolog_mlflow(self, qr: "QueryResult"):
        ...
        now_time = qr.query_data_time.strftime(
            "%Y-%m-%dT%H-%M-%S"
        )  # Not ISO format to make it a valid filename
    ...
    def log_to_mlflow(
        ...
        if self.assigned_dataset is not None:
            client.set_tag(
                run.info.run_id,
                MLFLOW_DATASET_TAG_NAME,
                self.assigned_dataset.dataset_id,
            )
        client.log_dict(run.info.run_id, self._to_dict(as_of), artifact_name)
        log_message(
            f'Saved the datasource state to MLflow (run "{run.info.run_name}") as "{artifact_name}"'
        )
        return run
    ...
    def load_from_serialized_state(state_dict: Dict) -> "Datasource":
        ...
        ds_state = DatasourceState(
            repo=state.repo, name=state.datasource_name, id=state.datasource_id
        )
        ...
        if state.dataset_id is not None:
            ds.load_from_dataset(
                state.dataset_id, state.dataset_name, change_query=False
            )
    ...
    def visualize(
        self, visualizer: Literal["dagshub", "fiftyone"] = "dagshub", **kwargs
    ) -> Union[str, "fo.Session"]:
        """
    ...
    def _generate_visualize_url(self) -> str:
        ...
        if self.assigned_dataset is not None:
            url = multi_urljoin(
                self.source.repoApi.repo_url,
                f"datasets/dataset/{self.assigned_dataset.dataset_id}/gallery",
            )
        else:
            url = self.source.url
    ...
    def _encode_query_for_frontend(self) -> str:
        ...
        params_encoded = base64.urlsafe_b64encode(params.encode("utf-8")).decode(
            "utf-8"
        )
    ...
    @retry(
        retry=retry_if_exception_type(LSInitializingError),
        wait=wait_fixed(3),
        stop=stop_after_attempt(5),
    )
    async def add_annotation_model_from_config(
        self, config, project_name, ngrok_authtoken, port=9090
    ):
        """
        ...
        """
        ls_api_endpoint = multi_urljoin(
            self.source.repoApi.host, self.source.repo, "annotations/de/api/projects"
        )
        ...
        projects = {
            project["title"]: str(project["id"]) for project in res.json()["results"]
        }
        ...
        if project_name not in projects:
            res = requests.post(
                ls_api_endpoint,
                headers={
                    "Authorization": f"Bearer {dagshub.auth.get_token()}",
                    "Content-Type": "application/json",
                },
                data=json.dumps(
                    {"title": project_name, "label_config": config.pop("label_config")}
                ),
            )
        ...
        await self.add_annotation_model(
            **config,
            port=port,
    ...
    async def add_annotation_model(
        ...
        if not ngrok_authtoken and project_name:
            raise ValueError(
                "As `ngrok_authtoken` is not specified, project will have to be added manually."
            )
        with get_rich_progress() as progress:
            ...
            progress.update(
                task, advance=1, description="Configured LS model backend container"
            )
            if res.status_code // 100 != 2:
                raise ValueError(f"Adding backend failed! Response: {res.text}")
            ...
            if ngrok_authtoken:
                if not self.ngrok_listener:
                    self.ngrok_listener = await ngrok.forward(
                        port, authtoken=ngrok_authtoken
                    )
                endpoint = self.ngrok_listener.url()
            else:
                endpoint = f"{LS_ORCHESTRATOR_URL}:{port}/"
            progress.update(
                task, advance=1, description="Configured any necessary forwarding"
            )

            if project_name:
                ls_api_endpoint = multi_urljoin(
                    self.source.repoApi.host, self.source.repo, "annotations/de/api"
                )
                res = requests.get(
                ...
                res = requests.post(
                    ...
                    data=json.dumps(
                        {"url": endpoint, "project": projects[project_name]}
                    ),
                )
                if res.status_code // 100 == 2:
                    progress.update(
                        task, advance=1, description="Added model to LS backend"
                    )
                ...
                print(
                    f"Connection Established! Add LS endpoint: {endpoint} to your project."
                )
    ...
    def annotate(self, fields_to_embed=None, fields_to_exclude=None) -> Optional[str]:
        ...
        return self.all().annotate(
            fields_to_exclude=fields_to_exclude, fields_to_embed=fields_to_embed
        )
    ...
    def send_datapoints_to_annotation(
        ...
        if len(datapoints) == 0:
            ...
            len(datapoints) > dagshub.common.config.recommended_annotate_limit
        ...
        init_url = multi_urljoin(
            self.source.repoApi.data_engine_url, "annotations/init"
        )
        resp = http_request(
            "POST", init_url, json=req_data, auth=self.source.repoApi.auth
        )
    ...
    def _launch_annotation_workspace(self):
        try:
            start_workspace_url = multi_urljoin(
                self.source.repoApi.annotations_url, "start"
            )
    ...
    def wait_until_ready(self, max_wait_time=300, fail_on_timeout=True):
        ...
        if max_wait_time:
            rich_console.log(
                f"Maximum waiting time set to {int(max_wait_time / 60)} minutes"
            )
        spinner = rich_console.status(
            "Waiting for datasource preprocessing to complete..."
        )
        with spinner:
            while True:
                self.source.get_from_dagshub()
    ...
    def __contains__(self, item):
        raise WrongOperatorError(
            "Use `ds.contains(a)` for querying instead of `a in ds`"
        )
    ...
    def contains(self, item: str):
        ...
        if type(item) is not str:
            return WrongOperatorError(
                f"Cannot use contains with non-string value {item}"
            )
    ...
    def startswith(self, item: str):
        ...
        if type(item) is not str:
            return WrongOperatorError(
                f"Cannot use startswith with non-string value {item}"
            )
    ...
    def endswith(self, item: str):
        ...
        if type(item) is not str:
            return WrongOperatorError(
                f"Cannot use endswith with non-string value {item}"
            )
    ...
    def _get_filtering_field(self) -> MetadataFieldSchema:
        ...
        for col in self.source.metadata_fields:
            if col.name == field_name:
                return col
        raise RuntimeError(
            f"Field {field_name} doesn't exist in the current uploaded metadata"
        )
    ...
    def import_annotations_from_files(
        ...
        **kwargs,
        ...
        annotation_dict = importer.remap_annotations(
            annotation_dict, remap_func=remapping_function
        )
        ...
        log_message(
            f'Done! Uploaded annotations for {len(tasks)} datapoints to field "{field}"'
        )
...
class MetadataContextManager:
    ...
    def update_metadata(
        self, datapoints: Union[List[str], str], metadata: Dict[str, Any]
    ):
        """
...
class DatasourceQuery(DataClassJsonMixin):
    ...
    select: Optional[List[Dict]] = field(
    ...
    filter: "QueryFilterTree" = field(
        ...
        metadata=config(
            field_name="query",
            encoder=QueryFilterTree.serialize,
            decoder=QueryFilterTree.deserialize,
        ),
    )
    ...
    def compose(
        ...
        op: str,
        other: Optional[
            Union[str, int, float, "DatasourceQuery", "QueryFilterTree", "Datasource"]
        ],
    ):
