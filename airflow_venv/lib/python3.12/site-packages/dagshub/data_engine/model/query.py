...
class QueryFilterTree:
    ...
    def compose(
        ...
        if self._column_filter_node is not None:
            ...
            raise RuntimeError(
                "is_null operation can only be done on a column (e.g. ds['col1'].is_null())"
            )
        elif op == "not":
            new_tree = Tree()
            ...
            if type(other) is not QueryFilterTree:
                raise RuntimeError(
                    f"Expected other argument to be a dataset, got {type(other)} instead"
                )
            if op not in ["and", "or"]:
                raise RuntimeError(
    ...
    def _column_filter_node(self) -> Node:
        return next(
            self._operand_tree.filter_nodes(lambda n: n.tag == UNFILLED_NODE_TAG), None
        )
    ...
    def _serialize_node(node: Node, tree: Tree) -> Dict:
        ...
        if operand in ["and", "or"]:
            ...
            return {
                operand: [
                    QueryFilterTree._serialize_node(child, tree)
        ...
        if operand == "not":
            ...
            query_op = fieldFilterOperandMap.get(
                operand
            ) or fieldFilterDateTimeFilterMap.get(operand)
            if query_op is None:
            ...
            value_type = (
                metadataTypeLookup[type(value)].value
                if type(value) in metadataTypeLookup
                else None
            )
            ...
            if query_op in dt_range_ops:
                ...
                res["filter"]["value"] = (
                    value if query_op is FieldFilterDateTimeFilter.TIMEOFDAY else 0
                )
                ...
                res["filter"]["valueRange"] = (
                    0 if query_op is FieldFilterDateTimeFilter.TIMEOFDAY else value
                )
    ...
    def _deserialize_node(node_dict: Dict, tree: Tree, parent_node=None) -> None:
        ...
        if op_type == "filter":
            ...
            if comparator.upper() == FieldFilterOperand.DATE_TIME_FILTER.value:
                ...
                converter = _metadataTypeCustomConverters.get(
                    value_type, lambda x: value_type(x)
                )
                value = converter(val["value"])
            as_of = val.get("asOf")
            node = Node(
                tag=comparator, data={"field": key, "value": value, "as_of": as_of}
            )
