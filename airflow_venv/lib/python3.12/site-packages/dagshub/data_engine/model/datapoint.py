...
class Datapoint:
    ...
    @staticmethod
    def from_gql_edge(
        edge: Dict, datasource: "Datasource", fields: List[MetadataSelectFieldSchema]
    ) -> "Datapoint":
        res = Datapoint(
        ...
        float_fields = {
            f.name for f in fields if f.valueType == MetadataFieldType.FLOAT
        }
        date_fields = {
            f.name for f in fields if f.valueType == MetadataFieldType.DATETIME
        }
    ...
    def get_blob(self, column: str, cache_on_disk=True, store_value=False) -> bytes:
        ...
        if isinstance(current_value, Path):
            ...
            content = _get_blob(
                blob_url,
                blob_location,
                self.datasource.source.repoApi.auth,
                cache_on_disk,
                True,
            )
            ...
            raise ValueError(
                f"Can't extract blob metadata from value {current_value} of type {type(current_value)}"
            )

    def download_file(
        self,
        target: Optional[Union[PathLike, str]] = None,
        ...
        download_files(
            [(self.download_url, target_path)], skip_if_exists=not redownload
        )
        return target_path
