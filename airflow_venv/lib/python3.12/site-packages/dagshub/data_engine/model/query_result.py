...
class QueryResult:
    ...
    def from_gql_query(
        ...
        fields = [
            dacite.from_dict(MetadataSelectFieldSchema, f, dacite_config)
            for f in raw_fields
        ]
        # If no fields - get the default datasource ones
        if len(fields) == 0:
            fields = [
                MetadataSelectFieldSchema.from_metadata_field_schema(mfs)
                for mfs in datasource.fields
            ]
        ...
        if edges is None:
            edges = []
        datapoints = [
            Datapoint.from_gql_edge(edge, datasource, fields) for edge in edges
        ]
        query_data_time = datetime.datetime.fromtimestamp(
            query_resp.get("queryDataTime"), tz=datetime.timezone.utc
        )
        ...
        return QueryResult(
            ...
            datasource=datasource,
            fields=fields,
            query_data_time=query_data_time,
        )
    ...
    def as_hf_dataset(
        ...
        if download_datapoints:
            ...
            for dp in df["path"]:
                new_paths.append(
                    str(target_dir / self.datasource.source.source_prefix / dp)
                )
            df["path"] = new_paths
    ...
    def get_blob_fields(
        ...
        *fields: str,
        ...
        if not fields:
            fields = tuple(
                [
                    ...
                    and f.name not in self.document_fields
                ]
            )
        ...
        for dp in self.entries:
            for fld in fields:
                ...
                download_task = (
                    dp,
                    fld,
                    dp.blob_url(field_value),
                    dp.blob_cache_location / field_value,
                )
        ...
        def _get_blob_fn(dp: Datapoint, field: str, url: str, blob_path: Path):
            blob_or_path = _get_blob(
                url, blob_path, auth, cache_on_disk, load_into_memory, path_format
            )
            if isinstance(blob_or_path, str) and path_format != "str":
                logger.warning(
                    f"Error while downloading blob for field {field} in datapoint {dp.path}:{blob_or_path}"
                )
        ...
        with progress:
            with ThreadPoolExecutor(max_workers=num_proc) as tp:
                ...
                for f in as_completed(futures):
                    ...
                    if exc is not None:
                        logger.warning(
                            f"Got exception {type(exc)} while downloading blob: {exc}"
                        )
    ...
    def predict_with_mlflow_model(
        ...
        token = get_token()
        os.environ["MLFLOW_TRACKING_USERNAME"] = UserAPI.get_user_from_token(
            token
        ).username
        ...
        with progress:
            for idx, local_paths in enumerate(
                ...
                for prediction, remote_path in zip(
                    ...
                    [
                        result.path
                        for result in self[idx * batch_size : (idx + 1) * batch_size]
                    ],
                ):
                    predictions[remote_path] = {
                        "data": {
                            "image": multi_urljoin(
                                self.datasource.source.root_raw_path, remote_path
                            )
                        },
            ...
            with self.datasource.metadata_context() as ctx:
                for remote_path in predictions:
                    ctx.update_metadata(
                        remote_path,
                        {
                            log_to_field: json.dumps(predictions[remote_path]).encode(
                                "utf-8"
                            )
                        },
                    )
    ...
    def _get_all_annotations(
        self, annotation_field: str
    ) -> List[IRImageAnnotationBase]:
        annotations = []
        for dp in self.entries:
    ...
    def export_as_yolo(
        ...
        if annotation_field is None:
            annotation_field = sorted(
                [f.name for f in self.fields if f.is_annotation()]
            )
        ...
        if annotation_type is None:
            ...
            log_message(
                f"Importing annotations of type {annotation_type} (most common type of annotation)."
            )
        ...
        for ann in annotations:
            ann.filename = os.path.join(
                self.datasource.source.source_prefix, ann.filename
            )
        ...
        yaml_path = dagshub_annotation_converter.converters.yolo.export_to_fs(
            context, annotations
        )
    ...
    def visualize(
        ...
        if visualizer == "fiftyone":
            ...
            send_analytics_event(
                "Client_DataEngine_VizualizeResults",
                repo=self.datasource.source.repoApi,
            )
