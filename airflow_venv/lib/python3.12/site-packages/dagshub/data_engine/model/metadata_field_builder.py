...
from dagshub.data_engine.dtypes import (
    DagshubDataType,
    MetadataFieldType,
    ReservedTags,
    ThumbnailType,
...
class MetadataFieldBuilder:
    ...
    def __init__(self, datasource: "Datasource", field_name: str):
        ...
        preexisting_schema = next(
            filter(lambda f: f.name == field_name, datasource.fields), None
        )
    ...
    def set_thumbnail(
        ...
        thumbnail_type: Optional[
            Literal["video", "audio", "image", "pdf", "text"]
        ] = None,
        ...
        if not is_thumbnail:
            self._set_or_unset_thumbnails(
                ReservedTags.THUMBNAIL_VIZ.value, is_thumbnail
            )
            return self
        ...
        try:
            ...
            raise ValueError(
                f"'{thumbnail_type}' is not a valid thumbnail type. Valid types are: {valid_types}"
            )
    ...
    def _get_backing_type(t: Union[Type, DagshubDataType]) -> MetadataFieldType:
        ...
        raise ValueError(
            f"{t} of type ({type(t)}) is not a valid primitive type or DagshubDataType"
        )
