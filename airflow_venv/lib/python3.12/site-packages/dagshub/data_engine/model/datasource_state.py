...
path_regexes = {
    DatasourceType.BUCKET: re.compile(
        r"(?P<schema>s3|gs|azure)://(?P<bucket>[\w\-._]+)(?P<prefix>/.*)?"
    ),
    DatasourceType.REPOSITORY: re.compile(
        r"repo://(?P<user>[\w\-_.]+)/(?P<repo>[\w\-_.]+)(?P<prefix>/.*)?"
    ),
}
...
class DatasourceState:
    ...
    def __post_init__(self):
        ...
        if (
            hasattr(self, "source_type")
            and self.source_type == DatasourceType.REPOSITORY
        ):
    ...
    def url(self) -> str:
        return multi_urljoin(
            self.repoApi.repo_url, f"datasets/datasource/{self.id}/gallery"
        )
    ...
    def file_path(
        self, path: Union[str, Datapoint, Mapping[str, Any]]
    ) -> PurePosixPath:
        """
    ...
    def import_metadata_from_file(
        self, datasource_name: str, file_path: Union[str, PathLike], path_column: str
    ):
