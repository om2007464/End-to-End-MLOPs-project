...
def _inside_colab():
    try:
        if (
            IPy.get_ipython()
            and "google.colab" in IPy.get_ipython().extension_manager.loaded
        ):
            return True
    except Exception:
        pass
...
def save_notebook(
    repo, path="", branch=None, commit_message=None, versioning="git", colab_timeout=40
) -> None:
    """
    ...
    if not _inside_notebook():
        log_message(
            "Trying to save a notebook while not being in an IPython environment. No notebook will be saved",
            logger,
        )
        return
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if _inside_colab():
            from google.colab import (
                _message,
            )  # If inside colab, this import is guaranteed

            notebook_ipynb = _message.blocking_request(
                "get_ipynb", timeout_sec=colab_timeout
            )
