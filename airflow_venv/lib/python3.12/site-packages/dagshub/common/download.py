...
def add_bucket_downloader(
    proto: Literal["gs", "s3", "azure"], func: BucketDownloaderFuncType
):
    """
    ...
    if proto in _bucket_downloader_map:
        logger.warning(
            f"Protocol {proto} already has a custom downloader function specified, overwriting it"
        )
    ...
    if len(files) > 1:
        ...
        with progress:
            with ThreadPoolExecutor(max_workers=threads) as tp:
                ...
                futures = [
                    tp.submit(download_fn, url, location) for (url, location) in files
                ]
                for f in as_completed(futures):
                    exc = f.exception()
                    if exc is not None:
                        logger.warning(
                            f"Got exception {type(exc)} while downloading file: {exc}"
                        )
