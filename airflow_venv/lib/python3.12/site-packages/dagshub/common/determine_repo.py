...
def parse_dagshub_remote(
    remote_url: urllib.parse.ParseResult, host_url: urllib.parse.ParseResult
) -> Optional[str]:
    """
    ...
    if not (
        remote_url.path.startswith(host_url.path) and remote_url.path.endswith(".git")
    ):
        return None
...
def determine_repo(
    path: Optional[Union[str, Path]] = None, host: Optional[str] = None
) -> Tuple[RepoAPI, str]:
    """
