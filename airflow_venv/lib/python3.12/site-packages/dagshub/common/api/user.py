...
class UserAPI:
    def __init__(
        self,
        user: Union[str, UserAPIResponse],
    ...
    def get_user_from_token(
        token_or_authenticator: Union[str, Any], host: Optional[str] = None
    ) -> "UserAPI":
        """
        ...
        if resp.status_code != 200:
            raise RuntimeError(
                f"Got HTTP status {resp.status_code} while trying to get user: {resp.content}"
            )
