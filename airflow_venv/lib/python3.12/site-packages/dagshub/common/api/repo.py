...
class RepoAPI:
    def __init__(
        self, repo: str, host: Optional[str] = None, auth: Optional[Any] = None
    ):
        """
    ...
    def get_repo_info(self) -> RepoAPIResponse:
        ...
        if res.status_code == 404:
            ...
            error_msg = (
                f"Got status code {res.status_code} when getting repository info."
            )
    ...
    def get_branch_info(self, branch: str) -> BranchAPIResponse:
        ...
        if res.status_code == 404:
            raise BranchNotFoundError(
                f"Branch {branch} not found in repo {self.repo_url}"
            )
        elif res.status_code >= 400:
    ...
    def get_connected_storages(self) -> List[StorageAPIEntry]:
        ...
        return [
            dacite.from_dict(StorageAPIEntry, storage_entry)
            for storage_entry in res.json()
        ]

    def list_path(
        self, path: str, revision: Optional[str] = None, include_size: bool = False
        ...
        res = self._http_request(
            "GET", self.content_api_url(path, revision), params=params
        )
    ...
    def list_storage_path(
        self, path: str, include_size: bool = False
    ) -> List[ContentAPIEntry]:
        """
        ...
        def _get():
            ...
            if res.status_code == 404:
                ...
                error_msg = (
                    f"Got status code {res.status_code} when listing path {path}"
                )
                logger.error(error_msg)
                logger.debug(res.content)
                raise RuntimeError(error_msg)
    ...
    def download(
        ...
        files = self._get_files_in_path(
            remote_path, revision, recursive, traverse_storages=traverse_storages
        )
        file_tuples = []
        ...
        if len(files) == 1 and files[0].path == remote_path:
            ...
            if local_path != Path("."):
                ...
                if local_path.is_dir():
                    remote_path = (
                        remote_path if keep_source_prefix else remote_path.name
                    )
