...
def init(
    ...
    if None in [repo_owner, repo_name] and (
        repo_owner is not None or repo_name is not None
    ):
    ...
    try:
        ...
        log_message(
            f"Repository {repo_name} doesn't exist, creating it under current user."
        )
    ...
    if dvc:
        ...
        if write:
            dvc_config_local[f"'remote \"{remote}\"'"] = {
                "auth": "basic",
                "user": token,
                "password": token,
            }
