...
class DAGsHubLogger:
    ...
    def log_metrics(self, metrics: Dict[str, Any] = None, step_num: int = 1, **kwargs):
        if self.should_log_metrics:
            ...
            self.unsaved_metrics.append(
                (copy_of_metrics, self.epoch_milisec(), step_num)
            )
    ...
    def init_metrics_file(self):
        ...
        self.metrics_file = open(
            self.metrics_path, "w", newline=""
        )  # newline='' required for csv writer
        self.metrics_file.write("Name,Value,Timestamp,Step\n")
        self.metrics_csv_writer = csv.writer(
            self.metrics_file, quoting=csv.QUOTE_NONNUMERIC
        )
