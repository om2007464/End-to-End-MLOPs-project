...
def add_dataset_to_repo(
    repo: "Repo", local_path: str, data_dir: str = DEFAULT_DATA_DIR_NAME
):
    """
...
def upload_file_to_s3(
    s3_client, local_path, bucket_name, remote_path, progress=None, task=None
):
    """
...
class Repo:
        ...
        **kwargs,
        ...
        if local_path.is_dir():
            ...
            if bucket:
                ...
                dir_to_upload.add_dir(
                    str(local_path), commit_message=commit_message, **kwargs
                )
        else:
            if bucket:
                self.upload_files_to_bucket(local_path, remote_path, **kwargs)
                ...
                self.upload_files(
                    [file_to_upload], commit_message=commit_message, **kwargs
                )

    @retry(
        retry=retry_if_exception_type(InternalServerErrorError),
        wait=wait_fixed(3),
    ...
    def upload_files(
        ...
        # uploading to completely blank repos
            ...
            upload_url = self.get_request_url(
                directory=directory_path, branch=self._api.default_branch
            )
        ...
        if not quiet:
            log_message(
                f'Uploading files ({len(files)}) to "{self._api.full_name}"...', logger
            )
        res = s.put(
            upload_url,
            data=data,
    ...
    def _log_upload_details(
        self, data: Dict[str, Any], res: httpx.Response, files, quiet: bool
    ):
        """
    ...
    def _poll_mirror_up_to_date(self):
        ...
        if self.current_progress is not None:
            task = self.current_progress.add_task(
                "Waiting for the mirror to sync", total=None
            )
            ...
            status = rich.status.Status(
                "Waiting for the mirror to sync", console=rich_console
            )
            status.start()
    ...
    def upload_files_to_bucket(
        self, local_path, remote_path, max_workers=config.upload_threads, **kwargs
    ):
        """
...
class DataSet:
    ...
    def add_dir(
        self, local_path, glob_exclude="", commit_message=None, **upload_kwargs
    ):
        """
            ...
            with progress:
                for root, dirs, files in os.walk(local_path):
                    ...
                    folder_task = progress.add_task(
                        f"Uploading files from {root}", total=len(files)
                    )

                    if commit_message is None:
                        commit_message = upload_kwargs.get(
                    ...
                    for batch in file_batches:
                        ...
                        progress.update(
                            total_task, completed=file_counter, refresh=True
                        )
    ...
    @staticmethod
    def get_file(
        file: Union[str, IOBase], path: os.PathLike = None
    ) -> FileUploadStruct:
        """
