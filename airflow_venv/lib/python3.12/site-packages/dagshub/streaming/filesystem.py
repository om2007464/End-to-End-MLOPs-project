...
class DagsHubFilesystem:
    ...
    def __init__(
        ...
        if not repo_url:
            ...
            if len(remotes) > 0:
                ...
                raise ValueError(
                    "No DagsHub git remote detected, please specify repo_url= argument or --repo_url flag"
                )
        ...
        self._listdir_cache: Dict[
            str, Optional[Tuple[List[ContentAPIEntry], bool]]
        ] = {}
        ...
        response = self._api_listdir(
            DagshubPath(self, self.project_root, Path(), Path())
        )
        if response is None:
            # TODO: Check .dvc/config{,.local} for credentials
            raise AuthenticationError(
                "DagsHub credentials required, however none provided or discovered"
            )
        ...
        for p, f in DagsHubFilesystem.already_mounted_filesystems.items():
            if is_subpath(p, self.project_root):
                raise FilesystemAlreadyMountedError(
                    self.project_root, f.parsed_repo_url.path[1:], f._current_revision
                )
    ...
    def get_remote_branch_head(self, branch):
        ...
        url = self.get_api_url(
            f"/api/v1/repos{self.parsed_repo_url.path}/branches/{branch}"
        )
    ...
    def get_remotes_from_git_config(self) -> List[str]:
        ...
        git_remotes = [
            urlparse(git_config[remote]["url"])
        ...
        for remote in git_remotes:
            ...
            remote = remote._replace(
                path=re.compile(r"(\.git)?/?$").sub("", remote.path)
            )
    ...
    def cleanup(self):
        ...
        if (
            hasattr(self, "project_root")
            and self.project_root in DagsHubFilesystem.already_mounted_filesystems
        ):
        ...
        if path.is_in_repo:
            if opener is not None:
                raise NotImplementedError(
                    "DagsHub's patched open() does not support custom openers"
                )
                ...
                try:
                    ...
                    if "r" in mode:
                        try:
                            ...
                            raise RuntimeError(
                                f"Couldn't download {path.relative_path} after multiple attempts"
                            )
                        if resp.status_code < 400:
                            ...
                            raise FileNotFoundError(
                                f"Error finding {path.relative_path} in repo or on DagsHub"
                            )
                        else:
                            raise RuntimeError(
    ...
    def os_open(self, path, flags, mode=0o777, *, dir_fd=None):
        ...
        if (
            dir_fd is not None
        ):  # If dir_fd supplied, path is relative to that dir's fd, will handle in the future
            logger.debug("fs.os_open - NotImplemented")
            raise NotImplementedError(
                "DagsHub's patched os.open() (for pathlib only) does not support dir_fd"
            )
        ...
        if path.is_in_repo:
            try:
                ...
                logger.debug(
                    "fs.os_open - failed to materialize path, os.open will throw"
                )
    ...
    def stat(self, path, *args, dir_fd=None, follow_symlinks=True):
        ...
        if dir_fd is not None or not follow_symlinks:
            ...
            raise NotImplementedError(
                "DagsHub's patched stat() does not support dir_fd or follow_symlinks"
            )
        ...
        if parsed_path.is_in_repo:
            ...
            if parsed_path.is_passthrough_path:
                ...
                return dagshub_stat_result(
                    self,
                    path,
                    is_directory=False,
                    custom_size=len(self._special_file()),
                )
            else:
                try:
                    ...
                    logger.debug(
                        f"cached_remote_parent_tree: {cached_remote_parent_tree}"
                    )
                    ...
                    if filetype == "file":
                        ...
                        raise RuntimeError(
                            f"Unknown file type {filetype} for path {str(parsed_path)}"
                        )
    ...
    def listdir(self, path="."):
        ...
        if parsed_path.is_in_repo:
            if parsed_path.is_passthrough_path:
                ...
                if (
                    0 < len_parts <= 3
                    and parsed_path.relative_path.parts[0] == ".dagshub"
                ):
                ...
                if resp is not None:
                    ...
                    self.remote_tree[str(parsed_path.relative_path)] = {
                        Path(f.path).name: f.type for f in resp
                    }
                    return encode_results(dircontents)
                else:
            ...
            if resp is not None:
                for f in resp:
                    ...
                    if name not in local_filenames:
                        yield dagshub_DirEntry(
                            self,
                            parsed_path / name,
                            f.type == "dir",
                            is_binary=is_bytes_path_arg,
                        )
        else:
    ...
    def _get_special_paths(
        ...
        def generate_entry(path, is_directory):
            if isinstance(path, str):
                path = Path(path)
            return dagshub_DirEntry(
                self, relative_to / path, is_directory=is_directory, is_binary=is_binary
            )
    ...
    def _api_listdir(
        self, path: DagshubPath, include_size: bool = False
    ) -> Optional[List[ContentAPIEntry]]:
        response, hit = self._check_listdir_cache(
            path.relative_path.as_posix(), include_size
        )
        if hit:
            return response
    ...
    def _check_listdir_cache(
        self, path: str, include_size: bool
    ) -> Tuple[Optional[List[ContentAPIEntry]], bool]:
    ...
    def _content_url_for_path(self, path: DagshubPath):
        if not path.is_in_repo:
            raise RuntimeError(
                f"Can't access path {path.absolute_path} outside of repo"
            )
    ...
    def _api_download_file_git(self, path: DagshubPath):
        resp = self.http_get(
            self._raw_url_for_path(path), headers=config.requests_headers, timeout=None
        )
        return resp
    ...
    def install_hooks(self):
        ...
        if is_inside_notebook() and not is_inside_colab():
            ...
            if (
                ...
                and hasattr(instance, "user_ns")
                and "open" in instance.user_ns
            ):
    ...
    def _install_framework_hooks(self):
        ...
        for framework in self.frameworks:
            if framework not in self._framework_override_map:
                logger.warning(
                    f"Framework {framework} not available for override, skipping"
                )
                continue
            ...
            for func in funcs:
                ...
                try:
                    ...
                    logger.warning(
                        f"Module [{module_name}] not found, so function [{func}] isn't being patched"
                    )
                    continue
                    ...
                    continue
                self.__class__.__unpatched[
                    f"{self._framework_key_prefix}{func}"
                ] = orig_fn
                if patch_class is not None:
                    setattr(
                        patch_class, func_name, self._passthrough_decorator(orig_fn)
                    )
                else:
                    setattr(
                        patch_module, func_name, self._passthrough_decorator(orig_fn)
                    )

    def _passthrough_decorator(
        self, orig_func, filearg: Union[int, str] = 0
    ) -> Callable:
        """
    ...
    def uninstall_hooks(cls):
        ...
        if hasattr(cls, f"_{cls.__name__}__unpatched"):
            ...
            if "notebook_open" in cls.__unpatched:
                ...
                instance = (
                    IPython.core.interactiveshell.InteractiveShell._instance
                )  # noqa
    ...
    def _uninstall_framework_hooks(cls):
        for func in list(
            filter(
                lambda key: key.startswith(cls._framework_key_prefix),
                cls.__unpatched.keys(),
            )
        ):
