...
def _make_message(
    method: Method, params: Optional[Mapping[str, Any]]
) -> "PipesMessage":
    return {
...
def _assert_defined_asset_property(value: Optional[_T], key: str) -> _T:
    return _assert_not_none(
        value, f"`{key}` is undefined. Current step does not target an asset."
    )
...
def _assert_single_asset(data: PipesContextData, key: str) -> None:
    ...
    if len(asset_keys) != 1:
        raise DagsterPipesError(
            f"`{key}` is undefined. Current step targets multiple assets."
        )
...
def _assert_defined_extra(extras: PipesExtras, key: str) -> Any:
    if key not in extras:
        raise DagsterPipesError(
            f"Extra `{key}` is undefined. Extras must be provided by user."
        )
    return extras[key]
...
def _assert_opt_param_type(
    value: _T, expected_type: Any, method: str, param: str
) -> _T:
...
def _assert_param_value(
    value: _T, expected_values: Iterable[_T], method: str, param: str
) -> _T:
...
def _normalize_param_metadata(
    ...
    for key, value in metadata.items():
        if not isinstance(key, str):
            ...
            _assert_param_value(
                value["type"], _METADATA_TYPES, method, f"{param}.{key}.type"
            )
...
class PipesBlobStoreMessageWriterChannel(PipesMessageWriterChannel):
    ...
    def buffered_upload_loop(self) -> Iterator[None]:
        ...
        try:
            thread = Thread(
                target=self._upload_loop, args=(is_session_closed,), daemon=True
            )
            thread.start()
            yield
        finally:
    ...
    def _upload_loop(self, is_session_closed: Event) -> None:
        ...
        while True:
            ...
            if self._buffer.empty() and is_session_closed.is_set():
                ...
                is_session_closed.is_set()
                or (now - start_or_last_upload).seconds > self._interval
            ):
                payload = "\n".join(
                    [json.dumps(message) for message in self.flush_messages()]
                )
                if len(payload) > 0:
...
class PipesDefaultContextLoader(PipesContextLoader):
    ...
    def load_context(self, params: PipesParams) -> Iterator[PipesContextData]:
        if self.FILE_PATH_KEY in params:
            path = _assert_env_param_type(
                params, self.FILE_PATH_KEY, str, self.__class__
            )
...
class PipesDefaultMessageWriter(PipesMessageWriter):
    ...
    def open(self, params: PipesParams) -> Iterator[PipesMessageWriterChannel]:
        if self.FILE_PATH_KEY in params:
            ...
            stream = _assert_env_param_type(
                params, self.BUFFERED_STDIO_KEY, str, self.__class__
            )
            if stream not in (self.STDERR, self.STDOUT):
                raise DagsterPipesError(
                    f'Invalid value for key "std", expected "{self.STDERR}" or "{self.STDOUT}" but'
...
DAGSTER_PIPES_CONTEXT_CLI_ARGUMENT = _env_var_to_cli_argument(
    DAGSTER_PIPES_CONTEXT_ENV_VAR
)
DAGSTER_PIPES_MESSAGES_CLI_ARGUMENT = _env_var_to_cli_argument(
    DAGSTER_PIPES_MESSAGES_ENV_VAR
)

DAGSTER_PIPES_CLI_PARSER = argparse.ArgumentParser(
    description="Dagster Pipes CLI interface"
)
    ...
    def get_opened_extras(self) -> PipesExtras:
        ...
        try:
            if spark.conf.get(
                ...
                return {
                    "cluster_driver_log_root": f"{cluster_log_destination}/{cluster_id}/driver"
                }
            else:
                return {}
...
class PipesContext:
    ...
    def __init__(
        ...
        self._data = self._io_stack.enter_context(
            context_loader.load_context(context_params)
        )
    ...
    def _write_message(
        self, method: Method, params: Optional[Mapping[str, Any]] = None
    ) -> None:
        if self._closed:
            raise DagsterPipesError(
    ...
    def asset_key(self) -> str:
        ...
        are in scope.
        """
        asset_keys = _assert_defined_asset_property(
            self._data["asset_keys"], "asset_key"
        )
    ...
    def asset_keys(self) -> Sequence[str]:
        ...
        """
        asset_keys = _assert_defined_asset_property(
            self._data["asset_keys"], "asset_keys"
        )
        return asset_keys
    ...
    def report_asset_check(
        ...
        check_name = _assert_param_type(
            check_name, str, "report_asset_check", "check_name"
        )
