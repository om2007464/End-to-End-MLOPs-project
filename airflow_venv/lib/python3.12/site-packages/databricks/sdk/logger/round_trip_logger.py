...
class RoundTrip:
    ...
    def generate(self) -> str:
        ...
        if self._debug_headers:
            for k, v in request.headers.items():
                sb.append(
                    f"> * {k}: {self._only_n_bytes(v, self._debug_truncate_bytes)}"
                )
        if request.body:
            sb.append(
                "> [raw stream]"
                if self._raw
                else self._redacted_dump("> ", request.body)
            )
        sb.append(f"< {self._response.status_code} {self._response.reason}")
        if (
            self._raw
            and self._response.headers.get("Content-Type", None) != "application/json"
        ):
    ...
    def _redacted_dump(self, prefix: str, body: str) -> str:
        ...
        try:
            ...
            return "\n".join(
                [f"{prefix}{line}" for line in json.dumps(raw, indent=2).split("\n")]
            )
