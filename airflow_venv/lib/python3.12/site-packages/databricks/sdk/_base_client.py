...
class _BaseClient:
    ...
    def do(
        ...
        response = retryable(self._perform)(
            method,
            url,
            query=query,
            headers=headers,
    ...
    def _perform(
        ...
        self._record_request_log(
            response, raw=raw or data is not None or files is not None
        )
    ...
    def _record_request_log(
        ...
        logger.debug(
            RoundTrip(
                response, self._debug_headers, self._debug_truncate_bytes, raw
            ).generate()
        )
...
class _StreamingResponse(BinaryIO):
    ...
    def __init__(
        self, response: requests.Response, chunk_size: Union[int, None] = None
    ):
    ...
    def read(self, n: int = -1) -> bytes:
        ...
        while remaining_bytes > 0 or read_everything:
            ...
            to_read = (
                bytes_available
                if read_everything
                else min(remaining_bytes, bytes_available)
            )
