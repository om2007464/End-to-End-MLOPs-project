...
class Config:
    ...
    password: str = ConfigAttribute(
        env="DATABRICKS_PASSWORD", auth="basic", sensitive=True
    )
        ...
        if not self.host and self.azure_workspace_resource_id:
            ...
            for environment in ALL_ENVS:
                ...
                if environment.azure_environment.name != azure_env:
                    continue
                if environment.dns_zone.startswith(
                    ".dev"
                ) or environment.dns_zone.startswith(".staging"):
                    continue
                return environment
    ...
    def _upstream_user_agent(self) -> str:
        return " ".join(
            f"{k}/{v}" for k, v in useragent._get_upstream_user_agent_info()
        )
    ...
    def sql_http_path(self) -> Optional[str]:
        ...
        if self.cluster_id:
            response = requests.get(
                f"{self.host}/api/2.0/preview/scim/v2/Me", headers=headers
            )
    ...
    def _known_file_config_loader(self):
        ...
        if profile not in profiles:
            raise ValueError(
                f"resolve: {config_path} has no {profile} profile configured"
            )
        raw_config = profiles[profile]
        logger.info(
            f'loading {profile} profile from {config_file}: {", ".join(raw_config.keys())}'
        )
    ...
    def _validate(self):
        ...
        raise ValueError(
            f"validate: more than one authorization method configured: {names}"
        )
    ...
    def init_auth(self):
        try:
            ...
            raise ValueError(
                f"{self._credentials_strategy.auth_type()} auth: {e}"
            ) from e
