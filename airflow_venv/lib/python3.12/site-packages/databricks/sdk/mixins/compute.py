    ...
    def select_spark_version(
        ...
        for version in sv.versions:
            ...
            if matches and long_term_support:
                matches = matches and (
                    ("LTS" in version.name) or ("-esr-" in version.key)
                )
    ...
    def select_node_type(
        ...
        for nt in types:
            ...
            if local_disk or local_disk_min_size is not None:
                ...
                local_disks = (
                    int(instance_type.local_disks) if instance_type.local_disks else 0
                )
                local_nvme_disks = (
                    int(instance_type.local_nvme_disks)
                ...
                if instance_type is None or (local_disks < 1 and local_nvme_disks < 1):
                    continue
                local_disk_size_gb = (
                    instance_type.local_disk_size_gb
                    if instance_type.local_disk_size_gb
                    else 0
                )
                local_nvme_disk_size_gb = (
                ...
                if (
                    local_disk_min_size is not None
                    and all_disks_size < local_disk_min_size
                ):
                    continue
