...
class _VolumesIO(BinaryIO):
    ...
    def close(self):
        ...
        if self._write:
            ...
            self._api.upload(
                self._path, contents=BytesIO(to_write), overwrite=self._overwrite
            )
        elif self._read:
    ...
    def __repr__(self) -> str:
        return (
            f"<_VolumesIO {self._path} {'read' if self.readable() else 'write'}=True>"
        )
    ...
    def __init__(self, path: str):
        if platform.system() == "Windows":
            self._path = pathlib.Path(
                str(path).replace("file:///", "").replace("file:", "")
            )
        else:
...
class _VolumesPath(_Path):
    def __init__(self, api: files.FilesAPI, src: Union[str, pathlib.Path]):
        self._path = pathlib.PurePosixPath(
            str(src).replace("dbfs:", "").replace("file:", "")
        )
        self._api = api
    ...
    def open(self, *, read=False, write=False, overwrite=False) -> BinaryIO:
        return _VolumesIO(
            self._api, self.as_string, read=read, write=write, overwrite=overwrite
        )
...
class DbfsExt(files.DbfsAPI):
    ...
    def copy(self, src: str, dst: str, *, recursive=False, overwrite=False):
        ...
        if src.is_dir:
            queue = [
                self._path(x.path)
                for x in src.list(recursive=recursive)
                if not x.is_dir
            ]
        else:
            queue = [src]
