...
class DecoratorMetaClass(type):
    def __new__(
        cls, name: str, bases: Tuple[type, ...], attributedict: Dict
    ) -> "DecoratorMetaClass":
...
class DecoratorBaseClass(metaclass=DecoratorMetaClass):
        ...
        try:
            if self._args_is_decorated_function(*args, **kwargs):
                ...
                if not isinstance(func, types.FunctionType) and (
                    self._staticmethod or self._classmethod
                ):
                    func = func.__func__
            ...
            raise TypeError(
                "'perfect.decorator' must wrap a function as argument or decorator"
            )
    ...
    def _args_is_decorated_function(cls, *args: Any, **kwargs: Any) -> bool:
        ...
        if (
            ...
            and (
                callable(args[0])
                or (
                    isinstance(args[0], (staticmethod, classmethod))
                    and not isinstance(args[0], types.FunctionType)
                )
            )
        ):
            func = args[0]
        ...
        if func is not None:
            if not isinstance(func, types.FunctionType) and isinstance(
                args[0], (staticmethod, classmethod)
            ):
                func.__func__
            return True
    ...
    def __get__(self, instance: Any, owner: Any) -> Callable:
        key = (instance, owner)
        function_map = cast(
            Dict[Tuple[Any, Any], Callable], getattr(self, "_function_map", {})
        )
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if wrapped_func is None:
            if not getattr(
                self, "_meta", False
            ) and not self._args_is_decorated_function(*args, **kwargs):
                raise TypeError(
                    "'perfect.decorator' must wrap a function as argument or decorator"
                )
